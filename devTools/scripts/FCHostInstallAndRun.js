...
let windowsLink = "https://mega.nz/file/kY5GmLRa#jwwgshNuKz0yAQDDeSs-jbPOOGX4Ap1ha_EKlWp-9-M";

// location to save the zip file to
const zipLocation = path.join("FCHost", "fchost", "PreCompiledFCHost.zip");

const currentDate = new Date();
const month   = currentDate.getUTCMonth() + 1; // months from 1-12
const day     = currentDate.getUTCDate();
const year    = currentDate.getUTCFullYear();
const epoch   = Math.round(currentDate.getTime() / 1000);
const backupLocation = path.join("FCHost", "fchost", `Release_PreUpdate_${month}-${day}-${year}_${epoch}`);
...
/**
 * Asks the user if they want to download FCHost from the given mega link
 * @param {string} link if this is an empty string then this function is a no-op
 */
async function download(link) {
	...
	// move old FCHost install to the backup path
	if (jetpack.exists(settings.FCHostPath) !== false) {
		console.log(`Moving existing install to ${backupLocation}`);
		jetpack.move(settings.FCHostPath, backupLocation);
	}
	...
	// delete zip file
	jetpack.remove(zipLocation);
	// set FCHostDownloadURL
	settings.FCHostDownloadURL = link;
	// save settings
	jetpack.write("settings.json", settings, {atomic: true});
}

// get the correct link for the given platform
let link = "";
...
if (
	...
	await inquirer
		.prompt([{
			type: "rawlist",
			name: "choice",
			message: `Do you want to download and extract a pre-compiled version of FCHost from '${link}'?`,
			choices: ["Yes", "No"],
			default: "Yes",
			loop: false
		}])
...
// FCHost exists but FCHostDownloadURL is not as expected
} else if (
	link !== "" &&
	settings.FCHostDownloadURL !== link
) {
	if (link !== "") {
		...
		await inquirer
			.prompt([{
				type: "rawlist",
				name: "choice",
				message: `FCHost has been updated since you last build/downloaded it. Do you want to download and extract an updated pre-compiled version of FCHost from '${link}'?`,
				choices: ["Yes", "No"],
				default: "Yes",
				loop: false
			}])
		...
		if (choice === "Yes") {
			...
			await inquirer
				.prompt([{
					type: "rawlist",
					name: "choice",
					message: `Are you going to manually build FCHost and/or skip this version?`,
					choices: ["Yes", "No"],
					default: "Yes",
					loop: false
				}])
			...
			if (choice === "Yes") {
				...
				// save settings
				jetpack.write("settings.json", settings, {atomic: true});
				console.log("Instructions for manually compiling FCHost are at: https://gitgud.io/pregmodfan/fc-pregmod/-/blob/pregmod-master/FCHost/HowToBuild.md");
				process.exit();
			} else {
				console.log("You can run this script at any time if you change your mind.");
				console.log("You can also manually compile FCHost if you wish.");
...
// TODO:@franklygeorge spawn FCHost as a separate process to allow this script to close
if (process.platform === "win32") {
	// run fchost
