...
import { redraw, svgToPng, toJpeg } from "../lib/imagegen";
...
export default function PhotoPortrait({ id, body }: { id: string; body: RefObject<SVGSVGElement | null> }) {
  ...
  const take = async (mode: "face" | "body") => {
    ...
    setBusy(mode);
    try {
      const png = await svgToPng(svg, mode === "face" ? 3 : 2.5);
  ...
  return (
    <div className="mb-2">
      ...
      {pic ? <img src={pic} alt={`photo of ${p.name}`} className="w-full max-h-72 object-cover object-top rounded-lg mb-1.5" /> : null}
      {ph?.face && ph?.body ? (
