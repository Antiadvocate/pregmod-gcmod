...
Gifts like this are never free. She's beautiful and well trained, and she's very quick to learn her way around the penthouse.`,
      options: [
        ...
        { id: "check", label: "Keep her, and have her watched", need: skill("hacking", 25),
          run: (c) => { const p = c.addSlave({ seed: "gift", age: 25, quality: 0.8, how: "a gift from the Owners' Association", devoted: true }); c.set("gift_id", p.id); c.set("gift_watched", true);
            return { text: `You put a tap on the dormitory network before she's unpacked. Three days later, she uses it.`, next: "caught", after: 3 }; } },
        { id: "decline", label: "Send her back with your regrets",
          run: (c) => { c.rep(-150);
            return { text: `The courier takes her back down in the lift. The membership committee is offended.`, end: "you sent the Association's gift back" }; } },
      ],
    },
    caught: {
      ...
      options: [
        { id: "turn", label: "Turn her. She reports what you tell her to.", need: (c) => { const p = c.s.people[String(c.flag("gift_id"))]; return p && read(p).trust > -20 ? null : "she'd have to trust you more"; },
          run: (c) => { const p = c.s.people[String(c.flag("gift_id"))]; c.treat(p, "recognition", 6, "turned instead of punished"); c.rep(300);
            return { text: `She agrees without much argument. "They never cared what happened to me either," she says. From now on the Association gets the reports you write.`, end: "you turned the Association's spy" }; } },
        ...
        { id: "sell", label: "Sell her back to the committee at twice her value",
          run: (c) => { const p = c.s.people[String(c.flag("gift_id"))]; if (p) c.remove(p, "sold", "sold back to the Association", Math.round(valuePerson(c.s, p) * 2)); c.rep(-100);
            return { text: `They pay, quietly. The gift has cost them double.`, end: "you sold the gift back" }; } },
      ],
    },
  },
};
...
At the Association dinner ${c.he("rival_owner")} says, loudly enough, that your household is soft, and that ${c.he("rival_owner")}'ll put ten thousand on the Widow against anyone you own. First blood, or submission. Everyone at the table waits for your answer.`,
      options: [
        { id: "accept", label: "Accept, and pick your champion", note: "she fights the Widow",
          ...
          run: (c, p) => {
            ...
            if (won) { c.cash(10000); c.rep(700); c.like("rival_owner", -20); c.treat(p, "recognition", 8, "won in the pit"); p.fame.prestige = Math.max(p.fame.prestige, 1) as 1; p.fame.why = "beat the Widow in the pit";
              return { text: `${p.name} goes down twice in the first minute and gets up both times. In the third she catches the Widow's wrist, turns it, and breaks it. The Widow bleeds first. ${c.n("rival_owner")} pays in front of everyone.`, end: `${p.name} beat the Widow` }; }
            ...
            return { text: `It lasts forty seconds. ${p.name} is carried out with her arm strapped to her chest. ${c.n("rival_owner")} collects the money with a smug smile.`, end: `${p.name} lost to the Widow` };
          } },
        { id: "buy", label: "Offer to buy the Widow instead", note: "¤30,000", need: cash(30000),
          run: (c) => { c.cash(-30000); const p = c.addSlave({ seed: "widow", age: 30, quality: 0.6, how: "the Widow, bought out from under a rival's pit" }); p.skills.combat = 85; p.body.muscle = 55; p.fame.prestige = 1; p.fame.why = "the Widow, unbeaten in the pit"; c.like("rival_owner", -10); c.rep(400);
            return { text: `There's a pause while ${c.n("rival_owner")} works out whether ${c.he("rival_owner")} can say no in front of this many people. ${c.He("rival_owner")} can't. The Widow comes home with you.`, end: "you bought the Widow" }; } },
        { id: "decline", label: "\"I don't fight my household for your entertainment.\"",
          run: (c) => { c.rep(-200); c.hope(4); c.like("rival_owner", -5);
            return { text: `Some of the other owners laugh at you. Your slaves hear about it by morning, and they're relieved.`, end: "you refused the pit" }; } },
      ],
    },
  },
};
      ...
      options: [
        { id: "sell", label: "Take the eight thousand. Let her go.",
          run: (c) => { c.cash(8000); if (c.subj) c.remove(c.subj, "free", `bought free by ${relation(c)}`); c.hope(10); c.standing(1);
            return { text: `She comes down in her work clothes, sees him, and runs into his arms. They leave together while your other slaves watch from the stairs.`, end: `${c.sn} went home with ${relation(c)}` }; } },
        { id: "ask", label: "Ask her what she wants",
          run: (c) => {
            ...
            if (stays) { c.treat(c.subj, "promise_kept", 8, "given the choice to leave, and stayed"); c.hope(5);
              return { text: `She thinks about it for a long time. Then she tells him she wants to stay with you, and goes back upstairs. He leaves heartbroken.`, end: `${c.sn} chose to stay` }; }
            c.cash(8000); if (c.subj) c.remove(c.subj, "free", `chose to go home with ${relation(c)}`); c.hope(8);
            return { text: `"Yes," she says, before you've finished the question. She leaves without even packing.`, end: `${c.sn} chose to go home` };
          } },
        { id: "full", label: "Full price or nothing",
          run: (c) => { c.treat(c.subj, "cruelty", 5, "he came for her and you named a price he couldn't pay");
            return { text: `He can't. He tries to argue and security walks him to the lift. ${c.sn} saw it all from the landing, and she's miserable for days.`, next: "back", after: 10 }; } },
        { id: "throw", label: "Have him thrown out",
          run: (c) => { c.treat(c.subj, "cruelty", 6, "the man who came for her was thrown out"); c.crime(1);
            return { text: `He's put out on the landward causeway without his letter. ${c.sn} finds out from the other slaves, and she's heartbroken.`, end: `you threw out the man who came for ${c.sn}` }; } },
      ],
    },
    back: {
      title: "He came back",
      text: (c) => `He's back, with the full amount in a bag. He's sold his house and everything else he owns to raise it.`,
      options: [
        { id: "take", label: "Take it", run: (c) => { c.cash(valuePerson(c.s, c.subj!)); if (c.subj) c.remove(c.subj, "free", `bought free by ${relation(c)}, at full price`); c.hope(6);
          return { text: `She goes home with him. He's broke, but happy.`, end: `${c.sn} went home, at full price` }; } },
        { id: "no", label: "Tell him she's no longer for sale", run: (c) => { c.treat(c.subj, "cruelty", 7, "not for sale, even at full price"); c.rep(-100);
          return { text: `Security has to drag him out of reception.`, end: `you kept ${c.sn} from the man who came for her` }; } },
      ],
    },
  },
};
      ...
      options: [
        { id: "take", label: "Take the year", note: "she's yours for 52 weeks, then free",
          run: (c) => { const p = c.addSlave({ seed: "heiress", age: 24, quality: 0.7, how: "signed herself over for a year against gambling debts" }); p.status = "indentured"; p.indenture_weeks = 52; p.bond = { ...p.bond, bond: 0, fear: 10, resentment: 15, hope: 60 }; c.set("heiress", p.id);
            return { text: `She signs the indenture without reading it and hands you back the pen. "What happens now?" she asks, suddenly nervous.`, next: "father", after: 6 }; } },
        ...
        { id: "forgive", label: "Tear the markers up",
          run: (c) => { c.standing(1); c.rep(100);
            return { text: `She can't believe it. A month later her father sends you a case of very old wine, so she must have told him anyway.`, end: "you forgave the heiress's debt" }; } },
      ],
    },
    father: {
      ...
      options: [
        { id: "release", label: "Take his money and release her",
          run: (c) => { const p = c.s.people[String(c.flag("heiress"))]; c.cash(22000); if (p) c.remove(p, "free", "bought out of her indenture by her father");
            return { text: `She goes home with him. She's relieved to be going.`, end: "her father bought her out" }; } },
        ...
        { id: "her", label: "Let her decide", show: (c) => !!c.s.people[String(c.flag("heiress"))],
          run: (c) => { const p = c.s.people[String(c.flag("heiress"))]!;
            if (read(p).devotion > 30) { c.treat(p, "promise_kept", 6, "chose to finish the year"); return { text: `"I'm staying," she says. "I signed it." Her father is stunned.`, end: "the heiress chose to stay" }; }
            c.cash(22000); c.remove(p, "free", "chose to go home with her father");
            return { text: `She leaves with her father immediately.`, end: "the heiress went home" }; } },
      ],
    },
  },
};
...
const twins: ArcDef = {
  ...
  beats: {
    lot: {
      ...
      text: (c) => `The broker has a pair of identical twins, twenty years old. He's selling them separately because separately is worth more, and a buyer from Ardent is already interested in one.
...
"Nine thousand each," he says. "Or eighteen for the pair, as a favour."`,
      options: [
        { id: "both", label: "Buy both", note: "¤18,000", need: cash(18000),
          run: (c) => { c.cash(-18000);
            ...
            return { text: `${a.name} and ${b.name}. They're so relieved not to be split up that they hold hands all the way to the dormitory, and push their beds together.`, next: "sick", after: 8 }; } },
        { id: "one", label: "Buy one", note: "¤9,000", need: cash(9000),
          run: (c) => { c.cash(-9000); const a = c.addSlave({ seed: "twinA", age: 20, quality: 0.5, how: "bought away from her twin, who went to Ardent", hostile: true }); c.treat(a, "cruelty", 5, "separated from her twin");
            return { text: `${a.name} screams when they separate them. The other one goes to Ardent. ${a.name} asks you every night for a week where her sister is, and then gives up.`, end: "you split the twins" }; } },
    ...
    sick: {
      ...
      text: (c) => {
        ...
        return `${b?.name ?? "One of them"} is sick — a fever that's gone into her chest — and ${a?.name ?? "her sister"} has been sleeping on the floor of the spa next to her for three nights. She asks to see you.
      ...
      options: [
        { id: "treat", label: "Pay for the clinic", note: "¤6,000", need: cash(6000),
          run: (c) => { c.cash(-6000); const [a, b] = String(c.flag("twins")).split(",").map((id) => c.s.people[id]); if (b) { b.health.illness = 0; b.health.health = clamp(b.health.health + 30, -100, 100); } c.treat(a, "promise_kept", 8, "you paid to save her sister"); c.treat(b, "kindness", 6, "you paid for the clinic");
            return { text: `The fever breaks on the fourth day. The next morning both of them bring you coffee to thank you.`, end: "you saved one twin for the other" }; } },
        { id: "take", label: "\"I'll take it out of you, then.\"",
          run: (c) => { c.cash(-6000); const [a, b] = String(c.flag("twins")).split(",").map((id) => c.s.people[id]); if (b) { b.health.illness = 0; b.health.health = clamp(b.health.health + 30, -100, 100); } if (a) resolveAct(c.s, a, "orders"); c.treat(a, "coercion", 5, "paid for her sister's treatment with herself");
            return { text: `She pays it with her body, and her sister gets better. She resents you for it.`, end: "one twin paid for the other" }; } },
        { id: "wait", label: "Let it run its course",
          run: (c) => { const [a, b] = String(c.flag("twins")).split(",").map((id) => c.s.people[id]); if (b) b.health.health = clamp(b.health.health - 25, -100, 100); c.treat(a, "cruelty", 6, "you let her sister stay sick");
            return { text: `She pulls through after two weeks, barely. Her sister hates you for it.`, end: "you let the twin's fever run" }; } },
      ],
    },
  },
};
