...
const heir: ArcDef = {
  ...
  beats: {
    will: {
      ...
      text: (c) => `The lawyer reads the will in your father's office with the blinds half down. It takes eleven minutes. You get the building, the household, and a folder of debts thick enough to stop a door.
...
${c.sn} — your father's head girl for nine years — is pouring the drinks. Her hand doesn't shake. She's listening to every word.`,
      options: [
        { id: "pay", label: "\"You'll get your money.\"", note: "a debt of ¤90,000, and your word on it",
          run: (c) => { c.set("heir_debt", 90000); c.set("heir_word", true); c.like("creditor", 5);
            return { text: `${c.n("creditor")} smiles like a man who has heard that before and been right to believe it about half the time. "Come by in two months. We'll see how you're getting on."`, next: "headgirl", after: 1 }; } },
        { id: "understanding", label: "Ask what the understanding actually was",
          run: (c) => { c.set("heir_debt", 90000); c.set("heir_asked", true); c.like("creditor", -5);
            return { text: `${c.n("creditor")} looks at you for a long moment. "Your father borrowed money. I lent it. That's the understanding." ${c.He("creditor")} finishes ${c.his("creditor")} drink. Across the room, ${c.sn} sets the decanter down a little too carefully. She knows something.`, next: "headgirl", after: 1 }; } },
        { id: "share", label: "Offer a share of the building instead of cash", note: "halves the debt; costs you ownership", need: skill("trading", 25),
          run: (c) => { c.set("heir_debt", 45000); c.set("heir_partner", true); c.s.arcology.ownership = Math.max(5, c.s.arcology.ownership - 8); c.like("creditor", 15); c.out.push("−8% ownership");
            return { text: `${c.n("creditor")} actually laughs. "He never would have offered that. All right. Eight points of the building and I'll cut the paper in half." You shake on it. ${c.His("creditor")} hand is dry and cold.`, next: "headgirl", after: 1 }; } },
        { id: "out", label: (c) => `Have her show ${c.him("creditor")} out`, note: "the debt stands, and so does the insult",
          run: (c) => { c.set("heir_debt", 110000); c.set("heir_defiant", true); c.like("creditor", -20); c.treat(c.subj, "recognition", 4, "trusted to throw out the creditor");
            return { text: `You tell ${c.sn} to see ${c.n("creditor")} to the lift. She does it with a courtesy so perfect it's an insult. ${c.n("creditor")} stops in the doorway. "Interest compounds," ${c.he("creditor")} says. "Your father knew that." The debt is a hundred and ten thousand now.`, next: "headgirl", after: 1 }; } },
      ],
    },
    headgirl: {
      title: "His girl",
...
"He used to let me sit here while he worked," she says. "I'd read. He'd swear at the numbers." She doesn't look at you. "I don't know what I am now."`,
      options: [
        { id: "sit", label: "\"Sit.\"",
        ...
        { id: "mine", label: "\"You're mine now. I'll show you what that means.\"", note: "you take her, here, in his chair",
          run: (c) => { if (c.subj) resolveAct(c.s, c.subj, "vaginal"); c.treat(c.subj, "cruelty", 6, "taken in his chair the week he died"); c.set("heir_hg", "taken");
            return { text: `You take her over his desk. She doesn't fight. She keeps her eyes on the photograph of him on the shelf the whole time, and when you're done she straightens her robe and asks if there's anything else. There isn't. She goes. She doesn't come back to the office.`, next: "payday", after: 6 }; } },
        { id: "sell", label: "Sell her. You don't want his ghosts in the house.",
          run: (c) => { if (c.subj) c.remove(c.subj, "sold", "sold the week her owner died", Math.round(valuePerson(c.s, c.subj) * 0.9)); c.set("heir_hg", "sold"); c.hope(-6);
            return { text: `A broker takes her on Thursday. She packs one bag and doesn't ask where she's going. The rest of the household watches her leave and draws its own conclusions.`, next: "payday", after: 6 }; } },
      ],
    },
    ledger: {
      ...
      options: [
        ...
        { id: "burn", label: "Burn it. It's his sin, not yours.",
          run: (c) => { c.set("heir_burned", true); c.treat(c.subj, "kindness", 3, "you kept his secret");
            return { text: `It takes a while to catch. ${c.sn} watches from the doorway and doesn't say anything, but she nods once when it's ash.`, next: "payday", after: 5 }; } },
      ],
    },
    payday: {
      ...
      options: [
        { id: "all", label: (c) => `Pay it all — ${money(heirDebt(c))}`, need: (c) => cash(heirDebt(c))(c),
          run: (c) => { c.cash(-heirDebt(c)); c.set("heir_debt", 0); c.like("creditor", 20); c.rep(200);
            return { text: `${c.n("creditor")} counts it twice, which is an insult, and then shakes your hand, which is an apology. "He'd have been surprised," ${c.he("creditor")} says. "I'll tell people."`, next: "end_paid", after: 0 }; } },
        ...
        { id: "time", label: "Ask for more time", need: (c) => (c.flag("heir_leverage") || c.skill("trading") >= 40 ? null : "needs trading 40, or leverage"),
          run: (c) => { c.like("creditor", -5);
            return { text: c.flag("heir_leverage") ? `${c.He("creditor")} knows what you have in the desk. "Take the time," ${c.he("creditor")} says, and it costs ${c.him("creditor")} something to say it.` : `You talk. ${c.He("creditor")} listens. At the end ${c.he("creditor")} gives you two months, and you both know it's because ${c.he("creditor")} thinks you'll be worth more then.`, next: "reckoning", after: 8 }; } },
...
Then ${c.he("creditor")} sends a note, handwritten. *${money(Math.max(heirDebt(c), 1))}, or the building. Friday.*`;
        ...
        return `${c.n("creditor")} is back, as promised, for the rest. ${money(heirDebt(c))}. ${c.He("creditor")}'s brought a bottle of something old from your father's year, which is either a courtesy or a threat.`;
      },
      options: [
        ...
        { id: "ruin", label: "Take the ledger to the Association", show: (c) => !!c.flag("heir_leverage") || !!c.flag("heir_ledger") && !c.flag("heir_burned") && !c.flag("heir_sold_ledger"),
          run: (c) => { c.set("heir_debt", 0); c.npc("creditor").status = "gone"; c.rep(400); c.standing(1);
            if (c.he("creditor") === "she") {
              ...
              return { text: `The Association moves faster than you expected. ${c.n("creditor")}'s accounts are frozen by Wednesday, and by the end of the month she's on a block in the Grand Exchange to cover what she owes. You buy her for a song. She stands in your office in a grey tunic and doesn't say a word.`, end: "the creditor was ruined, and ended up yours" };
            }
        ...
        { id: "kill", label: "Have it dealt with. Permanently.", need: (c) => (c.s.arcology.security >= 55 || c.s.arcology.mercenaries.hired ? null : "needs security 55 or mercenaries"),
          run: (c) => { c.set("heir_debt", 0); c.npc("creditor").status = "dead"; c.crime(5); c.rep(-150); c.household("coercion", 2, "the house heard what happened to the creditor");
            return { text: `It happens on a service stair at two in the morning and it's in the news by breakfast. Nobody asks you about it directly. The debt dies with ${c.him("creditor")}. So does some of the warmth in the way the concourse says your name.`, end: "the creditor was killed" }; } },
        { id: "sign", label: "Sign the building over", note: "the game goes on, but you answer to someone now",
          run: (c) => { c.s.arcology.ownership = Math.max(1, c.s.arcology.ownership - 30); c.set("heir_debt", 0); c.like("creditor", 30); c.rep(-300); c.out.push("−30% ownership");
            return { text: `You sign. ${c.n("creditor")} doesn't gloat. "You can stay on," ${c.he("creditor")} says. "Run it. You're better at it than he was." It's the kindest thing ${c.he("creditor")}'s ever said to you and you'd rather ${c.he("creditor")} hadn't.`, end: "you signed the building over and stayed on to run it" }; } },
      ],
    },
    end_paid: {
      title: "Paid",
      text: (c) => `Your father's debts are gone. The folder that could stop a door goes in the incinerator.${c.subj && c.subj.status === "owned" ? ` ${c.sn} watches it burn and then asks, carefully, what you'd like her to do now.` : ""}`,
      options: [
        { id: "hg", label: "Make her your Head Girl", show: (c) => !!c.subj && c.subj.status === "owned" && c.flag("heir_hg") === "kept",
          run: (c) => { if (c.subj) { c.subj.assignment = "be your Head Girl"; c.treat(c.subj, "recognition", 8, "made Head Girl of your house, not his"); }
            return { text: `She takes the post the way she took everything else — with a nod and no speech. But she moves her things out of his old room that night.`, end: "you paid your father's debts, and kept his head girl" }; } },
        { id: "done", label: "Pour a drink and go to bed",
          run: () => ({ text: `The building is yours. Actually yours. It feels exactly the same.`, end: "you paid your father's debts" }) },
      ],
    },
  },
};
...
Downstairs, ${c.sn} — one of ${c.n("deposed")}'s women, and yours now — is scrubbing the floor where the fighting was worst. She hasn't stopped scrubbing for four months.`,
      options: [
        ...
        { id: "shares", label: "Promise them shares in the building",
          run: (c) => { c.set("raider_shares", true); c.like("captain", 8);
            return { text: `"Shares," ${c.n("captain")} says, like ${c.he("captain")}'s tasting it. "They'll like that. They'll like it a lot less if it turns out to be a word." You'll have to make it more than a word.`, next: "envoy", after: 3 }; } },
        { id: "loot", label: "Let them take their pick of the old household for a week",
          run: (c) => { c.like("captain", 18); c.household("cruelty", 5, "given to the mercenaries for a week"); c.set("raider_loot", true); c.rumor(`the new owner gave ${c.n("deposed")}'s women to the soldiers`, -1);
            return { text: `The men cheer when ${c.n("captain")} tells them. The household doesn't make a sound. For a week the servants' quarters are loud at night and silent in the morning, and when it's over ${c.sn} goes back to scrubbing the same floor, harder.`, next: "envoy", after: 3 }; } },
        { id: "wait", label: "\"They'll be paid when the building pays.\"",
          run: (c) => { c.like("captain", -12); c.set("raider_mutter", true);
            return { text: `${c.n("captain")} doesn't argue. ${c.He("captain")} just nods and goes downstairs, and that evening there are fewer men on the doors than there were in the morning.`, next: "envoy", after: 3 }; } },
      ],
    },
    envoy: {
...
${c.sn} is standing behind you with a tray. You hear it rattle, once.`,
      options: [
        { id: "sell", label: "Take the money. Send her back.", note: "+¤80,000", show: (c) => !!c.subj && c.subj.status === "owned",
          run: (c) => { if (c.subj) c.remove(c.subj, "sold", `sent back to ${c.full("deposed")}`, 80000); c.set("raider_deal", true); c.like("deposed", 20);
            return { text: `${c.sn} goes down to the launch without a word. The envoy shakes your hand. That night the yacht's lights go off early. You'd like to believe it's leaving.`, next: "night", after: 8 }; } },
        { id: "no", label: "\"She stays. So do I.\"",
          run: (c) => { c.like("deposed", -20); c.treat(c.subj, "recognition", 6, "you refused to send her back to him");
            return { text: `The envoy closes the folder. "${c.He("deposed")} thought you'd say that." Behind you, ${c.sn} sets the tray down very gently, and when the envoy's gone she says the first thing she's ever said to you that wasn't an answer: "Thank you."`, next: "night", after: 7 }; } },
        { id: "hold", label: "Keep the envoy too", note: "she's worth something, and she knows things",
          run: (c) => { const p = c.addSlave({ seed: "envoy", age: 29, quality: 0.5, how: `came as ${c.full("deposed")}'s envoy and was kept`, hostile: true }); c.set("raider_envoy", p.id); c.like("deposed", -35);
            return { text: `Your men take her folder and her coat and her shoes. She doesn't scream, which you respect. ${p.name}, it turns out, has been ${c.n("deposed")}'s lawyer for six years and knows exactly how many guns are on that yacht.`, next: "squeeze", after: 1 }; } },
      ],
    },
    squeeze: {
      ...
      text: (c) => {
        ...
        return `${e?.name ?? "The envoy"} has been in a locked room for a day. She's sitting very straight on the edge of the bed when you come in.

"I'm not going to tell you anything," she says, "so you can skip whichever part of this you were planning."`;
      },
      options: [
        ...
        { id: "break", label: "Break her until she talks", note: "ugly, and it works",
          run: (c) => { const e = c.s.people[String(c.flag("raider_envoy"))]; if (e) { resolveAct(c.s, e, "discipline"); resolveAct(c.s, e, "restraint"); } c.treat(e, "cruelty", 8, "made to talk"); c.set("raider_plan", true);
            return { text: `It takes most of the night. By the end she's told you everything: forty men, the fishing fleet, the freight doors, the new moon. She tells you twice, to make sure you have it. She doesn't look at you after that.`, next: "night", after: 5 }; } },
        ...
        { id: "captain", label: "\"You handle it, Captain.\"", note: "how much does the captain like you?",
          run: (c) => {
            if (c.npc("captain").disposition >= 10) { c.npc("deposed").status = "owned"; c.like("captain", 10);
              return { text: `"On it." You listen to it happen on the radio. It's fast and ugly and it works, and at half past three ${c.n("captain")} says, "Got ${c.him("deposed")}."`, next: "fate", after: 0 }; }
            ...
            return { text: `There's a long pause on the radio. "Tell you what," ${c.n("captain")} says. "You handle it." The channel goes dead. By morning ${c.n("captain")} and ${c.his("captain")} men are gone, and so is ${c.n("deposed")}'s raid — they took each other's money and left together. You still have the building. Barely.`, next: "second", after: 6 };
          } },
      ],
    },
    second: {
      ...
      options: [
        { id: "sink", label: "Put it on the bottom of the harbour", need: (c) => (c.s.arcology.security >= 50 ? null : "needs security 50"),
          run: (c) => { c.npc("deposed").status = "dead"; c.rep(200); c.standing(-1); c.crime(3);
            return { text: `Divers, a limpet charge, and the yacht lists and goes down with its lights still on. ${c.n("deposed")} was aboard. The papers stop printing interviews.`, end: "you sank the old owner with his yacht" }; } },
        ...
        { id: "ignore", label: (c) => `Let ${c.him("deposed")} shout. You have a city to run.`,
          run: (c) => { c.standing(-1);
            return { text: `You let ${c.him("deposed")} shout. Over the next months the interviews get shorter and the yacht gets shabbier and one morning it's simply not there.`, end: "the old owner gave up and sailed away" }; } },
      ],
    },
    fate: {
      title: (c) => `${c.n("deposed")}`,
      text: (c) => `${c.full("deposed")} kneels on the penthouse carpet that used to be ${c.his("deposed")}. ${c.He("deposed")}'s bleeding from the scalp and hasn't stopped talking — lawyers, rights, the registry, the Association.

The household has come up to watch. Nobody told them to. ${c.subj && c.subj.status === "owned" ? `${c.sn} is at the front.` : ""}`,
      options: [
        { id: "hang", label: (c) => `Hang ${c.him("deposed")} from the atrium rail`, note: "the city will remember it",
          run: (c) => { c.npc("deposed").status = "dead"; c.rep(600); c.standing(-2); c.household("coercion", 3, "saw the old owner hanged"); c.hope(-5); c.treat(c.subj, "kindness", 5, "watched the man who owned her hang");
            return { text: `It's done at noon, where the shoppers can see. The household watches from the penthouse gallery. ${c.subj && c.subj.status === "owned" ? `${c.sn} watches longest.` : ""} Nobody broadcasts from the harbour after that.`, end: "you hanged the old owner in the atrium" }; } },
        { id: "ransom", label: (c) => `Ransom ${c.him("deposed")} back to ${c.his("deposed")} family`, note: "+¤60,000",
          run: (c) => { c.cash(60000); c.npc("deposed").status = "gone"; c.like("deposed", -40);
            return { text: `${c.His("deposed")} family pays within a week, grudgingly, in three instalments. ${c.n("deposed")} leaves in a hired car. ${c.He("deposed")}'ll hate you forever from somewhere far away, which is where you want ${c.him("deposed")}.`, end: "you ransomed the old owner" }; } },
        { id: "give", label: (c) => `Give ${c.him("deposed")} to ${c.his("deposed")} old household`, show: (c) => ownedAdults(c.s).length >= 2,
          run: (c) => { c.npc("deposed").status = "dead"; c.household("recognition", 6, "you let them have the man who owned them"); c.hope(8); c.rep(200);
            return { text: `You say it once, quietly, and leave the room. You don't ask what happens. In the morning the servants' quarters are calm for the first time since you came, and ${c.sn} has stopped scrubbing the floor.`, end: "you gave the old owner to the women he'd owned" }; } },
        { id: "keep", label: "Keep her. Collar her.", show: (c) => c.he("deposed") === "she",
          run: (c) => { const p = c.addSlave({ seed: "deposed", name: c.full("deposed"), age: 41, quality: 0.5, how: "owned this building until you took it, then tried to take it back", hostile: true }); c.npc("deposed").person = p.id; c.hope(6);
            return { text: `She screams when they put the collar on. Her old household listens from the stairs and nobody goes to help her. By evening she's in the servants' quarters in a grey tunic, and her women are teaching her how to fold sheets.`, end: "the old owner wears your collar" }; } },
      ],
    },
  },
};
...
Nine years older. Greyer. Still holding a glass by the stem like it might try something. ${c.He("old_owner")} owned you for four years. ${c.He("old_owner")} bought you for eleven hundred and sold you for nine, and in between ${c.he("old_owner")} taught you most of what you know about this trade by doing it to you.
      ...
      options: [
        { id: "toast", label: "Walk over and toast back",
          run: (c) => { c.like("old_owner", 10); c.set("climber_faced", true);
            return { text: `"Look at you," ${c.he("old_owner")} says, and actually sounds pleased. "I always said you'd go far. I didn't think it'd be up." ${c.He("old_owner")} asks about your building like an uncle. You leave first. Your hands don't shake until the lift.`, next: "papers", after: 3 }; } },
        { id: "cut", label: (c) => `Look straight through ${c.him("old_owner")} and leave`,
          run: (c) => { c.like("old_owner", -15); c.set("climber_cut", true);
            return { text: `You put your glass down on a waiter's tray and walk out. Behind you somebody laughs — not ${c.n("old_owner")}; somebody near ${c.him("old_owner")} — and the laugh follows you all the way home.`, next: "papers", after: 3 }; } },
        { id: "tell", label: (c) => `Tell the room who ${c.he("old_owner")} is to you`,
          run: (c) => { c.like("old_owner", -30); c.rep(-100); c.standing(1); c.set("climber_public", true);
            return { text: `You say it loud enough for the table: "${c.full("old_owner")}. My first owner. ${c.He("old_owner")} used to make me sleep in the laundry." The table goes quiet. ${c.n("old_owner")} smiles very thinly and says, "And look how well it turned out." Half the room thinks you're vulgar. The other half thinks you're brave. None of them will forget it.`, next: "papers", after: 3 }; } },
      ],
    },
    papers: {
...
Inside is a photocopy of your indenture. Your old name. Your old photograph — younger, thinner, frightened. And a registry stamp from ${c.arcology}'s own office, which nobody ever voided when you bought your way out, because nobody ever thought they'd need to.
      ...
      options: [
        { id: "void", label: "Pay the registrar to void it quietly", note: "¤25,000", need: cash(25000),
          run: (c) => { c.cash(-25000); c.set("climber_void", true);
            return { text: `The registrar takes your money with the expression of someone who has done this before for people you'd recognise. The record disappears. The original is still in ${c.n("old_owner")}'s safe, but in ${c.arcology} it no longer means anything.`, next: "claim", after: 4 }; } },
        { id: "hack", label: "Erase it from the registry yourself", need: skill("hacking", 30),
          run: (c) => { c.set("climber_void", true); c.set("climber_hacked", true);
            return { text: `It takes a night and most of a pot of coffee. When you're done, the person in the photograph never existed in ${c.arcology}'s records. You sit in the dark for a while afterwards.`, next: "claim", after: 4 }; } },
        ...
        { id: "laugh", label: "Throw it in the bin",
          run: (c) => { c.set("climber_ignored", true);
            return { text: `It's a piece of paper. You're the owner of an arcology. You throw it in the bin and go back to work, and it's still in the bin at the end of the day because you keep looking at it.`, next: "claim", after: 4 }; } },
      ],
    },
    claim: {
      ...
      text: (c) => c.flag("climber_void")
        ...
        : `The registrar calls you personally, which registrars never do. "Someone has filed a claim of ownership," she says carefully. "On you. With a valid original. I'm obliged to act on it in thirty days unless it's contested."
...
${c.n("old_owner")} has sent flowers.`,
      options: [
        { id: "own", label: "Say it in public before they can", show: (c) => !!c.flag("climber_void"),
          run: (c) => { c.rep(200); c.standing(2); c.household("recognition", 4, "you told the city what you used to be"); c.hope(6); c.like("old_owner", -10);
            return { text: `You give a short interview on the concourse. Yes, you were a slave. Yes, for six years. Yes, you own the building now. Your household hears it on the screens in the dormitory, and ${c.subj?.name ?? "one of them"} asks you that night, very quietly, whether it's true. You tell her it is.`, next: "end", after: 2 }; } },
        { id: "court", label: "Contest it in the registry court", show: (c) => !c.flag("climber_void"), note: "¤15,000 in lawyers", need: cash(15000),
          run: (c) => { c.cash(-15000); const household = ownedAdults(c.s); const devoted = household.filter((p) => read(p).devotion > 25).length;
            ...
            c.rep(-200);
            return { text: `You win on a technicality after a week of hearings where your household is asked, under oath, what you're like to them. Not enough of them said anything good. ${c.n("old_owner")} smiled through all of it.`, next: "end", after: 2 }; } },
        { id: "night", label: (c) => `Give ${c.him("old_owner")} the night ${c.he("old_owner")} asked for`, show: (c) => !c.flag("climber_void") && !!c.flag("climber_called"), note: "and get the original",
          run: (c) => { c.set("climber_knelt", true); c.like("old_owner", 25);
            return { text: `You go to ${c.his("old_owner")} apartment alone. It's exactly what ${c.he("old_owner")} said it would be, and you still know how. In the morning ${c.he("old_owner")} burns the original in an ashtray in front of you and pours you coffee. You drink it. You don't tell anyone, ever.`, next: "end", after: 2 }; } },
      ...
      text: (c) => `${c.subj?.name ?? "One of your women"} brings your coffee in the morning and doesn't leave.
      ...
      options: [
        { id: "truth", label: "Tell her the truth, all of it",
          run: (c) => { c.treat(c.subj, "recognition", 8, "you told her what you used to be"); c.set("climber_told", true);
            return { text: `You tell her about the laundry, and the block, and what you did to get out. She listens without interrupting. At the end she says, "So it's possible," and you realise she means getting out, and you don't correct her.`, end: "you made your peace with your papers" }; } },
        { id: "strong", label: "\"It made me who I am. It'll do the same for you.\"",
          run: (c) => { c.treat(c.subj, "coercion", 3, "told her it would make her strong");
            return { text: `She nods like she's been given an instruction. You watch her go and think about whether that's what you meant.`, end: "you told the house what you used to be, and what you are now" }; } },
        { id: "none", label: "\"That's not your concern.\"",
          run: (c) => { c.treat(c.subj, "neglect", 2, "told it wasn't her concern");
            return { text: `"No," she agrees, and goes. That evening you hear the others asking her what you said, and her saying, "Nothing."`, end: "you kept your past to yourself" }; } },
      ],
    },
  },
};
...
const broker: ArcDef = {
  ...
  beats: {
    book: {
      ...
      text: (c) => `${c.full("rival_broker")} comes to see the building. ${c.He("rival_broker")} admires everything out loud and prices it under ${c.his("rival_broker")} breath.
...
It's a fair price. That's what worries you.`,
      options: [
        { id: "sell", label: "Sell it", note: "+¤40,000",
          run: (c) => { c.cash(40000); c.set("broker_sold_book", true); c.like("rival_broker", 20);
            return { text: `${c.n("rival_broker")} flips through it on the spot, nodding. "I'll look after them," ${c.he("rival_broker")} says. Twelve years of your life, in ${c.his("rival_broker")} jacket pocket.`, next: "block", after: 5 }; } },
        { id: "fake", label: (c) => `Sell ${c.him("rival_broker")} a book full of dead numbers`, need: skill("trading", 50), note: "+¤40,000, and an enemy when it's found out",
          run: (c) => { c.cash(40000); c.set("broker_fake", true); c.like("rival_broker", 10);
            return { text: `You spend a night writing it. Half the captains are dead, one's in prison, the man in Odessa is a police informant. ${c.n("rival_broker")} pays in cash and leaves happy. ${c.He("rival_broker")}'ll be happy for about a month.`, next: "block", after: 5 }; } },
        ...
        { id: "partner", label: "Offer a partnership instead", need: skill("trading", 35),
          run: (c) => { c.set("broker_partner", true); c.like("rival_broker", 15);
            return { text: `You'll source, ${c.he("rival_broker")}'ll ship, you split it. ${c.n("rival_broker")} thinks about it for the length of a cigarette and then shakes your hand. Your market prices will be kinder for a while.`, next: "block", after: 5 }; } },
      ],
    },
    block: {
      title: "On the block",
...
She sees you in the second row. She doesn't look away.`,
      options: [
        { id: "buy", label: "Buy her", note: "¤7,000", need: cash(7000),
          run: (c) => { c.cash(-7000); const p = c.addSlave({ seed: "sold_one", name: c.full("sold_one"), age: 34, quality: 0.1, how: "you sold her eight years ago; you bought her back", hostile: true }); c.npc("sold_one").status = "owned"; c.npc("sold_one").person = p.id; p.health.health = -10;
            return { text: `Nobody bids against you. She's brought up to your office in the evening. She stands with her hands at her sides and says, "You told me it was a good house."`, next: "yours", after: 0 }; } },
        { id: "free", label: "Buy her freedom through a lawyer, anonymously", note: "¤12,000", need: cash(12000),
          run: (c) => { c.cash(-12000); c.npc("sold_one").status = "gone"; c.set("broker_freed", true);
            return { text: `A lawyer you've never met buys lot fourteen and walks her out of the building with a ticket and a sealed envelope. She never finds out who paid. You tell yourself that's the point.`, next: "rival", after: 6 }; } },
        { id: "pass", label: "Look at your programme until the lot is over",
          run: (c) => { c.set("broker_passed", true);
            return { text: `The bidding is slow. She goes for five thousand to a buyer in a good suit — ${c.full("rival_broker")}'s man. As she's led off she looks back at the second row one more time.`, next: "rival", after: 5 }; } },
      ],
    },
    yours: {
      title: (c) => `${c.n("sold_one")}`,
      text: (c) => `She's been in the house a week. She works. She doesn't talk to you unless she has to. The others have noticed and they're careful around her, the way you're careful around a dog that's been hit.
      ...
      options: [
        { id: "sorry", label: "Tell her you're sorry",
          run: (c) => { const p = c.s.people[c.npc("sold_one").person ?? ""]; c.treat(p, "kindness", 5, "you said you were sorry"); c.set("broker_sorry", true);
            return { text: `She laughs, which you didn't expect. "Sorry," she says. "I had a baby in that good house. They took him at six weeks." She waits to see what your face does. Then she goes to bed.`, next: "rival", after: 4 }; } },
        { id: "free", label: "Free her", note: "she leaves the household",
          run: (c) => { const p = c.s.people[c.npc("sold_one").person ?? ""]; if (p) c.remove(p, "free", "freed by the broker who sold her"); c.hope(8); c.set("broker_freed", true);
            return { text: `You hand her the papers. She reads every line. At the door she stops. "It doesn't make us even," she says. "But it's something." Your household watches her walk out of the front of the building like a citizen.`, next: "rival", after: 4 }; } },
        { id: "hg", label: "Make her your Head Girl", note: "she knows exactly how houses go wrong",
          run: (c) => { const p = c.s.people[c.npc("sold_one").person ?? ""]; if (p) { p.assignment = "be your Head Girl"; p.skills.management = { ...p.skills.management, general: 50 }; } c.treat(p, "recognition", 8, "made Head Girl by the broker who sold her");
            return { text: `She stares at you. Then she says, "Fine," and by the end of the week she has rearranged the entire household, fired the cook, and put the two youngest girls in rooms with locks on the inside.`, next: "rival", after: 4 }; } },
        { id: "use", label: "\"You're mine again. Act like it.\"",
          run: (c) => { const p = c.s.people[c.npc("sold_one").person ?? ""]; if (p) resolveAct(c.s, p, "orders"); c.treat(p, "cruelty", 5, "told to act like property");
            return { text: `"Yes," she says. "I remember how." She kneels right there in the corridor, and it is the most frightening thing you've seen in years.`, next: "rival", after: 4 }; } },
      ],
    },
    rival: {
      ...
      options: [
        { id: "buy", label: (c) => `Buy her from ${c.him("rival_broker")}, whatever it costs`, show: (c) => !!c.flag("broker_passed"), note: "¤20,000", need: cash(20000),
          run: (c) => { c.cash(-20000); const p = c.addSlave({ seed: "sold_one2", name: c.full("sold_one"), age: 34, quality: 0.1, how: "sold by you, bought by your rival, bought back by you", hostile: true }); c.npc("sold_one").person = p.id; c.like("rival_broker", -10);
            return { text: `${c.n("rival_broker")} names a price that's an insult and you pay it without blinking, in front of everyone. ${c.n("sold_one")} unclips her own leash and hands it to ${c.him("rival_broker")} on the way past.`, end: "you bought back the woman you sold, twice" }; } },
        ...
        { id: "peace", label: (c) => `Meet ${c.him("rival_broker")} and make peace`, need: skill("trading", 40),
          run: (c) => { c.like("rival_broker", 30); c.set("broker_partner", true);
            return { text: `You meet on a boat, which is neutral ground for people like you. By the second bottle you've carved up the Gulf between you like it's a roast. It's not friendship. It's better than friendship: it's business.`, end: "you made peace with your rival" }; } },
...
const investor: ArcDef = {
  ...
  beats: {
    call: {
      ...
      text: (c) => `The fund's video calls always start with the chair's face frozen for three seconds, which ${c.full("chair")} says is the connection and you suspect is a power move.
...
${c.He("chair")} unfreezes. "Questions?"`,
      options: [
        { id: "double", label: "\"I'll beat the numbers by a third.\"", note: "harder targets; bigger reward",
          run: (c) => { c.set("inv_base", c.s.arcology.cash); c.set("inv_bold", true); c.like("chair", 15);
            return { text: `${c.n("chair")} writes something down, which ${c.he("chair")} never does. "Then we'll hold you to it." The call ends. Your targets have gone up.`, next: "q1", after: 13 }; } },
...
${hit ? `${c.n("chair")} scrolls through the figures in silence and then says, "Well." It's the warmest thing ${c.he("chair")} has ever said.` : `${c.n("chair")} scrolls through the figures in silence. "You're short," ${c.he("chair")} says. "I'd like to hear why."`}`;
      },
      options: [
        ...
        { id: "cook", label: "Fix the numbers before the call", show: (c) => c.s.arcology.cash < target(c, 1), need: skill("hacking", 30),
          run: (c) => { c.set("inv_cooked", true); c.set("inv_hits", 1);
            return { text: `By nine the numbers say what they need to say. ${c.n("chair")} is satisfied. Somewhere in the fund's systems there's now a difference between the building on paper and the building you're sitting in, and it has your fingerprints on it.`, next: "visit", after: 6 }; } },
        { id: "sell", label: "Sell someone before the call to make the number", show: (c) => c.s.arcology.cash < target(c, 1),
          ...
          run: (c, p) => { if (!p) return { text: "" }; const v = Math.round(valuePerson(c.s, p)); c.remove(p, "sold", "sold to make a quarterly number", v); c.hope(-5);
            ...
            return { text: `${p.name} is on a transport by eight. At nine, you ${hit ? "make the number" : "are still short, but closer"}. ${c.n("chair")} says nothing about how.`, next: "visit", after: 6 }; } },
        { id: "short", label: "Explain, honestly", show: (c) => c.s.arcology.cash < target(c, 1),
          run: (c) => { c.set("inv_hits", 0); c.like("chair", -5);
            return { text: `${c.n("chair")} listens. "One miss," ${c.he("chair")} says. "Everyone gets one." ${c.He("chair")} doesn't say what happens after that. ${c.He("chair")} doesn't need to.`, next: "visit", after: 6 }; } },
      ],
    },
    visit: {
      title: (c) => `${c.n("chair")} comes to see the building`,
      text: (c) => `${c.full("chair")} is smaller in person and walks faster than you. ${c.He("chair")} wants to see everything: the brothel, the kitchens, the dormitories, the books. ${c.He("chair")} asks the Madam three questions you didn't know the answers to.

At the end of the day, in the penthouse, ${c.he("chair")} takes ${c.his("chair")} shoes off and looks at your household the way you look at a menu.`,
      options: [
        { id: "offer", label: "Offer one of them for the night",
          ...
          run: (c, p) => { if (!p) return { text: "" }; resolveAct(c.s, p, "oral"); c.treat(p, "coercion", 3, `given to the fund's chair for a night`); c.like("chair", 25);
            return { text: `${p.name} goes with ${c.n("chair")} to the guest suite. In the morning ${c.n("chair")} is in an excellent mood and ${p.name} is very quiet. The next review, you suspect, will be generous.`, next: "q2", after: 7 }; } },
        ...
        { id: "cold", label: "Keep it professional and send them to their hotel",
          run: (c) => { c.like("chair", -5);
            return { text: `${c.n("chair")} puts ${c.his("chair")} shoes back on without a word. In the car ${c.he("chair")} makes a call you can't hear.`, next: "q2", after: 7 }; } },
      ],
    },
    q2: {
      ...
      options: [
        ...
        { id: "buyout", label: "Buy the fund out", note: "¤250,000", need: cash(250000),
          run: (c) => { c.cash(-250000); c.s.arcology.ownership = Math.min(100, c.s.arcology.ownership + 30); c.npc("chair").status = "gone"; c.out.push("+30% ownership");
            return { text: `${c.n("chair")} is genuinely surprised, which you'll treasure. The fund sells. It's your building now, all of it that matters, and nobody reviews you ever again.`, end: "you bought the building out from under the fund" }; } },
        { id: "fight", label: "Refuse to hand over and dare them to remove you", show: (c) => !q2hit(c), need: (c) => (c.s.arcology.security >= 50 ? null : "needs security 50"),
          run: (c) => { c.npc("chair").disposition = -60; c.rep(-200); c.standing(1); c.set("inv_seized", true);
            return { text: `The replacement arrives on Friday with two lawyers and is met at the lift by your guards. They go back down. The fund can sue you in a court the arcology doesn't recognise. The city finds the whole thing thrilling.`, end: "you seized the building from the fund that owned it" }; } },
        { id: "go", label: "Go quietly", show: (c) => !q2hit(c), note: "you stay as the owner's manager; the fund takes a cut",
          run: (c) => { c.s.arcology.ownership = Math.max(1, c.s.arcology.ownership - 15); c.out.push("−15% ownership"); c.like("chair", 10);
            return { text: `The replacement turns out to be a nervous man called Gerald who knows nothing about slaves. Within a month he's asking you to run it for him. You do, for a cut. It's a smaller cut than before.`, end: "the fund took more of the building, and you run it for them" }; } },
      ],
    },
    final: {
...
"I'm leaving the fund," ${c.he("chair")} says. "Starting my own. I want you with me, or I want you to stay here and be my competition. Either's fine. I just want to know which."`,
      options: [
        ...
        { id: "rival", label: "\"Competition, then.\"", run: (c) => { c.like("chair", -5); c.rep(200);
          return { text: `${c.n("chair")} laughs, genuinely. "Good. I'd have been disappointed." You're both going to enjoy this.`, end: "you and the chair are rivals now" }; } },
      ],
    },
  },
};
...
${c.He("sibling")} looks at your household the way ${c.he("sibling")} looked at your toys when you were children: already deciding which ones ${c.he("sibling")}'d take.`,
      options: [
        { id: "party", label: "Throw a party in their honour", note: "¤8,000", need: cash(8000),
          run: (c) => { c.cash(-8000); c.rep(300); c.like("sibling", 5); c.set("prod_party", true);
            return { text: `Half the Association comes. ${c.n("sibling")} is charming to everyone and tells three of them stories about you as a child that are true and humiliating. The papers call it the party of the season. Mother hears about it by morning, which was the point.`, next: "letter", after: 4 }; } },
        { id: "gift", label: "Give them one of your women for the week",
          ...
          run: (c, p) => { if (!p) return { text: "" }; c.like("sibling", 20); c.treat(p, "coercion", 4, "lent to the owner's sibling for a week"); c.set("prod_lent", p.id);
            return { text: `${c.n("sibling")} is delighted, in the way ${c.he("sibling")} used to be delighted with borrowed things. ${p.name} comes back at the end of the week with a new bracelet and nothing to say about it.`, next: "letter", after: 4 }; } },
...
It's signed with her full name, as though you might not know who she was.`,
      options: [
        ...
        { id: "ally", label: "Propose to your sibling that you split it", need: skill("trading", 25), run: (c) => { c.set("prod_ally", true); c.like("sibling", 20);
          return { text: `${c.n("sibling")} is quiet on the phone for a long time. "She'll hate that," ${c.he("sibling")} says at last. "All right." For the first time since you were twelve, you're on the same side.`, next: "move", after: 8 }; } },
        { id: "refuse", label: "Write back that you don't want it", run: (c) => { c.set("prod_refused", true); c.rep(100);
          return { text: `*Keep it,* you write. *I have a city.* You post it before you can change your mind. ${c.n("sibling")} sends flowers, which is somehow the most insulting thing ${c.he("sibling")}'s ever done.`, next: "move", after: 8 }; } },
      ],
    },
    move: {
...
She could have kept it. She brought it to you.`,
      options: [
        { id: "reward", label: "Reward her for bringing it to you", show: (c) => !c.flag("prod_ally"),
          run: (c) => { c.treat(c.subj, "recognition", 8, "she brought you your sibling's note instead of taking the offer"); c.set("prod_loyal", true);
            return { text: `You ask her what she wants. She asks for a window. You give her a room with one, facing the sea. She doesn't say thank you; she just looks at it for a long time.`, next: "will", after: 10 }; } },
        { id: "freeher", label: "Let her take the offer, if she wants it", show: (c) => !c.flag("prod_ally"),
          run: (c) => { const stays = c.subj ? read(c.subj).devotion > 35 : false;
            if (stays) { c.treat(c.subj, "promise_kept", 8, "offered freedom, and stayed"); c.hope(5); return { text: `She reads the note again and puts it in the fire. "I don't want ${c.his("sibling")} flat," she says. "I want this one." Word goes round the house by morning.`, next: "will", after: 10 }; }
        ...
        { id: "back", label: "Return the favour: bribe their household manager", need: cash(15000), show: (c) => !c.flag("prod_ally"),
          run: (c) => { c.cash(-15000); c.like("sibling", -20); c.set("prod_sabotage", true);
            return { text: `${c.n("sibling")}'s manager starts sending you the weekly accounts. You have your sibling's whole operation on your desk every Monday. It's thinner than you thought.`, next: "will", after: 10 }; } },
        { id: "trust", label: "\"I thought about it too.\"", show: (c) => !!c.flag("prod_ally"),
          run: (c) => { c.like("sibling", 15);
            return { text: `${c.n("sibling")} laughs, and it's the old laugh, from before. "Good," ${c.he("sibling")} says. "Then neither of us is stupid." You hang up still allies.`, next: "will", after: 10 }; } },
        { id: "betray", label: "Break the alliance first", show: (c) => !!c.flag("prod_ally"),
          run: (c) => { c.like("sibling", -50); c.set("prod_betrayed", true); c.set("prod_race", true); c.set("prod_start", worth(c.s) - 50000);
            return { text: `You call your mother's lawyer that afternoon. By evening ${c.n("sibling")} knows. ${c.He("sibling")} doesn't call. ${c.He("sibling")} never calls again.`, next: "will", after: 10 }; } },
      ],
    },
    will: {
      ...
      text: (c) => {
        ...
        return `Rain on the family plot, and a hundred people under black umbrellas pretending they came for her.

The lawyer reads it in the library afterwards. Your holding is valued at ${money(mine)}. ${c.n("sibling")}'s at ${money(theirs(c))}.${c.flag("prod_ally") && !c.flag("prod_betrayed") ? " Neither of you broke the alliance. The lawyer reads that clause twice, as if the words might change." : ""}`;
      },
      options: [
        { id: "win", label: "Take what's yours", show: (c) => !!won(c) && !c.flag("prod_ally") || !!c.flag("prod_betrayed") && !!won(c),
          run: (c) => { c.cash(200000); c.rep(1000); c.like("sibling", -30); c.npc("sibling").status = "gone";
            return { text: `The coast is yours. ${c.n("sibling")} leaves the library without looking at you, and you realise you'd expected to feel more than you do.`, end: "you inherited the coast" }; } },
        ...
        { id: "lose", label: "Congratulate them", show: (c) => !won(c) && !(c.flag("prod_ally") && !c.flag("prod_betrayed")),
          run: (c) => { c.like("sibling", 15); c.rep(-200);
            return { text: `You shake ${c.n("sibling")}'s hand in front of everyone. ${c.He("sibling")} holds on a second too long. "You did better than I thought you would," ${c.he("sibling")} says, and means it, and that's worse.`, end: "your sibling inherited the coast" }; } },
        { id: "gave", label: "You already said you didn't want it", show: (c) => !!c.flag("prod_refused"),
          run: (c) => { c.like("sibling", 25); c.rep(200);
            return { text: `The lawyer reads your letter aloud. There's a murmur. ${c.n("sibling")} looks at you across the table like ${c.he("sibling")}'s never seen you before.`, end: "you gave up the coast for your city" }; } },
      ],
    },
  },
};
