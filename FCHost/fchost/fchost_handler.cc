...
void FCHostHandler::OnLoadEnd(CefRefPtr<CefBrowser> browser,
                              CefRefPtr<CefFrame> frame,
                              int httpStatusCode) {
    CEF_REQUIRE_UI_THREAD();

    /* CEF doesn't implement a native '<input type=color>' handler, so we just hack one into the javascript after the page is loaded.  If this ever gets fixed upstream in CEF, we can lose all this code.
     * We'll initialize tinyColorPicker, attach a mutation observer to the entire document, and anytime a new color input is added, we'll steal its events and connect it to TCP.
     * Note that this copy of TCP is slightly modified to avoid event loops since we need to fire onChange events in the render callback for FC itself, so don't blindly try to update it. */
    frame->ExecuteJavaScript(
        "if (!window.FCHostColorMutationObserver) {\
            ...
            window.FCHostColorMutationObserver = new MutationObserver((mutationList) => { \
                $('input[type=color]').colorPicker({doRender: false, renderCallback: function ($elm, toggled) { const evt = new Event('change', {target:$elm[0], bubbles:false}); $elm[0].dispatchEvent(evt); }});\
            });\
            window.FCHostColorMutationObserver.observe(document.body.parentElement, {childList: true, subtree: true});\
        }", frame->GetURL(), 0);
}
