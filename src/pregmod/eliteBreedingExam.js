...
App.Interact.eliteBreedingExam = function(slave = null) {
	...
	if (V.arcologies[0].FSHedonisticDecadenceLaw2 === 1){
		checks.push(
			[
				slave => slave.butt >= 5,
				`Must have enormous butt`
			],
			[
				slave => slave.butt >= 2000,
				`Must have enormous breasts`
			],
		);
	} else if (V.arcologies[0].FSGenderFundamentalistLawBeauty + V.arcologies[0].FSGenderRadicalistLawBeauty > 0){
		if (V.arcologies[0].FSAssetExpansionist > 20) {
			checks.push(
				[
					slave => slave.butt.isBetween(5, 8, true),
					`Must have huge butt`
				],
				[
					slave => slave.boobs.isBetween(1600, 4300, true),
			...
			checks.push(
				[
					slave => slave.butt.isBetween(3, 4, true),
					`Must have a healthy butt`
				],
				[
					slave => slave.boobs.isBetween(500, 800, true),
					`Must have nice breasts`
				],
			);
		}
		...
		if (V.arcologies[0].FSSlimnessEnthusiastLaw === 1) {
			checks.push([
				slave => slave.butt <= 1,
				`Must have a flat rear`
			]);
			checks.push([
				slave => slave.boobs <= 300,
				`Must have a flat chest`
			]);
		} else {
			checks.push([
				slave => slave.butt <= 2,
