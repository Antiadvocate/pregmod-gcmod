...
App.UI.editGenetics = function() {
	...
	/**
	 * @param {FC.GenePoolRecord} s the slave to get the full name of
	...
	/**
	 * @param {ReadonlyDeep<Partial<FC.GenePoolRecord>>} s the slave to get the full name of
	...
	/**
	 * @param {ReadonlyDeep<Partial<FC.GenePoolRecord>>} s
	 * @returns {HTMLButtonElement}
	 */
	function nameButton(s) {
		const button = App.UI.DOM.makeElement("button", currentFullName(s), "slavepicker");
		button.setAttribute("data-slave", String(s.ID));
		button.style.width = "100%";
		return button;
	}

	/**
	 * @param {FC.GenePoolRecord} s
	...
	/**
	 * @param {FC.GenePoolRecord} s the slave to describe
	 * @returns {string} their age description
	 */
	function ageDesc(s) {
		let age = s.actualAge + s.birthWeek/52.0;
	 ...
	 */
	function chemicalsDesc(s) {
		let res = [];
		if (s.chem > 0) {
			res.push('Immune system damaged (' + s.chem.toFixed(0) + ').');
		}
		if (s.addict > 0) {
			res.push('Addicted to aphrodisiacs (' + s.addict.toFixed(0) + ').');
		}
		return res.join(' ');
	}

	/**
	 * @param {FC.HumanID} id The id of the parent slave
	...
	jQuery(function() {
		...
		_(Object.values(V.genePool)).sortBy(['birthName', 'birthSurname']).forEach(function(s) {
		...
		jQuery('.slavepicker').each(function() {
			let el = jQuery(this);
			let id = /** @type {FC.HumanID} */ (Number(el.attr('data-slave')));
