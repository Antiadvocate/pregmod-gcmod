globalThis.FutureSocieties = (function() {
	...
	function validAdoptions(arcID) {
		...
		// apply game rules
		if (!V.seeIncest) {
			societies.deleteAll("FSIncestFetishist");
		}
		if (!V.seePreg) {
			societies.deleteAll("FSRepopulationFocus");
		}
	...
	function diplomaticFSes(arc1, arc2) {
		...
		if (shared.includes("FSSupremacist")) {
			...
			if (arc1.FSSupremacistRace !== arc2.FSSupremacistRace) {
				shared.deleteAll("FSSupremacist");
		...
		if (shared.includes("FSSubjugationist")) {
			...
			if (arc1.FSSubjugationistRace !== arc2.FSSubjugationistRace) {
				shared.deleteAll("FSSubjugationist");
	...
	function ValidateFacilityDecoration(facility) {
		...
		// Do not add new BC to this. Create a patch following the instructions in `/src/data/patches/patch.js` instead.
		// @ts-ignore BC
	...
	function FSHighestDecoration() {
		...
		decorationList.deleteAll(`FSNullDecoration`); // FSNull doesn't have decorations
		...
		const arcSupportTexts = (fs, arc) => {
			switch (fs) {
				...
				case "FSIntellectualDependency": return [
					`${arc.name} believes implicitly that all slaves should be mentally dependent on their owner.`,
