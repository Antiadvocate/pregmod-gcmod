App.UI.OptionsGroup = (function() {
	...
	class OptionButtonRow extends Row {
		...
		/**
		 * Add a TextBox where the user can input a value.
		 *
		 * By default the TextBox will only accept inputs of the same Type as previous options(if any)
		 * @param {object} [params]
		 * @param {string} [params.unit] Will be displayed next to the TextBox
		 * @param {boolean} [params.large=false] If set to true the TextBox will render as 'full-width'
		 * @param {boolean} [params.forceString=false] Force the TextBox to accept String inputs
		 *
		 * Note: If forceString is true any value entered into the TextBox will be interpreted as a String.
		 * @returns {OptionButtonRow}
		 */
		...
		/**
		 * @param {function(any):void} callback gets executed on every click of this button. Doesn't get executed for other buttons. Selected value is given as argument.
		 */
	...
	return class OptionsGroup {
		 ...
		 * Can be overwritten by some options, the behavior is the same as with the default function.
		 *
		 * @param {function():void} refresh
		 * @returns {OptionsGroup}
		 */
