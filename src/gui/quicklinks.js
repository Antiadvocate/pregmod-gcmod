...
App.UI.quickMenu = (function() {
	// setup passage lists
	const jumpFrom = getPassagesWithTag("jump-from-safe").map(passage => passage.title);
	const jumpTo = getPassagesWithTag("jump-to-safe").map(passage => passage.title);
	const hidden = getPassagesWithTag("jump-hidden").map(passage => passage.title);
	const noHistory = getPassagesWithTag("no-history").map(passage => passage.title);
	...
	const notifications = cleanPassageMapping({
		"Future Society": () => V.sideBarOptions.notifyFS === 1 && (FutureSocieties.availCredits() > 0 || V.FSReminder),
		"Incubator": () => V.sideBarOptions.notifyIncubator === 1 && V.incubator.capacity > 0 && App.Facilities.Incubator.readySlaves() > 0,
		"Manage Corporation": () => V.sideBarOptions.notifyCorp === 1 && V.corp.SpecToken > 0 && V.corp.SpecTimer === 0,
	});
