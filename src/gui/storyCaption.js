...
App.UI.StoryCaption.render = function() {
	const pass = passage();
	const passageSafe = getPassagesWithTag("jump-from-safe")
	...
	if (V.ui !== "start") {
		...
		const reminders = App.UI.DOM.makeElement("span", App.UI.DOM.link("Reminders", () => App.Reminders.dialog()), null);
		if (V.reminders.find(r => r.week <= V.week)) {
			reminders.append(" ", App.UI.DOM.makeElement("span", "\uf06a", ["icons", "noteworthy"]));
		}
		fragment.append(
			App.UI.DOM.generateLinksStrip([
				reminders,
				App.UI.DOM.link("Log", App.PersonalLog.dialog)
			])
		);
	...
	function slavesGrid(container) {
		...
		App.UI.DOM.appendNewElement("div", container, `${slaveCount()}`);
	}
	...
	function startingGirls() {
		...
		// @ts-expect-error In starting girls we know that there is always an active slave
		let cost = startingSlaveCost(V.temp.activeSlave);
		...
		if (V.temp.limitedCheatStart || V.cheatMode) {
