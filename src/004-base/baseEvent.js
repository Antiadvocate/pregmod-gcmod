...
App.Events.BaseEvent = class BaseEvent {
	...
	toJSON() {
		...
		return Serial.createReviver(`new App.Events.${this.constructor.name}($ReviveData$.actors, $ReviveData$.params)`, reviveData);
	}
	...
	castActors(firstActor) {
		...
		for (; i < prereqs.length; ++i) {
			const qualified = getSlaves().filter(s => !this.actors.includes(s.ID) && prereqs[i].every(p => p(s)));
			if (qualified.size === 0) {
				return false; // a required actor was not found
			}
			this.actors.push(qualified.pluck().key);
		}
