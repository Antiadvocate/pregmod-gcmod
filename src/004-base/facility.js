...
/** @type {{[key: string]: FC.FacilityFramework}} */
App.Data.Facilities = {};
...
App.Entity.Facilities.Job = class {
	...
	assignmentLinkElement(ID, passage, callback, linkText) {
		...
		return App.UI.DOM.assignmentLink(getSlave(ID), this.desc.assignment, passage, callback, linkText);
	}
	 ...
	 */
	employees() {
		return [...this.employeesIDs()].map(id => getSlave(id, true));
	}
...
App.Entity.Facilities.Facility = class {
	...
	constructor(desc, jobs, manager) {
		...
		this._jobs = {};
		/** @protected @type {{[key: FC.Assignment]: App.Entity.Facilities.Job}} */
		this._jobsByAssignment = {};
		...
		for (const jn in this.desc.jobs) {
			...
			const job = this._jobs[jn];
			job.facility = this;
			job.desc = desc.jobs[jn];
			this._jobsByAssignment[job.desc.assignment] = job;
		}
	...
	set capacity(capacity) {
		const stateObj = V[this.desc.baseName];
		if (stateObj && typeof stateObj === "object") {
			stateObj.capacity = capacity;
		} else {
			V[this.desc.baseName] = capacity;
		}
	}
	 ...
	 */
	rule(name) {
		return (typeof V[this.desc.baseName] === "object") ? V[this.desc.baseName].rules[name] : V[name];
	}

	/**
	 * Sets a rule value. Only use this on a facility migrated to an object in V.
	 * @param {string} name
	 * @param {number} value
	 */
	setRule(name, value) {
		V[this.desc.baseName].rules[name] = value;
	}

	/**
	 * Returns a list slave aspects (for example, living conditions or specific surgeries) that are controlled
	 * by the facility and cannot be overridden in SI or by RA.
	...
	getControlledAspects(assignment) {
		if (assignment === this.desc.manager?.assignment) {
			return new Set(this.desc.manager.commonControlledAspects);
		} else {
			return new Set(this._jobsByAssignment[assignment].desc.commonControlledAspects);
		}
	}
	 ...
	 */
	upgrade(name) {
		return (typeof V[this.desc.baseName] === "object") ? V[this.desc.baseName].upgrades[name] : this.option("Upgrade" + name);
	}

	/**
	 * Sets an upgrade value. Only use this on a facility migrated to an object in V.
	 * @param {string} name
	 * @param {number} value
	 */
	setUpgrade(name, value) {
		V[this.desc.baseName].upgrades[name] = value;
	}
	...
	/**
	 * Whether the facility is located within the penthouse or in the broader arcology
	 * @returns {boolean}
	 */
	get isInPenthouse() {
		return this.penthouseFacility;
	}

	set isInPenthouse(val) {
		this.penthouseFacility = val;
	}
...
App.Entity.Facilities.FacilitySingleJob = class extends App.Entity.Facilities.Job {
	...
	assignmentLinkElement(ID, targetPassage, callback, linkText) {
		linkText = linkText || this.desc.position;
		return App.UI.DOM.assignmentLink(getSlave(ID), this.desc.assignment, "",
			(slave, assignment) => {
				if (callback) {
