App.Desc.Player.butt = function(PC = V.PC) {
	...
	function anus() {
		...
		if (PC.cervixImplant >= 2) {
			r.push(`You have a special pump in your anal cavity that converts fluids to inert filler`);
			if (PC.cervixImplantTarget === "all") {
				r.push(`to be divided between your fillable implants.`);
			} else if (PC.cervixImplantTarget === "TnA") {
				r.push(`to be split between your breast and butt implants.`);
			} else if (PC.cervixImplantTarget === "mombod") {
				r.push(`primarily for your abdominal implant, with a little set aside for your breasts.`);
			} else if (PC.cervixImplantTarget === "bottomHeavy") {
				r.push(`for your butt implants with a little set aside for your belly.`);
			} else if (PC.cervixImplantTarget === "bellyImplant") {
				r.push(`for your abdominal implant.`);
			} else if (PC.cervixImplantTarget === "boobsImplant") {
				r.push(`for your breast implants.`);
			} else if (PC.cervixImplantTarget === "buttImplant") {
				r.push(`for your butt implants.`);
			} else {
				r.push(`for fillable implants.`);
			}
			r.push(`While it is intended to make anal sex more interesting, it has the unfortunate side-effect of requiring you to be on a special diet to prevent complications.`);
		}
		if (PC.mpreg === 1) {
			if (!hasAnyArms(PC)){
