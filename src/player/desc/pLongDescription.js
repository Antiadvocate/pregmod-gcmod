App.Desc.Player.longDescription = function(PC = V.PC) {
	...
	r.push(
		`Looking down,`,
		App.Desc.Player.boobs(PC)
	);
	r.push(
		`${PC.boobs >= 300 ? "Past them" : "And past that"},`,
		App.Desc.Player.belly(PC),
	);
	r.toParagraph();
	if (PC.belly >= 100000) {
	...
	r.push(App.Desc.Player.waist(PC));
	r.push(`It flows into your`);
	r.push(App.Desc.Player.butt(PC));
	r.toParagraph();
	if (!isMovable(PC)) {
	...
	r.push(App.Desc.Player.crotch(PC));
	r.toParagraph();

	r.push(App.Desc.Player.health(PC));
	r.toParagraph();
