	...
	if (PC.career === "servant") {
		if (PC.preg > 0) {
			...
			if (PC.preg >= 41) {
				...
				if (PC.pregSource === SID.FORMERMASTER) {
		...
		if (PC.preg > 0) {
			...
			if (PC.preg >= 41) {
				...
				if (PC.pregMood === 1) {
					r.push(`Even in the final stages of pregnancy, you make sure the slaves attending you are treated as if they were your favorite clients.`);
				} else if (PC.pregMood === 2) {
					r.push(`Your hormones practically rule you, leading you to demand your slaves to be prepared to pleasure you at a moment's notice. Your needy cunt hungers for dick and you don't care`);
					if (V.seeDicks !== 0) {
						r.push(`what it is attached to`);
				...
				r.push(`Something startling happened this week; while enjoying a slave, your belly button popped out!`);
			} else if (PC.preg === PC.pregData.normalBirth / 8 && PC.pregSource > 0) {
				const babyDaddy = asSlave(findFather(PC.pregSource));
				/* Should be handled in prPregnancy ln46
				if (babyDaddy) {
					babyDaddy.counter.PCKnockedUp++;
				}
				*/
