 ...
 */
App.UI.playerSalon = function(PC, cheat = false) {
	...
	function eyewear() {
		...
		el.append(App.Medicine.Modification.eyeSelector(PC, cheat));
		return el;
	}
	...
	function pattern() {
		...
		el.append(App.Medicine.Salon.pattern(PC, cheat));
	...
	function hair() {
		...
		if (showChoices || cheat) {
			if (PC.hLength > 1) {
				if (cheat) {
					option = options.addOption("Natural color", "origHColor", PC)
						.showTextBox();
					for (const color of App.Medicine.Modification.Color.Primary) {
					...
					options.addCustomOption("")
						.addButton("Sync body hair color", () => {
							PC.eyebrowHColor = PC.origHColor;
							PC.pubicHColor = PC.origHColor;
							PC.underArmHColor = PC.origHColor;
							App.UI.reload();
						});
				}
				// Color
	...
	function bodyHair() {
		...
		// Eyebrows
		if (PC.eyebrowHStyle !== "bald" || cheat) {
