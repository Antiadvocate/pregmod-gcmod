...
App.UI.SlaveInteract.physicalRegimen = function(slave, refresh) {
	...
	function drugs() {
		...
		if (facilityControlledAspects.has(FacilityControlledAspect.DRUGS)) {
			App.UI.DOM.appendNewElement("div", title, ` ${His} drug regiment is managed by ${his} assignment.`, ["note", "indent"]);
			el.append(title);
			return el;
		}
	...
	function curatives() {
		...
		if (facilityControlledAspects.has(FacilityControlledAspect.CURATIVES)) {
			App.UI.DOM.appendNewElement("div", el, ` ${His} curatives regiment is managed by ${his} assignment.`, ["note", "indent"]);
			return el;
		}
	...
	function aphrodisiacs() {
		...
		if (facilityControlledAspects.has(FacilityControlledAspect.APHRODISIACS)) {
			App.UI.DOM.appendNewElement("div", el, ` ${His} aphrodisiacs regiment is managed by ${his} assignment.`, ["note", "indent"]);
			return el;
		}
	...
	function fertility() {
		...
		if (slave.ovaries === 1 || slave.mpreg === 1 || slave.preg > 0) {
			...
			if (slave.preg < -2) {
				...
				} else if (V.menstruation === 1 && V.menstruationKnown === 1 && slave.fertLate < 0 && !slave.pregKnown) {
				...
				if (slave.preg <= 0) {
					if (facilityControlledAspects.has(FacilityControlledAspect.CONTRACEPTIVES)) {
						note += `${His} contraceptives regiment is managed by ${his} assignment`;
					} else if (slave.preg === 0 && V.policies.contraceptivesBan === 0) {
						linkArray.push(App.UI.DOM.link(
							`Use contraceptives`,
							() => {
								slave.preg = -1;
				...
				} else if (slave.preg > 0 && slave.breedingMark === 1 && V.propOutcome === 1 && FutureSocieties.isActive('FSRestart') && V.eugenicsFullControl !== 1 && (slave.pregSource === SID.PC || slave.pregSource === SID.ELITE)) {
			...
			if (note !== "") {
				App.UI.DOM.appendNewElement("div", fertilityBlock, note, ["note", "indent"]);
			}
		...
		if (slave.cervixImplant > 0 && slave.cervixImplantTubing > 0) {
			const hasFillableBellyImplant = slave.bellyImplant >= 0;
			const hasFillableBoobsImplant = slave.boobsImplant > 0 && ["fillable", "advanced fillable", "hyper fillable"].includes(slave.boobsImplantType);
			const hasFillableButtImplant = slave.buttImplant > 0 && ["fillable", "advanced fillable", "hyper fillable"].includes(slave.buttImplantType) && slave.cervixImplant === 0; // Forced impossible due to butt implants not being compatible.

			let titleCervix = document.createElement('div');
			titleCervix.append("Cervix micropump control: ");

			if (slave.cervixImplantTarget === "all") {
				titleCervix.append("Pump to all fillable implants. ");
			} else if (slave.cervixImplantTarget === "TnA") {
				titleCervix.append("Pump to breast and butt implants. ");
			} else if (slave.cervixImplantTarget === "mombod") {
				titleCervix.append("Pump to belly and breast implants, simulating pregnancy. ");
			} else if (slave.cervixImplantTarget === "bottomHeavy") {
				titleCervix.append("Pump most to butt implants and some to belly implant. ");
			} else if (slave.cervixImplantTarget === "boobsImplant") {
				titleCervix.append("Pump to breast implants. ");
			} else if (slave.cervixImplantTarget === "buttImplant") {
				titleCervix.append("Pump to butt implants. ");
			} else if (slave.cervixImplantTarget === "bellyImplant") {
				titleCervix.append("Pump to belly implant. ");
			} else if (slave.cervixImplantTarget === "none") {
				titleCervix.append("Pump disabled. ");
			}
			fertilityBlock.appendChild(titleCervix);

			linkArray = [];
			if (slave.cervixImplantTarget !== "none") {
				linkArray.push(App.UI.DOM.link(
					`Disable pump`,
					() => {
			...
			if (slave.cervixImplantTarget !== "bellyImplant" && hasFillableBellyImplant) {
				linkArray.push(App.UI.DOM.link(
					`Belly`,
					() => {
			...
			if (slave.cervixImplantTarget !== "boobsImplant" && hasFillableBoobsImplant) {
				linkArray.push(App.UI.DOM.link(
					`Breasts`,
					() => {
						slave.cervixImplantTarget = "boobsImplant";
						refresh();
					},
				));
			}
			if (slave.cervixImplantTarget !== "buttImplant" && hasFillableButtImplant) {
				linkArray.push(App.UI.DOM.link(
					`Butt`,
					() => {
						slave.cervixImplantTarget = "buttImplant";
						refresh();
					},
				));
			}
			if (slave.cervixImplantTarget !== "TnA" && hasFillableBoobsImplant && hasFillableButtImplant) {
				linkArray.push(App.UI.DOM.link(
					`Tits and Ass`,
					() => {
						slave.cervixImplantTarget = "TnA";
						refresh();
					},
				));
			}
			if (slave.cervixImplantTarget !== "mombod" && hasFillableBellyImplant && hasFillableBoobsImplant) {
				linkArray.push(App.UI.DOM.link(
					`Faux Pregnancy`,
					() => {
						slave.cervixImplantTarget = "mombod";
						refresh();
					},
				));
			}
			if (slave.cervixImplantTarget !== "bottomHeavy" && hasFillableButtImplant && hasFillableBellyImplant) {
				linkArray.push(App.UI.DOM.link(
					`Pear`,
					() => {
						slave.cervixImplantTarget = "bottomHeavy";
						refresh();
					},
				));
			}
			if (slave.cervixImplantTarget !== "all" && hasFillableButtImplant && hasFillableBellyImplant && hasFillableBoobsImplant) {
				linkArray.push(App.UI.DOM.link(
					`All`,
					() => {
						slave.cervixImplantTarget = "all";
						refresh();
					},
				));
			}
			...
			if (slave.preg > 0 && slave.broodmother === 0 && slave.pregKnown === 1 && slave.eggType === "human") {
				if ((slave.assignment !== Job.DAIRY || V.dairyPregSetting === 0) && (slave.assignment !== Job.FARMYARD || V.farmyardBreeding === 0)) {
					...
					if (WL - reservedNursery === 0) {
						...
						const freeTanks = (V.incubator.capacity - tankSlaveCount());
	...
	function breederEligibility() {
		...
		if (V.propOutcome === 1 && FutureSocieties.isActive('FSRestart')) {
			if (slave.breedingMark === 0 && slave.fuckdoll === 0 && slave.eggType === "human" && isFertile(slave, true) && slave.preg <= 0) {
	...
	function hormones() {
		...
		if (facilityControlledAspects.has(FacilityControlledAspect.HORMONES)) {
			el.append(title);
			App.UI.DOM.appendNewElement("div", el, ` ${His} hormonal regiment is managed by ${his} assignment.`, ["note", "indent"]);
			return el;
		}
	...
	function diet() {
		...
		if (facilityControlledAspects.has(FacilityControlledAspect.DIET)) {
			App.UI.DOM.appendNewElement("div", el, ` ${His} diet is managed by ${his} assignment.`, ["note", "indent"]);
			return el;
		}
		...
		if (((isFertile(slave, true) && slave.preg === 0) || (slave.geneticQuirks.superfetation === 2 && canGetPregnant(slave, true) && V.geneticMappingUpgrade !== 0)) && V.dietFertility === 1) {
