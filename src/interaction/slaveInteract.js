/**
 * @param {FC.HumanID} slaveID
 * @returns {DocumentFragment}
 */
App.UI.SlaveInteract.mainPage = function(slaveID) {
	const el = new DocumentFragment();
	if (!slaveID) {
		throw new Error(`The last passage ("${lastPassage()}") did not set V.temp.AS properly`);
	}
	...
	if (V.aiChat === 1) {
		tabs.push({
			title: "Chat",
			id: "chat-tab",
			get node() { return App.UI.SlaveInteract.chat(slave); }
		});
	}
