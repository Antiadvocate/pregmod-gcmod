...
App.Interact.sellSlave = function(slave) {
	...
	function buyerLink(key) {
		...
		r.push(App.UI.DOM.link(
			...
			() => {
				...
				if (allowsBoomerang && slaveCanBoomerang() && (!boomerang.actor || boomerang.weeks > 15)) {
					boomerang.actor = clone(slave);
					boomerang.weeks = 1;
					boomerang.buyer = key;
					boomerang.actor.assignment = Job.REST;
					boomerang.stats = {
						PCpregSource: SID.GENERIC, PCmother: SID.GENERIC, PCfather: SID.GENERIC, boomerangMother: [], boomerangFather: [], boomerangPregSources: [], boomerangMotherTank: [], boomerangFatherTank: [], boomerangRelationship: 0, boomerangRivalry: 0, boomerangBody: 0
					};
					if (V.temp.AS === V.PC.pregSource) {
						boomerang.stats.PCpregSource = V.temp.AS;
					}
					if (V.PC.mother === V.temp.AS) {
						boomerang.stats.PCmother = V.temp.AS;
					}
					if (V.PC.father === V.temp.AS) {
						boomerang.stats.PCfather = V.temp.AS;
					}
					for (const s of getSlaves().values()) { // TODO: review for hyper fertility compatibility
						if (s.ID !== V.temp.AS) {
							if (s.mother === V.temp.AS) {
								boomerang.stats.boomerangMother.push(s.ID);
							}
							if (s.father === V.temp.AS) {
								boomerang.stats.boomerangFather.push(s.ID);
							}
							if (V.temp.AS === s.pregSource) {
								boomerang.stats.boomerangPregSources.push(s.ID);
							}
						}
					}
					if (V.incubator.capacity > 0) {
						for (const child of getTankSlaves().values()) {
							if (V.temp.AS === child.mother) {
								boomerang.stats.boomerangMotherTank.push(child.ID);
							}
							if (V.temp.AS === child.father) {
								boomerang.stats.boomerangFatherTank.push(child.ID);
							}
						}
					}
					if (V.nursery > 0) {
					...
					boomerang.actor.sisters = 0;
					boomerang.actor.daughters = 0;
					if (slave.relationship > 0) {
						boomerang.stats.boomerangRelationship = slave.relationshipTarget;
					}
					if (slave.rivalry > 0) {
						boomerang.stats.boomerangRivalry = slave.rivalryTarget;
					}
					if (slave.bodySwap > 0) {
						const myBody = getSlaves().find(s => s.origBodyOwnerID === V.temp.AS)?.value;
						if (myBody) {
							boomerang.stats.boomerangBody = myBody.ID;
						}
					}
				}
	...
	/**
	 * @returns {Map<string, slaveBuyerData>}
	 */
	function getBuyers() {
		/** @type {Map<string, slaveBuyerData>} */
		const buyers = new Map([
			...
			["preg fetishist", {
				...
				get requirements() { return ((isFertile(slave, true) && V.seePreg !== 0) && cost > 10000); },
				percentOdds: 60,
				get completeSale() {
					const r = [];
					let slaveImpact;
			...
			["organ crafter", {
				...
				get requirements() { return ((isFertile(slave, true) && V.seePreg !== 0) && slave.mpreg === 1); },
				percentOdds: 60,
				get completeSale() {
					const r = [];
					let slaveImpact;
			...
			["womb filler", {
				...
				get requirements() { return ((isFertile(slave, true) && V.seePreg !== 0) && V.seeHyperPreg === 1 && V.seeExtreme === 1 && slave.vagina === 0); },
				percentOdds: 60,
				get completeSale() {
					const r = [];
					let slaveImpact;
			...
			["eugenics arcology", {
				...
				get requirements() { return ((isFertile(slave, true) && V.seePreg !== 0) && slave.intelligence > 50) && slave.face >= 55 && slave.faceShape !== "masculine"; },
				percentOdds: 90,
				get completeSale() {
					const r = [];
					let slaveImpact;
			...
			["repopulationist arcology", {
				...
				get requirements() { return ((isFertile(slave, true) && V.seePreg !== 0) && slave.hips > 1 && slave.vagina === 0); },
				percentOdds: 60,
				get completeSale() {
					const r = [];
					let slaveImpact;
			...
			["virgin trader", {
				...
				get requirements() { return ((isFertile(slave, true) && V.seePreg !== 0) && slave.vagina === 0); },
				percentOdds: 60,
				get completeSale() {
					const r = [];
					let slaveImpact;
			...
			["volume breeder", {
				...
				get requirements() { return (V.seePreg !== 0 && FutureSocieties.isActive('FSPaternalist') && cost < 10000 && slave.health.condition > 40 && slave.ovaries === 1 && isFertile(slave, true)); },
				percentOdds: 50,
				get completeSale() {
					const r = [];
					let slaveImpact;
		...
		if (V.peacekeepers.state === 3) {
			...
			const boolToNum = (/** @type {boolean} */ b) => !b ? 0 : 1;
			/** @type {Record<string, function(App.Entity.SlaveState): number>} */
			const peacekeepersTasteEval = {
				...
				"fertile virgins": s => boolToNum(s.vagina >= 0 && s.vagina <= 1) * ((s.vagina === 0 ? 0.6 : 0) + (isFertile(s, true) ? 0.6 : 0) + (s.anus === 0 ? 0.15 : 0) - 0.2),
				...
				"oppai loli": s => boolToNum(s.physicalAge < 15) * Math.clamp((15-s.visualAge) / 3, 0, 1) * Math.clamp((s.boobs - 1000) / 1000, 0, 1.4),
				"horny MILFs": s => boolToNum(s.physicalAge > 32 && s.devotion > 0) * Math.clamp((s.visualAge - 32) / 5, 0, 1) * Math.clamp((s.energy + s.devotion) / 150, 0, 1.2),
				"beautiful young sex slaves": s => boolToNum(s.physicalAge < 28) * Math.clamp((28-s.visualAge) / 5, 0, 1) * Math.clamp(s.face / 80, 0, 1.2) * Math.clamp(Beauty(s) / 150, 0, 1.2),
			};
		...
		} else if (![0, "bleached"].includes(slave.anusTat) && (slave.buttTat !== 0) && (random(1, 100) > 80)) {
