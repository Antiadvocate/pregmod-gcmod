...
App.UI.SlaveInteract.work = function(slave, refresh) {
	...
	switch (slave.assignment) {
		...
		default:
			...
			if ((V.brothel + V.club + V.dairy + V.farmyard + App.Entity.facilities.servantsQuarters.capacity + V.masterSuite + V.spa + V.nursery + V.clinic + V.schoolroom + V.cellblock + V.arcade + V.HGSuite) > 0) {
	...
	function useSlaveDisplay() {
		...
		if (slave.fuckdoll === 0) {
			...
			if ( // TODO: Make these part-time assignments.
				!V.temp.slaveIdsImpregnanted?.has(slave.ID) &&
					canGetPregnant(slave, !fertKnown(slave)) && // Unknown fertility
					(
						slave.geneticQuirks.superfetation !== 2 || V.geneticMappingUpgrade !== 0 || slave.womb.length < 1
					) &&
					slave.fuckdoll === 0 &&
					V.seePreg !== 0
			) {
				// Sets variable to prevent repeat interactions this week.
				const setImpreg = () => {
				...
				if (canImpreg(slave, V.PC, !fertKnown(slave)) || canFemImpreg(slave, V.PC, !fertKnown(slave))) {
					sexOptions.push({
						text: `Impregnate ${him} yourself`,
						scene: () => {
							setImpreg();
							return App.Interact.fPCImpreg(slave);
						},
					});
				}
				if (canImpreg(slave, slave, !fertKnown(slave)) || canFemImpreg(slave, slave, !fertKnown(slave))) {
					sexOptions.push({text: `Use ${his} own seed to impregnate ${him}`, scene: () => {
						setImpreg();
						return App.Interact.fSlaveSelfImpreg(slave);
					}});
				}
				// @ts-ignore - ignore scene error as this is handled correctly in click()
			...
			if (V.seeIncest === 1 && totalRelatives(slave) > 0) {
				const availableRelatives = getSlaves().filter(s => areRelated(s, slave) && isSlaveAvailable(s));
				if (availableRelatives.size > 1) {
					sexOptions.push({
						text: `Fuck ${him} with one of ${his} close relatives`,
						// @ts-ignore
						scene: new App.Interact.fRelationChoosePartner(slave),
					});
				} else if (availableRelatives.size === 1) {
					const rel0 = availableRelatives.atIndex(0).value;
					sexOptions.push({
						text: `Fuck ${him} with ${his} ${relativeTerm(slave, rel0)} ${SlaveFullName(rel0)}`,
						scene: () => App.Interact.fRelation(slave, rel0),
					});
				} else {
			...
			if (canGetPregnant(slave) && (slave.geneticQuirks.superfetation !== 2 || V.geneticMappingUpgrade !== 0) && V.seePreg !== 0) { // TODO: Add fert known stuff and fem preg
		...
		if (activeSlaveRepSacrifice > 0) {
			...
			TODO: fix this interaction. Consider that it "takes a week" but is not tied to endweek. Consider how easy it is to cheese for rep gains.
			 ...
			 Make this a part-time assignment.
			sexOptions.push({
			...
			function click(sexOption) {
				...
				if (sexOption.goto) {
					...
					if (sexOption.scene instanceof App.Interact.BaseChoosePartnerRenderer) {
						...
						// run the scene first, storing the results
						const output = sexOption.scene();
						// refresh the host passage, to pick up changes to slave
						refresh();
						// display the results
						document.getElementById(miniSceneId).replaceChildren(output);
					}
				} else {
					// just refresh
					refresh();
