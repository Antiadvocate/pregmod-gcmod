...
App.UI.SlaveInteract.wardrobe = function(slave, contentRefresh) {
	...
	function contents() {
		...
		// Industrial Milker slaves are completely stripped naked and cannot have clothing changed.
		if (slave.assignment === Job.DAIRY && V.dairyRestraintsSetting === 2) {
			frag.append(industrialMilker());
		} else if (slave.fuckdoll === 0) {
			frag.append(filtersDOM());
			if (!facilityControlledAspects.has(FacilityControlledAspect.CLOTHES)) {
	...
	/**
	 * Adds text for industrial milker to prevent updating clothing.
	 */
	function industrialMilker() {
		const el = document.createElement("p");
		el.append(`${slave.slaveName} has been assigned to the industrial milker and cannot have their clothing changed.`);
		return el;
	}
	...
	function clothes() {
		...
		if (slave.fuckdoll === 0) {
			...
			if (facilityControlledAspects.has(FacilityControlledAspect.CLOTHES)) {
				App.UI.DOM.appendNewElement("div", clothingDiv, ` ${His} clothes are managed by ${his} assignment.`, ["note", "indent"])
			} else {
		...
		if (facilityControlledAspects.has(FacilityControlledAspect.COLLAR)) {
			clothingDiv.appendChild(label);
			App.UI.DOM.appendNewElement("div", clothingDiv, ` ${His} collar is managed by ${his} assignment.`, ["note", "indent"]);
		} else {
			// Choose her own
	...
	function mask() {
		...
		if (facilityControlledAspects.has(FacilityControlledAspect.FACE_ACCESSORY)) {
			el.appendChild(label);
			App.UI.DOM.appendNewElement("div", el, ` ${His} mask is managed by ${his} assignment.`, ["note", "indent"]);
			return el;
		} else {
			// Choose her own
	...
	function mouth() {
		...
		if (facilityControlledAspects.has(FacilityControlledAspect.MOUTH_ACCESSORY)) {
			el.appendChild(label);
			App.UI.DOM.appendNewElement("div", el, ` ${His} gag is managed by ${his} assignment.`, ["note", "indent"]);
			return el;
		} else {
			// Choose her own
	...
	function buttplug() {
		...
		if (facilityControlledAspects.has(FacilityControlledAspect.BUTTPLUG)) {
			el.appendChild(label);
			App.UI.DOM.appendNewElement("div", el, ` ${His} buttplug is managed by ${his} assignment.`, ["note", "indent"]);
			return el;
		} else {
	...
	function vaginalAccessory() {
		...
		if (facilityControlledAspects.has(FacilityControlledAspect.VAGINAL_ACCESSORY)) {
			el.appendChild(label);
			App.UI.DOM.appendNewElement("div", el, ` ${His} vaginal accessory is managed by ${his} assignment.`, ["note", "indent"]);
			return el;
		} else {
	...
	function vaginalAttachment() {
		...
		if (facilityControlledAspects.has(FacilityControlledAspect.VAGINAL_ATTACHMENT)) {
			el.appendChild(label);
			App.UI.DOM.appendNewElement("div", el, ` ${His} vaginal accessory attachment is managed by ${his} assignment.`, ["note", "indent"]);
			return el;
		} else {
	...
	function dickAccessory() {
		...
		if (facilityControlledAspects.has(FacilityControlledAspect.DICK_ACCESSORY)) {
			el.appendChild(label);
			App.UI.DOM.appendNewElement("div", el, ` ${His} dick accessory is managed by ${his} assignment.`, ["note", "indent"]);
			return el;
		} else {
	...
	function chastity() {
		...
		if (facilityControlledAspects.has(FacilityControlledAspect.CHASTITY)) {
			el.appendChild(label);
			App.UI.DOM.appendNewElement("div", el, ` ${His} chastity is managed by ${his} assignment.`, ["note", "indent"]);
			return el;
		} else {
		...
		for (const name of optionsArray) {
			...
			if (V.boughtItem.toys.chastity === 0) {
				continue;
			}
