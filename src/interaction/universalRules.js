...
App.UI.universalRules = function() {
	...
	function permissions() {
		...
		if (V.brothel + V.club + V.dairy + V.farmyard + App.Entity.facilities.servantsQuarters.capacity + V.arcade + V.schoolroom + V.spa + V.nursery + V.clinic + V.masterSuite + V.cellblock > 0) {
	...
	function babies() {
		...
		if (V.incubator.capacity > 0) {
			if (App.Entity.facilities.incubator.capacity - (tankSlaveCount() + FetusGlobalReserveCount("incubator")) > 0) {
		...
		if (V.nursery > 0) {
			if (V.nurseryCribs - (V.nurseryChildren + FetusGlobalReserveCount("nursery")) > 0) {
