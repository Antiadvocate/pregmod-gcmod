...
App.Art.applyMorphs = function(slave, scene, p, isAnimating) {
	...
	*/
	...
	let volumeScalar = 1;
	if (p.height < 175) {
		volumeScalar = Math.pow((p.height / 175), 3);
	} else {
		volumeScalar = p.height / 175;
	}
	const g8femaleBaseVolume = 62102.0459;
	const scaledVolume = (g8femaleBaseVolume*volumeScalar);
	const zeroVolume = 61505.6077;
	let targetVolume = (zeroVolume*volumeScalar) + ((slave.boobs-200)*2);

	const morphsVolumeData = {
		"boobShapeSmall": {volume: 61505.6077, exp: 0.398263}, // decreasing morph
		"boobShapeNormal":  {volume: 80462.8041, exp: 1.861887},
		"boobShapeSaggy": {volume:128579.3797, exp: 2.226023},
		"boobShapeDownward":  {volume: 79409.9395, exp: 1.912144},
		"boobShapePerky":  {volume: 84824.5792, exp: 1.995920},
		"boobShapeSpherical": {volume: 84965.5049, exp: 1.915347},
		"boobShapeTorpedo":  {volume: 69027.9047, exp: 1.524038},
		"boobShapeWide":  {volume: 64473.8535, exp: 1.328441},
	};


	/**
	 * basically it derives morph strength from morph volume in relation to base volume
	 * baseVolume -> morphVolume
	 * given targetVolume == morphVolume it returns 1.0
	 * given targetVolume == baseVolume it returns 0.0
	 * anything in between or out of bounds will scale based on exponent
	 * exponent is derived from baseVolume -> 0.5 morphVolume -> 1.0 morphVolume
	 * @param {number} baseVolume
	 * @param {number} morphVolume
	 * @param {number} exponent
	 * @param {number} targetVolume
	 * @param {number} scale
	 */
	function morphStrengthFromVolume(baseVolume, morphVolume, exponent, targetVolume, scale) {
		const vBaseScaled = baseVolume * scale;
		const vMorphScaled = morphVolume * scale;

		const delta = vMorphScaled - vBaseScaled;
		const num = targetVolume - vBaseScaled;
		const ratio = num / delta;

		if (ratio < 0) { return null; }
		return Math.pow(ratio, 1 / exponent);
	}

	// compensate for -200 boobs offset
	let offsetBonus = ((slave.boobs - 300) / (1000 - 300)) * 400;
	offsetBonus = Math.max(0, Math.min(offsetBonus, 400));
	targetVolume += offsetBonus;

	if (targetVolume < scaledVolume) {
		// negative small breasts
		let s = morphStrengthFromVolume(g8femaleBaseVolume, morphsVolumeData.boobShapeSmall.volume, morphsVolumeData.boobShapeSmall.exp, targetVolume, volumeScalar);
		morphs.push(["boobShapeSmall", Math.min(s, 1)]);
	} else {
		switch (slave.boobShape) {
			case "normal":
				morphs.push(["boobShapeNormal", morphStrengthFromVolume(g8femaleBaseVolume, morphsVolumeData.boobShapeNormal.volume, morphsVolumeData.boobShapeNormal.exp, targetVolume, volumeScalar)]); break;
			case "perky":
				morphs.push(["boobShapePerky", morphStrengthFromVolume(g8femaleBaseVolume, morphsVolumeData.boobShapePerky.volume, morphsVolumeData.boobShapePerky.exp, targetVolume, volumeScalar)]); break;
			case "saggy":
				morphs.push(["boobShapeSaggy", morphStrengthFromVolume(g8femaleBaseVolume, morphsVolumeData.boobShapeSaggy.volume, morphsVolumeData.boobShapeSaggy.exp, targetVolume, volumeScalar)]); break;
			case "torpedo-shaped":
				morphs.push(["boobShapeTorpedo", morphStrengthFromVolume(g8femaleBaseVolume, morphsVolumeData.boobShapeTorpedo.volume, morphsVolumeData.boobShapeTorpedo.exp, targetVolume, volumeScalar)]); break;
			case "downward-facing":
				morphs.push(["boobShapeDownward", morphStrengthFromVolume(g8femaleBaseVolume, morphsVolumeData.boobShapeDownward.volume, morphsVolumeData.boobShapeDownward.exp, targetVolume, volumeScalar)]); break;
			case "wide-set":
				morphs.push(["boobShapeWide", morphStrengthFromVolume(g8femaleBaseVolume, morphsVolumeData.boobShapeWide.volume, morphsVolumeData.boobShapeWide.exp, targetVolume, volumeScalar)]); break;
			case "spherical": {
				let key = (slave.nipples === "flat" || slave.nipples === "inverted" || !p.applyNipples) ? "boobShapeSpherical" : "boobShapeSphericalNippleFix";
				morphs.push([key, morphStrengthFromVolume(g8femaleBaseVolume, morphsVolumeData.boobShapeSpherical.volume, morphsVolumeData.boobShapeSpherical.exp, targetVolume, volumeScalar)]);
				break;
			}
		}
	}
