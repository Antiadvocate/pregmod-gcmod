...
// cSpell: ignore IDBPDatabase, indicies, probablyDoesntMatter
...
App.Art.GenAI.reactiveImageDB = (function() {
	...
	async function createDB() {
		return AI_IDB.openDB('AIImages-Reactive', 1, {
			blocked: () => {
	...
	function fuzzyCompareSlaves(s1, s2) {
		...
		const dealBreakers = [
			...
			App.Art.GenAI.TattooPromptPart,
		...
		for (const DealBreaker of dealBreakers) {
			...
			if (!_.isEqual(p1.positive(), p2.positive())) {
	...
	function getImage(slaves, options = {}) {
		...
		// Currently when there's a large amount of images being generated for the active slave, it'll spam the server with requests and eventually crash.
		// Maybe re-enable some other time if people find it important
		//
	...
	async function regenAllImages() {
		getSlaves().forEach((s) => getImage([s], {forceRegenerate: true}));
	}
