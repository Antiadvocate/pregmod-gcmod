/** <<SlaveArt>> SugarCube macro. For parameter details see {@link App.Art.SlaveArtElement} */
Macro.add("SlaveArt", {
	handler() {
...
App.Art.humanToArtSlave = (actor) => {
	if (actor.ID === SID.PC) {
		...
		return /** @type {FC.SlaveState} */ (actor);
	}
};
...
async function renderAIArt(slave, imageSize, imageNum = null) {
	...
	try {
		...
		imgElement.setAttribute("title", `${slave.custom.aiDisplayImageIdx + 1}/${slave.custom.aiImageIds.length}${imageData.week ? " Week " + imageData.week:""}`);
	} catch (e) {
...
App.Art.aiArtElement = function(slave, imageSize, isEventImage = null) {
	...
	/**
	 * Adds an orientation class to the surrounding .imageRef so landscape renders can take a wider column.
	 * This fixes landscape images (e.g. 768x512) looking too small when the UI was previously sized for portrait.
	 * @param {HTMLImageElement} imgEl
	 */
	const updateAIOrientationClass = (imgEl) => {
		const refDiv = container.closest('.imageRef');
		if (!refDiv) { return; }
		const apply = () => {
			if (!imgEl.naturalWidth || !imgEl.naturalHeight) { return; }
			const isLandscape = imgEl.naturalWidth > imgEl.naturalHeight;
			refDiv.classList.toggle('ai-landscape', isLandscape);
			refDiv.classList.toggle('ai-portrait', !isLandscape);
		};
		if (imgEl.complete) {
			apply();
		} else {
	...
	const reactiveSpecific = {
		...
		setImage: (imageSrc, eventId) => {
			...
			updateAIOrientationClass(newImg);
		},
		/**
		 *
	...
	const staticSpecific = {
		...
		refresh: () => {
			renderAIArt(slave, imageSize, slave.custom.aiDisplayImageIdx)
				...
				updateAIOrientationClass(imgElement);
				}).catch((e) => {
