App.Neighbor.Interact = function() {
	...
	const interact = (function() {
		...
		function list() {
			...
			if (!V.temp.activeArcologyIdx) {
				V.temp.activeArcologyIdx = 0;
			}
			...
			$(document).one(':passagedisplay', () => nd.select(V.temp.activeArcologyIdx));
		...
		function replaceDetails(arcID) {
			V.temp.activeArcologyIdx = arcID;
		...
		function controlActions(arcID) {
			...
			if (V.arcologies[arcID].government === "your trustees") {
				...
				const agentPartner = getSlaves().find((s) => s.assignment === Job.AGENTPARTNER && s.relationshipTarget === agent.ID)?.value;
				if (agentPartner) {
