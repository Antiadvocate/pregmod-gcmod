App.Corporate.manage = function() {
	...
	function structure() {
		...
		if (App.Corporate.getStored("Incorporated") === 0) {
			...
			App.UI.DOM.appendNewElement("h1", f, `Corporation Overview`); // ${f.append(options.render())}
			...
			/* 
			>isn't there an option to disable notification in the settings? I believe you can turn off corp notifications from there
			... Might be more noticeable if it was on the corp page itself
			App.UI.DOM.appendNewElement("tab", f,
				...
				App.UI.DOM.link("Notify you about available specializations?", () => {
					if (Dialog.isOpen()) {
						Dialog.close();
					}
			...
			if (App.Corporate.getStored("Div") > 1) {
				...
				addDiv(frag,
					App.UI.DOM.link("Dissolve Division", () => {
						...
						Dialog.create("Dissolve Division");
						const frag = new DocumentFragment();
	...
	function dissolveLink() {
		...
		p.append(App.UI.DOM.link("Dissolve the corporation", () => {
			...
			Dialog.create("Dissolve Corporation");
			const frag = new DocumentFragment();
