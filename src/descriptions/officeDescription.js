...
App.Desc.officeDescription = function(lastElement) {
	...
	function paragraph1() {
		...
		if (V.clubAdsSpending >= 5000) {
			...
			const clubSlaves = getSlaves().filter(s => s.assignment === Job.CLUB);
			if (clubSlaves.size > 0) {
				const modeledSlave = clubSlaves.randomValue();
		...
		if (V.brothelAdsSpending >= 5000) {
			...
			const brothelSlaves = getSlaves().filter(s => s.assignment === Job.BROTHEL);
			if (brothelSlaves.size > 0) {
				const modeledSlave = brothelSlaves.randomValue();
				const {he, his} = getPronouns(modeledSlave);
	...
	function paragraph2() {
		...
		const slavesLength = slaveCount();
		if (slavesLength > 50) {
			r.push(`innumerable`);
		} else if (slavesLength > 20) {
			r.push(`many`);
		} else if (slavesLength > 10) {
			r.push(`numerous`);
		} else if (slavesLength > 5) {
			r.push(`handful of`);
		} else {
			r.push(`few`);
		}
						...
						switch (V.assistant.fsAppearance) {
							...
							case "gender radicalist":
								...
								if (V.arcologies[0].FSGenderRadicalistLawFuta === 1) {
									r.push(`${hisA} rear. A pair of balls hang beneath ${hisA} tight pussy.`);
								} else if (V.arcologies[0].FSGenderRadicalistLawFuta === 2) {
									r.push(`${hisA} anus. A pair of heavy balls hang from ${hisA} crotch.`);
									...
									r.push(`${hisA} anus. A tiny pair of balls hang from ${hisA} crotch.`);
								} else {
									r.push(`${hisA} rear.`);
								}
								break;
	...
	function printTrinkets() {
		...
		frag.append(
			...
			App.UI.DOM.linkReplace(`${num(V.trinkets.size)} items`, trinkets()), // TODO: this doesn't take slave trinkets into account
			` in it.`,
		);
		...
		function trinkets() {
			...
			for (const [trinketDesc, value] of V.trinkets) {
				if (!value.slaveTrinket) {
					trinketElements.push(capFirstChar(trinketDesc));
				} else {
					trinketElements.push(trinketPluralReplacer(trinketDesc, value.data));
					plurals = true;
				}
			}
	 ...
	 *
	 * @param {string} desc
	 * @param {Set<FC.SlaveTrinketData>} value
	 * @returns {HTMLElement}
	 */
	function trinketPluralReplacer(desc, value) {
		const his = (value.atIndex(0).id && getSlave(value.atIndex(0).id) ? getPronouns(getSlave(value.atIndex(0).id)).possessive : "their");
		const he = (value.atIndex(0).id && getSlave(value.atIndex(0).id) ? getPronouns(getSlave(value.atIndex(0).id)).pronoun : "they");
		...
		switch (desc) {
			...
			case "a cloth napkin":
				if (value.size === 1) {
					App.Events.addNode(el, [`A cloth napkin skillfully folded into the shape of`, value.atIndex(0).napkinShape, `given to you by`, slaveSentence(value)]);
				} else {
					const r = [];
			...
			case "a framed shot from porn starring":
				if (value.size === 1) {
					App.Events.addNode(el, [`A framed shot from porn starring`, slaveSentence(value), `, ${value.atIndex(0).extra}`]);
				} else {
					const r = [];
					const sl = [];
		...
		/**
		 * @param {Set<FC.SlaveTrinketData>} slaveArray
		 * @returns {DocumentFragment}
		 */
		function slaveSentence(slaveArray) {
			return App.UI.DOM.toSentence(Array.from(slaveArray).map(s => slaveName(s)));
		}
	}
};
