...
App.Update.Slave = function(slave, genepool = false) {
	...
	App.Utils.setNonexistentProperties(slave, {
		earShape: "normal",
		earT: "none",
		earTColor: "hairless",
		earTEffect: "none",
	...
	if (!slave.eye) {
		...
		const origEye = getProp(slave, "origEye", "brown");
		setGeneticEyeColor(slave, origEye);
		if (getProp(slave, "eyes") === -4) {
			eyeSurgery(slave, "both", "remove");
		} else {
			if (getProp(slave, "eyesImplant") === 1) {
				eyeSurgery(slave, "both", "cybernetic");
			}
			if (getProp(slave, "eyes") === -3) {
				eyeSurgery(slave, "both", "glass");
			} else if (getProp(slave, "eyes") === -2) {
				eyeSurgery(slave, "both", "blind");
			} else if (getProp(slave, "eyes") === -1) {
				eyeSurgery(slave, "both", "blur");
			}
			const eyeColor = getProp(slave, "eyeColor", "brown");
			const pupil = getProp(slave, "pupil", "circular");
			const sclerae = getProp(slave, "sclerae", "white");
			setEyeColorFull(slave, eyeColor, pupil, sclerae, "both");
			if (typeof slave.geneticQuirks.heterochromia === "string") {
				setEyeColor(slave, slave.geneticQuirks.heterochromia, "left");
			}
		}
	}
	deleteProps(slave, "origEye", "eyes", "eyesImplant", "eyeColor", "pupil", "sclerae");
	...
	if (slave.rivalry !== 0) {
		const test = getSlave(slave.rivalryTarget, true);
		if (!test) {
			slave.rivalry = 0;
	...
	if (slave.relationship > 0) {
		const test = getSlave(slave.relationshipTarget, true);
		if (!test) {
	...
	function updateSlaveBoobPotential(slave) {
		const slaveGenes = genepool ? slave : Array.isArray(V.genePool) ? V.genePool.find(s => s.ID === slave.ID): getGenePoolRecord(slave);
