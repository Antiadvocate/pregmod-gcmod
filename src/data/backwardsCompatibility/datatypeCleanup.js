 ...
 */
App.Update.SlaveDataSchemeCleanup = (function() {
	"use strict";
	...
	function SlaveDataSchemeCleanup(slave) {
		...
		migrateHealth(slave);
		App.Update.migratePronouns(slave);

		App.Utils.deleteProperties(slave, ["assignmentVisible", "tired"]);
	}
	...
	function migrateRules(slave) {
		if (!slave.hasOwnProperty("rules")) {
			...
			App.Utils.moveProperties(slave.rules, slave, {
		...
		App.Utils.deleteProperties(slave, [
	...
	function migratePorn(slave) {
		if (!slave.hasOwnProperty("porn")) {
			...
			App.Utils.moveProperties(slave.porn, slave, {
	...
	function migrateSkills(slave) {
		if (!slave.hasOwnProperty("skill")) {
			...
			App.Utils.moveProperties(slave.skill, slave, {
				anal: "analSkill",
				combat: "combatSkill",
	...
	function migrateCounters(slave) {
		if (!slave.hasOwnProperty("counter")) {
			...
			App.Utils.moveProperties(slave.counter, slave, { // new <= old
				anal: "analCount",
	...
	function migrateCustomProperties(slave) {
		if (!slave.hasOwnProperty("custom")) {
			...
			if ('customImageFormat' in slave && 'customImage' in slave && slave.customImage !== "" && slave.customImage !== undefined) {
				c.image = {
					filename: /** @type {string} */ (slave.customImage),
					format: /** @type {FC.ImageFormat} */ (slave.customImageFormat) || "png"
				};
			}
			App.Utils.deleteProperties(slave, ["customImageFormat", "customImage"]);

			App.Utils.moveProperties(c, slave, { // new <= old
				tattoo: "customTat",
				label: "customLabel",
				desc: "customDesc",
				title: "customTitle",
		...
		if (!slave.custom.hasOwnProperty("aiImageIds")) {
			if ('aiImageId' in slave.custom && slave.custom.aiImageId !== null) {
				slave.custom.aiImageIds = [/** @type {number} */ (slave.custom.aiImageId)];
 ...
 */
App.Update.SlaveDatatypeCleanup = (function SlaveDatatypeCleanup() {
	"use strict";
	...
	function slaveBoobsDatatypeCleanup(slave) {
		...
		slave.lactation = /** @type {FC.LactationType} */ (Math.clamp(+slave.lactation, 0, 2) || 0);
	...
	function slaveButtDatatypeCleanup(slave) {
		...
		slave.anus = /** @type {FC.AnusType} */ (Math.clamp(+slave.anus, 0, 4) ?? 0);
	...
	function slaveBellyDatatypeCleanup(slave) {
		...
		slave.cervixImplantTubing = Math.clamp(+slave.cervixImplantTubing, 0, 1) || 0;
		if (typeof slave.cervixImplantTarget !== "string") {
			slave.cervixImplantTarget = "bellyImplant";
		}
		if (slave.cervixImplant) {
			if ((!["fillable", "advanced fillable", "hyper fillable"].includes(slave.boobsImplantType) && ["all", "TnA", "mombod", "boobsImplant"].includes(slave.cervixImplantTarget)) || (!["fillable", "advanced fillable", "hyper fillable"].includes(slave.buttImplantType) && ["all", "TnA", "bottomHeavy", "buttImplant"].includes(slave.cervixImplantTarget))) {
				slave.cervixImplantTarget = "bellyImplant";
			}
			if (slave.cervixImplantTarget === "bellyImplant" && slave.bellyImplant < 0) {
				slave.cervixImplantTarget = "none";
			}
		}
	...
	function slaveGenitaliaDatatypeCleanup(slave) {
		...
		slave.vaginaLube = /** @type {FC.VaginaLubeType} */ (Math.clamp(+slave.vaginaLube, 0, 2) ?? 0);
		slave.labia = /** @type {FC.LabiaType} */ (Math.clamp(+slave.labia, 0, 3) ?? 0);
		slave.clit = /** @type {FC.ClitType} */ (Math.clamp(+slave.clit, 0, 5) ?? 0);
		slave.foreskin = Math.max(+slave.foreskin, 0) || 0;
		slave.dick = Math.max(+slave.dick, 0) || 0;
		if (slave.dick) {
			slave.prostate = /** @type {FC.ProstateType} */ (Math.clamp(+slave.prostate, 0, 3) ?? 1);
		}
	...
	function slaveDietDatatypeCleanup(slave) {
		...
		slave.dietCum = /** @type {FC.dietCumType} */ (Math.clamp(+slave.dietCum, 0, 2) ?? 0);
		slave.dietMilk = /** @type {FC.dietMilkType} */ (Math.clamp(+slave.dietMilk, 0, 2) ?? 0);
	...
	function slaveRelationDatatypeCleanup(slave) {
		...
		slave.relationship = /** @type {FC.RelationShipKind} */ (Math.clamp(+slave.relationship, -3, 5) ?? 0);
		slave.relationshipTarget = Math.max(+slave.relationshipTarget, 0) || 0;
		slave.rivalryTarget = Math.max(+slave.rivalryTarget, 0) || 0;
		slave.rivalry = /** @type {FC.RivalryType} */ (Math.clamp(+slave.rivalry, 0, 3) ?? 0);
	...
	function slaveMiscellaneousDatatypeCleanup(slave) {
		...
		slave.tankBaby = /** @type {0|1|2|3} */ (Math.clamp(+slave.tankBaby, 0, 3) ?? 0);
		...
		slave.indentureRestrictions = /** @type {FC.IndentureType} */ (Math.clamp(+slave.indentureRestrictions, 0, 2) ?? 0);
		slave.hears = /** @type {FC.Hearing} */ (Math.clamp(+slave.hears, -2, 0) ?? 0);
		...
		slave.PLimb = /** @type {0 | 1 | 2 | 3} */ (Math.clamp(+slave.PLimb, 0, 3) ?? 0);
		...
		for (const partner of Array.from(slave.partners)) {
 ...
 * @deprecated Future cleanup should be handled in `/src/data/patches/patch.js` and `/src/data/verification/zVerify.js`.
 *
 * @see App.Update.SlaveDatatypeCleanup
 */
App.Update.PCDatatypeCleanup = (function PCDatatypeCleanup() {
	"use strict";
	 ...
	 * @param {FC.PlayerState} PC
	 */
	function PCOldVersions(PC) {
		App.Utils.assignMissingDefaults(
			PC,
			{
				slaveName: getProp(PC, "name"),
				slaveSurname: getProp(PC, "surname"),
				birthName: getProp(PC, "name"),
				birthSurname: getProp(PC, "surname", ""),
				eye: {
					origColor: getProp(PC, "origEye"),
					left: {
						iris: getProp(PC, "eyeColor"),
						pupil: getProp(PC, "pupil"),
						sclera: getProp(PC, "sclerae"),
					},
					right: {
						iris: getProp(PC, "eyeColor"),
						pupil: getProp(PC, "pupil"),
				...
				skill: {
					trading: getProp(PC, "trading"),
					warfare: getProp(PC, "warfare"),
					hacking: getProp(PC, "hacking"),
					slaving: getProp(PC, "slaving"),
					engineering: getProp(PC, "engineering"),
					medicine: getProp(PC, "medicine"),
					cumTap: getProp(PC, "cumTap"),
				},
				counter: {
					birthsTotal: getProp(PC, "births"),
					birthElite: getProp(PC, "birthElite"),
					birthMaster: getProp(PC, "birthMaster"),
					birthDegenerate: getProp(PC, "birthDegenerate"),
					birthClient: getProp(PC, "birthClient"),
					birthArcOwner: getProp(PC, "birthArcOwner"),
					birthCitizen: getProp(PC, "birthCitizen"),
					birthFutaSis: getProp(PC, "birthFutaSis"),
					birthSelf: getProp(PC, "birthSelf"),
					birthLab: getProp(PC, "birthLab"),
					birthOther: getProp(PC, "birthOther"),
					laborCount: getProp(PC, "laborCount"),
					slavesFathered: getProp(PC, "slavesFathered"),
					slavesKnockedUp: getProp(PC, "slavesKnockedUp"),
					storedCum: getProp(PC, "storedCum"),
				}
			},
			basePlayer()
		);
	...
	function PCPhysicalDatatypeCleanup(PC) {
		...
		if (typeof PC.health === "number") {
			...
			setProp(PC, "health", {condition: condition});
		}

		setProp(PC, "health", getProp(PC, "health", {}));

		PC.majorInjury = Math.max(+PC.majorInjury, 0) || 0;
		App.Utils.assignMissingDefaults(PC.health, {
			condition: Math.clamp(getProp(PC.health, "condition", 0), -100, 100),
			shortDamage: Math.max(+getProp(PC.health, "shortDamage", 0), 0),
			longDamage: Math.max(+getProp(PC.health, "longDamage", 0), 0),
			illness: Math.max(+getProp(PC.health, "illness", 0), 0),
			tired: Math.clamp(getProp(PC.health, "tired", 0), 0, 100),
		});
		updateHealth(PC);
	...
	function PCFaceDatatypeCleanup(PC) {
		if (!PC.eye) {
			PC.eye = new App.Entity.EyeState();
			const origEye = getProp(PC, "origEye", "brown");
			setGeneticEyeColor(PC, origEye);
			if (getProp(PC, "eyes") === -4) {
				eyeSurgery(PC, "both", "remove");
			} else {
				if (getProp(PC, "eyesImplant") === 1) {
					eyeSurgery(PC, "both", "cybernetic");
				}
				if (getProp(PC, "eyes") === -3) {
					eyeSurgery(PC, "both", "glass");
				} else if (getProp(PC, "eyes") === -2) {
					eyeSurgery(PC, "both", "blind");
				} else if (getProp(PC, "eyes") === -1) {
					eyeSurgery(PC, "both", "blur");
				}
				const eyeColor = getProp(PC, "eyeColor", "brown");
				const pupil = getProp(PC, "pupil", "circular");
				const sclerae = getProp(PC, "sclerae", "white");
				setEyeColorFull(PC, eyeColor, pupil, sclerae, "both");
				if (typeof PC.geneticQuirks.heterochromia === "string") {
					setEyeColor(PC, PC.geneticQuirks.heterochromia, "left");
				}
			}
		}
		deleteProps(PC, "origEye", "eyes", "eyesImplant", "eyeColor", "pupil", "sclerae");
	...
	function PCBellyDatatypeCleanup(PC) {
		...
		PC.cervixImplantTubing = Math.clamp(+PC.cervixImplantTubing, 0, 1) || 0;
		if (typeof PC.cervixImplantTarget !== "string") {
			PC.cervixImplantTarget = "bellyImplant";
		}
		if (PC.cervixImplant) {
			if ((!["fillable", "advanced fillable", "hyper fillable"].includes(PC.boobsImplantType) && ["all", "TnA", "mombod", "boobsImplant"].includes(PC.cervixImplantTarget)) || (!["fillable", "advanced fillable", "hyper fillable"].includes(PC.buttImplantType) && ["all", "TnA", "bottomHeavy", "buttImplant"].includes(PC.cervixImplantTarget))) {
				PC.cervixImplantTarget = "bellyImplant";
			}
			if (PC.cervixImplantTarget === "bellyImplant" && PC.bellyImplant < 0) {
				PC.cervixImplantTarget = "none";
			}
		}
	...
	function PCSkillsDatatypeCleanup(PC) {
		App.Utils.assignMissingDefaults(PC, {skill: {}});
	...
	function PCMiscellaneousDatatypeCleanup(PC) {
		...
		App.Utils.deleteProperties(PC, ["indenture", "indentureRestrictions"]);
 ...
 * @deprecated Future cleanup should be handled in `/src/data/patches/patch.js` and `/src/data/verification/zVerify.js`.
 */
App.Update.EconomyDatatypeCleanup = function() {
 ...
 * @deprecated Future cleanup should be handled in `/src/data/patches/patch.js` and `/src/data/verification/zVerify.js`.
 */
App.Update.FacilityDatatypeCleanup = (function() {
	"use strict";
	...
	function BrothelDatatypeCleanup() {
		...
		App.Utils.moveProperties(V.brothelBoost, V.brothelBoost, { // FFS, spell it right...
			eligible: "eligable"
		});
 ...
 */
App.Update.GenePoolRecordCleanup = (function() {
	"use strict";
	...
	function GenePoolRecordCleanup(slave) {
		...
		App.Utils.deleteProperties(slave, [
			...
			"cervixImplant", "cervixImplantTubing", "cervixImplantTarget",
			"scar",
			"choosesOwnChastity",
			"pregControl",
			"death",
...
App.Update.RARuleDatatypeCleanup = function() {
	...
	function cleanupConditions(cond) {
		if ('specialSlaves' in cond && 'excludeSpecialSlaves' in cond && cond.excludeSpecialSlaves !== undefined) {
		...
		if ('specialSlaves' in cond && cond.specialSlaves !== undefined) {
			...
			let assignment = [];
			if ('assignment' in cond && cond.specialSlaves === -1) { // include: duplicate regular assignments
				assignment = /** @type {Array} */ (cond.assignment);
				for (const f of fwh) {
					if (assignment.includes(f.jobs[f.defaultJob].assignment)) {
						assignment.push(f.manager.assignment);
					}
				}
			} else if ('assignment' in cond && cond.specialSlaves === 1) { // only: replace regular assignments
				let newAssignments = [];
				assignment = /** @type {Array} */ (cond.assignment);
				for (const assign of assignment) {
					let found = false;
					for (const f of fwh) {
						if (assign === f.jobs[f.defaultJob].assignment) {
		...
		if ('function' in cond && 'data' in cond && cond.function !== undefined) {
			try {
	...
	function cleanupSetters(set) {
		...
		switch (set.collar) {
			...
			case "porcelain mask":
				...
				set.collar = null;
				break; // @ts-ignore
			case "bit gag": // @ts-ignore
			case "dildo gag": // @ts-ignore
			case "massive dildo gag": // @ts-ignore
			case "ball gag": // @ts-ignore
			case "ring gag":
 ...
 */
App.Update.migratePronouns = function(subject) {
