App.Events.REFIPregnancy = class REFIPregnancy extends App.Events.BaseEvent {
	...
	execute(node) {
		...
		if (isFertile(eventSlave, true) || ((eventSlave.ovaries === 1 || eventSlave.mpreg === 1) && eventSlave.pubertyXX === 0)) {
		...
		function steer() {
			...
			if (canDoVaginal(eventSlave) && eventSlave.vagina > 0) {
				...
				if (eventSlave.mpreg === 1) {
					...
					blueballs = (V.PC.dick && canGetPregnant(eventSlave, !fertKnown(eventSlave)));
				}
