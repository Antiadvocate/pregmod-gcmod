App.Events.recFSEgyptianRevivalist = class recFSEgyptianRevivalist extends App.Events.BaseEvent {
	...
	execute(node) {
		...
		if (secondSlave.dick > 0) {
			...
			slave.anus = /** @type {FC.GenePoolRecord["anus"]} */ (Math.max(slave.anus, secondSlave.dick > 4 ? 2 : 1));
			if (slave.preg >= 1) {
		...
		if (slave.dick > 0) {
			...
			secondSlave.anus = /** @type {FC.GenePoolRecord["anus"]} */ (Math.max(secondSlave.anus, slave.dick > 4 ? 2 : 1));
