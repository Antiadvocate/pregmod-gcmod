App.Events.PStripClubAftermath = class PStripClubAftermath extends App.Events.BaseEvent {
	...
	execute(node) {
		...
		if (V.eventResults.strip === 1) {
			...
			App.Events.addParagraph(node, r);
			r = [];
			r.push(App.UI.DOM.makeElement("span", `Enslaving ${him} will cost ${cashFormat(contractCost)}.${((cost - contractCost) > 0) ? ` Alternatively, you could sell ${him}, bringing in ${cashFormat(cost - contractCost)}.` : ``}`, "note"));
			App.Events.addParagraph(node, r);
			r = [];
		...
		if (V.eventResults.strip !== 3) {
			...
			if ((cost - contractCost) > 0) {
				responses.push(new App.Events.Result(`Sell ${him}`, sell));
			}
		} else {
		...
		function sell() {
			const el = new DocumentFragment();
			const r = [];
			cashX((cost - contractCost), "slaveTransfer", slave);
			if (V.eventResults.strip === 1) {
				// wants to get out of poverty and starvation
				r.push(`${He} sobs with gratitude as you lead ${him} to your penthouse. ${He} gets a look of worry when the purchaser's agent comes to complete the transaction, but ${he} stays silent and the transaction completes peacefully.`);
				// TODO: suggested variation: violent/uncooperative transaction
			} else {
				// expects to become your sex slave
				r.push(`${He} smiles with gratitude as you lead ${him} to your penthouse. Before ${he} knows what is happening the purchaser's agent has ${him} subdued and the transaction is completed swiftly.`);
			}
			App.Events.addParagraph(el, r);
			return el;
		}
