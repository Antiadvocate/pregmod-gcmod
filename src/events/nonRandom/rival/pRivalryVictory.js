App.Events.pRivalryVictory = function() {
	...
	function execute() {
		...
		App.UI.DOM.appendNewElement("div", result, App.UI.DOM.link(
			...
			() => {
				...
				if (V.rival.state === 2) {
					...
					r.push(`${HisR} arcology will go to satisfy ${hisR} great debts, but you will still profit <span class="yellowgreen">immensely</span> from your victory${(V.rival.hostageState > 0 && hostageActor) ? `, and acquire ${hostageActor.slaveName} as a slave` : ``}.`);
		...
		function hostageBounty() {
			if (V.rival.hostageState === 0 || !hostageActor) {
				return;
			}
			...
			App.UI.DOM.appendNewElement("div", result2, App.UI.DOM.link(
				...
				() => {
					const {He} = getPronouns(hostageActor);
					jQuery(result2).empty().append(`You make sure to post a bounty sufficient to ensure that ${hostageActor.slaveName}, your rival's psychological warfare tool, does not escape or go unrescued, depending on one's point of view. ${He} is quickly picked out of the chaos of your rival's escape and delivered to you.`);
					V.rival.hostageState = 2;
					cashX(-10000, "war");
				}
			));
