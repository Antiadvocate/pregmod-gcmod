	...
	function execute() {
		...
		switch (V.rival.FS.name) {
			...
			case "Youth Preferentialism":
				hostage.wife = GenerateNewSlave("XX");
		}
		...
		const {woman2, girl2, he2, his2} = getPronouns(hostage.wife || {pronoun : 0}).appendSuffix("2");
		let r = [];
		...
		if (isPCCareerInCategory("wealth")) {
			if (V.PC.career === "wealth" || V.PC.career === "trust fund" || V.PC.actualAge > 16) {
				...
				if (hostageActor.actualAge >= V.PC.actualAge + 6) {
			...
			if (V.PC.career === "servant" || V.PC.career === "handmaiden" || V.PC.actualAge > 16) {
				...
				if (hostageActor.actualAge >= V.PC.actualAge + 6) {
					...
					if (hostageActor.counter.birthsTotal > 0 && V.seePreg) {
					...
					if (V.seePreg && (hostageActor.counter.birthsTotal > 0 || V.PC.counter.birthMaster >= 1)) {
						r.push(`together, and later,`);
						if (hostageActor.counter.birthsTotal > 0) {
							if (V.PC.counter.birthMaster >= 8) {
		...
		switch (V.rival.FS.name) {
			case "Racial Subjugationism":
				r.push(`I'm going to treat ${him} well. Train ${him}. ${He}'s a fine Head Girl prospect, don't you think? ${He}'ll have lots of nice ${V.arcologies[0].FSSubjugationistRace} girls to make love to whenever ${he} wants. Isn't that right, ${SlaveFullName(hostageActor)}?" The slave nods uncertainly.`);
				break;
			case "Racial Supremacism":
				r.push(`I'm going to destroy ${him}. ${He}'s got a whipping post with ${his} name on it, right between a couple of ${V.arcologies[0].FSSupremacist} bitches. Isn't that right, ${SlaveFullName(hostageActor)}?" The slave begins to scream, and the noise goes on and on as the slave is tied up and scourged.`);
				break;
			case "Gender Radicalism":
				r.push(`I'm going to treat ${him} well. Train ${him}. Teach ${him} ${his} proper role in society, and make ${him} happy here. Isn't that right, ${hostageActor.slaveName}?" The slave nods uncertainly.`);
				break;
			case "Gender Fundamentalism":
				r.push(`${he}'s going to be my special pet. ${He}'ll have everything ${he} could ever want, as long as ${he} has a dick up ${his} ass. Isn't that right, ${SlaveFullName(hostageActor)}?" The slave nods hesitantly, but with evident arousal.`);
				break;
			case "Paternalism":
				r.push(`I'm going to destroy ${him}. Holes first, of course. I'll have to get more inventive after that, and I'll leave ${his} vocal cords for last. That way, ${he}'ll be able to beg. Isn't that right, ${SlaveFullName(hostageActor)}?" The slave begins to scream, and the noise goes on and on. And on.`);
				break;
			case "Degradationism":
				r.push(`${he}'s going to be my lover. ${He}'ll have everything ${he} could ever want, and I'll be sure to let ${him} know all about what you do to your slaves. Isn't that right, ${SlaveFullName(hostageActor)}?" The slave nods uncertainly.`);
				break;
			case "Repopulation Focus":
			...
			case "Eugenics":
				...
				break;
			case "Body Purism":
				r.push(`I'm going to give ${him} a nice pair of fake tits. No reason to be excessively clever when I can just make ${his} tits so huge ${he} won't be able to walk. Isn't that right, ${SlaveFullName(hostageActor)}?" The slave begins to sob.`);
				break;
			case "Transformation Fetishism":
				r.push(`${he}'s going to be my lover. I'll be sure to let ${him} know all about what you do to your slaves, and rest assured, I'll never treat ${him} that way. Isn't that right, ${SlaveFullName(hostageActor)}?" The slave nods uncertainly.`);
				break;
			case "Youth Preferentialism":
				r.push(`I've set ${him} up with a nice motherly ${woman2} already. I believe they'll be a perfect match. Isn't that right, ${SlaveFullName(hostageActor)}?" The slave nods uncertainly.`);
				break;
			case "Maturity Preferentialism":
				r.push(`I've set ${him} up with a nice young ${girl2} already. I believe they'll be a perfect match. Isn't that right, ${SlaveFullName(hostageActor)}?" The slave nods uncertainly.`);
				break;
			case "Slimness Enthusiasm":
				r.push(`I think I'll make ${his} IV line permanent. After all, ${he}'s never going off breast growth hormones. I'd estimate ${he} shouldn't be able to walk within a month. Isn't that right, ${SlaveFullName(hostageActor)}?" The slave begins to sob.`);
				break;
			case "Asset Expansionism":
				r.push(`${he}'s going to be my lover. I'll be sure to let ${him} know all about what you do to your slaves, and rest assured, I'll never give ${him} growth hormones, not one single dose. Isn't that right, ${SlaveFullName(hostageActor)}?" The slave nods uncertainly.`);
				break;
			case "Pastoralism":
				r.push(`${he}'s going to be my spotter. Girls should be strong, smoking hot ladies, not disgusting cows. Isn't that right, ${SlaveFullName(hostageActor)}?" The slave nods uncertainly.`);
				break;
			case "Cummunism":
				r.push(`${he}'s going to be my personal milk dispenser. I shouldn't have to chain ${him} to my desk after the first month, since ${his} udders will pin ${him} in place. Isn't that right, ${SlaveFullName(hostageActor)}?" The slave begins to sob.`);
				break;
			case "Physical Idealism":
				r.push(`${he}'s going to learn to enjoy yourself. I'm going to force ${his} face full of food, ${his} cunt full of dicks and discover ${his} deepest, darkest desires. Wouldn't you like that, ${SlaveFullName(hostageActor)}?" The slave begins to sob. "To start, how about a weigh in? ${hostageActor.weight < 0 ? "Just skin and bones, how dreadful" : "So thin and shapeless, such a pity"}, but don't worry, I'll have ${him} nice and plump soon enough."`);
				break;
			case "Hedonistic Decadence":
				r.push(`${he}'s going to be my spotter. Girls should be strong, smoking hot ladies, not disgusting obese slobs. Isn't that right, ${SlaveFullName(hostageActor)}?" The slave nods uncertainly.`);
				break;
			case "Chattel Religionism":
				r.push(`${he}'s going to be the perfect courtesan. No preaching, no praying, just good, clean sex for money. Isn't that right, ${SlaveFullName(hostageActor)}?" The slave nods uncertainly.`);
				break;
			case "Multiculturalism":
				r.push(`${he}'s going to be the ideal holy whore. Thousands of citizens are going to find rapture inside ${him}. Isn't that right, ${SlaveFullName(hostageActor)}?" The slave nods uncertainly.`);
				break;
			case "Intellectual Dependency":
				...
				if (hostageActor.intelligence < 0) {
					r.push(`Shame ${he}'s an idiot though. But ${he} can learn and improve still, I'll see to that.`);
				}
				r.push(`Isn't that right, ${SlaveFullName(hostageActor)}?" The slave nods begrudgingly.`);
				break;
			case "Slave Professionalism":
				r.push(`${he}'s going to become a moron${(hostageActor.intelligence < 0) ? `, though ${he} isn't exactly smart to begin with`:``}. No time for higher thought when all ${his} brain power is caught up with how to satisfy ${his} raging libido. Isn't that right, ${SlaveFullName(hostageActor)}?" The slave begins to sob.`);
				break;
			case "Petite Admiration":
				r.push(`I've set ${him} up with a particularly lovely giant. I believe they'll be a perfect match, even though ${he}'s half ${his2} size. Isn't that right, ${SlaveFullName(hostageActor)}?" The slave nods uncertainly.`);
				break;
			case "Statuesque Glorification":
				r.push(`I've set ${him} up with a lovely little dwarf already. I believe they'll be a perfect match, even though ${he2}'s half ${his} size. Isn't that right, ${SlaveFullName(hostageActor)}?" The slave nods uncertainly.`);
				break;
			default:
				...
				r.push(`nonsense, and what your slaves suffer because of it. Isn't that right, ${SlaveFullName(hostageActor)}?" The slave nods uncertainly.`);
		}
