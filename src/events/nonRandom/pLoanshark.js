App.Events.pLoanshark = class pLoanshark extends App.Events.BaseEvent {
	...
	execute(node) {
		...
		App.Events.addResponses(node, [
			...
			getSlaves().some(slave => slaveCost(slave) > loan.full)
		...
		function pay() {
			...
			V.loans.deleteAll(V.loans.find(loan => loan.name === 'shark'));
		...
		function refuse() {
			...
			if (V.cash > loan.full || getSlaves().some(slave => slaveCost(slave) > loan.full)) {
