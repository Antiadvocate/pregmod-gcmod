...
App.Events.PSchoolSuggestion = class PSchoolSuggestion extends App.Events.BaseEvent {
	...
	execute(node) {
		...
		App.Events.addParagraph(node, [`"I see good taste is scarce within these walls," suddenly stated a young man previously quiet in his corner. "I say quality and strength is where the game should be played and there's nothing better than the girls from the Hippolyta Academy."`]);
		...
		App.UI.DOM.appendNewElement("div", node, `Selecting a school will cost ${cashFormat(startFee)} and add minor upkeep costs.${(V.pedoMode === 0 ? ` In addition, slaves schooled at a local branch of the Slave School, the Utopian Orphanage, l'École des Enculées, or the Gymnasium-Academy will be at or close to ${(V.minimumSlaveAge > 10 ? 'the arcology\'s legal minimum age' : '10 years old')} upon graduation, though the majority of the slaves offered by the school in question will be graduates from outside campuses.` : ``)}`, "note");
