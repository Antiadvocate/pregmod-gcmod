...
App.Events.PCoupAttempt = class PCoupAttempt extends App.Events.BaseEvent {
	execute(node) {
		...
		const doubleAgent = ["agent", "trapper"].includes(traitor.type) ? 1 : 0;
		...
		if (traitorActor !== 0 && (traitor.type === "trapper" || traitor.type === "agent")) {
			App.Events.addParagraph(node, r);
			r = [];
			r.push(`Every now and then you catch sight of ${traitorActor.slaveName} seeding disorder through the ranks.`);
			if (random(1, 100) > traitorActor.intelligence + traitorActor.intelligenceImplant) {
				traitorDeath = 1;
			}
		}
		...
		if ((secExpUnits / 2) + V.arcologies[0].FSRomanRevivalistLaw + V.arcologies[0].FSNeoImperialistLaw1 + V.arcologies[0].FSAntebellumRevivalistLaw2 + V.mercenaries + V.personalArms + V.hackerSupport + doubleAgent > 8) {
			...
			if (traitorActor !== 0 && !["agent", "trapper"].includes(traitor.type)) {
				r.push(`${traitorActor.slaveName} was killed in the fighting.`);
				traitor.actor = 0;
				traitor.stats = {};
				traitor.type = 0;
			}
		...
		if (traitorActor !== 0 && traitorDeath !== 1) {
			if (["agent", "trapper"].includes(traitor.type)) {
				r.push(`${traitorActor.slaveName} returns to your service having completed the task assigned to ${him}.`);
				traitorActor.assignment = Job.REST;
			} else if (traitor.type === "hostage") {
				r.push(`${traitorActor.slaveName} wanders back to the penthouse, battered and bloody, but alive.`);
				traitorActor.assignment = Job.REST;
				setHealth(traitorActor, traitorActor.health.condition, traitorActor.health.condition - traitorActor.health.shortDamage + 80, traitorActor.health.longDamage, traitorActor.health.illness, traitorActor.health.tired);
			} else {
				r.push(`${traitorActor.slaveName} was captured and has been returned to you.`);
				traitorActor.assignment = "stay confined";
				setHealth(traitorActor, traitorActor.health.condition, traitorActor.health.condition - traitorActor.health.shortDamage + 80, traitorActor.health.longDamage, traitorActor.health.illness, traitorActor.health.tired);
				traitorActor.origin = "$He was your slave, but you freed $him, which $he repaid by participating in a coup attempt against you. It failed, and $he is again your chattel.";
			}
			restoreTraitor();
		} else if (traitorActor !== 0) {
			r.push(`${traitorActor.slaveName}'s body was found among the dead. ${He} died sabotaging the Daughters' efforts.`);
			traitor.actor = 0;
			traitor.stats = {};
			traitor.type = 0;
		}
		...
		const choices = [];
		choices.push(new App.Events.Result(`Sell your prisoners immediately`, sell, (traitorActor !== 0) ? `${traitorActor.slaveName} will not be sold.` : ""));
		choices.push(new App.Events.Result(`Give your prisoners to those who suffered property damage`, property, (traitorActor !== 0) ? `${traitorActor.slaveName} will not be sold.` : ""));
