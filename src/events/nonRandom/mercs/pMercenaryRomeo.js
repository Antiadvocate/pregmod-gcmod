App.Events.PMercenaryRomeo = class PMercenaryRomeo extends App.Events.BaseEvent {
	...
	castActors() {
		...
		const juliet =
			...
			getSlaves().filter(s => s.fetish !== Fetish.MINDBROKEN && s.fuckdoll === 0 && ["serve in the club", "serve the public", "whore", "work in the brothel"].includes(s.assignment)).randomValue() ||
			// second try - find a slave who had public exposure and is hopefully not important to the PC personally
			getSlaves().filter(s => (s.counter.publicUse > 0) && (s.newGamePlus === 0) && (s.relationship > -3) && !["be your Concubine", "serve in the master suite"].includes(s.assignment)).randomValue();
		if (juliet) {
