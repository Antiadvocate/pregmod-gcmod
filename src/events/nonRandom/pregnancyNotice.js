...
App.Events.PregnancyNotice.unreservedTanks = () => {
	...
	return (V.incubator.capacity - tankSlaveCount()) - FetusGlobalReserveCount("incubator");
};

App.Events.PregnancyNotice.unreservedCribs = () => {
	if (!V.nurseryCribs) { return 0; }
	return (V.nurseryCribs - infantCount()) - FetusGlobalReserveCount("nursery");
};
...
App.Events.PregnancyNotice.validFetus = (mother, fetus) => {
	return (
		fetus.age === 2 && fetus.noticeData.fate === "undecided" ||
		(fetus.age === 6 && ["wait", "undecided"].includes(fetus.noticeData.fate)) ||
		// Week 35 is the last one before the first labor check
		(fetus.age === 35 && ["lateWait", "undecided"].includes(fetus.noticeData.fate)) ||
		...
		(mother.ID !== SID.PC && mother.weekAcquired === V.week && fetus.age > 6 && fetus.noticeData.fate === "undecided")
	);
};
...
App.Events.PregnancyNotice.Event = (node, mother) => {
	...
	const motherIsPC = (mother.ID === SID.PC);
	// @ts-expect-error weekAcquired doesn't exist on PlayerState
	const motherIsNew = (mother.ID === SID.PC) ? false : (mother.weekAcquired === V.week);
	...
	if (motherIsPC) {
		...
		motherDiv.append(App.UI.DOM.makeElement("div", `${pronounsForSlaveProp(mother, mother.origin)}`));
	}
	...
	function updateProcessed() {
		...
		let unprocessedFetuses =  mother.womb.filter((fetus) => (App.Events.PregnancyNotice.validFetus(mother, fetus)));
		if (unprocessedFetuses.length === 0) {
			...
			let bulkChoice = {
				/** @type {"incubator"|"nursery"|"nothing"|"wait"|"lateWait"|"undecided"|"terminate"|"transplant"} */
				choice: "undecided"
			};
			...
			if (unprocessedFetuses.filter((fetus) => fetus.age < 35).length !== 0) {
				bulkOption.addValue("Ask me again in the 35th week", "lateWait");
			}
	...
	function fetusInfo(fetus, noteSpan) {
		...
		const fatherIsPC = (typeof father !== "string" && father.ID === SID.PC);
		...
		while (fakeChild.actualAge < V.idealAge) {
			ageSlave(fakeChild);
			// TODO:@franklygeorge handle puberty and teeth
		}
			...
			if (fetus.age === 2) {
				...
				if (fatherIsPC) {
					...
					intro.push(
						...
						(fetus.fatherID === SID.ELITE || fetus.fatherID === SID.LAB) ? "are": "is",
						`the father.`
					);
		...
		if (canTransplant === -1) {
			if (fetus.motherID === SID.PC) {
				intro.push(`The child was transplanted from your womb.`);
			} else {
				let origMom = getSlave(fetus.motherID);
				if (origMom) {
					intro.push(`The child was transplanted from`, App.SlaveAssignment.saSlaveName(origMom), `'s womb.`);
				} else {
					let momRecord = getGenePoolRecord(fetus.motherID);
					if (momRecord) {
						intro.push(`The child was transplanted from`, SlaveFullName(momRecord), `'s womb.`);
					} else {
						intro.push(`The child was transplanted from someone else's womb.`);
					}
				}
			}
		}
		...
		if (fetus.age < 35) {
			fetusOption.addValue("Ask me again in the 35th week", "lateWait", () => {
				fetus.reserve = "";
			});
		}
		if (canTransplant !== 0) {
		...
		if (fetus.noticeData.fate === "transplant") {
			if (canTransplant === -1) {
				let originalMother = (fetus.motherID === SID.PC) ? V.PC : getSlave(fetus.motherID);
				// @ts-expect-error PlayerState not assignable to SlaveState
				const popup = (originalMother.ID === SID.PC) ? App.UI.DOM.makeElement("div", "yourself"): App.UI.DOM.slaveDescriptionDialog(originalMother, SlaveFullName(originalMother));
