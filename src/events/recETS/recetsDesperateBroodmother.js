App.Events.recetsDesperateBroodmother = class recetsDesperateBroodmother extends App.Events.BaseEvent {
	...
	execute(node) {
		...
		const pram = {
			minAge: V.fertilityAge + 4, maxAge: (V.pedoMode === 1 ? 18 : 24), disableDisability: 1, ageOverridesPedoMode: 1
		};
		...
		/* We don't generate a real child unless the minimum slave age is 3 or less--otherwise "generateRelatedSlave" throws an error. We do
			however have to create a dummy child--with pronouns--here so that the IDE and compiler don't choke on the references later in the event.
		*/
		let child = baseSlave();
