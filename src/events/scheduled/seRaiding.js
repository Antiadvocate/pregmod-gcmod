	...
	execute(node) {
		...
		for (let ser = 0; ser < 3; ser++) {
			...
			switch (origin) {
				...
				case "nun":
					...
					break;
				case "monk":
					r.push(`is a reclusive monastic community in a mountain range between two current conflict zones. The members of the community have withdrawn from the collapsing social order of the world and established a commune in their mountain retreat, filling their days with peaceful reflection and prayer. Though the monks have no true leader, one among them has emerged as the most pious of the brothers.`);
					break;
				case "journalist":
		...
		function createRaidee(origin) {
			...
			switch (origin) {
				case "housewife":
					...
					if (isFertile(slave) && V.seePreg !== 0) {
						...
						if (slave.preg > 0) {
							...
							WombImpregnate(slave, slave.pregType, SID.GENERIC, slave.preg);
							slave.pregKnown = 1;
				...
				case "doctor":
					...
					break;
				case "nun":
					slave = GenerateNewSlave("XX", {minAge: 18, maxAge: 70, disableDisability: 1});
					slave.career = "a nun";
					slave.anus = 0;
					slave.vagina = 0;
					...
					break;
				case "monk":
					slave = GenerateNewSlave("XY", {minAge: 18, maxAge: 70, disableDisability: 1});
					slave.career = "a nun"; // will become "a monk" in convertCareer if male pronouns are enabled
					slave.anus = 0;
					slave.vagina = 0;
					slave.weight = random(-100, -20);
					slave.attrXX = random(10, 50);
					...
					slave.behavioralFlaw = "devout";
					slave.sexualFlaw = "repressed";
					slave.clothes = "a penitent nuns habit"; // TODO: fix when we have suitable masculine clothing options
					break;
				case "journalist":
					slave = GenerateNewSlave("XX", {
				...
				case "investor":
					...
					break;
				case "scientist":
					slave = GenerateNewSlave("", {minAge: 28, maxAge: 55, disableDisability: 1});
					...
					slave.clothes = "conservative clothing"; // TODO: fix when we have suitable masculine clothing options
					break;
				case "lawyer":
					slave = GenerateNewSlave("", {
		...
		function raidingAssault(slave, origin, targetEscape) {
			...
			switch (origin) {
				...
				case "monk":
					if (targetEscape > MercCapture) {
						r.push(`As the ${V.mercenariesTitle} close in on the commune, they are spotted by a group of young monks tending to their grounds. Before your ${V.mercenariesTitle} can close the distance, they retreat behind the walls of their community and seal the gate behind them.`);
					} else if (raidEscape === 1) {
						r.push(`The monks flee into their inner sanctum as your ${V.mercenariesTitle} approach and bar the doors behind them as they go. When the last barrier is pried open, your ${V.mercenariesTitle} discover the monks have committed suicide as a group rather than be taken prisoner.`);
					} else {
						r.push(`Unarmed and defenseless, the monks are easily cowed by your ${V.mercenariesTitle} and taken away one by one to the VTOLs. The most pious brother prays vehemently until ${he} is dragged out of the inner sanctum in cuffs. The pious monk spends the VTOL ride engaged in fervent prayer as ${his} brothers are raped by your mercenaries. ${He} naïvely believes that ${his} devotion and piety will see ${him} spared from the same fate as ${his} brothers.`);
						newSlave(slave);
						// skip New Slave Intro
					}
					break;
