App.Events.SEWedding = class SEWedding extends App.Events.BaseEvent {
	...
	execute(node) {
		...
		if (V.weddingPlanned === 3 && brides.some(b => !isFertile(b))) {
			if (brides.every(b => !isFertile(b, !fertKnown(b)) || isMenstruating(b))) {
		...
		if ([1, 3].includes(V.weddingPlanned)) {
			...
			const anyFertUnknown = brides.some(b => !fertKnown(b)); // checks if any brides have unknown fertility cycles
			...
			if (V.weddingPlanned === 3) { // Impregnation ceremony
				if (!solo && brides.every((b) => b.fetish === Fetish.MINDBROKEN)) {
					...
					r.push(`Then you pull ${his} ${knees} up to give your guests a good view as you fuck ${his} ${fertKnown(slave1) ? `fertile` : `potentially fertile`}`);
					if (slave1.mpreg === 1) {
						r.push(`asshole.`);
					} else {
						r.push(`cunt.`);
					...
					r.push(`You'll fuck ${himC} repeatedly over the next few days, ${anyFertUnknown ? `hopefully ` : ``}ensuring impregnation.`);
				} else if (!solo && brides.every((b) => b.devotion + b.trust >= 175)) {
					r.push(`Then, you`);
					const {
						He,
					...
					r.push(`${He} is left to face your guests, watching raptly as you fuck ${his} ${fertKnown(slave1) ? `fertile` : `potentially fertile`}`);
					if (slave1.mpreg === 1) {
						r.push(`asshole.`);
					} else {
						r.push(`cunt.`);
					...
					r.push(`You'll fuck them repeatedly over the next few days, ${anyFertUnknown ? `hopefully ` : ``}ensuring impregnation.`);
				} else if (!solo && brides.every((b) => b.devotion < -20 && b.trust > 20)) {
					const {
						He,
						he, his, him
					...
					r.push(`${He} is left to face them, watching sullenly as you fuck ${his} ${fertKnown(slave1) ? `fertile` : `potentially fertile`}`);
					if (slave1.mpreg === 1) {
						r.push(`asshole.`);
					} else {
						r.push(`cunt.`);
					...
					r.push(`You'll fuck them repeatedly over the next few days, ${anyFertUnknown ? `hopefully ` : ``}ensuring impregnation, despite their efforts to defy you.`);
				} else if (!solo && brides.every((b) => b.devotion < -20)) {
					const {
						He,
						he, his, him, women
					...
					r.push(`You'll fuck them repeatedly over the next few days, ${anyFertUnknown ? `hopefully ` : ``}ensuring impregnation, despite their protesting.`);
				} else if (!solo && brides.every((b) => b.devotion >= -20)) {
					const {
						He,
						he, his, him, wife
					...
					for (let i = 0; i < ML; i++) {
						...
						if (slave.fetish === Fetish.MINDBROKEN) {
							...
							r.push(`Then you pull ${his} ${knees} up to give your guests a good view as you fuck ${his} ${fertKnown(slave) ? `fertile` : `potentially fertile`}`);
							if (slave.mpreg === 1) {
								r.push(`asshole.`);
							} else {
								r.push(`cunt.`);
							...
							r.push(`${He} is left to face your guests, watching raptly as you fuck ${his} ${fertKnown(slave) ? `fertile` : `potentially fertile`}`);
							if (slave.mpreg === 1) {
								r.push(`asshole.`);
							} else {
								r.push(`cunt.`);
							...
							r.push(`up to give your guests a good view of the consummation. ${He} is left to face them, watching sullenly as you fuck ${his} ${fertKnown(slave) ? `fertile` : `potentially fertile`}`);
							if (slave.mpreg === 1) {
								r.push(`asshole.`);
							} else {
								r.push(`cunt.`);
							...
							r.push(`${He} is left to face them, watching sullenly as you fuck ${his} ${fertKnown(slave) ? `fertile` : `potentially fertile`}`);
							if (slave.mpreg === 1) {
								r.push(`asshole.`);
							} else {
								r.push(`cunt.`);
					...
					r.push(`You'll fuck them repeatedly over the next few days, ${anyFertUnknown ? `hopefully ` : ``}ensuring`);
				...
				if (!solo && brides.every((b) => b.fetish === "pregnancy" && b.fetishStrength > 60)) {
					if (brides.every((b) => b.devotion + b.trust >= 175)) {
						...
						r.push(`As hateful pregnancy fetishists, <span class="hotpink">getting ${anyFertUnknown ? `impregnanted` : `pregnant`} was the best part of the ceremony.</span>`);
						brides.forEach(slave => slave.devotion += 1);
					} else if (brides.every((b) => b.devotion < -20)) {
						r.push(`As pregnancy fetishists, getting ${anyFertUnknown ? `forcefully impregnanted` : `raped pregnant`} by someone they hate leave them with very mixed feelings.`);
					} else if (brides.every((b) => b.devotion >= -20)) {
						r.push(`As pregnancy fetishists, <span class="hotpink">they thoroughly enjoyed getting ${anyFertUnknown ? `inseminated` : `knocked up`} in such an ostentatious fashion.</span>`);
						brides.forEach(slave => slave.devotion += 10);
					} else {
						const pregFet = [];
						...
						for (const slave of brides) {
							...
							if (slave.devotion + slave.trust >= 175) {
								...
								r.push(`<span class="hotpink">in spite of ${his} hate, getting ${!fertKnown(slave) ? `impregnanted` : `pregnant`} was the best part of the ceremony for ${slave.slaveName}</span>`);
								slave.devotion += 1;
							} else if (slave.devotion < -20) {
								r.push(`for ${slave.slaveName}, getting ${!fertKnown(slave) ? `forcefully impregnanted` : `raped pregnant`} by someone ${he} hates leaves ${him} with very mixed feelings`);
							} else {
								r.push(`<span class="hotpink">${slave.slaveName} thoroughly enjoyed getting ${!fertKnown(slave) ? `inseminated` : `knocked up`} in such an ostentatious fashion</span>`);
								slave.devotion += 10;
							}
						}
					...
					for (const slave of brides) {
						...
						if (slave.fetish === "pregnancy" && slave.fetishStrength > 60) {
							if (slave.devotion + slave.trust >= 175) {
								...
								r.push(`As a hateful pregnancy fetishist, <span class="hotpink">getting ${!fertKnown(slave) ? `impregnanted` : `pregnant`} was the best part of the ceremony</span> for ${slave.slaveName}.`);
								slave.devotion += 1;
							} else if (slave.devotion < -20) {
								r.push(`As a pregnancy fetishist, getting ${!fertKnown(slave) ? `forcefully impregnanted` : `raped pregnant`} by someone ${slave.slaveName} hates leaves ${him} with very mixed feelings.`);
							} else {
								r.push(`As a pregnancy fetishist, <span class="hotpink">${slave.slaveName} thoroughly enjoyed getting ${!fertKnown(slave) ? `inseminated` : `knocked up`} in such an ostentatious fashion.</span>`);
								slave.devotion += 10;
							}
						}
					}
