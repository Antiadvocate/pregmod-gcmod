	...
	actorPrerequisites() {
		return [
			[ // single event slave
				...
				s => canGetPregnant(s, !fertKnown(s)),
				s => canImpreg(s, V.PC, !fertKnown(s)),
			...
			if (V.PC.belly >= 30000) {
				...
				if (eventSlave.dick > 0 && eventSlave.chastityPenis === 0) {
					...
					if (eventSlave.dick > 10) {
						...
						if (cumShot >= 1) {
							t.push(`Like a loose firehose, ${he} sprays a jet of semen haphazardly across you, ${himself} and ${his} surroundings, thoroughly soaking everything in range.`);
						} else if (cumShot >= .1) {
							t.push(`You block most of ${his} cum from splashing across your face, but it still ends up all over the place.`);
						} else {
							t.push(`As if it is any condolence, at least ${his} loads are small even though ${he} still shakes them everywhere.`);
						}
						facialDiverted = false;
					}
				}
