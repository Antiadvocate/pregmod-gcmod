	...
	execute(node) {
		...
		switch (eventSlave.clothes) {
			...
			case "a slutty magical girl costume":
				if (canTalk(eventSlave)) {
					t.push(Spoken(eventSlave, `"I don't remember this sinking into me so much... And this fold? Was that always there?"`));
					t.push(He);
				}
				t.push(`runs ${his} hand down ${his} bodysuit, clearly bothered by what ${he}'s feeling.`);
				break;
			case "a superhero outfit":
				...
				t.push(`tugs at ${his} leotard, clearly uncomfortable of how much it shows.`);
				break;
			case "a red priestess outfit":
			case "a coat and corset":
				if (canTalk(eventSlave)) {
					t.push(Spoken(eventSlave, `"I should be able to close this, right? So why won't it stay shut..."`));
					t.push(He);
				}
				t.push(`pulls ${his} outfit closed before letting it fall open again, clearly distressed.`);
				break;
			default:
				if (canTalk(eventSlave)) {
