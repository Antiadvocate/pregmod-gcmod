App.Events.RESSSolitaryDesperation = class RESSSolitaryDesperation extends App.Events.BaseEvent {
	...
	execute(node) {
		...
		if (V.PC.belly < 5000){
			if (canPenetrate(V.PC) && (canDoAnal(eventSlave) || canDoVaginal(eventSlave)) && canWalk(eventSlave)) {
		...
		function exploit(){
			...
			if (canDoVaginal(eventSlave) && canDoAnal(eventSlave)) {
				r.push(`In ${his} desperation ${he} even guides your cock into ${his} butt without prompting.`);
			}
