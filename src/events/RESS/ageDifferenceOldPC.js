App.Events.RESSAgeDifferenceOldPC = class RESSAgeDifferenceOldPC extends App.Events.BaseEvent {
	...
	execute(node) {
		...
		if (eventSlave.mother !== SID.PC && eventSlave.father !== SID.PC) {
		...
		App.Events.addResponses(node, [
			...
			fertKnown(eventSlave) && isFertile(eventSlave)
		...
		function afternoon() {
			...
			WombImpregnate(eventSlave, eventSlave.pregType, SID.GENERIC, 1);
