App.Events.RESSPenitent = class RESSPenitent extends App.Events.BaseEvent {
	...
	execute(node) {
		...
		App.Events.addResponses(node, [
			...
			(canImpreg(eventSlave, V.PC, !fertKnown(eventSlave)))
		...
		function babyPray() {
			...
			if (eventSlave.fetish !== "humiliation" && fetishChangeChance(eventSlave) > random(1, 100)) {
				...
				eventSlave.fetishKnown = 1;
				eventSlave.fetishStrength = 10;
				r.push(`${He} has come to associate <span class="fetish gain">${!fertKnown(eventSlave) ? `carrying your children` : `being bred`}</span> with sexual fulfillment.`);
			}
