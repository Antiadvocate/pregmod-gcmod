...
App.Events.RESSKitchenMolestation = class RESSKitchenMolestation extends App.Events.BaseEvent {
	...
	execute(node) {
		...
		const targetSlaves = getSlaves().filter(s => targetJobs.includes(s.assignment) && s.ID !== eventSlave.ID);
		const PC = V.PC;
		...
		function improve() {
			...
			for (const s of targetSlaves.values()) {
		...
		function turnTables() {
			...
			if (canGetPregnant(eventSlave) && eventSlave.mpreg === 1) {
				...
				if (pregSources.size > 0) {
					knockMeUp(eventSlave, 50, 1, pregSources.randomValue().ID);
				}
			}
			for (const slave of targetSlaves.values()) {
				if (slave.trust < 50) {
					slave.trust += 4;
				}
			}
