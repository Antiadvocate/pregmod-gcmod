App.Events.RESSEscapee = class RESSEscapee extends App.Events.BaseEvent {
	...
	execute(node) {
		...
		function nothing() {
			...
			eventSlave.trust += 2;
			getSlaves().forEach(function(s) { if (s.devotion <= 20) { s.trust += 8; } });
		...
		function confine() {
			...
			getSlaves().forEach(function(s) { if (s.devotion <= 20) { s.trust += 4; } });
		...
		function flog() {
			...
			getSlaves().forEach(function(s) { if (s.devotion <= 20) { s.trust -= 5; } });
		...
		function amputate() {
			...
			getSlaves().forEach(function(s) { if (s.devotion <= 20) { s.trust -= 10; } });
