App.Events.RESSBreastExpansionBlues = class RESSBreastExpansionBlues extends App.Events.BaseEvent {
	...
	execute(node) {
		...
		App.Events.addResponses(node, [
			...
			(canGetPregnant(eventSlave, !fertKnown(eventSlave)) && PC.dick !== 0)
				? new App.Events.Result(`Give ${his} bust a boost by ${fertKnown(eventSlave) ? `knocking` : `attempting to knock`} ${him} up`, boost, virginityWarning())
				: new App.Events.Result(),
		...
		function boost() {
			...
			r.push(`${He} knows full well what you meant now, and <span class="devotion inc">lets you have your way</span> with ${his} body every time you catch ${him} in a vulnerable moment or complaining about ${his} tits.`);
			if (fertKnown(eventSlave)) {
				r.push(`By the week's end, scans reveal that your seed has taken root; <span class="lime">${he}'s pregnant.</span> As ${his} breasts grow to feed ${his} coming child, ${he} will likely be too distracted by ${his} swelling middle to complain about their added weight.`);
			}
			eventSlave.trust -= 5;
