App.Events.RESSMillenary = class RESSMillenary extends App.Events.BaseEvent {
	...
	execute(node) {
		...
		function everyone() {
			...
			slave.devotion += 10;
			seX(slave, "oral", "slaves", "penetrative", (slaveCount() * 2));
			getSlaves().forEach(function(s) { if (s.ID !== slave.ID) { s.counter.oral++; } });
			if (canDoVaginal(slave)) {
				r.push(VCheck.Vaginal(slave, 1));
			} else {
				r.push(VCheck.Anal(slave, 1));
		...
		function drive() {
			...
			seX(slave, "anal", "slaves", "penetrative", slaveCount() + 1);
			getSlaves().forEach(function(s) { if (s.ID !== slave.ID) { s.counter.penetrative++; } });
			return r;
		}
