App.Events.RESSDevotedExhibition = class RESSDevotedExhibition extends App.Events.BaseEvent {
	...
	execute(node) {
		...
		function leave() {
			...
			r.push(`down towards a bench; you secure ${him} so ${he} can ${hasAnyLegs(eventSlave) ? "crouch or kneel" : "lay"}, naked and vulnerable. As you leave, ${he} understands, and glances around the balcony.`);
			let bestialityCount = 0;
			let analCount = jsRandom(5, 15);
			let vaginalCount = jsRandom(5, 15);
			let mammaryCount = jsRandom(5, 15);
			let oralCount = jsRandom(5, 15);
			if (V.seeBestiality === 1 && eventSlave.fetish === Fetish.BESTIALITY && jsRandom(1, 4) === 2) { // 1 out of 4 chance
				r.push(`<span class="bold">While looking for a citizen to entice ${he} notices a dog paying attention to ${him} and so ${he} wiggles ${his} butt enticingly in its direction.</span>`);
				r.push(`The dog takes the bait and moments later ${he} is getting fucked by a dog ${eventSlave.counter.bestiality === 0 ? "for the first time." : `once again.`}`);
				bestialityCount += 1;
				if (jsRandom(1, 10) === 5) {
					r.push(`The commotion attracts another dog and as soon as the first dog finishes and its knot dies down the other dog mounts ${him}.`);
					bestialityCount += jsRandom(1, 3);
				}
			} else {
				r.push(`Fixing on the nearest citizen paying attention to ${him}, ${he} looks him straight in the eye`);
				switch (eventSlave.fetish) {
					case "submissive":
						r.push(`<span class="bold">and begs him submissively to do whatever he wants to ${him}.</span>`);
						break;
					case "cumslut":
						r.push(`<span class="bold">and begs him to come over and feed ${him} his`);
						...
						r.push(`cum.</span>`);
						oralCount += jsRandom(2, 7);
						break;
					case "humiliation":
						r.push(`<span class="bold">as ${he} begins to wordlessly writhe and blush with the <span class="devotion inc">excitement</span> of this humiliating situation.</span>`);
						eventSlave.devotion += 2;
						break;
					case "buttslut":
						r.push(`<span class="bold">and then turns as best ${he} can to offer him ${his} ${canDoAnal(eventSlave) ? "backdoor" : "warm hole"}, begging him to stuff his cock ${canDoAnal(eventSlave) ? `up ${his} ass`: `in ${his} vagina since he can't use ${his} ass`}.</span>`);
						break;
					case "boobs":
						r.push(`<span class="bold">and swings ${his} torso from side to side, bouncing ${his} breasts around lewdly.</span>`);
						mammaryCount += jsRandom(2, 7);
						break;
					case "pregnancy":
						r.push(`<span class="bold">and begs him to come over and shoot his seed into ${canDoVaginal(eventSlave) ? canGetPregnant(eventSlave, !fertKnown(eventSlave)) ? `${his} fertile womb` : `${his} vagina` : `${him}`}.</span>`);
						vaginalCount += jsRandom(2, 7);
						break;
					case "dom":
					case "sadist":
						r.push(`<span class="bold">and challenges him to come over and fuck ${him}.</span>`);
						break;
					case "masochist":
						r.push(`<span class="bold">and, applying a bit of reverse psychology, begins to beg him not to spank ${his} helpless behind.</span>`);
						break;
					case "bestiality":
						r.push(`<span class="bold">and begs him to treat ${him} like the bitch ${he} is.</span>`);
						break;
					default:
						r.push(`<span class="bold">and implores him to fuck ${him}.</span>`);
				}
				if (
					V.seeBestiality === 1 && jsRandom(1, 10) === 5 &&
					(canDoVaginal(eventSlave) || canDoAnal(eventSlave) || canDoOral(eventSlave))
				) { // 1 out of 10 chance
					if (jsRandom(1, 2) === 1) { // 50/50 chance
						r.push(`<span class="bold">Before the man gets to ${him} a group of dogs take interest instead.</span>`);
						r.push(`They fight each other a little for who will go first and before ${he} knows it ${his} ${canDoVaginal(eventSlave) ? "vagina" : canDoAnal(eventSlave) ? "anus" : "mouth"} is being fucked by a dog.`);
						const isViolent = jsRandom(0, 1);
						r.push(`Once the first dog finishes and its knot dies down one of the other dogs ${isViolent ? `<span class="health dec">violently</span> ` : ""}takes its place.`);
						if (isViolent) {
							healthDamage(eventSlave, 1);
						}
						bestialityCount += jsRandom(4, 10);
					} else {
						r.push(`Before the man gets to ${him} a dog takes interest and starts sniffing ${his} ${canDoVaginal(eventSlave) ? "vagina" : canDoAnal(eventSlave) ? "anus" : "mouth"}.`);
						const isViolent = jsRandom(0, 1);
						r.push(`After a few licks the dog mounts ${him}${isViolent ? ` <span class="health dec">violently</span>` : ""}.`);
						if (isViolent) {
							healthDamage(eventSlave, 1);
						}
						bestialityCount += 1;
						if (jsRandom(1, 10) === 5) { // 1 out of 10 chance
							r.push(`The commotion attracts another dog and as soon as the first dog finishes and its knot dies down the other dog mounts ${him}.`);
							bestialityCount += jsRandom(1, 3);
						}
					}
			...
			// do all the deeds
			analCount = Math.max(0, analCount - bestialityCount);
			vaginalCount = Math.max(0, vaginalCount - bestialityCount);
			mammaryCount = Math.max(0, mammaryCount - bestialityCount);
			oralCount = Math.max(0, oralCount - bestialityCount);
			repX(1250, "event", eventSlave);
			if (mammaryCount > 0) {
				seX(eventSlave, "mammary", "public", "penetrative", mammaryCount);
			}
			if (oralCount > 0 && canDoOral(eventSlave)) {
				seX(eventSlave, "oral", "public", "penetrative", oralCount);
			}
			if (analCount > 0 && canDoAnal(eventSlave)) {
				seX(eventSlave, "anal", "public", "penetrative", vaginalCount);
			}
			if (vaginalCount > 0 && canDoVaginal(eventSlave)) {
				seX(eventSlave, "vaginal", "public", "penetrative", vaginalCount);
			}
			if (bestialityCount !== 0) {
				for (let i = 0; i < bestialityCount; i++){
					if (canDoVaginal(eventSlave) && jsRandom(1, 2) === 1) { // 50/50 chance
						seX(eventSlave, "vaginal", "animal", "bestiality", 1);
					} else if (canDoAnal(eventSlave)) {
						seX(eventSlave, "anal", "animal", "bestiality", 1);
					} else {
						seX(eventSlave, "oral", "animal", "bestiality", 1);
					}
				}
				if (eventSlave.counter.bestiality > 10 && eventSlave.fetish !== Fetish.BESTIALITY && fetishChangeChance(eventSlave) > random(1, 100)) {
					eventSlave.fetish = Fetish.BESTIALITY;
					eventSlave.fetishKnown = 1;
					eventSlave.fetishStrength = 10;
					r.push(`Being fucked by so many animals has awoken something deep inside ${eventSlave.slaveName}. ${He} realizes that <span class="fetish gain">${he} enjoys doing depraved things with ${his} animal friends.</span>`);
				} else if (eventSlave.fetish === Fetish.BESTIALITY) {
					r.push(`<span class="fetish inc">Being fucked by dogs increases ${his} depravity.</span>`);
					eventSlave.fetishStrength += jsRandom(3, 6);
				}
			}
			...
			if (eventSlave.dick > 0) {
				if (eventSlave.vagina > 0) {
					r.push(`${his} own ejaculate${canDoVaginal(eventSlave) ? ", cum, and ": " and "}pussyjuice`);
				} else {
					r.push(`${his} own ejaculate`);
				}
			} else {
				r.push(`${canDoVaginal(eventSlave) ? "cum and ": ""}pussyjuice`);
			}
			const holeDesc = canDoVaginal(eventSlave) ? canDoAnal(eventSlave) ? "holes are" : "vagina is" : "butthole is";
			r.push(`all over ${his} thighs${canDoVaginal(eventSlave) || canDoAnal(eventSlave) ? `, and ${his} well-fucked ${holeDesc} dripping semen` : ``}. ${He}'s certainly worked hard <span class="reputation inc">improving your reputation.</span>`);
			r.toParagraph();
			return r.container();
		}
		...
		function like() {
			...
			if (eventSlave.fetish === Fetish.SUBMISSIVE) {
				...
				} else if (!isFertile(eventSlave, true)) {
