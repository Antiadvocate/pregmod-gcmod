...
App.Events.RESSVocalDisobedience = class RESSVocalDisobedience extends App.Events.BaseEvent {
	...
	execute(node) {
		...
		function sentence() {
			...
			r.push(`to anyone who pays for the rest of the week, and keep ${him} fed and cleaned. ${His} world will become nothing but silent, featureless darkness and dick. <span class="cash inc">You earn some money off ${him},</span> and by the end of the night ${his}`);
			if (canDoAnal(eventSlave) && canDoVaginal(eventSlave)) {
				r.push(`holes get`);
			} else {
				r.push(`hole gets`);
