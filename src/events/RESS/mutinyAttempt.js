App.Events.RESSMutinyAttempt = class RESSMutinyAttempt extends App.Events.BaseEvent {
	...
	execute(node) {
		...
		function lock() {
			...
			getSlaves().forEach(function(s) { if (s.devotion < -50) { s.trust += 5; } });
		...
		function mute() {
			...
			eventSlave.voice = 0;
			getSlaves().forEach(function(s) { if (s.devotion < -50) { s.trust += 10; } });
		...
		function genderbender() {
			...
			getSlaves().forEach(function(s) { if (s.devotion < -50) { s.trust -= 15; } });
