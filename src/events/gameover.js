...
App.Events.Gameover = function() {
	...
	switch (V.temp.gameover) {
		...
		case "Idiot Ball 3 Totally Not Idiot Ball 2 Again":
			...
			r.push(`When you awake, it hits you like a truck; you idiotically enslaved your ${V.PC.race} ass by decreeing all${(V.temp.gameover === "Idiot Ball 2 The Dumbassening") ? `non-${V.arcologies[0].FSSupremacistRace}` : V.arcologies[0].FSSubjugationistRace}${(V.PC.race !== "mixed race") ? "s" : " individuals"} slaves, and since you are now a slave, lack the authority to revert the policy. The story of your remaining years may be worth telling, as is your legendary blunder, but it must be told elsewhere.`);
			if (V.temp.gameover === "Idiot Ball 2 The Dumbassening") {
		...
		default:
			r.push (`Game Over`);
			console.error(`Unhandled V.temp.gameover value of '${V.temp.gameover}'`);
	}
	App.Events.addParagraph(node, r);
