App.Events.RESiblingPlease = class RESiblingPlease extends App.Events.BaseEvent {
	...
	impregCheck(slave) {
		...
		return isFertile(slave, !fertKnown(slave)) && !isMenstruating(slave) && ((slave.ovaries === 1 && slave.vagina >= 0) || slave.mpreg === 1) && canBreed(slave, V.PC);
	}
	...
	execute(node) {
		...
		function impregMom() {
			...
			if (mom.preg === -1) {
				t.push(`You know it probably won't take today, but you've already ordered ${V.assistant.name} to discontinue ${his2} contraceptive regimen, and you'll be repeating this ritual all week.`);
				if (fertKnown(mom)) {
					t.push(`${He2}'s <span class="pregnant">bound to end up pregnant.</span>`);
				} else {
					t.push(`${He2}'s probably going to end up pregnant.`);
				}
			} else {
				t.push(`You won't know if it's taken right away, but you'll be repeating this ritual all week,`);
				if (fertKnown(mom)) {
					t.push(`so ${he2}'s <span class="pregnant">going to end up pregnant.</span>`);
				} else {
					t.push(`so ${he2}'s likely going to end up pregnant.`);
				}
			}
			if (isFertile(mom)) {
				knockMeUp(mom, 100, 2, -1);
			}
			...
			dau.devotion += 2;
			dau.trust += 5;
			t.push(`${dau.slaveName} is <span class="devotion inc">excited</span> to${fertKnown(mom) ? `` : ` potentially`} have a sibling on the way and <span class="trust inc">grateful</span> that you fulfilled ${his} request,`);
			if (disobedient) {
				mom.devotion -= 4;
				mom.trust -= 4;
				t.push(`but ${mom.slaveName} <span class="devotion dec">hates that ${fertKnown(mom) ? `it's yours` : `${he2} might be carrying your child`} </span> and is <span class="trust dec">terrified</span> of your control over ${his2} body.`);
				...
				mom.devotion += 1;
				mom.trust -= 1;
				t.push(`and while ${mom.slaveName} <span class="trust dec">fears your arbitrary control of ${his2} body,</span> ${he2} feels <span class="devotion inc">closer to you</span> because ${he2}'s${fertKnown(mom) ? `` : ` potentially`} carrying the child that ${dau.slaveName} wanted so badly.`);
			}
			t.toParagraph();
		...
		function impregDau() {
			...
			if (dau.preg === -1) {
				t.push(`You know it probably won't take today, but you've already ordered ${V.assistant.name} to discontinue ${his} contraceptive regimen, and you'll be repeating this ritual all week.`);
				if (fertKnown(dau)) {
					t.push(`${He}'s <span class="pregnant">bound to end up pregnant.</span>`);
				} else {
					t.push(`${He}'s probably going to end up pregnant.`);
				}
			} else {
				t.push(`You won't know if it's taken right away, but you'll be repeating this ritual all week,`);
				if (fertKnown(dau)) {
					t.push(`so ${he}'s <span class="pregnant">going to end up pregnant.</span>`);
				} else {
					t.push(`so ${he}'s likely going to end up pregnant.`);
				}
			}
			t.toParagraph();

			if (isFertile(dau)) {
				knockMeUp(dau, 100, 2, -1);
			}
			dau.devotion += 5;
			dau.trust += 5;
			t.push(`${dau.slaveName} is <span class="devotion inc">overjoyed</span> to be${fertKnown(dau) ? `` : ` potentially`} carrying a child, and <span class="trust inc">grateful</span> that you understood the heart of ${his} request, even if you didn't fulfil it exactly the way ${he} was originally hoping.`);
			if (dau.fetish === "pregnancy") {
				if (!dau.fetishKnown) {
					...
					t.push(`${His} pregnancy fetish <span class="fetish inc">has strengthened</span> since you${fertKnown(dau) ? `` : ` tried`} put a new baby in ${him}.`);
					dau.fetishStrength += 10;
				}
		...
		function impregBoth() {
			...
			if (fertKnown(dau) && fertKnown(mom)) {
				...
				t.push(`hopeful that repeating this ritual all week will result in both of them getting pregnant with your children.`);
			}
			...
			dau.devotion += 2;
			dau.trust += 5;
			t.push(`${dau.slaveName} is <span class="devotion inc">excited</span> to${fertKnown(dau) && fertKnown(mom) ? `` : ` potentially`} have both a sibling and a child on the way and <span class="trust inc">grateful</span> that you fulfilled ${his} request,`);
			if (disobedient) {
				mom.devotion -= 5;
				mom.trust -= 8;
				t.push(`but ${mom.slaveName} <span class="devotion dec">hates that you're the father</span> of both${fertKnown(dau) && fertKnown(mom) ? `` : ` possible`} children and is <span class="trust dec">terrified</span> of your control over ${his2} and ${his2} ${daughter}'s bodies.`);
			} else if (mom.devotion > 50 || mom.fetish === "pregnancy") {
				mom.devotion += 4;
				mom.trust += 2;
				t.push(`while ${mom.slaveName} is <span class="devotion inc">just as excited</span> about ${his2} upcoming child and grandchild, and <span class="trust inc">hopes and trusts</span> you'll allow ${him2} to provide good advice for ${his2}${fertKnown(dau) ? `` : ` potentially`} pregnant ${daughter}.`);
			} else {
				mom.trust -= 4;
		...
		function decline() {
			dau.trust += 2;
			return [`You tell ${dau.slaveName} that you're satisfied with ${his} service for you as an only child, and you aren't going to knock up ${mom.slaveName}${dau.father === SID.PC ? " again" : ""} just to make a sibling for ${him}. Like the good slave ${he} is, ${he} accepts your decision and <span class="trust inc">appreciates</span> your attention.`];
		}
	}
};
