...
App.Events.RERoyalBlood = class RERoyalBlood extends App.Events.BaseEvent {
	...
	execute(el) {
		...
		const king = generateKing(); // King first to get his ID for the Queens pregSource and the children's father id

		const queen = generateQueen(); // Queen second to get an ID for her children to relate to.
		const {
			He3,
			his3, he3, him3, himself3, wife3
		} = getPronouns(queen).appendSuffix('3');

		const prince = generatePrince();
		const {
			His2,
			his2, he2, himself2
		...
		if (activeSF) {
			if (V.rep > princessSF) {
				...
				choices.push(new App.Events.Result(null, null, `You lack the reputation to capture and enslave a princess with ${V.SF.Lower}.`));
			}
		}
		...
		if (V.seeDicks > 0) {
			...
			if (activeSF) {
				if (V.rep > princeSF) {
					...
					choices.push(new App.Events.Result(null, null, `You lack the necessary reputation to capture and enslave the crown prince with ${V.SF.Lower}.`));
				}
			}
			...
			if (activeSF) {
				if (V.rep > princeAndPrincessSF) {
					...
					choices.push(new App.Events.Result(null, null, `You lack the necessary reputation to capture and enslave both the crown prince and princess with ${V.SF.Lower}.`));
				}
			}
		}
		...
		const courtLadiesName = (V.pedoMode ? "the princess's friends" : "court ladies");
		if (V.cash >= courtLadiesCost) {
			choices.push(new App.Events.Result(
				`Transfer a respectable quantity of credits for a handful of ${courtLadiesName}.`,
				getCourtLadies,
				`It will cost about ${cashFormat(courtLadiesCost)} to enslave them.`
			));
		} else {
			choices.push(new App.Events.Result(null, null, `You lack the necessary funds to enslave a handful of ${courtLadiesName}.`));
		}

		if (activeSF) {
			if (V.rep > courtLadiesCostSF) {
				choices.push(new App.Events.Result(
					`Dispatch ${V.SF.Lower} on a night time raid to acquire a handful of ${courtLadiesName}.`,
					() => getCourtLadies(false),
					`You will be disliked for this action and trade will be damaged.`
				));
			} else {
				choices.push(new App.Events.Result(null, null, `You lack the necessary reputation to capture and enslave a handful of ${courtLadiesName} with ${V.SF.Lower}.`));
			}
		}


		// PRINCESS AND COURT
		const princessAndCourtCost = 500000;
		const princessAndCourtSF = 15000;
		if (V.pedoMode) {
			if (V.cash >= princessAndCourtCost) {
				choices.push(new App.Events.Result(
					`Offer a generous amount of cash to aid the new arcology in exchange for both the princess and ${his} friends.`,
					getPrincessAndCourt,
					`It will cost about ${cashFormat(princessAndCourtCost)} to enslave the princess and ${his} friends.`
				));
			} else {
				choices.push(new App.Events.Result(null, null, `You lack the necessary funds to enslave the princess and ${his} friends.`));
			}
		}

		if (V.pedoMode && activeSF) {
			if (V.rep >= princessAndCourtSF) {
				choices.push(new App.Events.Result(
					`Dispatch ${V.SF.Lower} on a night time raid to acquire the princess and ${his} friends.`,
					() => getPrincessAndCourt(false),
					`You will be despised for this action, and trade will be greatly damaged.`
				));
			} else {
				choices.push(new App.Events.Result(null, null, `You lack the necessary reputation to capture and enslave the princess and ${his} friends with ${V.SF.Lower}.`));
			}
		}
		...
		if (activeSF) {
			if (V.rep >= queenSF) {
				...
				choices.push(new App.Events.Result(null, null, `You lack the necessary reputation to capture and enslave the Queen with ${V.SF.Lower}.`));
			}
		}
		...
		if (activeSF) {
			if (V.rep >= princessAndQueenSF) {
				...
				choices.push(new App.Events.Result(null, null, `You lack the necessary reputation to capture and enslave both the Queen and princess with ${V.SF.Lower}.`));
			}
		}
		...
		if (!V.pedoMode && activeSF) {
			if (V.rep >= queenAndCourtSF) {
				...
				choices.push(new App.Events.Result(null, null, `You lack the necessary reputation to capture and enslave the Queen and ${his3} ladies with ${V.SF.Lower}.`));
			}
		}
		...
		if (V.limitFamilies !== 1) {
			...
			if (activeSF) {
				choices.push(new App.Events.Result(
					`Dispatch ${V.SF.Lower} on a night time raid to take everyone and everything of value.`,
		...
		function commitKingToMissingTable() {
			V.missingTable[king.ID] = missingTableRecordFromHumanState(king, king.ID, {yours: false, dead: true}, "King");
		}
		...
		function getPrinceAndPrincess(buy = true) {
			...
			App.UI.DOM.appendNewElement("p", text, `Eventually they arrive in your penthouse, faces flushed with embarrassment at their compromising position. The former prince is beside ${himself2} with rage, but seems to be holding ${himself2} back for ${his2} little ${relativeTerm(prince, princess)}'s sake, while ${he} struggles to maintain a façade of poise and grace. ${princessArrives()} However, the prince's submission to life as a slave is another question entirely.`);

			// the queen is missing
			princess.mother = SID.GENERIC;
			setMissingParents(princess);
			prince.mother = princess.mother;
			commitKingToMissingTable();
		...
		function getCourtLadies(buy = true) {
			const text = new DocumentFragment();
			const ladiesName = (V.pedoMode ? "friends of the princess" : "court ladies");
			const arrivalDesc = (V.pedoMode
				? `When the young noblewomen arrive at your penthouse, they're decidedly disoriented.`
				: `When the ladies arrive at your penthouse, they seem almost relieved at the opulence of their new surroundings.`
			);
			if (buy) {
				App.UI.DOM.appendNewElement("p", text, `You take a tablet and peruse the various ${ladiesName} on offer by the fledgling arcology. You single out a few interesting individuals for purchase that might suit your tastes, but the slapdash descriptions provided for each slave by the revolutionaries make it likely that any similarities to your aesthetic leanings will be coincidental. Once you are satisfied with your selection, you electronically transfer the credits to the new arcology and soon receive a receipt for your donation and an estimated delivery schedule.`);
		...
		function getPrincessAndCourt(buy = true) {
			...
			if (buy) {
				...
				App.UI.DOM.appendNewElement("p", text, `Seizing a tablet, you quickly send a message to The Colonel. After dark, a flight of VTOLs land in the new arcology laden with troops. When they take off again they have the princess and a gaggle of ${his} terrified friends chained together.`);
				V.arcologies[0].prosperity -= 17;
				const share = queenAndCourtSF / 17;

				// Friends
				for (let i = 0; i < 3; i++) {
					const slave = generateOneCourtLady();
					repX(-share * 4, "event", slave);
					newSlave(slave); // skip New Slave Intro
				}

				// Princess
				repX(-share * 5, "event", queen);
				newSlave(princess); /* skip New Slave Intro */
			}

			App.UI.DOM.appendNewElement("p", text, `Eventually they all arrive in your penthouse. The princess presents the perfect image of a demure yet composed royal. ${His} clearly practiced façade of poise and grace fades under scrutiny, however. ${princessArrives()} Meanwhile, ${his} friends are decidedly disoriented. ${ladiesArrive()}`);
		...
		function getQueen(buy = true) {
			...
			if (buy) {
				...
				App.UI.DOM.appendNewElement("p", text, `The man seems somewhat surprised that you want to purchase the Queen and the Queen alone, given the breadth and quality of his other merchandise available, but not unduly so. Soon enough negotiations begin and a short time later an equitable price is agreed upon. Your new peer even throws in a slight discount, in exchange for allowing him to use the Queen himself for the remainder of the night.`);
		...
		function getPrincessAndQueen(buy = true) {
			...
			App.UI.DOM.appendNewElement("p", text, `Eventually they both arrive in your penthouse. The princess is flushed with embarrassment at ${his} compromising position, struggling to maintain a façade of poise and grace. ${princessArrives()} The Queen, on the other hand, seems almost relieved and basks in the opulence of ${his3} new surroundings. ${queenArrives()}`);
		...
		function getEntireCourt(buy = true) {
			function everyoneArrives() {
				...
				const ladiesArrivalDesc = (V.pedoMode
					? `the friends of the princess are decidedly disoriented`
					: `the ladies seem comforted by the opulence of their new surroundings`
				);
				r.push(`seems almost relieved and basks in the opulence of ${his3} new surroundings. ${queenArrives()} Lastly, ${ladiesArrivalDesc}. ${ladiesArrive()}`);
				return r;
			}
			...
			if (buy) {
				...
				r.push(`and a gaggle of terrified ${ladiesName}.`);
				...
				r.push(`a gaggle of terrified ${ladiesName}, and as much loot as they could carry.`);
				App.Events.addParagraph(text, r);
		...
		function generateTemplate(sex) {
			...
			/* We set the minimum age high enough to make sure that she was at least 19 when the prince was born, *and* to minimize the chance of the prince-princess age gap
				being compressed so far that the two become twins.
			*/
			const minQueenAge = V.minimumSlaveAge + 21;

			if (sex === "XX") { // queen is generated after king, so we can key her race/nationality to his
				slave = GenerateNewSlave(sex, {
					minAge: minQueenAge, maxAge: 43, ageOverridesPedoMode: 1, disableDisability: 1, nationality: king.nationality, race: king.race
				});
				slave.slaveSurname = slave.birthSurname = king.birthSurname; // can this be made to account for languages with different male/female surnames?
			} else if (V.realRoyalties === 1) {
				/** @type {FC.Race} */
				let race;
				...
				slave = GenerateNewSlave(sex, {
					minAge: minQueenAge, maxAge: 43, ageOverridesPedoMode: 1, disableDisability: 1, nationality: nationality, race: race
				});
				...
				slave = GenerateNewSlave(sex, {
					minAge: minQueenAge, maxAge: 43, ageOverridesPedoMode: 1, disableDisability: 1
				});
			}
			slave.anus = 0;
			slave.weight = 0;
		...
		function generatePrincess() {
			const slave = generateRelatedSlave(prince, "younger sister", undefined, 0, 2, 3);
		...
		function generatePrince() {
			const slave = generateRelatedSlave(queen, "son", undefined, 0, 19);
			slave.father = king.ID;
			...
			// moved to finalTweaks(): muscles = 50
			slave.intelligence = random(15, 100);
			slave.intelligenceImplant = 15;
			// skills moved to finalTweaks(): anal 0, oral 0, combat 40
			setHealth(slave, jsRandom(30, 60), 0, 0, 0, 0);
			slave.behavioralFlaw = either("arrogant", "bitchy");
			setMissingParents(slave); // set .father so the princess will be a full sibling
		...
		function generateQueen() {
			const slave = generateTemplate("XX");
			...
			while (slave.boobs < 300 || slave.boobs > 1000) {
			...
			slave.pubicHStyle = "waxed";
			slave.underArmHStyle = "waxed";
			slave.shoulders = either(-1, 0, 1);
			slave.hips = 1;
			slave.butt = random(2, 4);
			slave.intelligence = random(15, 100);
			slave.intelligenceImplant = 30;
			// skills moved to finalTweaks(): entertainment 45
			slave.counter.birthsTotal = 2;
			setHealth(slave, jsRandom(30, 60), 0, 0, 0, 0);
			slave.behavioralFlaw = either("arrogant", "bitchy");
			if (V.seePreg !== 0) {
				slave.pregSource = king.ID;
				slave.preg = 7;
				slave.pregType = 1;
				slave.pregWeek = 7;
				slave.pregKnown = 1;
		...
		function generateKing() {
			const slave = generateTemplate("XY");
			// give the king a missing parent ID
			slave.ID = generateMissingParentID();
			return slave;
		}

		/** @returns {FC.SlaveState} */
		function generateOneCourtLady() {
			const minAge = (V.pedoMode ? princess.actualAge - 3 : 21);
			const maxAge = (V.pedoMode ? princess.actualAge + 3 : V.retirementAge -2);
			const slave = GenerateNewSlave("XX", {
				minAge: minAge, maxAge: maxAge, disableDisability: 1, nationality: princess.nationality
			});
			...
			slave.prestige = 1;
			const ladyName = (V.pedoMode ? "friend of the princess" : "lady of the court");
			slave.prestigeDesc = `$He was once a ${ladyName} of an ancient kingdom.`;
			...
			slave.pubicHStyle = "waxed";
			slave.underArmHStyle = "waxed";
			slave.shoulders = jsEither([-1, 0, 1]);
			while (slave.natural.boobs < 300 || slave.natural.boobs > 1000) {
				slave.natural.boobs = rollBreast(0);
			}
			if (slave.actualAge <= 15) {
				slave.boobs = Math.min(slave.boobs, slave.natural.boobs);
				slave.butt = Math.min(slave.butt, 1);
			} else {
				slave.boobs = slave.natural.boobs;
				slave.butt = 1;
			}
			if (slave.actualAge <= 13) {
				slave.hips = /** @type {FC.GenePoolRecord["hips"]} */ (Math.min(slave.hips, 1));
			} else {
				slave.hips = 1;
			}
			slave.anus = 0;
		...
		// apply some prince and queen traits after princess generated, to prevent unintentional inheritance
		function finalTweaks() {
			queen.skill.entertainment = 45;
			prince.muscles = 50;
			prince.skill.anal = 0;
			prince.skill.oral = 0;
			prince.skill.combat = 40;
		}
		...
		function queenArrives() {
			return `Yet, it seems likely that ${his3} relief has more to do with saving ${him3} from a lifetime of gang rape at the mercy of ${his3} former subjects, than it does the familiar luxury. ${He3} submits to biometric scanning obediently and without ${(V.seePreg !== 0) ? `fuss, during which you discover to ${his3} surprise that ${he3} is pregnant with the child of the former King. You notice ${he3} cradles ${his3} ever so slightly rounded stomach protectively.` : `fuss.`}`;
		}
		...
		function princeArrives() {
			return `When ${he2} arrives in your penthouse, the former prince is beside ${himself2} with rage. As soon as he sees a chance, ${he2} breaks free and attempts to fight ${his2} way out of your penthouse. ${His2} attempt at freedom is futile, however, and ${he2} is soon overwhelmed by your guards and dragged back to your office for punishment. It will take some times before the valiant princes submits to life as a slave.`;
		}

		function ladiesArrive() {
			const arrivalText = (V.pedoMode
				? `You're faced with a little knot of profoundly frightened but defiant girls. Ripped suddenly from their families and lives of luxury, they have no idea how to face their new circumstances.`
