...
App.Events.REArcologyInspection = class REArcologyInspection extends App.Events.BaseEvent {
	...
	execute(node) {
		...
		const agentLover = agent ? getSlaves().find(s => (s.ID === agent.relationshipTarget && s.assignment === Job.AGENTPARTNER))?.value : undefined;
			...
			if (agentLover) {
				...
				if (agent.fetish === Fetish.SUBMISSIVE) {
					...
					if (V.PC.dick > 0 && (agent.vagina >= 0 || agent.mpreg === 1)) {
						...
						if (canImpreg(agent, V.PC, !fertKnown(agent))) {
							const pregText = tryKnockMeUp(agent, 25, 2,	V.PC);
							if (agent.pregKnown) {
								t.push(pregText);
							}
						} else {
							t.push(`Both of you know, of course, that you can't actually get ${him} pregnant${isFertile(agent, true) && fertKnown(agent) ? ` this week` : ``}, but the fantasy is fun anyway.`);
						}
						if (agent.mpreg === 1) {
						...
						if (canImpreg(V.PC, agent)) {
							const pregText = tryKnockMeUp(V.PC, 25, 2, agent);
							if (V.PC.pregKnown) {
								t.push(pregText);
							}
						} else {
							t.push(`All three of you know, of course, that ${he} can't actually get you pregnant${isFertile(V.PC, true) ? ` this week` : ``}, but the fantasy is fun anyway.`);
						}
				...
				if (agent.fetish === Fetish.SUBMISSIVE) {
					...
					if (V.PC.dick > 0 && (agent.vagina >= 0 || agent.mpreg === 1)) {
						...
						if (canImpreg(V.PC, agent)) {
							...
							t.push(`Both of you know, of course, that ${he} can't actually get you pregnant${isFertile(V.PC, true) ? ` this week` : ``}, but the fantasy is fun anyway.`);
						}
						seX(V.PC, "vaginal", agent, "penetrative", 4);
					} else if (V.PC.pregKnown === 1) {
