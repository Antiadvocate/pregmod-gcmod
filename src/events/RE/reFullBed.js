App.Events.REFullBed = class REFullBed extends App.Events.BaseEvent {
	...
	execute(node) {
		...
		App.Events.addResponses(node, [
			...
			(canImpreg(bedSlaves[0], V.PC, !fertKnown(bedSlaves[0])) && canImpreg(bedSlaves[1], V.PC), !fertKnown(bedSlaves[1])) ? new App.Events.Result("Tire yourself out with some babymaking", babymaking, fuckNote()) : new App.Events.Result(),
		...
		function babymaking() {
			...
			t.push(`${bedSlaves[0].slaveName} is <span class="devotion inc">honored to ${fertKnown(bedSlaves[0]) ? `be bred by you` : `bear your children`} </span> and <span class="trust inc">snuggles even closer.</span>`);
			if (virgin0) {
			...
			tryKnockMeUp(bedSlaves[0], 100, bedSlaves[0].mpreg, V.PC);

			t.push(`${bedSlaves[1].slaveName} is <span class="devotion inc">thrilled to ${fertKnown(bedSlaves[1]) ? `carry` : `be potentially carrying`} your child</span> and <span class="trust inc">happily embraces the gift inside ${him2}.</span>`);
			if (virgin1) {
			...
			tryKnockMeUp(bedSlaves[1], 100, bedSlaves[1].mpreg, V.PC);
