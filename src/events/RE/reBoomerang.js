App.Events.REBoomerang = class REBoomerang extends App.Events.BaseEvent {
	...
	get weight() {
		return Math.trunc(getBoomerang().weeks / 5);
	}
	...
	execute(node) {
		...
		if (boomerang.actor === 0 || boomerang.buyer === 0) { // purely for typing
			return node;
		}
		const slave = boomerang.actor;
		const weeks = boomerang.weeks - 1;
		const pregWeeks = boomerang.weeks - 1;
		boomerang.weeks = 0;
		const buyer = boomerang.buyer;
		boomerang.buyer = 0;
		...
		*/
		...
		switch (buyer) {
			...
			case "obsessed fan":
				...
				if (V.seePreg !== 0 && canGetPregnant(slave, true, true)) {
			...
			case "concert hall":
				...
				if (V.seePreg !== 0) {
					if (slave.ovaries) {
						...
						slave.pregSource = either(SID.CITIZEN, SID.GENERIC);
						slave.pregWeek = slave.preg;
						slave.pregKnown = 1;
						SetBellySize(slave);
					}
		...
		function reSlave() {
			...
			if (boomerang.stats.PCpregSource > 0 && V.PC.preg > 0 && V.PC.pregSource === SID.GENERIC) {
				V.PC.pregSource = slave.ID;
			}
			if (boomerang.stats.PCmother > 0) {
				V.PC.mother = slave.ID;
			}
			if (boomerang.stats.PCfather > 0) {
				V.PC.father = slave.ID;
			}
			for (const s of getSlaves().values()) {
				if (boomerang.stats.boomerangMother.includes(s.ID)) {
					s.mother = slave.ID;
				}
				if (boomerang.stats.boomerangFather.includes(s.ID)) {
					s.father = slave.ID;
				}
				if (boomerang.stats.boomerangPregSources.includes(s.ID) && s.preg > 0 && s.pregSource === SID.GENERIC) {
					s.pregSource = slave.ID;
				}
				WombChangeID(s, boomerang.stats.missingParentTag, slave.ID);
			}
			if (V.incubator.capacity > 0) {
				for (let tank of getTankSlaves().values()) {
					if (boomerang.stats.boomerangMotherTank.includes(tank.ID)) {
						tank.mother = slave.ID;
					}
					if (boomerang.stats.boomerangFatherTank.includes(tank.ID)) {
						tank.father = slave.ID;
					}
				}
			}
			if (V.nursery > 0) {
				for (let crib of getInfants().values()) {
					if (boomerang.stats.boomerangMotherTank.includes(crib.ID)) {
						crib.mother = slave.ID;
					}
					if (boomerang.stats.boomerangFatherTank.includes(crib.ID)) {
						crib.father = slave.ID;
					}
				}
			}
			if (boomerang.stats.boomerangRelationship > 0) {
				const boomerangRelationship = getSlave(boomerang.stats.boomerangRelationship);
			...
			if (boomerang.stats.boomerangRivalry > 0) {
				const boomerangRivalry = getSlave(boomerang.stats.boomerangRivalry);
				if (boomerangRivalry) {
			...
			if (boomerang.stats.boomerangBody > 0) {
				const boomerangBody = getSlave(boomerang.stats.boomerangBody);
				if (boomerangBody) {
