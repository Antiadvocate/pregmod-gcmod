...
App.Events.RERelativeRecruiter = class RERelativeRecruiter extends App.Events.BaseEvent {
	...
	_chooseTargetRelative(slave) {
		...
		const gp = getGenePoolRecord(slave);
		if (!gp) {
			console.error(`Slave with ID ${slave.ID} is missing their genepool entry.`);
			slave.canRecruit = 0;
			return false;
		}
	...
	_getTargetRelativeChoices(slave, gp) {
		let recruit = [];
		if (!gp) {
			gp = getGenePoolRecord(slave);
		}

		const pushWeight = (value, weight=1) => recruit.push(...Array(weight).fill(value));

		if ((slave.mother === SID.GENERIC || (slave.mother < -999 && !(slave.mother in V.missingTable))) && V.seeDicks !== 100) {
			const momAge = getParentAgeRange(slave, false);
			if (momAge.max >= momAge.min) {
				pushWeight("mother", 3);
			}
		}
		if ((slave.father === SID.GENERIC || (slave.father < -999 && !(slave.father in V.missingTable))) && V.seeDicks !== 0) {
		...
		if (slave.genes === "XX" && slave.daughters < 3 && (slave.actualAge > V.minimumSlaveAge + V.fertilityAge + 1) && slave.trueVirgin !== 1) {
			if (V.seeDicks !== 100) {
		...
		} else if (slave.genes === "XY" && slave.daughters < 3 && (slave.actualAge > V.minimumSlaveAge + V.potencyAge + 1)) {
			if (V.seeDicks !== 100) {
				pushWeight("daughter", 2);
			}
			if (V.seeDicks !== 0) {
		...
		function makeSlave() {
			...
			function applyBackground(slave) {
				...
				function applyBedMILF() { // assumes XX
					...
					slave.voice = 2;
					if (V.seePreg !== 0 && random(1, 5) === 1) {
						let setPreg = false;
						if (slave.ovaryAge < 47) {
							setPreg = true;
						} else if (slave.physicalAge < 70 && random(1, 5) === 1) {
							setPreg = true;
							slave.ovaryAge = 46;
						}
						if (setPreg) {
			...
			const genepoolRec = getGenePoolRecord(that.actors[0]);
			...
			if (that.params.relative === "mother") {
				newSlave.counter.birthsTotal += getSlaves().asArray().reduce((acc, cur) => acc + (sameDad(eventSlave, cur) ? 1 : 0), 0);
			}
		...
		function buySlave() {
			...
			switch (that.params.relative) {
				...
				case "twin": { // siblings
					...
					} else if (eventSlave.mother === SID.GENERIC) {
						setMissingParents(eventSlave);
