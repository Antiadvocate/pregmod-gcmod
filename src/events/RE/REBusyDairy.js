App.Events.REBusyDairy = class REBusyDairy extends App.Events.BaseEvent {
	...
	execute(node) {
		...
		/** @returns {string} */
		function share() {
			const dairySlaves = getSlaves().filter(s => s.assignment === Job.DAIRY);
			cashX(-1000, "event");
			for (const slave of dairySlaves.values()) {
				slave.devotion += 4;
				SimpleSexAct.Slaves(slave, dairySlaves.filter(s => s.ID !== slave.ID).randomValue());
