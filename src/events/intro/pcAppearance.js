...
App.UI.Player.appearance = function(options, summary = false) {
	...
	if (V.PC.vagina > -1) {
		options.addOption("You would prefer being penetrated in the", "preferredHole", V.PC)
			.addValueList([["No preference", -1], ["Vagina", 1], ["Anus", 0]]);
	}
		...
		if (V.PC.vagina !== -1 && V.PC.pubertyXX === 1) {
			...
			if (V.cheatMode) {
				...
				options.addOption("Pregnancy Source", "pregSource", V.PC)
					.addValueList([
						["Unknown", SID.GENERIC],
						["Self-impregnation", SID.PC],
						["Citizen", SID.CITIZEN],
						["Former Master", SID.FORMERMASTER],
						["Male arc owner", SID.ARCOWNER],
						["Client", SID.CLIENT],
						["Societal Elite", SID.ELITE],
						["Designer baby", SID.LAB],
						["Futanari Sister", SID.FUTA],
						["Rapist", SID.RAPIST],
					])
					.showTextBox()
					.pulldown();
