...
App.Intro.summary = function() {
	...
	if (V.freshPC === 1 || V.saveImported === 0) {
		...
		V.PC.underArmHColor = V.PC.hColor;
	}
	...
	function introContent() {
		...
		const cheatStart = () => {
			...
			if (V.freshPC === 1 || V.saveImported === 0) {
				if (isInGenePool(SID.PC)) {
					deleteGenePoolRecord(V.PC, true);
				}
				addToGenePool(V.PC);
			}
		...
		linkArray.push(
			App.UI.DOM.link(
				...
				() => {
					continueNormal();
					V.temp.limitedCheatStart = true;
				},
				[],
				"Starting Girls",
				"Allow you to set how much cash you start with"
			)
		);
		linkArray.push(
			App.UI.DOM.link(
				"Cheat Start with a few random arcology slaves",
				() => {
					V.temp.newCheatGirls = true;
					cheatStart();
				},
				[],
				"Starting Girls",
				"Intended for debugging; Including debugging of the hero slave system; May have unexpected effects"
			)
		);
	...
	function modContent() {
		...
		// TODO: Move me to scenarios/scenario starts in the future. Right now I just have no place better to be. - Pregmodder
		if (V.menstruation === 1) {
	...
	function slavesContent() {
		...
		options.addOption("Hero slave lolifier", "lolifier").addComment("For those unsatisfied with lolimode's version.")
			.addValue("Enabled", 1).on().addValue("Disabled", 0).off();
			...
			App.UI.DOM.link(
				...
				() => {
					...
					if (V.targetArcology.fs !== "New") {
						...
						if (!App.Data.Arcology.Terrain.includes(V.targetArcology.terrain)) {
							V.targetArcology.terrain = App.Data.Arcology.Terrain.random();
						}
						V.terrain = V.targetArcology.terrain;
						if (!App.Data.Arcology.Continents.includes(V.targetArcology.continent)) {
