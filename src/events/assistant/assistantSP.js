...
App.Events.assistantSP = class assistantSP extends App.Events.BaseEvent {
	...
	execute(node) {
		...
		const slave = getSlaves().find(s => (s.vagina > 0 || s.anus > 0) && s.rules.release.masturbation === 1)?.value ?? getSlaves().randomValue();
		...
		if (getSlaves().some(s => s.piercing.genitals.smart)) {
