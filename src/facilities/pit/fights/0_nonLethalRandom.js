		...
		function postFight(parent) {
			...
			if (V.pit.virginities === "all") {
				if (loser.vagina === 0 && canDoVaginal(loser) && loser.anus === 0 && canDoAnal(loser)) {
					...
					seX(loser, oral, winner, 'penetrative');
					...
					if (canPenetrate(winner) && canImpreg(loser, winner, !fertKnown(loser)) && loser.mpreg) {
						r.push(loserProtest);

						tryKnockMeUp(loser, 50, 1, winner);
					}

					seX(loser, anal, winner, 'penetrative');
				} else if (loser.anus === 0 && canDoVaginal(loser) && canDoAnal(loser)) {
					r.push(`${He} pushes ${loser.slaveName}'s back down onto the mat, forces ${his2} ${hasBothLegs(loser) ? `legs apart` : !hasAnyLegs(loser) ? `hips steady` : `leg aside`},`);

					if (winner.fetish === "buttslut" || (canPenetrate(winner) && canImpreg(loser, winner, !fertKnown(loser)) && loser.mpreg && winner.fetish === "pregnancy")) {
					...
					if (canPenetrate(winner) && canImpreg(loser, winner, !fertKnown(loser))) {
						r.push(loserProtest);

						tryKnockMeUp(loser, 50, 0, winner);
					}

					seX(loser, vaginal, winner, 'penetrative');
				...
				if (loser.vagina === 0 && canDoVaginal(loser)) {
					...
					r.push(`${canTalk(loser)
						? `${loser.slaveName} starts to scream a protest to stop ${winner.slaveName} raping ${him2} pregnant, but ${winner.slaveName} grinds ${his2} face into the mat to shut ${him2} up`
						: `${loser.slaveName} tries to gesture a protest before ${winner.slaveName} fills ${his2} ${fertKnown(loser) ? `fertile` : `potentially fertile`} pussy with cum, but ${winner.slaveName} grabs ${his2} ${hasBothArms(loser) ? `hands and pins them` : hasAnyArms(loser) ? `hand and pins it` : `neck firmly`} to keep ${him2} from complaining`}.`);
