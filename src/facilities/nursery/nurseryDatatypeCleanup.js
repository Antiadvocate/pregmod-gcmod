...
App.Facilities.Nursery.ChildDatatypeCleanup = function(child) {
	...
	function childCosmeticsDatatypeCleanup(child) {
		...
		delete child.scars; // The scars property is deprecated
...
App.Facilities.Nursery.InfantDatatypeCleanup = function(child) {
	...
	if (getInfants().find((s) => s.ID === child.ID && (s.birthWeek !== child.birthWeek || s.actualAge !== child.actualAge)) !== undefined ||
	// @ts-expect-error V.slaves and V.incubator.tanks were/are an array at this point in time
