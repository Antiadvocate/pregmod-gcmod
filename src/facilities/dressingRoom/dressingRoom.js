...
App.UI.DressingRoom.render = function() {
	...
	if (!App.UI.DressingRoom.modelId || !getSlave(App.UI.DressingRoom.modelId)) {
		...
		const defaultModelChoices = V.favorites?.length ? V.favorites.map(slaveId => getSlave(slaveId)) : getSlaves().asArray();
		...
		function createCell(clothingName, clothesProperties) {
			...
			function createLabel() {
				...
				function createComment(type) {
					...
					} else if (customClothesPrompts[clothingName]) {
						comment = customClothesPrompts[clothingName][type];
					}
			...
			function ifClothesMatch(elem) {
				...
				switch (searchMode) {
					...
					case 'positive prompts':
						match = !!customClothesPrompts[elem.id]?.positive.toLowerCase().match(query) || !!customClothesPrompts[elem.id]?.positive.toLowerCase().match(query);
						break;
					case 'negative prompts':
						match = !!customClothesPrompts[elem.id]?.negative.toLowerCase().match(query) || !!customClothesPrompts[elem.id]?.negative.toLowerCase().match(query);
						break;
					case 'fs loves':
