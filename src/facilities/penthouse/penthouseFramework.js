...
App.Entity.Facilities.PenthouseJobs = {
	...
	SubordinateSlave: class extends App.Entity.Facilities.Job {
		...
		assignmentLinkElement(ID, passage, callback, linkText) {
			...
			return App.UI.DOM.assignmentLink(getSlave(ID), this.desc.assignment, "Subordinate Targeting",
				(slave, assignment) => {
					if (callback) {
	...
	Cow: class extends App.Entity.Facilities.Job {
		...
		checkRequirements(slave) {
			...
			if (slave.lactation === 0 && (slave.balls === 0 || cumMilkingBlocked)) {
				r.push(`${slave.slaveName} is not lactating` + ((V.seeDicks > 0 && slave.balls === 0) ? ' or producing semen.' : '.'));
