App.UI.inspectTankSettings = function(isFetus, isPCMother = false) {
	...
	if (!isFetus) {
		child = getTankSlave(V.temp.AS);
		tankSetting = child.incubatorSettings;
	} else {
		if (isPCMother) {
			fetus = V.PC.womb[V.temp.activeFetus];
		} else {
			fetus = getSlave(V.temp.AS).womb[V.temp.activeFetus];
		}
	...
	function content() {
		...
		section = App.UI.DOM.makeElement("div");
		if (incubator.upgrade('reproduction') === 1 && incubator.upgrade('pregAdaptation') === 1 && tankSetting.pregAdaptation === 1) {
			linkArray = [];
		...
		if (incubator.upgrade('reproduction') === 1 && incubator.upgrade('pregAdaptation') === 1 && !isFetus) {
