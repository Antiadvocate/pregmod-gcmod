 ...
 * @property {string} varSuffix
 * @property {(slave: FC.SlaveState) => number} classifySlave
 * @property {() => number} classifyLocalPreference
 */

/** @type {{[key: string]: AdCategory}} */
App.Ads.Categories = {};
...
App.Ads.report = function(building, preview = false) {
	...
	function payBonus() {
		if (!preview) {
			/** @type {"brothelAds"|"clubAds"} */
			const cashID = `${building}Ads`;
			if (building === "brothel") {
				const adsIncome = DL * random(20, 30);
				V.facility[building].adsIncome += adsIncome;
				cashX(adsIncome, cashID);
			} else if (building === "club") {
				repX(DL * random(5, 10), cashID);
			}
		}
	}
	...
	/**
	 * @param {keyof App.Ads.Categories} category
	...
	function links(category) {
		...
		switch (category) {
			...
			case "age":
				...
				break;
			default:
				console.log(`You done fucked up. Building "${building}" in category "${String(category)}"`);
		}
