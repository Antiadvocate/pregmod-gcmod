 ...
 * @param {boolean} [cheat=false] if true, will hide scenes, prevent damage to slaves, and keep the player from being billed for mods.
 * @param {boolean} [disableLinks=false] if true, then we will disable any links that would lead out of the current passage
 */
App.UI.bodyModification = function(slave, cheat = false, disableLinks = false) {
			...
			if (slave.anusTat === "bleached") {
				linkArray.push(
					App.UI.DOM.link(
						...
						() => {
							tattooChoice = 0;
							modReaction += App.Medicine.Modification.setTattoo(slave, "anus", tattooChoice, cheat);
							refresh();
						}
					)
				);
