...
App.Facilities.Dairy.dairy = class Dairy extends App.Facilities.Facility {
	...
	get rules() {
		// TODO: Add a setting to respect Virginity. Default will take virginities.
		// Variables below preserve the current setting so effects know which setting they are transitioning from.
		let currentFeederSetting = V.dairyFeedersSetting;
		let currentPregSetting = V.dairyPregSetting;
		let currentStimulatorSetting = V.dairyStimulatorsSetting;
		let currentRestraintSetting = V.dairyRestraintsSetting;
		return [
			{
				...
				options: [
					{
						...
						link: `Deactivate`,
						value: 0,
						handler: App.UI.DialogHandler(() => {
							// This ensures the current settings gets updated correctly after someone selects a setting.
							const output =	this._feederEffect(currentFeederSetting);
							currentFeederSetting = 0;
							return output;
						}),
					},
					{
				...
				options: [
					{
						...
						value: 0,
						handler: App.UI.DialogHandler(() => {
							const output = this._pregEffects(currentPregSetting);
							currentPregSetting = 0;
							return output;
						}),
					},
					{
						...
						handler: App.UI.DialogHandler(() => {
							const output = this._pregEffects(currentPregSetting);
							currentPregSetting = 1;
							return output;
						}),
						note: `${V.arcologies[0].FSGenderRadicalistResearch === 1 ? `Requires sodomizers to be active in order to impregnate anal wombs.` : ``}`
				...
				options: [
					{
						...
						value: 0,
						handler: App.UI.DialogHandler(() => {
							const option = this._stimulatorEffects(currentStimulatorSetting);
							currentStimulatorSetting = 0;
							return option;
						}),
					},
					{
				...
				options: [
					{
						...
						value: 0,
						handler: App.UI.DialogHandler(() => {
							const option = this._restraintEffects(currentRestraintSetting);
							currentRestraintSetting = 0;
							return option;
						}),
					},
					{
	...
	/**
	 * Effects for Feeder Options
	 * @param {number} prevSetting
	 */
	_feederEffect(prevSetting) {
		const text = [];
		const cowCount = this.facility.hostedSlaves();

		// Industrial and more than 1 slave
		if (V.dairyRestraintsSetting === 2 && cowCount > 2) {
			if (prevSetting === 2) {
				text.push(`In unison, the milking machines withdraw their feeders from the slaves' throats. The slaves gag and cough, strings of feeding fluid and saliva running between their lips and the heads of the feeding phalli.`);
				if (V.dairyFeedersSetting === 1) {
					text.push(`These remain close to their faces so that the slaves can suck them off once they get hungry, which they will, soon.`);
				}
				text.push(`The slaves' mouths and tongues are very tired, and most of them rest with their mouths open and their tongues hanging out.`);
			} else if (V.dairyFeedersSetting === 2 && prevSetting < 2) {
	...
	/**
	 * Effects for Pregnancy Options
	 * @param {number} prevSetting
	 */
	_pregEffects(prevSetting) {
		const text = [];
		let vaginaCount = 0;
		let fertileCount = 0;
		if (V.dairyPregSetting >= 2) { // Industrial - will ignore chastity belts as they should be removed via other actions.
			for (const slave of this.facility.employees()) {
				let removed = false;
				const {He, him, his} = getPronouns(slave);
				if (slave.vagina.isBetween(-1, 3) && V.dairyPrepUpgrade === 0) { // 0, 1, 2
				...
				if ((slave.broodmother > 0 || slave.bellyImplant !== -1) && !removed) {
				...
				if (!removed && slave.vagina.isBetween(-1, 3) && V.dairyPrepUpgrade === 1 ) {
					text.push(`The milking machine detects ${slave.slaveName}'s ${vaginaDesc(slave, true)} is too tight for its massive dildo and is <span class="change positive">mercilessly fucked and stretched</span> by the preparatory raper.`);
					if (slave.vagina === 0) {
						text.push(`This breaks in ${his} <span class="virginity loss">virgin pussy.</span>`);
					}
					slave.vagina = 3;
				}
				if (!removed && slave.vagina > -1) {
					vaginaCount++;
					// Don't need to check mPreg here as will have vagina
					if (isFertile(slave, !fertKnown(slave) && !isMenstruating(slave))) {
						fertileCount++;
					}
				}
			}
		...
		// Industrial
		if (vaginaCount > 2 && V.dairyRestraintsSetting === 2) {
			// Going from none or normal to industrial.
			if (V.dairyPregSetting >= 2 && prevSetting < 2) {
				if (prevSetting === 1) {
					text.push(`In unison, the milking machines withdraw their dildos from the slaves' vaginas.`);
				}

				text.push(`The auxiliary drug injectors hiss as the slaves are filled with drugs that promote natural lubrication. The slaves begin to shift awkwardly as they feel their pussies begin to drool slick female fluids. Once a machine judges that its slave's cunt is sufficiently wet, it readies a gigantic dildo. The slaves cannot see their own groins, but as soon as the heads of the dildos touch their pussylips, they begin to`);
				...
				text.push(`As the massive phalli begin to ejaculate ${fertileCount > 0 ? `fertility drugs and semen` : `curatives and drugs`}, they drive all resistance out of the poor cows.`);
			} else if (V.dairyPregSetting < 2 && prevSetting >= 2) {
				text.push(`In unison, the milking machines withdraw their monstrous dildos from the slaves' stretched cunts. Their pussies' overcharged production of natural lubricant produces a gush of pent-up female fluids from each loose vagina as the phalli slide clear.`);
				if (V.dairyFeedersSetting < 2) {
					text.push(`The slaves moan with relief at the sudden reduction in fullness. Being penetrated like that must be quite uncomfortable.`);
				} else {
	...
	/**
	 * Effects for Anal Stimulator Options
	 * @param {number} prevSetting
	 */
	_stimulatorEffects(prevSetting) {
		const text = [];
		let cowCount = this.facility.hostedSlaves();
		let mpregCount = 0;

		// Ignores chastity as industrial settings strip slaves.
		if (V.dairyStimulatorsSetting === 2) {
			for (const slave of this.facility.employees()) {
				if (slave.anus.isBetween(-1, 3)) {
					const {He, him, his} = getPronouns(slave);
					if (V.dairyPrepUpgrade === 0) {
						...
						text.push(`The milking machine detects ${slave.slaveName}'s ${anusDesc(slave, true)} is too tight for its massive anal dildo and is <span class="change positive">mercilessly fucked and stretched</span> by the preparatory raper.`);
						if (slave.anus === 0) {
							text.push(`This breaks in ${his} <span class="virginity loss">virgin anus.</span>`);
						}
						slave.anus = 3;
					}
				}

				if (slave.mpreg === 1 && isFertile(slave, !fertKnown(slave) && !isMenstruating(slave))) {
					mpregCount++;
				}
			}
		}
		...
		// Industrial
		if (V.dairyStimulatorsSetting === 2 &&  cowCount > 2) {
			// This works for both setting 0 and setting 1.
			...
			text.push(`they begin to ${V.dairyFeedersSetting < 2 ? 'scream and struggle instinctively' : 'struggle wildly'}. As the constant assrape that will define their existences for the foreseeable future begins in earnest, their ${V.dairyFeedersSetting < 2 ? 'whining' : 'wriggling'}`);
			...
			if (mpregCount > 0 && V.dairyPregSetting > 0) {
				text.push(`Periodically, the dildos expel ${V.dairyPregSetting > 1 ? `a massive amount of ` : ``}cum and fertility drugs into slaves with a fertile anal womb.`);
			}
		} else if (V.dairyStimulatorsSetting < 2 && prevSetting > 0) {
			text.push(`In unison, the milking machines withdraw their ${prevSetting === 2 ? 'gargantuan ' : ''}dildos from the slaves' ${prevSetting === 2 ? 'loosened ' : ''} anuses.`);
	...
	/**
	 * Effects for Restraint Options
	 * @param {number} prevSetting
	 */
	_restraintEffects(prevSetting) {
		const text = [];
		let cowCount = this.facility.hostedSlaves();
		...
		if (V.dairyRestraintsSetting > 0) {
			for (const slave of this.facility.employees()) {
				if (slave.indentureRestrictions > 1 || (slave.indentureRestrictions > 0 && V.dairyRestraintsSetting === 1)) {
					...
					const {He, he, him} = getPronouns(slave);
					text.push(`${slave.slaveName}'s milking machine declines to restrain ${him}, since ${he} is an Elite breeder and ineligible for restrained milking. <span class="yellow">${He} has been kicked out of ${this.facility.name}.</span>`);
					removeJob(slave, Job.DAIRY);
					cowCount--;
				} else if (V.dairyRestraintsSetting === 2) {
					// Strip slave for Industrial Processing
					App.Facilities.Dairy.strip(slave);
				}
			}
		}
		if (cowCount > 1) {
			if (V.dairyRestraintsSetting === 1 && prevSetting === 0) {
			...
			} else if (V.dairyRestraintsSetting === 0 && prevSetting === 1) {
			...
			} else if (V.dairyRestraintsSetting === 2 && prevSetting < 2) {
				if (S.Milkmaid) {
