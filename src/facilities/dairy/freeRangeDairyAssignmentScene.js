...
App.Facilities.Dairy.freeRangeAssignmentScene = function(slave) {
	...
	const fertUnknown = !fertKnown(slave) && !isMenstruating(slave);
	const potentiallyFertile = isFertile(slave, fertUnknown);
	...
	if (App.Entity.facilities.dairy.employeesIDs().size > 1 && canSee(slave)) {
		...
		App.Events.addParagraph(node, r);
		r = [];
		if (slave.fetish === "boobs" && getSlaves().filter(s => s.assignment === Job.DAIRY && s.lactation > 0).size > 0) {
			aroused = true;
			...
			if (slave.sexualFlaw === "breeder" && potentiallyFertile) {
		...
		const cow = getSlaves().find(s => s.assignment === Job.DAIRY && s.ID !== slave.ID && s.balls >= 10 && s.dick > 0 && isVirile(slave))?.value;
		if (cow) {
			const {
				He2, His2,
				he2, his2, him2,
			...
			if (V.dairyStimulatorsSetting === 1 && canDoAnal(cow)) {
				if (cow.prostate !== 0) {
			...
			} else if (V.MilkmaidID !== 0 && (S.Milkmaid.dick > 4 || S.Milkmaid.clit > 4) && canPenetrateAny(S.Milkmaid) && canDoAnal(cow)) {
	...
	if (assayedSlave) {
		...
		if (assayedSlave && assayedSlave.assignment === Job.DAIRY) {
			assayType = relationshipTerm(slave);
		}
	}
	if (assayType) {
		const {
	...
	if (V.MilkmaidID === 0 && V.dairyStimulatorsSetting === 0) {
		...
		if (V.MilkmaidID !== 0) {
			...
			if ((S.Milkmaid.dick > 4 || S.Milkmaid.clit > 4) && canPenetrateAny(S.Milkmaid)) {
				r.push(`Most often, ${he2} uses ${his2} ${dickOrClit(S.Milkmaid)} to penetrate the cows.`);
			} else {
				r.push(`Lacking ${S.Milkmaid.dick > 0 ? `penile equipment` : `a clit`} of appropriate size, ${he2} relies on ${his2} tongue to stimulate the cow's sexual organs.`);
			}
		...
		if (V.dairyStimulatorsSetting > 0) {
			...
			// TODO: Use when virginity setting is added to dairy
		 ...
		 if (!canDoAnal(slave)) {
				r.push(`The machine stays inactive to respect the chastity device protecting ${his} ${anusDesc(slave, true)}.`);
		 } else {
				...
				if (slave.anus === 0) {
					r.push(`<span class="virginity loss">This breaks in ${his} ${anusDesc(slave, true)}.</span>`);
					slave.anus = 1;
				}
				...
				// Handle mpreg
				if (V.dairyPregSetting > 0 && slave.mpreg === 0 && potentiallyFertile) {
					r.push(`The machine detects a ${fertUnknown ? ` potentially` : ''} fertile anal womb and releases a large amount of potent seed and fertility drugs.`);
				}
				if (slave.dick > 0) {
					if (slave.prostate !== 0) {
					...
					r.push(`The dildo fucks ${his} ${slave.prostate > 0 ? 'prostate' : 'ass'} extra hard to make up for ${his} ${slave.dick > 0 ? 'restrained' : 'lack of'} penis.`);
				}
			}
			App.Events.addParagraph(node, r);
			r = [];
		...
		if (V.dairyPregSetting > 0) {
			...
			if (slave.vagina > -1) {
				// TODO: Use line below when virginity setting is added to dairy
				...
				if (!canDoVaginal(slave)) {
					r.push(`The automatic dildo stays inactive in order to respect the chastity device protecting ${his} ${vaginaDesc(slave, true)}.`);
				} else {
					r.push(`In order to provide adequate stimulation for ${his}`);
					if (slave.vagina > 3) {
						...
						r.push(`well-used cunt, the automatic dildo is equipped with a large phallus. It is nicely ribbed for extra stimulation.`);
					} else {
						r.push(`pussy, the automatic dildo is equipped with a fair-sized silicone phallus.`);
					}
					r.push(`Lubrication spurts from a hole at the tip, giving the artificial phallus a very lifelike pre-dripping look.`);
					r.push(`Gently, the mechanical arm pushes the phallus into ${slave.slaveName}'s vagina.`);
					if (slave.vagina === 0) {
						r.push(`<span class="virginity loss">This breaks in ${his} ${vaginaDesc(slave, true)}.</span>`);
						slave.vagina = 1;
					}
					...
					r.push(`Soon, the automatic impregnator pumps the dildo in and out of ${his} body${potentiallyFertile && slave.mpreg === 0 ? `frequently releasing large loads of potent cum directly into ${his} womb.` : `.`}`);
				}
			}
		}
	}
	...
	if (slave.dick > 0) {
		...
		if (isVirile(slave)) {
			...
			if (slave.pubertyXY === 0 && slave.balls > 0) {
				r.push(`${His} immature balls fail to contribute to ${his} ejaculation, but this exotic fluid is collected regardless.`);
			} else if (slave.balls > 0) {
				r.push(`${His} ${ballsDesc(slave, true)} fail to add much to ${his} cum, greatly reducing it's volume.`);
			} else if (slave.balls === 0) {
				r.push(`${His} ballslessness impedes cum-production, but ejaculating feels nice, anyway.`);
			}
		}
	}
	if (slave.dick < 1 && slave.balls > 0) {
	...
	App.Events.addParagraph(node, r);
	r = [];
	if (slave.genes === GenderGenes.FEMALE && slave.prostate > 0 && slave.balls === 0 && slave.vagina > -1) {
		r.push(`${slave.slaveName}`);
		r.push(`has no balls, but a functional prostate gland. It's attached to ${his} urethra, so ${he} squirts`);
		if (slave.prostate > 2) {
			r.push(`massively`);
		} else {
			r.push(`copiously`);
	...
	if (V.dairyPregSetting > 0 && potentiallyFertile && canDoVaginal(slave)) {
	...
	if (slave.dick > 0) {
		r.push(`The suction on ${his} dick stops, allowing ${his} abused member to ${canAchieveErection(slave) ? `go soft and ` : ``}have some rest.`);
	}
	if (slave.balls > 0) {
		r.push(`${His} balls rest${slave.scrotum > 0 ? `, dangling down and relieved` : ''}. They have already started to produce more cum. They seem to wait for their next chance to unload.`);
	}
	App.Events.addParagraph(node, r);
