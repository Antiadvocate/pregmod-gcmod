/**
 * Expects V.dairyRestraintsSetting >= 2 && ((V.dairyStimulatorsSetting >= 2) || (V.dairyFeedersSetting >= 2) || (V.dairyPregSetting >= 2)
 * Slave will be fully stripped of all clothing and chastity and can be of any devotion and trust level.
...
App.Facilities.Dairy.industrialAssignmentScene = function(slave) {
	...
	// Set clothing and accessories - fully strip them (this is done weekly in the dairy Report).
	App.Facilities.Dairy.strip(slave);
	App.Events.drawEventArt(node, slave, "no clothing");

	// Will the vaginal impregnator be active? They can either be potentially fertile OR known to be pregnant.
	const impregnator = (isFertile(slave, true) || slave.pregKnown === 1) && slave.mpreg === 0;
	...
	if (slave.sexualFlaw === "self hating") { /* GETTING IN */
		...
		if (hasAnyArms(slave)) {
			if (V.dairyStimulatorsSetting > 1) {
				...
				if (V.dairyPregSetting > 1 && impregnator) {
	...
	} else if ((slave.sexualFlaw === SexualFlaw.BREEDER) && impregnator) {
		...
		if (canTalk(slave)) {
			...
			if (V.dairyPregSetting > 2 && impregnator) {
			...
			} else if ((V.dairyPregSetting > 1) && impregnator) {
	...
	if (slave.anus < 3 || (slave.vagina < 3 && slave.vagina > -1)) {
		...
		if (V.dairyPregSetting > 1 && slave.vagina > -1 && slave.vagina < 3) {
			r.push(`${His} pussy is nowhere near ready to accept the passage of drugs ${impregnator ? `from the giant dildo` : `and cum in and new slaves out`}. The new machine lubes ${him} thoroughly and then`);
			if (slave.vagina === 0) {
				r.push(`<span class="virginity loss">takes ${his} virginity</span>`);
				slave.vagina = 1;
			} else {
		...
		if (V.dairyStimulatorsSetting > 1 && slave.anus < 3) {
			...
			if (slave.anus === 0) {
				r.push(`<span class="virginity loss">virgin anus</span>`);
				slave.anus = 1;
			} else {
				r.push(`rectum`);
			}
		...
		} else if (V.dairyStimulatorsSetting > 1 && slave.anus < 3) {
	...
	if (V.dairyPregSetting > 1 && slave.vagina > -1) {
	...
	} else if (V.dairyPregSetting > 0 && slave.vagina > -1) {
		if (slave.devotion > 95) {
		...
		if (slave.vagina === 0) {
			r.push(`This breaks in ${his} <span class="virginity loss">virgin pussy.</span>`);
		}
	}
	if (V.dairyStimulatorsSetting > 1) {
	...
	} else if (V.dairyPregSetting > 0 && impregnator) {
		r.push(`The dildo in ${his} vagina ejaculates cum and fertility drugs.`);
	} else if (V.dairyPregSetting > 0 && slave.vagina > -1) {
		r.push(`The ${V.dairyPregSetting > 1 ? 'massive ' : ''}dildo in ${his} vagina ejaculates medication and fucks ${him} ${V.dairyPregSetting ? 'relentlessly' : 'mechanically'}.`);
	}
	...
	if (slave.balls > 0) {
		if (V.dairyStimulatorsSetting > 0) {
			...
			if (V.dairyFeedersSetting > 1) {
				r.push(`${He} tries to scream around the dildo in ${his} mouth.`);
			}
		}
		if (slave.scrotum > 0) {
