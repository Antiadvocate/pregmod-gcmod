...
App.UI.mediaStudio = function() {
	...
	r.push(`${num(summary.actors)} of your ${num(slaveCount())} slaves are currently featured in porn. You are spending a total of ${cashFormatColor(summary.totalCost)} promoting ${num(summary.promoted)} of them.`);
	r.toParagraph();

	App.UI.DOM.appendNewElement("h2", t, `Genre Coverage`);

	// the returns below is valid and also attempting to disable this warning doesn't work
	/**
	 * @template {number} Num
	 * @param {Num} number
	 * @returns {`<span class="gray">-</span>` | `${Num}`}
	 */
	...
	if ((V.seeImages === 1) && (V.seeSummaryImages === 1)) {
		batchRenderer = new App.Art.SlaveArtBatch(getSlaves().map(s => s.ID), 1);
