App.Facilities.ServantsQuarters.servantsQuarters = class ServantsQuarters extends App.Facilities.Facility {
	constructor() {
		...
		const decommissionHandler = () => {
			let sq = App.Entity.facilities.servantsQuarters;
			sq.capacity = 0;
			sq.name = "the Servants' Quarters";
			sq.decoration = "standard";
			sq.setUpgrade('monitoring', 0);
			sq.setRule('stewardessImpregnates', 0);
		};
	...
	get decorations() {
		...
		const res = FS[this.facility.decoration];

		if (!res) {
			throw new Error(`Unknown decoration value of '${this.facility.decoration}' found in decorations().`);
		}
	...
	get expand() {
		return {
			desc: `${this.facility.nameCaps} has room to keep ${numberWithPluralOne(this.facility.capacity, "slave")} while they serve. There ${this.facility.hostedSlaves() === 1 ? `is currently ${num(this.facility.hostedSlaves())} slave` : `are currently ${num(this.facility.hostedSlaves())} slaves`} serving in ${this.facility.name}.`,
	...
	get upgrades() {
		return [
			{
				object: V.servantsQuarters.upgrades,
				property: "monitoring",
				tiers: [
					{
						value: 0,
						...
						cost: 10000 * globalThis.upgradeMultiplier('engineering') * globalThis.upgradeMultiplier('hacking'),
						handler: () => globalThis.IncreasePCSkills('hacking', 0.1),
	...
	get rules() {
		return [
			{
				object: V.servantsQuarters.rules,
	...
	get staffing() {
		...
		} else if (capacity >= slavesLength * 0.95) {
		...
		} else if (capacity >= slavesLength * 0.5) {
		...
		div.append(` They can currently help reduce the maintenance of up to ${num(capacity)} slaves, and you have ${num(slavesLength)}.`);
