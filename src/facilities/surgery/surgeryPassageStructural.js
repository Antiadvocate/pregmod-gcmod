	...
	function content() {
				...
				if (slave.shouldersImplant === 0 || V.surgeryUpgrade === 1) {
					if (slave.shoulders < 2) {
						linkArray.push(App.Medicine.Surgery.makeLink(
							new App.Medicine.Surgery.Procedures.BroadenShoulders(slave),
							refresh, cheat, facilityControlledAspects
						));
					}
					if (slave.shoulders > -2) {
						linkArray.push(App.Medicine.Surgery.makeLink(
							new App.Medicine.Surgery.Procedures.NarrowShoulders(slave),
							refresh, cheat, facilityControlledAspects
						));
					}
				}
		...
		function hips() {
			...
			if (slave.indentureRestrictions > 0) {
				...
				if (slave.hipsImplant === 0 || V.surgeryUpgrade === 1) {
					if (slave.hips < 2 || (slave.hips < 3 && V.surgeryUpgrade === 1)) {
						linkArray.push(App.Medicine.Surgery.makeLink(
							new App.Medicine.Surgery.Procedures.BroadenPelvis(slave),
							refresh, cheat, facilityControlledAspects
						));
					}
					if (slave.hips > -2) {
						linkArray.push(App.Medicine.Surgery.makeLink(
							new App.Medicine.Surgery.Procedures.NarrowPelvis(slave),
							refresh, cheat, facilityControlledAspects
						));
					}
				}
		...
		function limbLength() {
			...
			if (slave.indentureRestrictions < 1) {
				if (slave.heightImplant === 0) {
					linkArray.push(App.Medicine.Surgery.makeLink(
						new App.Medicine.Surgery.Procedures.IncreaseHeight(slave),
						refresh, cheat, facilityControlledAspects
					));
					linkArray.push(App.Medicine.Surgery.makeLink(
						new App.Medicine.Surgery.Procedures.DecreaseHeight(slave),
						refresh, cheat, facilityControlledAspects
					));
		...
		function tendons() {
			...
			if (slave.heels === 0 && hasAnyNaturalLegs(slave) && V.seeExtreme === 1) {
				if (slave.indentureRestrictions < 1) {
					linkArray.push(App.Medicine.Surgery.makeLink(
						new App.Medicine.Surgery.Procedures.ShortenTendons(slave),
						refresh, cheat, facilityControlledAspects
					));
				}
			}
			if (slave.heels === 1) {
				linkArray.push(App.Medicine.Surgery.makeLink(
					new App.Medicine.Surgery.Procedures.ReplaceTendons(slave),
					refresh, cheat, facilityControlledAspects
				));
			}
			App.Events.addNode(el, r, "div");
		...
		function prostheticInterface() {
			...
			if (!hasAllNaturalLimbs(slave)) {
				if (slave.PLimb === 0 && isProstheticAvailable(slave, "interfaceP1")) {
					linkArray.push(App.Medicine.Surgery.makeLink(
						new App.Medicine.Surgery.Procedures.BasicPLimbInterface(slave),
						refresh, cheat, facilityControlledAspects
					));
				}
				if (slave.PLimb !== 2 && isProstheticAvailable(slave, "interfaceP2")) {
					linkArray.push(App.Medicine.Surgery.makeLink(
						new App.Medicine.Surgery.Procedures.AdvancedPLimbInterface(slave),
						refresh, cheat, facilityControlledAspects
					));
				}
				if (slave.PLimb !== 3 && isProstheticAvailable(slave, "interfaceP3")) {
					linkArray.push(App.Medicine.Surgery.makeLink(
						new App.Medicine.Surgery.Procedures.QuadrupedalPLimbInterface(slave),
						refresh, cheat, facilityControlledAspects
					));
				}
			}
