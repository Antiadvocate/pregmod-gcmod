...
App.UI.surgeryPassageExotic = function(slave, refresh, cheat = false) {
	...
	function content() {
		...
		function geneTesting() {
			...
			App.UI.DOM.appendNewElement("li", slaveGeneTest, `Genetic intelligence index: ${geneRecord.intelligence} out of 100`);
			App.UI.DOM.appendNewElement("li", slaveGeneTest, `Genetic face index: ${geneRecord.face} out of 100`);
		...
		function geneTherapy() {
			...
			function retroVirus() {
				...
				/** @type {keyof FC.GeneticQuirks} */
				let gene;
				for (gene in slave.geneticQuirks) {
				...
				if (canEditGenes) {
					el.append(App.UI.DOM.makeSelect(options, null, gene => {
						jQuery(description).empty().append(describeGene(/** @type {keyof FC.GeneticQuirks} */ (gene)));
					}));
				}
				...
				/**
				 * @param {keyof FC.GeneticQuirks} selectedGene
				 * @returns {DocumentFragment}
				 */
