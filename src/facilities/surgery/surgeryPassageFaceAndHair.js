...
App.UI.surgeryPassageHairAndFace = function(slave, refresh, cheat = false) {
	...
	function content() {
				...
				if (slave.indentureRestrictions > 1) {
					...
					App.UI.DOM.appendNewElement("div", el,
						App.Medicine.Surgery.makeLink(
							new App.Medicine.Surgery.Procedures.HairRemoval(slave),
							refresh, cheat, facilityControlledAspects
						),
						"choices");
				}
			} else {
		...
		function eyebrows() {
			...
			if (slave.eyebrowHStyle !== "bald") {
				...
				if (slave.indentureRestrictions > 1) {
					...
					App.UI.DOM.appendNewElement("div", el,
						App.Medicine.Surgery.makeLink(
							new App.Medicine.Surgery.Procedures.EyebrowRemoval(slave),
							refresh, cheat, facilityControlledAspects
						),
						"choices");
				}
		...
		function armpitHair() {
			...
			if ((slave.underArmHStyle !== "bald" && slave.underArmHStyle !== "hairless") || (slave.pubicHStyle !== "bald" && slave.pubicHStyle !== "hairless")) {
				...
				if (slave.indentureRestrictions > 1) {
					...
					App.UI.DOM.appendNewElement("div", el,
						App.Medicine.Surgery.makeLink(
							new App.Medicine.Surgery.Procedures.BodyHairRemoval(slave),
							refresh, cheat, facilityControlledAspects
						),
						"indent");
				}
		...
		function faceShape() {
			...
			if (slave.indentureRestrictions > 1) {
				...
				for (const [key, shape] of faceMap) {
					...
					linkArray.push(App.Medicine.Surgery.makeLink(
						new App.Medicine.Surgery.Procedures.FaceShape(slave, key),
						refresh, cheat, facilityControlledAspects
					));
				}
				linkArray.push(App.Medicine.Surgery.makeLink(
					new App.Medicine.Surgery.Procedures.FaceAttractiveness(slave),
					refresh, cheat, facilityControlledAspects
				));
			...
			if (slave.indentureRestrictions < 2 && slave.faceImplant <= 95) {
				if (slave.ageImplant > 1) {
					...
					App.UI.DOM.appendNewElement("div", el,
						App.Medicine.Surgery.makeLink(
							new App.Medicine.Surgery.Procedures.AgeLift(slave),
							refresh, cheat, facilityControlledAspects
						),
						"choices");
				}
		...
		function topEars() {
			...
			} else if (slave.earT === "round hyena") {
				r.push(`a pair of round hyena ears adorning ${his} head.`);
			} else if (slave.earT === "pointy hyena") {
				r.push(`a pair of pointy hyena ears adorning ${his} head.`);
			...
			} else if (slave.earT === "rat") {
				r.push(`a pair of small rat ears adorning ${his} head.`);
			} else if (slave.earT === "mouse") {
				r.push(`a pair of large mouse ears adorning ${his} head.`);
			...
			if (slave.indentureRestrictions >= 2) {
				...
				if (slave.earT !== "none") {
					...
					if (slave.earT !== "round hyena") {
						linkArray.push(App.Medicine.Surgery.makeLink(
							new App.Medicine.Surgery.Procedures.TopEarReshape(slave, "round hyena"),
							refresh, cheat));
					}
					if (slave.earT !== "pointy hyena") {
						linkArray.push(App.Medicine.Surgery.makeLink(
							new App.Medicine.Surgery.Procedures.TopEarReshape(slave, "pointy hyena"),
							refresh, cheat));
					}
					if (slave.earT !== "fox") {
						linkArray.push(App.Medicine.Surgery.makeLink(
					...
					if (slave.earT !== "rat") {
						linkArray.push(App.Medicine.Surgery.makeLink(
							new App.Medicine.Surgery.Procedures.TopEarReshape(slave, "rat"),
							refresh, cheat));
					}
					if (slave.earT !== "mouse") {
						linkArray.push(App.Medicine.Surgery.makeLink(
							new App.Medicine.Surgery.Procedures.TopEarReshape(slave, "mouse"),
							refresh, cheat));
					}
					if (slave.earT !== "horse") {
						linkArray.push(App.Medicine.Surgery.makeLink(
		...
		function lips() {
			...
			const surgeries = App.Medicine.Surgery.sizingProcedures.lips(slave, App.Medicine.Surgery.allSizingOptions(facilityControlledAspects));
			const surgeryLinks = surgeries.map(s => App.Medicine.Surgery.makeLink(s, refresh, cheat, facilityControlledAspects));
