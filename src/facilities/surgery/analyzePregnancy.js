	...
	function scanFetus(i) {
		...
		if (fetus.age >= 2 || cheat) {
			...
			if (V.incubator.capacity > 0) {
				if (fetus.reserve === "incubator") {
					...
					if ("tankSetting" in fetus) {
						linkArray.push(App.UI.DOM.link(
							...
							() => {
								V.temp.AS = mother.ID;			// Undefined if mother is PC, but harmlessly so
								V.temp.activeFetus = i;
							},
							[],
				...
				} else if ((V.incubator.capacity - tankSlaveCount()) - FetusGlobalReserveCount("incubator") > 0) {
					App.UI.DOM.appendNewElement("div", el, App.UI.DOM.link(
			...
			if (V.nursery > 0) {
				...
				} else if ((V.nurseryCribs - infantCount()) - FetusGlobalReserveCount("nursery") > 0) {
					App.UI.DOM.appendNewElement("div", el, App.UI.DOM.link(
...
App.UI.analyzePregnancy = function() {
	...
	if (lastPassage() === "Incubator") {
		V.storedLink = "Incubator";
	} else if (lastPassage() === "Slave Interact") {
