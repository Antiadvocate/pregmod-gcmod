...
App.UI.surgeryPassageLower = function(slave, refresh, cheat = false) {
	...
	function content() {
		...
		function butt() {
			...
			const surgeries = App.Medicine.Surgery.sizingProcedures.butt(slave, App.Medicine.Surgery.allSizingOptions(facilityControlledAspects));
		...
		function fertility() {
			...
			if (isFertile(slave, true) && !isMenstruating(slave) && slave.preg === 0) {
				linkArray.push(
		...
		function sexDescription() {
			...
			if (slave.vagina > -1 && V.seeExtreme === 1) {
				if (slave.indentureRestrictions < 1 && (slave.breedingMark !== 1 || V.propOutcome === 0 || V.eugenicsFullControl === 1 || !FutureSocieties.isActive('FSRestart'))) {
					linkArray.push(App.Medicine.Surgery.makeLink(
