...
App.UI.surgeryPassageUpper = function(slave, refresh, cheat = false) {
	...
	function content() {
		...
		function boobImplants() {
			...
			App.Events.addNode(el, r, "div");
			const surgeries = App.Medicine.Surgery.sizingProcedures.boobs(slave, App.Medicine.Surgery.allSizingOptions(facilityControlledAspects));
		...
		function belly() {
			...
			if (slave.pregKnown > 0) {
				...
				if (slave.cervixImplantTubing) {
					r.push(`It's tubing forms a complex vascular system throughout ${his} body to deliver filler to any fillable implants ${he} has.`);
				}
			...
			if (slave.bellyImplant < 0 && slave.cervixImplant && slave.cervixImplantTubing) {
				if (slave.cervixImplant === 1) {
					r.push(`${He} has a micropump filter installed in ${his} cervix`);
				} else if (slave.cervixImplant === 2) {
					r.push(`${He} has a micropump filter installed in ${his} rectum`);
				} else if (slave.cervixImplant === 3) {
					r.push(`${He} has a micropump filter installed in both ${his} holes`);
				}
				if (slave.cervixImplantTubing) {
					r.push(`feeding into a complex vascular system that runs throughout ${his} body, delivering filler where desired.`);
				}
			}
			App.Events.addNode(el, r, "div");
			if (slave.indentureRestrictions >= 2) {
			...
			} else if (slave.ovaries === 0 && slave.mpreg === 0) { // while intended for the belly implant, the tubing allows for far more options. It may only be applied if the slave DOES NOT have a womb! Which, naturally, is poorly tracked.
				if (slave.cervixImplant !== 1 && slave.cervixImplant !== 3 && V.cervixImplants >= 1 && slave.vagina > -1) { /* slave should have vagina */
					linkArray.push(App.Medicine.Surgery.makeLink(
						new App.Medicine.Surgery.Procedures.CervixPump(slave),
						refresh, cheat, facilityControlledAspects
			...
			if (slave.cervixImplant > 0 && slave.cervixImplantTubing === 0 && V.cervixImplantUpgrade > 0) {
				linkArray.push(App.Medicine.Surgery.makeLink(
					new App.Medicine.Surgery.Procedures.CervixPumpTubing(slave),
					refresh, cheat, facilityControlledAspects
				));
			}
			App.UI.DOM.appendNewElement("div", el, App.UI.DOM.generateLinksStrip(linkArray), "choices");
