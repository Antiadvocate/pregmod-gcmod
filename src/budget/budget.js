...
App.Budget.table = function(budgetType) {
	...
	if (V.temp.showEmptyBudgetEntries[budgetType] === undefined) {
		V.temp.showEmptyBudgetEntries[budgetType] = 0;
	}
	...
	function generateRowCategory(node, category) {
		const r = [];
		if (V[income][category] || V[expenses][category] || V.temp.showEmptyBudgetEntries[budgetType]) {
	...
	function generateRowGroup(title, name) {
		...
		const r = [];
		if (groupIn || groupEx || V.temp.showEmptyBudgetEntries[budgetType]) {
