 ...
 * @param {object} [extras]
 * @param {string} [extras.closeButtonText="Close Report"] if an empty string is provided then the close button will be disabled
 * @param {Function} [extras.callback] this function will be called each time the accordion is collased or expanded. It will be passed false for expanded and true for collapsed
 * @returns {DocumentFragment}
 */
App.UI.DOM.accordion = function(
	head,
	content,
	collapsed = true,
	extras = {},
	...
	extras = extras ?? {};
	App.Utils.assignMissingDefaults(extras, {
		closeButtonText: "Close Report",
		callback: undefined,
	});
	...
	closeButton.textContent = extras.closeButtonText;
	closeButton.onclick = () => {
	...
	if (content) {
		if (extras.closeButtonText !== "") {
			content.append(closeButton);
		}
		App.UI.DOM.elementToggle(button, [content], collapsed, extras.callback);
		fragment.append(content);
	} else {
 ...
 * @param {Function} [callback=undefined] this function will be called each time the element is toggled. It will be passed false for open and true for closed
 */
App.UI.DOM.elementToggle = function(toggleElement, content, startHidden = true, callback = undefined) {
 ...
 * @returns {HTMLAnchorElement}
 */
App.UI.DOM.createExternalLink = (text, url) => {
...
/**
 * Creates a high definition canvas of a given width and height.
 * Stolen/borrowed from https://stackoverflow.com/a/65124939
 * Note that the plotted points should use the prescaled width and height.
 * @param {number} width
 * @param {number} height
 * @returns {HTMLCanvasElement}
 */
App.UI.DOM.createCanvas = (width, height) => {
	...
	// turn off anti-aliasing for the canvas
