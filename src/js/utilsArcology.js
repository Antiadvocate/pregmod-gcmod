...
/**
 * Returns the percentage (0-100) of the social acceptance of the slaves penetrating citizens in YOUR arcology.
 * Accepts a slave as an optional argument.
 *
 * @param {FC.SlaveState} [slave=null]
 * @returns {number}
 */
globalThis.penetrativeSocialUse = function(slave = null) {
	...
	if (total >= 40) { // Once dicks are trendy, availability starts to increase interest
		let haveDick = getSlaves().filter(s => s.dick > 0).size;
		if (haveDick) {
			let withDick = (haveDick / slaveCount()); // percentage of slaves with dicks
			total += withDick > .2 ? 15 * withDick : 0;
			let toyHole = getSlaves().filter(s => s.toyHole === "dick").size;
			total += 10 * (toyHole / haveDick);
			let penetrativeAverage = 0;
			getSlaves().filter(s => s.dick > 0).forEach(s1 => {
