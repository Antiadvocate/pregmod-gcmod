...
App.Events.SEBirth = class SEBirth extends App.Events.BaseEvent {
	...
	castActors() {
		this.actors = getSlaves().filter(s => isInLabor(s)).map(s => s.ID);
...
globalThis.birth = function(slave, {birthStorm = false, cSection = false, artRenderer = null} = {}) {
	...
	function birthCalc() {
		...
		if (slave.broodmother > 0) {
			...
			babiesBeingBorn = WombBirth(slave, slave.pregData.minLiveBirth); // Normal human pregnancy - 32 week is minimal gestation time for live birth.
		}
	...
	function postBirthReaction() {
		...
		if (slave.fetish !== Fetish.MINDBROKEN && slave.fuckdoll === 0) {
			...
			if (FutureSocieties.isActive('FSRestart')) {
				r = [];
				if (slave.breedingMark === 1 && V.propOutcome === 1 && (slave.pregSource === SID.PC || slave.pregSource === SID.ELITE)) {
	...
	function destinationForBabies() {
		...
		if (numBeingBorn > 0) {
			...
			if (FutureSocieties.isActive('FSRestart') && slave.breedingMark === 1 && V.propOutcome === 1 && (slave.pregSource === SID.PC || slave.pregSource === SID.ELITE)) {
				r.push(`${His} ${childrenAre} collected by the ${societalElite} to be raised into upstanding members of the new society.`);
			} else if (slave.breedingMark === 1 && V.propOutcome === 1 && (slave.pregSource === SID.PC || slave.pregSource === SID.ELITE) && V.DefaultBirthDestination === "the market") {
			...
			} else if (slave.pregSource === SID.FUTA && V.DefaultBirthDestination === "the market") {
				...
				choices.append(choice);
				if (slave.breedingMark === 1 && (slave.pregSource === SID.PC || slave.pregSource === SID.ELITE) && V.propOutcome === 1) {
					choice = document.createElement("div");
					choice.append(
						App.UI.DOM.link(
	...
	function broodmotherBirth() {
		...
		if (random(1, 2) === 1) {
			...
			switch (slave.assignment) {
				...
				case Job.CONCUBINE:
					if (slave.pregSource === SID.PC && slave.relationship === -3) {
						if (random(1, 20) > suddenBirth) {
	...
	function deadBabiesResponse(babies) {
		...
		if (
			...
			(slave.pregSource === SID.PC || slave.pregSource === SID.ELITE)
		) {
			...
			switch (slave.clothes) {
				...
				case "a superhero outfit":
					if (slave.fetish === Fetish.MINDBROKEN) {
						...
						r.push(`Quickly ${he} attempts to remove ${his} leotard but fails to do so before having to push out ${babyIntro}.`);
						if (slave.fetish === "humiliation") {
							r.push(`${He} could use ${his} cape to hide ${his} shame, but ${he} finds it far more exciting to put on a show.`);
						} else {
							r.push(`${He} makes use of ${his} cape to hide what's happening between ${his} legs.`);
						}
						r.push(`The tight material stretches as ${his} child is born into it and with a little help ${he}'s freed to finish giving birth.`);
					}
					break;
				...
				case "a streetwear outfit":
					if (slave.fetish === Fetish.MINDBROKEN) {
						r.push(`${babies}, indifferent to who may be watching ${his} exposed`);
						if (slave.mpreg === 1) {
							r.push(`ass.`);
						} else {
							r.push(`pussy.`);
						...
						r.push(`Quickly ${he} spreads ${his} legs apart and begins pushing out ${babyIntro}.`);
						if (slave.fetish === "humiliation") {
							r.push(`${He} is thrilled that ${his} skirt reveals what's happening between ${his} legs.`);
						} else if (slave.devotion > 95) {
							r.push(`${He} does ${his} best to hold a rebellious pose until the birth is complete.`);
						} else {
							r.push(`${He} does ${his} best to hide what's happening between ${his} legs.`);
						}
					}
				...
				case "a leather outfit":
					if (slave.fetish === Fetish.MINDBROKEN) {
						r.push(`${babies}, indifferent to who may be watching. The`);
						if (slave.mpreg === 1) {
							r.push(`seat`);
						} else {
							r.push(`crotch`);
						}
						r.push(`of ${his} leather pants strains as ${he} crowns against it, but wins out and holds ${his} child in place. ${He} struggles to continue ${his} tasks, oblivious to ${his} stalled birth, until someone manages to pull down the tight garment and allow ${his} body to finish giving birth.`);
					} else {
						r.push(`Quickly, ${he} attempts to struggle out of ${his} leather pants but fails to do so before having to push out ${babyIntro}. ${He} crowns against the}`);
						if (slave.mpreg === 1) {
							r.push(`seat`);
						} else {
							r.push(`crotch`);
						}
						r.push(`of ${his} pants, unable to make any progress with childbirth due to the tight material. ${His} struggles are obvious to anyone that looks at ${him},`);
						if (slave.fetish === "humiliation") {
							r.push(`but that only makes it more exciting.`);
						} else {
							r.push(`so ${he} bears with it.`);
						}
						r.push(`With a little help, ${he} manages to escape ${his} leather prison and finish giving birth.`);
					}
					break;
				...
				case "a turtleneck and miniskirt":
					if (slave.fetish === Fetish.MINDBROKEN) {
						...
						r.push(`Quickly ${he} spreads ${his} legs apart and begins pushing out ${babyIntro}.`);
						if (slave.fetish === "humiliation") {
							r.push(`${He} is thrilled that ${his} miniskirt reveals what's happening between ${his} legs.`);
						} else {
							r.push(`${He} does ${his} best to hide what's happening between ${his} legs.`);
						}
					}
				...
				case "a tropical outfit":
					if (slave.fetish === Fetish.MINDBROKEN) {
						...
						r.push(`Quickly ${he} spreads ${his} legs apart and begins pushing out ${babyIntro}.`);
						if (slave.fetish === "humiliation") {
							r.push(`${He} is thrilled that ${his} grass skirt reveals what's happening between ${his} legs.`);
						} else {
							r.push(`${He} does ${his} best to hide what's happening between ${his} legs.`);
						}
					}
				...
				case "a valkyrie outfit":
					if (slave.fetish === Fetish.MINDBROKEN) {
						...
						if (slave.fetish === "humiliation") {
							...
							r.push(`${He} makes use of ${his} coat to hide what's happening between ${his} legs.`);
						}
					}
					break;
				...
				case "a coat and corset":
					if (slave.fetish === Fetish.MINDBROKEN) {
						r.push(`${babies}, indifferent to who ${he} may be flashing ${his}`);
						if (slave.mpreg === 1) {
							r.push(`ass at.`);
						} else {
							r.push(`pussy at.`);
						...
						r.push(`Quickly ${he} spreads ${his} legs apart and begins pushing out ${babyIntro}.`);
						if (slave.fetish === "humiliation") {
							r.push(`${He} is thrilled that ${he} can flash what's happening between ${his} legs at passerby.`);
						} else {
							r.push(`${He} makes use of ${his} coat to hide what's happening between ${his} legs.`);
						}
					}
				...
				case "a babydoll":
					if (slave.fetish === Fetish.MINDBROKEN) {
						...
						r.push(`Quickly ${he} spreads ${his} legs apart and begins pushing out ${babyIntro}.`);
						if (slave.fetish === "humiliation") {
							r.push(`${He} is thrilled that ${his} babydoll reveals what's happening between ${his} legs.`);
						} else {
							r.push(`${He} does ${his} best to hide what's happening between ${his} legs.`);
						}
					}
				...
				case "a baroque dress":
					if (slave.fetish === Fetish.MINDBROKEN) {
						...
						r.push(`Quickly ${he} spreads ${his} legs apart and begins pushing out ${babyIntro}.`);
						if (slave.fetish === "humiliation") {
							r.push(`${He} is disappointed that ${his} petticoat hides what's happening between ${his} legs.`);
						} else {
							r.push(`${He} does ${his} best to hide what's happening between ${his} legs.`);
						}
					}
				...
				case "a wedding dress":
					if (slave.fetish === Fetish.MINDBROKEN) {
						...
						r.push(`Quickly ${he} spreads ${his} legs apart and begins pushing out ${babyIntro}.`);
						if (slave.fetish === "humiliation") {
							r.push(`${His} gown might hide what's happening between ${his} legs, but being seen heavily pregnant and in the throes of labor in a wedding dress excites ${him}.`);
						} else {
							r.push(`${He} does ${his} best to hide what's happening between ${his} legs, but being heavily pregnant, in the throes of labor, and wearing a wedding dress certainly draws a crowd.`);
						}
					}
					break;
				...
				case "a slutty magical girl costume":
					if (slave.fetish === Fetish.MINDBROKEN) {
						r.push(`${babies}, indifferent to who may be watching. The crotch of ${his} bodysuit bulges lewdly before slipping to the side of ${his} crowning newborn, clearing the way for childbirth.`);
					} else {
						r.push(`Quickly ${he} spreads ${his} legs apart and shifts the crotch of ${his} bodysuit aside before beginning to push out ${babyIntro}. ${He} can't hide what's happening between ${his} legs,`);
						if (slave.fetish === "humiliation") {
							r.push(`but that only makes it more exciting.`);
						} else {
							r.push(`so ${he} bears with it and thanks the stars ${he} isn't birthing a load of tentacles.`);
						}
					}
					break;
				...
				case "slave rags":
					if (slave.fetish === Fetish.MINDBROKEN) {
						...
						r.push(`Quickly ${he} spreads ${his} legs apart and begins pushing out ${babyIntro}. ${His} rags can't hide what's happening between ${his} legs,`);
						if (slave.fetish === "humiliation") {
							r.push(`but that only makes it more exciting.`);
						} else {
							r.push(`so ${he} bears with it.`);
			...
			if (slave.fetish === Fetish.MINDBROKEN) {
				...
				switch (slave.clothes) {
					...
					case "a courtesan dress":
						...
						break;
					case "a superhero outfit":
						r.push(`superhero suit`);
						break;
					case "a streetwear outfit":
						r.push(`rebellious outfit`);
						break;
					case "a country outfit":
						r.push(`short skirt`);
						break;
					case "a turtleneck and miniskirt":
						...
						break;
					case "a red priestess outfit":
						r.push(`priestess outfit`);
						break;
					case "an eastern european outfit":
						r.push(`eastern european outfit`);
						break;
					case "a samba outfit":
						r.push(`samba outfit`);
						break;
					case "a tribal outfit":
						r.push(`tribal outfit`);
						break;
					case "a tropical outfit":
						r.push(`tropical outfit`);
						break;
					case "a valkyrie outfit":
						r.push(`valkyrie outfit`);
						break;
					case "a baroque dress":
						r.push(`baroque dress`);
						break;
					case "a cocktail dress":
						r.push(`cocktail dress`);
						break;
					case "an ice queen outfit":
						r.push(`ice queen outfit`);
						break;
					case "a magical girl costume":
						r.push(`magical girl costume`);
						break;
					case "a slutty magical girl costume":
						r.push(`slutty magical girl costume`);
						break;
					case "a sundress":
						r.push(`sundress`);
						break;
					case "a wedding dress":
						r.push(`wedding dress`);
						break;
					case "an armored dress":
						r.push(`armored dress`);
						break;
					case "a coat and corset":
						r.push(`coat and corset`);
						break;
					case "a babydoll":
						r.push(`babydoll lingerie`);
						break;
					case "slave rags":
						r.push(`rags`);
						break;
...
globalThis.sendNewbornsToFacility = function(mom, babiesBeingBorn, sendAll) {
	...
	for (const ovum of babiesBeingBorn) {
		if ((ovum.reserve === "incubator" || sendAll) && tankSlaveCount() < V.incubator.capacity) {
		...
		} else if ((ovum.reserve === "nursery" || sendAll) && infantCount() < V.nurseryCribs) {
