...
globalThis.setPregType = function(actor) {
	...
	const reproductionFormulaApplies = (V.reproductionFormula === 1) && (actor.ID === SID.PC ? !canEatFood(asPlayer(actor)) : (V.week - asSlave(actor).weekAcquired > 0));
	...
	if (actor.broodmother < 1) { // Broodmothers should be not processed here. Necessary now.
		if (typeof actor.readyOva === "number" && actor.readyOva !== 0) {
			...
			if (actor.eggType === "horse" || actor.eggType === "cow") {
				...
				if (actor.ID === SID.PC) { // temp until player birth is redone
...
globalThis.knockMeUp = function(target, chance, hole, fatherID) {
	...
	if (V.seePreg !== 0) {
		...
		if (jsRandom(0, 99) < (chance + (V.reproductionFormula * ((target.pregSource <= 0) ? ((target.ID === SID.PC) ? 0 : 10) : 20)))) {
			if (target.mpreg === hole || hole === 2) {
				if (target.pregWeek <= 0) {
					target.preg = 1;
					target.pregSource = (!fatherID ? SID.GENERIC : fatherID);
					if (target.ID !== SID.PC) {
						target.pregWeek = 1;
					}
				}
 ...
 * Technically deprecates canImpreg() and canFemImpreg(). Does not check for chastity.
...
/** Returns the amount of fetuses currently destined (reserved) for the incubator @returns {number} */
globalThis.getIncubatorReserved = function() {
	return FetusGlobalReserveCount("incubator");
};

/** Returns the amount of fetuses currently destined (reserved) for the nursery @returns {number} */
globalThis.getNurseryReserved = function() {
...
globalThis.findFather = function(fatherID) {
	...
	father = getSlave(fatherID, true);

	if (father === undefined) {
		// fast methods didn't find them so do a search through all HumanState objects
		father = getHumanStates(true).find((s) => s.ID === fatherID);
	}
