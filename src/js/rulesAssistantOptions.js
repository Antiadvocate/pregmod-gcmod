...
// cSpell:ignore onoffswitch, rajs
...
/**
 * @returns {HTMLDivElement} container for the RA UI
 */
App.RA.options = (function() {
	 ...
	 */
	function rulesAssistantOptionsWrapper() {
		const div = document.createElement("div");
		div.classList.add("rajs");
		rulesAssistantOptions(div);
		return div;
	}
	...
	class NoteElement extends Element {
		 ...
		 */
		constructor(note, ...extraCssClasses) {
			super(App.UI.DOM.makeElement("div", note, ["note", "grid-all-columns", ...extraCssClasses]));
		}
	}
	...
	class ListSelector extends ElementWithLabel {
		...
		setValue(what) {
			// Array case to handle NippleShape
			this.value.value = (what === null || (Array.isArray(what) && !what.length)) ? noDefaultSetting.value : what;
		}
		...
		loadNewRule() {
			...
			try {
				...
				if (Array.isArray(rule)) {
					rule.forEach(r => {
						r.ID = generateNewID();
						V.defaultRules.push(App.Update.RARuleDatatypeCleanup(r));
					});
				} else {
					rule.ID = generateNewID();
					V.defaultRules.push(App.Update.RARuleDatatypeCleanup(rule));
				}
				reload();
			} catch (e) {
	...
	/** Returns true if any of the genetic mods is unlocked */
	function isAnyGeneModUnlocked() {
		let formulas = [V.RapidCellGrowthFormula, V.immortalityFormula, V.bioEngineeredFlavoringResearch, V.optimizedSpermFormula, V.enhancedProductionFormula, V.optimizedBreedingFormula];
		return formulas.some(f => f === 1);
	}
	...
	class Root extends Element {
		...
		constructor(element) {
			super(element);
			this.appendChild(new RuleSelectorContainer());
			if (V.defaultRules.length === 0) {
				return;
			}
			...
			// This way the Gene Mod tab doesn't show up empty
			if (isAnyGeneModUnlocked() || V.geneticMappingUpgrade >= 2) {
	...
	class RuleSelectorContainer extends Element {
		constructor() {
			...
			if (V.defaultRules.length === 0) {
				...
				this.appendChild(new EmptyRulesTransformations());
				this.appendChild(new AutoSurgeryFacilitiesSwitch());
			} else {
				this.appendChild(new RuleSelector());
				this.appendChild(new RuleTransformations());
				this.appendChild(new AutoSurgeryFacilitiesSwitch());
			}
		}
	}
	...
	class EmptyRulesTransformations extends Options {
		constructor() {
			super();
	...
	class RuleSelector extends List {
		constructor() {
			super(null, V.defaultRules.map(i => [App.UI.DOM.makeElement("span", i.name, ["rajs-rules-inner"]), i]), false, false, false);

			this.onchange = function(rule) {
				if (V.currentRule === rule.ID) {
					return;
				}
				V.currentRule = rule.ID;
				reload();
		...
		_appendContentTo(container) {
			// Overwrite to control the CSS classes applied to the label
			container.append("Current rule: ");
			App.UI.DOM.appendNewElement("span", container, current_rule.name, ["current-rule-name"]);
			container.appendChild(this.element);
		}
	}

	// buttons for doing transformations on rules
	class RuleTransformations extends Options {
		constructor() {
			super();
	...
	class AutoSurgeryFacilitiesSwitch extends ButtonItem {
		constructor() {
			super("Apply autosurgery to slaves outside of the penthouse", false, (V.rulesAssistantFacilitiesAutosurgery === true));
		}

		onchange() {
			V.rulesAssistantFacilitiesAutosurgery = !V.rulesAssistantFacilitiesAutosurgery;
		}
	}
	...
	class ApplicationLog extends Element {
		render() {
			...
			elem.innerHTML = getSlaves().map(slave => DefaultRules(slave)).join("");
	...
	class ActivationConditionContainer extends Element {
		constructor() {
			...
			const header = new HeaderElement("Activation Condition").element;
			const content = document.createElement("div");

			content.append(
				new NoteElement("Controls which slaves this rule will be applied to.").element,
				new ConditionBuilder().element,
				new SpecificInclusionExclusion().element,
				new ApplyRuleOnce().element,
			);

			App.UI.DOM.elementToggle(header, [content], false);
	...
	class SlaveSelection extends ButtonList {
		constructor() {
			...
			getSlaves().forEach(slave => this.appendChild(new ButtonItem(
	...
	class ApplyRuleOnce extends ButtonItem {
		constructor() {
			super("Only apply once per slave", false, (current_rule.condition.applyRuleOnce === true));
		}
	...
	class AppearanceTab extends Tab {
		constructor() {
			...
			if (V.boughtItem.toys.chastity === 1) {
	...
	class AutosurgeryTab extends Tab {
		constructor() {
			super();
			this.appendChild(new NoteElement("ONLY applied at the end of the week!", "noteworthy"));
			this.appendChild(new NoteElement("ONLY applied if slave's current assignment won't revert or perform it automatically!", "noteworthy"));
			...
			this.appendChild(new BreastShapeSurgeryList());
			this.appendChild(new BreastLiftSurgerySwitch());
			this.appendChild(new BreastMeshSurgerySwitch());
			this.appendChild(new AreolaeSurgeryList());
			this.appendChild(new AreolaeShapeSurgeryList());
	...
	class NippleGrowthList extends ListSelector {
	...
	class SmartFetishList extends ListSelector {
		constructor() {
			/** @type {Array<FC.SmartPiercingSetting[]|[string, FC.SmartPiercingSetting]>} */
			const pairs = [
				["off"],
				["All sex", "all"],
				["No sex", "none"],
	...
	class BreastShapeSurgeryList extends RadioSelector {
		constructor() {
			const items = [
				["normal", "normal"],
				["perky", "perky"],
			...
			super("Breast shape", items);
			this.setValue(current_rule.set.surgery.boobShape);
			this.onchange = (value) => current_rule.set.surgery.boobShape = value;
		}
	}

	class BreastLiftSurgerySwitch extends BooleanSwitch {
		constructor() {
			super(`Breast lift on saggy and downward facing breasts`);

			this.setValue(current_rule.set.surgery.boobLift);
			this.onchange = (value) => current_rule.set.surgery.boobLift = value;
		}
	}

	class BreastMeshSurgerySwitch extends BooleanSwitch {
		constructor() {
			super(`Supportive mesh to preserve breast shape`);

			this.setValue(current_rule.set.surgery.boobMesh);
			this.onchange = (value) => current_rule.set.surgery.boobMesh = value;
		}
	}

	class AreolaeSurgeryList extends RadioSelector {
		constructor() {
			const items = [
				["normal", 0],
				["large", 1],
			...
			super("Areolae size", items);
			this.setValue(current_rule.set.surgery.areolae);
			this.onchange = (value) => current_rule.set.surgery.areolae = value;
		}
	}

	class AreolaeShapeSurgeryList extends RadioSelector {
		constructor() {
			const items = [
				["circle", "circle"],
				["start", "star"],
			...
			super("Areolae shape", items);
			this.setValue(current_rule.set.surgery.areolaeShape);
			this.onchange = (value) => current_rule.set.surgery.areolaeShape = value;
		}
	}
	...
	class HipsAndShoulderSurgeryList extends ListSelector {
		...
		*/
		constructor(label, target) {
			...
			super(`${label.toUpperFirst()} change`, items, true);
			this.setValue(current_rule.set.surgery[target]);
			this.onchange = (value) => current_rule.set.surgery[target] = value;
		}
	}

	class SizingImplantType extends ListSelector {
		constructor(label, target) {
			...
			this.onchange = (value) => current_rule.set.surgery[`${target}ImplantTypes`] = value ? [value] : null;
		}
	}
	...
	class AutomaticAssignmentList extends ListSelector {
		constructor() {
			...
			const facilitiesToSkip = [facilities.penthouse, facilities.pit, facilities.armory, facilities.arcologyAgent, facilities.incubator];
			const facilityManagersToSkip = [facilities.pit, facilities.arcologyAgent, facilities.incubator, facilities.arcade];
	...
	return rulesAssistantOptionsWrapper;
})();
