...
globalThis.transplantFetus = (mother, receptrix, fetuses, dialogInsteadOfPassageTraversal = false) => {
	...
	cashX(forceNeg(calculateTransplantingCost(fetuses.length)), (receptrix.ID === SID.PC) ? "PCmedical": "slaveSurgery");
	V.surgeryType = "transplant";
	if (dialogInsteadOfPassageTraversal) {
		Dialog.create("Transplant Fetus");
...
globalThis.transplantingTool = (
	...
	if (V.PC.vagina !== -1 && mother.ID !== SID.PC && V.PC.preg >= 0 && V.PC.preg < 4 && V.PC.pregType < 8 && V.PC.physicalAge < 70) {
		...
		option.addValue("Use your own womb", SID.PC);
	}
	...
	if (V.receptrix !== 0) {
		...
		transplantCommitButton.customRefresh(() => {
			...
			processing.forEach(fetus => {
				...
				if (jDiv && jDiv.length !== 0) {
					...
					jDiv.empty().append(
						...
						(V.receptrix === SID.PC) ? "your": App.SlaveAssignment.saSlaveName(getSlave(V.receptrix)),
						(V.receptrix === SID.PC) ? "": "'s",
						" womb."
					);
				}
			...
			// transplant fetuses
			transplantFetus(mother, (V.receptrix === SID.PC) ? V.PC : getSlave(V.receptrix), processing, dialogInsteadOfPassageTraversal);
		...
		if (transplantingCount === 1) {
			transplantCommitOption.addValue(`Transplant fetus into ${(V.receptrix === SID.PC) ? "your womb" : SlaveFullName(getSlave(V.receptrix)) + "'s womb"}`, true);
		} else {
			transplantCommitOption.addValue(`Transplant ${num(transplantingCount)} fetuses into ${(V.receptrix === SID.PC) ? "your womb" : SlaveFullName(getSlave(V.receptrix)) + "'s womb"}`, true);
		}
