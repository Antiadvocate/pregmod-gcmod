...
App.MainView.useGuard = function() {
	...
	linkArray.push(App.UI.DOM.passageLink(`Abuse ${him}`, "Gameover", () => { V.temp.gameover = "idiot ball"; }));
...
App.MainView.full = function() {
		...
		if (V.releaseID >= 1000 || ["0.9", "0.8", "0.7", "0.6"].includes(V.ver)) {
			if (V.releaseID < App.Version.release) {
				newError().append(App.UI.DOM.makeElement("span", "INCOMPATIBILITY WARNING:", "major-warning"),
					` Your saved game was created using version: ${V.ver}, release: ${V.releaseID}${("commitHash" in V && V.commitHash !== "") ? `, commit ${V.commitHash}` : ""}. You must run `,
	...
	function walkPast() {
		...
		const slave = getSlaves().filter(s => ![Job.BODYGUARD, Job.FUCKTOY].includes(s.assignment) && (!getPersonalAttention(s.ID, "torture") || onBedRest(V.PC, true))).randomValue();
		if (slave) {
