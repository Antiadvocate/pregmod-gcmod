...
globalThis.CategoryAssociatedGroup = Object.freeze({
	...
	ABORTIONS: [ // maybe should be "scenarios" instead?
		'abortions'
	],
	EDICTS: [
		'edicts'
...
globalThis.calculateCosts = (function() {
	...
	function predictTotalSlaveCosts() {
		let loopCosts = 0;
		let slaves = getSlaves(true);
		for (const slave of slaves.values()) {
			loopCosts += getSlaveCost(slave);
		}
		const reducibleUpkeep = Math.trunc(loopCosts * 0.2);
		const servantCapacity = totalServantCapacity();
		if (slaves.size > servantCapacity) {
			loopCosts -= Math.trunc(reducibleUpkeep / slaves.size * servantCapacity);
		} else {
	...
	function getTotalSlaveCosts() {
		...
		for (const slave of getSlaves(true).values()) {
		...
		const reducibleUpkeep = Math.trunc(loopCosts * 0.2);
		const totalSlaveCount = slaveCount();
		const servantCapacity = totalServantCapacity();
  // This ensures we don't overcount servant cost reduction when we have extra capacity
  const savedPerSlot = reducibleUpkeep / (servantCapacity > totalSlaveCount ? servantCapacity : totalSlaveCount);
	...
	function getIncubatorCosts() {
		const facInc = App.Entity.facilities.incubator;
		return V.facilityCost * facInc.capacity * (10 +
			0.2 * facInc.upgrade('weight') +
			0.2 * facInc.upgrade('muscles') +
			0.2 * facInc.upgrade('reproduction') +
			0.2 * facInc.upgrade('growthStims') +
			0.2 * facInc.upgrade('organs') +
			0.5 * facInc.upgrade('reproduction') +
			0.5 * facInc.upgrade('speed')
		);
	}

	function getIncubatorSlavesCosts() {
		let incubator = App.Entity.facilities.incubator;
		const incubatorCostBase =
			incubator.upgrade('weight') +
			incubator.upgrade('muscles') +
			incubator.upgrade('reproduction') +
			incubator.upgrade('growthStims') +
			incubator.upgrade('organs');
		let sum = 0;
		for (const tank of getTankSlaves().values()) {
			sum += ((incubatorCostBase + tank.incubatorSettings.pregAdaptationPower) * 500);
		}
		return sum;
	}

	function getServantsQuartersCosts() {
		return (0.2 * App.Entity.facilities.servantsQuarters.upgrade('monitoring') * App.Entity.facilities.servantsQuarters.capacity * V.facilityCost);
	}
 ...
 * @param {FC.RelationShipKind} relationship
	...
	switch (assignment) {
		...
		case Job.QUARTER:
			if (living === LivingRule.NORMAL) {
				...
				cost += (App.Entity.facilities.servantsQuarters.decoration === 'Degradationist') ? rulesCost * 0.90 : rulesCost;
			}
			break;
		case Job.CELLBLOCK:
...
globalThis.cashX = function(cost, what, who) {
	...
	// detailed record keeping performed for slaves only
...
globalThis.repX = function(rep, what, who) {
	...
	if (rep > 0) {
		// record the slave, if available
		if (typeof who !== 'undefined' && who.ID !== SID.PC) {
