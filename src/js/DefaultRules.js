...
globalThis.DefaultRules = function(slave, options) {
	...
	if (ruleIds.length !== 0 && !options?.aiPromptsOnly) {
		...
		if (slave.fuckdoll === 0) {
			ProcessClothing(slave, rule, facilityControlledAspects);
			ProcessCollar(slave, rule, facilityControlledAspects);
			ProcessMask(slave, rule, facilityControlledAspects);
			ProcessGag(slave, rule, facilityControlledAspects);
			ProcessEyewear(slave, rule);
			ProcessEarwear(slave, rule);
			ProcessDildos(slave, rule, facilityControlledAspects);
			ProcessDickAccessories(slave, rule, facilityControlledAspects);
			ProcessAnalAccessories(slave, rule, facilityControlledAspects);
			ProcessChastity(slave, rule, facilityControlledAspects);
		...
		if (isFertile(slave, true) || slave.pregWeek < 0) {
			ProcessContraceptives(slave, rule, facilityControlledAspects);
		}
		if (slave.preg > 0 && slave.pregKnown === 1 && slave.broodmother === 0) {
			ProcessAbortions(slave, rule);
		}
		ProcessDrugs(slave, rule, facilityControlledAspects);
		ProcessEnema(slave, rule);
		ProcessDiet(slave, rule, facilityControlledAspects);
		ProcessCuratives(slave, rule, facilityControlledAspects);
		ProcessAphrodisiacs(slave, rule, facilityControlledAspects);
		ProcessPenisHormones(slave, rule, facilityControlledAspects);
		ProcessFemaleHormones(slave, rule, facilityControlledAspects);
		ProcessPregnancyDrugs(slave, rule);
		if (slave.fuckdoll === 0) {
			ProcessLivingStandard(slave, rule, facilityControlledAspects);
			ProcessRest(slave, rule, facilityControlledAspects);
			ProcessSpeech(slave, rule);
			ProcessRelationship(slave, rule);
			ProcessRelease(slave, rule);
			ProcessLactation(slave, rule, facilityControlledAspects);
	 ...
	 * @param {FC.SlaveState} slave
	 * @param {FC.RA.RuleSetters} rule
	 * @param {Set<FC.Facilities.FacilityControlledAspect>} facilityControlledAspects
	 */
	function ProcessClothing(slave, rule, facilityControlledAspects) {
		// apply clothes to slave
		if ((rule.clothes !== undefined) && (rule.clothes !== null) && (!rule.choosesOwnClothes)) {
			if (slave.clothes !== rule.clothes && !facilityControlledAspects.has(FacilityControlledAspect.CLOTHES)) {
		...
		if ((rule.choosesOwnClothes !== undefined) && (rule.choosesOwnClothes !== null)) {
			if (slave.choosesOwnClothes !== rule.choosesOwnClothes && !facilityControlledAspects.has(FacilityControlledAspect.CLOTHES)) {
	 ...
	 * @param {Set<FC.Facilities.FacilityControlledAspect>} facilityControlledAspects
	 */
	function ProcessCollar(slave, rule, facilityControlledAspects) {
		// apply collar to slave
		if ((rule.collar !== undefined) && (rule.collar !== null)) {
			if (slave.collar !== rule.collar && !facilityControlledAspects.has(FacilityControlledAspect.COLLAR)) {
				let m = "";
	 ...
	 * @param {Set<FC.Facilities.FacilityControlledAspect>} facilityControlledAspects
	 */
	function ProcessMask(slave, rule, facilityControlledAspects) {
		// apply faceAccessory to slave
		if ((rule.faceAccessory !== undefined) && (rule.faceAccessory !== null)) {
			if (slave.faceAccessory !== rule.faceAccessory && !facilityControlledAspects.has(FacilityControlledAspect.FACE_ACCESSORY)) {
	 ...
	 * @param {Set<FC.Facilities.FacilityControlledAspect>} facilityControlledAspects
	 */
	function ProcessGag(slave, rule, facilityControlledAspects) {
		// apply mouthAccessory to slave
		if ((rule.mouthAccessory !== undefined) && (rule.mouthAccessory !== null)) {
			if (slave.mouthAccessory !== rule.mouthAccessory && !facilityControlledAspects.has(FacilityControlledAspect.MOUTH_ACCESSORY)) {
				let m = "";
	 ...
	 * @param {Set<FC.Facilities.FacilityControlledAspect>} facilityControlledAspects
	 */
	function ProcessDildos(slave, rule, facilityControlledAspects) {
		// apply vaginal dildos to slave
		if (slave.vagina === 0) {
			ProcessVVirginDildos(slave, rule, facilityControlledAspects);
		} else if ((slave.vagina > 0) && (slave.anus === 0)) {
			ProcessAVirginDildos(slave, rule, facilityControlledAspects);
		} else if (slave.vagina > 0) {
			ProcessNonVirginDildos(slave, rule, facilityControlledAspects);
	 ...
	 * @param {Set<FC.Facilities.FacilityControlledAspect>} facilityControlledAspects
	 */
	function ProcessVVirginDildos(slave, rule, facilityControlledAspects) {
		// apply vaginal dildos to vaginal virgins
		if ((rule.virginAccessory !== undefined) && (rule.virginAccessory !== null)) {
			if (slave.vaginalAccessory !== rule.virginAccessory && !facilityControlledAspects.has(FacilityControlledAspect.VAGINAL_ACCESSORY)) {
	 ...
	 * @param {Set<FC.Facilities.FacilityControlledAspect>} facilityControlledAspects
	 */
	function ProcessAVirginDildos(slave, rule, facilityControlledAspects) {
		// apply vaginal dildos to anal virgins
		if ((rule.aVirginAccessory !== undefined) && (rule.aVirginAccessory !== null)) {
			if (slave.vaginalAccessory !== rule.aVirginAccessory && !facilityControlledAspects.has(FacilityControlledAspect.VAGINAL_ACCESSORY)) {
	 ...
	 * @param {Set<FC.Facilities.FacilityControlledAspect>} facilityControlledAspects
	 */
	function ProcessNonVirginDildos(slave, rule, facilityControlledAspects) {
		// apply vaginal dildos to non-virgins
		if ((rule.vaginalAccessory !== undefined) && (rule.vaginalAccessory !== null)) {
			if (slave.vaginalAccessory !== rule.vaginalAccessory && !facilityControlledAspects.has(FacilityControlledAspect.VAGINAL_ACCESSORY)) {
	 ...
	 * @param {Set<FC.Facilities.FacilityControlledAspect>} facilityControlledAspects
	 */
	function ProcessVaginalAttachments(slave, rule, facilityControlledAspects) {
		...
		if (slave.vaginalAccessory === "none" && slave.vaginalAttachment !== "none") {
			...
			if (slave.vaginalAttachment !== rule.vaginalAttachment && !facilityControlledAspects.has(FacilityControlledAspect.VAGINAL_ATTACHMENT)) {
	 ...
	 * @param {Set<FC.Facilities.FacilityControlledAspect>} facilityControlledAspects
	 */
	function ProcessDickAccessories(slave, rule, facilityControlledAspects) {
		// apply dick accessories to slave
		if (slave.dick > 0) {
			if (slave.anus === 0) {
				if ((rule.aVirginDickAccessory !== undefined) && (rule.aVirginDickAccessory !== null)) {
					if (slave.dickAccessory !== rule.aVirginDickAccessory && !facilityControlledAspects.has(FacilityControlledAspect.DICK_ACCESSORY)) {
				...
				if ((rule.dickAccessory !== undefined) && (rule.dickAccessory !== null)) {
					if (slave.dickAccessory !== rule.dickAccessory && !facilityControlledAspects.has(FacilityControlledAspect.DICK_ACCESSORY)) {
	 ...
	 * @param {Set<FC.Facilities.FacilityControlledAspect>} facilityControlledAspects
	 */
	function ProcessChastity(slave, rule, facilityControlledAspects) {
		// apply chastity to slave
		if (rule.chastityVagina !== undefined && rule.chastityVagina !== null) {
			if (slave.vagina > -1) {
				if (!facilityControlledAspects.has(FacilityControlledAspect.CHASTITY)) {
		...
		if (rule.chastityPenis !== undefined && rule.chastityPenis !== null) {
			if (slave.dick > 0) {
				if (slave.chastityPenis !== rule.chastityPenis && !facilityControlledAspects.has(FacilityControlledAspect.CHASTITY_PENIS)) {
	 ...
	 * @param {Set<FC.Facilities.FacilityControlledAspect>} facilityControlledAspects
	 */
	function ProcessAnalAccessories(slave, rule, facilityControlledAspects) {
		// apply buttplugs and buttplug accessories to slave
		if (slave.chastityAnus !== 1) {
			if (slave.anus === 0) {
				ProcessAnalVirginButtplugs(slave, rule, facilityControlledAspects);
			} else {
				ProcessNonVirginButtplugs(slave, rule, facilityControlledAspects);
			}
		}
	 ...
	 * @param {Set<FC.Facilities.FacilityControlledAspect>} facilityControlledAspects
	 */
	function ProcessAnalVirginButtplugs(slave, rule, facilityControlledAspects) {
		// apply buttplugs to virgins
		if ((rule.aVirginButtplug !== undefined) && (rule.aVirginButtplug !== null)) {
			if (slave.buttplug !== rule.aVirginButtplug && !facilityControlledAspects.has(FacilityControlledAspect.BUTTPLUG)) {
	 ...
	 * @param {Set<FC.Facilities.FacilityControlledAspect>} facilityControlledAspects
	 */
	function ProcessNonVirginButtplugs(slave, rule, facilityControlledAspects) {
		// apply buttplugs to non-virgins
		if ((rule.buttplug !== undefined) && (rule.buttplug !== null)) {
			if (slave.buttplug !== rule.buttplug && !facilityControlledAspects.has(FacilityControlledAspect.BUTTPLUG)) {
	 ...
	 * @param {Set<FC.Facilities.FacilityControlledAspect>} facilityControlledAspects
	 */
	function ProcessContraceptives(slave, rule, facilityControlledAspects) {
		if ((rule.preg !== undefined) && (rule.preg !== null)) {
			if (facilityControlledAspects.has(FacilityControlledAspect.CONTRACEPTIVES)) {
				return;
			}
			if (rule.preg === true && slave.preg === 0 && (V.menstruation === 0 || slave.fertKnown === 1 || slave.physicalAge >= V.fertilityAge)) {
	 ...
	 * @param {Set<FC.Facilities.FacilityControlledAspect>} facilityControlledAspects
	 */
	function ProcessDrugs(slave, rule, facilityControlledAspects) {
		...
		if (facilityControlledAspects.has(FacilityControlledAspect.DRUGS)) {
			return;
		}
	 ...
	 * @param {Set<FC.Facilities.FacilityControlledAspect>} facilityControlledAspects
	 */
	function ProcessDiet(slave, rule, facilityControlledAspects) {
		...
		*/
		...
		function dietRule(slave, rule) {
			if (rule.diet === "healthy" && slave.diet !== "healthy") {
				...
				if ((isFertile(slave) && slave.preg === 0) || (slave.geneticQuirks.superfetation === 2 && canGetPregnant(slave, true) && V.geneticMappingUpgrade !== 0)) {
		...
		if (!facilityControlledAspects.has(FacilityControlledAspect.DIET)) {
	 ...
	 * @param {Set<FC.Facilities.FacilityControlledAspect>} facilityControlledAspects
	 */
	function ProcessCuratives(slave, rule, facilityControlledAspects) {
		if ((rule.curatives !== undefined) && (rule.curatives !== null)) {
			if (slave.curatives !== rule.curatives && !facilityControlledAspects.has(FacilityControlledAspect.CURATIVES)) {
				if (rule.curatives === 2) {
	 ...
	 * @param {Set<FC.Facilities.FacilityControlledAspect>} facilityControlledAspects
	 */
	function ProcessAphrodisiacs(slave, rule, facilityControlledAspects) {
		if ((rule.aphrodisiacs !== undefined) && (rule.aphrodisiacs !== null)) {
			if (!facilityControlledAspects.has(FacilityControlledAspect.APHRODISIACS) && slave.aphrodisiacs !== rule.aphrodisiacs) {
	 ...
	 * @param {string} source
	 */
	function applyHormones(slave, facilityControlledAspects, hormones, slaveClass, source) {
		if (!_.isNil(hormones) && !facilityControlledAspects.has(FacilityControlledAspect.HORMONES)) {
	 ...
	 * @param {Set<FC.Facilities.FacilityControlledAspect>} facilityControlledAspects
	 */
	function ProcessPenisHormones(slave, rule, facilityControlledAspects) {
		if (slave.dick > 0) {
			if (slave.balls === 0) {
				applyHormones(slave, facilityControlledAspects, rule.gelding, "a gelding", sourceRecord.gelding);
			} else if (slave.balls > 0) {
				applyHormones(slave, facilityControlledAspects, rule.XY, "a shemale", sourceRecord.XY);
			}
		}
	}
	 ...
	 * @param {Set<FC.Facilities.FacilityControlledAspect>} facilityControlledAspects
	 */
	function ProcessFemaleHormones(slave, rule, facilityControlledAspects) {
		if ((slave.vagina > -1) && (slave.dick === 0)) {
			applyHormones(slave, facilityControlledAspects, rule.XX, "a female", sourceRecord.XX);
		}
	}
	 ...
	 * @param {Set<FC.Facilities.FacilityControlledAspect>} facilityControlledAspects
	 */
	function ProcessLivingStandard(slave, rule, facilityControlledAspects) {
	 ...
	 * @param {Set<FC.Facilities.FacilityControlledAspect>} facilityControlledAspects
	 */
	function ProcessRest(slave, rule, facilityControlledAspects) {
	 ...
	 * @param {Set<FC.Facilities.FacilityControlledAspect>} facilityControlledAspects
	 */
	function ProcessLactation(slave, rule, facilityControlledAspects) {
		if ((rule.lactationRules !== undefined) && (rule.lactationRules !== null)) {
			if (slave.rules.lactation !== rule.lactationRules && !facilityControlledAspects.has(FacilityControlledAspect.LACTATION)) {
		...
		if (slave.piercing.genitals.smart || hasSmartVibe) {
			if (rule.clitSetting !== undefined && rule.clitSetting !== null) {
				...
				if (fetish === "random") {
					...
					let choices = ["vanilla", "oral", "anal", "boobs", "submissive", "dom", "humiliation", "masochist", "sadist"];
					if (V.seePreg) { choices.push("pregnancy"); }
					if (V.seeBestiality) { choices.push("bestiality"); }
					fetish = either(...choices);
				}
...
globalThis.removeFromRulesToApplyOnce = function(slave) {
	for (const rule of Object.keys(V.rulesToApplyOnce)) {
		if (V.rulesToApplyOnce[rule].includes(slave.ID)) {
			V.rulesToApplyOnce[rule].deleteAll(slave.ID);
		}
	}
};
