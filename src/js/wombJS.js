...
App.Entity.Fetus = class {
	...
	constructor(age, fatherID, mother) {
		...
		this.noticeData = {
			/** @type {("undecided"|"nothing"|"terminate"|"transplant"|"incubator"|"nursery"|"wait"|"lateWait")} */
			fate: "undecided",
...
globalThis.WombImpregnateClone = function(actor, fCount, mother, age) {
	...
	const motherOriginal = getGenePoolRecord(mother) || mother;
...
globalThis.getFetusTwins = function(fetus) {
	if (fetus.twinID && fetus.twinID !== "") {
		return getHuman(fetus.motherID).womb.filter((tFetus) => (tFetus.twinID === fetus.twinID && tFetus !== fetus));
	}
	return null;
};
...
globalThis.MissingParentIDCorrection = function(actor) {
	...
	actor.womb
		...
		.forEach(ft => ft.genetics.father = MID);
};
...
globalThis.FetusGlobalReserveCount = function(reserveType) {
	...
	getSlaves().forEach(function(slave) {
...
/**
 * @param {FC.HumanState} actor
 * @param {"incubator"|"nursery"|"both"} type
 * @returns {number}
 */
globalThis.WombReserveCount = function(actor, type) {
	if (type === "both") {
		return WombReserveCount(actor, "incubator") + WombReserveCount(actor, "nursery");
	}
...
/**
 * checks to see if `mother` has an ovum that originally came from `donatrix`
 * @param {FC.HumanState} mother the mother to check for a transplanted ovum
 * @param {FC.HumanState} donatrix the actor that may have given the transplanted ovum
 * @returns {boolean} returns true if mother has an ovum from donatrix
 */
globalThis.hasTransplantFrom = (mother, donatrix) => {
	return mother.womb.some((fetus) => fetus.motherID === donatrix.ID);
};
