...
globalThis.SlaveStatsChecker = (function() {
	...
	/**
	 * call as SlaveStatsChecker.hasImplants(slave)
	 * @param {FC.HumanState} slave
	 * @returns {boolean}
	 */
	function hasImplants(slave){
		return slave.boobsImplant > 0 || slave.buttImplant > 0 || slave.lipsImplant > 0;
	}
	 ...
	 * @returns {boolean}
	 */
	function hasAllImplants(slave){
		return slave.boobsImplant > 0 && slave.buttImplant > 0 && slave.lipsImplant > 0;
	}

	/**
	 * call as SlaveStatsChecker.isSurgicallyImproved(slave)
	 * @param {FC.HumanState} slave
	 * @returns {boolean}
	 */
	function isSurgicallyImproved(slave){
		return globalThis.isSurgicallyImproved(slave);
	}
}());
...
/**
 * @param {FC.HumanState} slave
 * @param {boolean} ignoreCycle ignoreCycle=true will not take into account the slaves fertility cycle, but will return false if they are menstruating.
 * @param {boolean} ignoreMensturation ignoreMensturation=true will also ignore menstruation.
 * @returns {boolean}
 */
globalThis.canGetPregnant = function(slave, ignoreCycle = false, ignoreMensturation = false) {
	if (!slave) {
		return null;
	} else if (slave.preg === -1) { /* contraceptives check */
		return false;
	} else if (!isFertile(slave, ignoreCycle)) { /* check other fertility factors */
		return false;
	} else if (!ignoreMensturation && isMenstruating(slave)) {
		return false;
...
globalThis.canWalk = function(slave) {
	...
	// player exclusives
	const player = asPlayer(slave);
	if (player) {
		if (player.physicalImpairment > 1) {
			return false;
		}
	}
	return true;
...
globalThis.isHindered = function(slave) {
	...
	if (player) {
		if (onBedRest(player)) {
			return true;
		} else if (player.physicalImpairment !== 0) {
			return true;
		// } else if (player.energy > 95 || player.need > 0) {
		//	return true;
		}
	}
	return false;
...
/**
 * Returns false if the slave has something blocking their mouth
 * @param {FC.HumanState} slave
 * @returns {boolean}
 */
globalThis.canDoOral = function(slave) {
	if (!slave) {
		return null;
	} else if (typeof slave.mouthAccessory === "string" && !["none", ""].includes(slave.mouthAccessory)) {
		return false;
	}
	return true;
};
...
/**
 * Checks if the slave is menstruating this week.
 * @param {FC.HumanState} slave
 * @returns {boolean}
 */
globalThis.isMenstruating = function(slave) {
	return V.menstruation === 1 &&
		...
		slave.broodmother === 0 &&
		(slave.preg === 0 || slave.preg === -1) &&
		slave.fertPeak === 2 &&
		isFertile(slave, true);
};

/**
 * Checks if fertility is known.
 * @param {FC.HumanState} slave
 * @returns {boolean}
 */
globalThis.fertKnown = function(slave) {
	return (V.menstruationKnown === 1 && slave.fertKnown === 1) || V.menstruationKnown === 0;
};
