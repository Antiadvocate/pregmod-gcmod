...
globalThis.hasSurgeryRule = function(slave, rules) {
	if (!V.rulesAssistantFacilitiesAutosurgery) {
		// preserves old behaviour - no surgery is applied to slaves in facilities
		if (!assignmentVisible(slave)) {
			return false;
		}
	} else {
		if (assignmentOutside(slave)) {
			return false;
		}
	}
...
App.RA.ruleDeepAssign = function deepAssign(target, source, sourceRecord, sourceName) {
	...
	function isClitSet(rules) {
		return !(rules.clitSetting === null && rules.clitSettingEnergy === null && rules.clitSettingXX === null && rules.clitSettingXY === null);
	}

	// reset target if source sets clitSettings, to prevent low priority rules overriding
	if (isClitSet(source)) {
		target.clitSetting = null;
		target.clitSettingEnergy = null;
		target.clitSettingXX = null;
		target.clitSettingXY = null;
	}
