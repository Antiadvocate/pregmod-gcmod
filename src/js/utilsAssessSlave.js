...
globalThis.canImproveIntelligence = function(slave) {
	let origIntel = getGenePoolRecord(slave)?.intelligence || slave.intelligence;
...
/**
 * @param {Pick<FC.HumanState, "slaveSurname" | "slaveName"> & {nationality?: FC.HumanState["nationality"]}} actor
 * @returns {string}
 * TODO:@franklygeorge rename to `fullName`
 */
globalThis.SlaveFullName = function(actor) {
	const pair = actor.slaveSurname ? [actor.slaveName, actor.slaveSurname] : [actor.slaveName];
	if ((V.surnameOrder !== 1 && ["Cambodian", "Chinese", "Ancient Chinese Revivalist", "Hungarian", "Japanese", "Edo Revivalist", "Korean", "Mongolian", "Taiwanese", "Vietnamese"].includes(actor?.nationality)) || (V.surnameOrder === 2)) {
		pair.reverse();
	}
	return pair.join(" ");
};
 ...
 */
globalThis.perceivedGender = function(slave, spoken = false) {
	let degree = 0;
