 ...
 * @param {boolean} ignoreCycle
 * @param {boolean} ignoreMensuration
 * @returns {boolean}
 */
globalThis.canImpreg = function(slave1, slave2, ignoreCycle = false, ignoreMensuration = false) {
	if (!slave1 || !slave2) {
		return null;
		...
		return false;
	} else if (!isVirile(slave2)) {
		return false;
	} else if (!canBreed(slave1, slave2)) {
		return false; /* pregmod end */
	} else if (!canGetPregnant(slave1, ignoreCycle, ignoreMensuration)) {
 ...
 * @returns {boolean}
 */
globalThis.canFemImpreg = function(slave1, slave2, ignoreCycle = false, ignoreMensuration = false) {
	if (!slave1 || !slave2) {
		return null;
	} else if (slave2.dick > 0) {
		return false;
...
globalThis.cumAmount = function(slave) {
	let cum = 1;
	const balls = slave.ID === SID.PC ? slave.balls - slave.ballsImplant : slave.balls;
...
globalThis.intimidationDegree = function(intimidatee, intimidater) {
	...
	if (intimidater.ID === SID.PC) {
		...
		if (asPlayer(intimidater)?.rumor === "force") {
			degree += 1;
		}
	}
	...
	if (intimidater.horn !== "none") {
		if (intimidatee.behavioralFlaw === "devout") {
			degree += 2;
		} else if (["curved succubus horns", "backswept horns", "two long oni horns"].includes(intimidater.horn)) {
			degree++;
		} else if (intimidater.horn === "cow horns" && intimidater.boobs < 800) {
			degree++;
		}
	}
...
globalThis.canLift = function(liftee, lifter) {
	...
	if (lifter.boobs >= 15000 || lifter.boobsImplant >= 10000) {
		return false;
	}
