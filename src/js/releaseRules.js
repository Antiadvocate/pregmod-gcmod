...
App.Utils.sexAllowed = function sexAllowed(slaveA, slaveB) {
	...
	} else if (haveRelationshipP(asSlave(slaveA), slaveB)) {
		return (slaveA.rules.release.partner === 1) && (slaveB.rules.release.partner === 1);
	} else if (App.Utils.isDevelopmentFacilityLeader(asSlave(slaveA), asSlave(slaveB))) {
		return slaveA.rules.release.facilityLeader === 1;
	} else if (App.Utils.isDevelopmentFacilityLeader(asSlave(slaveB), asSlave(slaveA))) {
...
App.Utils.hasFamilySex = function hasFamilySex(slave) {
	...
	return getSlaves().some(s => areRelated(slave, s) && this.sexAllowed(slave, s));
};
