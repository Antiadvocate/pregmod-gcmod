...
globalThis.BeautyArray = function(slave) {
	...
	function calcIntelligenceBeauty(slave) {
		if (arcologyInfo.fsActive('FSSlaveProfessionalism')) {
			adjustBeauty("Intelligence: Slave Professionalism", ((FSValues.FSSlaveProfessionalism / 10) * (slave.intelligence / 10))); /* 100 */
	...
	function calcFSNotFuckdollBeauty(slave) {
		...
		if (arcology.FSBodyPurist > 20) {
			...
			if (slave.vagina === 0 && slave.counter.reHymen === 0) {
				adjustBeauty("True Virgin: Body Purist", (30 * (FSValues.FSBodyPurist / 100)));
			} else if (slave.vagina === 0) {
				adjustBeauty("Virgin: Body Purist", (5 * (FSValues.FSBodyPurist / 100)));
			}

			if (slave.anus === 0 && slave.counter.anal === 0) {
				adjustBeauty("Unsullied Anal Virgin: Body Purist", (30 * (FSValues.FSBodyPurist / 100)));
			} else if (slave.anus === 0) {
				adjustBeauty("Anal Virgin: Body Purist", (5 * (FSValues.FSBodyPurist / 100)));
			}
		}
	...
	function calcMiscNotFuckdollBeauty(slave) {
		...
		if (V.racialAttractivenessBonus === 2 && !FutureSocieties.isActive("FSSupremacist") && slave.race === arcology.FSSupremacistRace) {
			const preferredRace = App.Data.misc.filterRaces.get(arcology.FSSupremacistRace);
			if (slave.origRace === arcology.FSSupremacistRace) {
				adjustBeauty(`Regional Preference: ${preferredRace}`, 4);
			} else {
				adjustBeauty(`Regional Preference: Fake ${preferredRace}`, 2);
			}
		} else if (V.racialAttractivenessBonus === 1) {
			if (slave.race === "white") {
	...
	function calcRepopulationPregBeauty(slave) {
		if (slave.preg > slave.pregData.normalBirth / 1.33) {
			...
			adjustBeauty("Preg Beauty", (FSValues.FSRepopulationFocus / 10)); /* 10 */
		} else if (slave.preg > 0 && slave.collar === "preg biometrics" && slave.pregKnown === 1) {
	...
	function calcRestartPregBeauty(slave) {
		if (slave.breedingMark === 1 && V.propOutcome === 1) {
			if ((slave.preg > slave.pregData.normalBirth / 8) && ((slave.pregSource === SID.PC) || (slave.pregSource === SID.ELITE))) {
	 ...
	 * @param {FC.SlaveState} slave
	 */
	function calcPregCycleBeauty(slave) {
		if (fertKnown(slave) && isFertile(slave)) {
			adjustBeauty("Repopulation: Fertile", FSValues.FSRepopulationFocus / 2.5); // 40 - this counters the malice for non-pregnant slaves u get from the FS
		} else if (isMenstruating(slave)) {
			adjustBeauty("Repopulation: Menstruation", -FSValues.FSRepopulationFocus / 10); // -10
		}
	}
	...
	function calcFSMiscBeauty(slave) {
		...
		if (arcologyInfo.fsActive('FSSlaveProfessionalism')) {
			if (slave.energy > 80) {
				adjustBeauty("High Sex Drive: Slave Professionalism", (FSValues.FSSlaveProfessionalism / 100) * -(slave.energy)); /* -80 to -100 */
			} else if (slave.energy <= 40 && slave.devotion > 50) {
				adjustBeauty("Low Sex Drive: Slave Professionalism", (FSValues.FSSlaveProfessionalism / 100) * (10 - (slave.energy / 4))); /* +10 to 0 */
			}
		} else if (arcologyInfo.fsActive('FSIntellectualDependency')) {
	...
	function calcAgeBeauty(slave) {
		if (slave.physicalAge === V.minimumSlaveAge) {
			...
			if (slave.physicalAge === V.fertilityAge && canGetPregnant(slave, !fertKnown(slave)) && (arcologyInfo.fsActive('FSRepopulationFocus') || arcologyInfo.fsActive('FSGenderFundamentalist')) && !arcologyInfo.fsActive('FSRestart')) {
		...
		} else if (slave.physicalAge === V.fertilityAge && canGetPregnant(slave, !fertKnown(slave)) && (arcologyInfo.fsActive('FSRepopulationFocus') || arcologyInfo.fsActive('FSGenderFundamentalist')) && !arcologyInfo.fsActive('FSRestart')) {
...
globalThis.FResultArray = (function() {
	...
	function calcWorksWithRelationship(slave) {
		const fre = getSlaves().find(s => haveRelationshipP(slave, s) && sameAssignmentP(slave, s));
		if (fre) {
	...
	function calcSlaveProfessionalismBonus(slave) {
		if (slave.devotion > 50) {
			if (slave.energy <= 40) {
				adjustFResult(`Devotion/Energy: Slave Professionalism`, 4);
			} else {
				adjustFResult(`Devotion/Energy: Slave Professionalism`, 2);
			}
	...
	function calcAge(slave) {
		if ((FutureSocieties.isActive('FSRepopulationFocus') || FutureSocieties.isActive('FSGenderFundamentalist')) && slave.physicalAge === V.minimumSlaveAge && slave.physicalAge === V.fertilityAge && canGetPregnant(slave, !fertKnown(slave))) {
		...
		} else if ((FutureSocieties.isActive('FSRepopulationFocus') || FutureSocieties.isActive('FSGenderFundamentalist')) && slave.physicalAge === V.fertilityAge && canGetPregnant(slave, !fertKnown(slave))) {
			adjustFResult(`Young and Fertile: Repopulationist/Gender Fundamentalist`, 1);
			if (slave.birthWeek === 0) {
...
globalThis.slaveCostBeauty = function(slave, isStartingSlave, followLaws, isSpecial, modifiers) {
	...
	function calcAgeCost(slave) {
		if (slave.physicalAge === V.minimumSlaveAge && slave.physicalAge === V.fertilityAge && canGetPregnant(slave, !fertKnown(slave)) && (arcologyInfo.fsActive('FSRepopulationFocus') || arcologyInfo.fsActive('FSGenderFundamentalist'))) {
			if (slave.birthWeek === 0) {
			...
			(slave.physicalAge === V.fertilityAge && canGetPregnant(slave, !fertKnown(slave)) && (arcologyInfo.fsActive('FSRepopulationFocus') || arcologyInfo.fsActive('FSGenderFundamentalist')))
		) {
			if (slave.birthWeek === 0) {
