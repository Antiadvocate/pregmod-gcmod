...
App.UI.SlaveSummaryRenderers = function() {
	...
	const shortRenderers = {
		...
		drugs: function(slave, c) {
			...
			} else if (V.menstruation && !fertKnown(slave) && V.menstruationKnown > 0 && (slave.ovaries > 0 || slave.mpreg > 0) && slave.pubertyXX === 1 && slave.physicalAge <= V.fertilityAge) {
				makeSpan(c, "Prepub", styles);
			...
			} else if (V.menstruation === 1 && V.menstruationKnown === 1 && (slave.ovaries === 1 || slave.mpreg === 1) && slave.fertLate < 0 && !slave.pregKnown) {
				makeSpan(c, "Preg?", styles);
			} else if (V.menstruation === 1 && fertKnown(slave) && slave.fertPeak > 0 && slave.preg === 0 && (slave.ovaries === 1 || slave.mpreg === 1)) {
				makeSpan(c, "Safe", styles);
			} else if (V.menstruation === 1 && V.menstruationKnown === 0 && slave.fertPeak <= 0 && slave.preg === 0 && (slave.ovaries === 1 || slave.mpreg === 1)) {
				makeSpan(c, "Danger", styles);
			} else if (V.menstruation === 1 && V.menstruationKnown > 0 && !slave.fertKnown && !slave.pregKnown && (slave.ovaries === 1 || slave.mpreg === 1)) {
				makeSpan(c, "Fert?", styles);
			} else if (V.menstruation === 1 && V.menstruationKnown === 2 && !slave.pregKnown && (slave.ovaries === 1 || slave.mpreg === 1)) {
				makeSpan(c, "Fert", styles);
	...
	const longRenderers = {
		...
		drugs: function(slave, c) {
			...
			if (slave.bellyImplant > -1) {
				...
				makeSpan(c, "Menstruating.", styles);
			} else if (V.menstruation === 1 && V.menstruationKnown === 1 && (slave.ovaries === 1 || slave.mpreg === 1) && slave.fertLate < 0 && !slave.pregKnown) {
				makeSpan(c, "Missed period.", styles);
			} else if (V.menstruation === 1 && fertKnown(slave) && slave.fertPeak > 0 && slave.preg === 0 && (slave.ovaries === 1 || slave.mpreg === 1)) {
				makeSpan(c, "Safe week.", styles);
			} else if (V.menstruation === 1 && V.menstruationKnown === 0 && slave.fertPeak <= 0 && slave.preg === 0 && (slave.ovaries === 1 || slave.mpreg === 1)) {
				makeSpan(c, "Risky week.", styles);
			} else if (V.menstruation === 1 && V.menstruationKnown > 0 && !slave.fertKnown && !slave.pregKnown && (slave.ovaries === 1 || slave.mpreg === 1)) {
...
App.UI.SlaveSummary = function() {
	...
	function render(slave) {
		...
		if (slave.custom.label) {
			helpers.makeSpan(para, `${capFirstChar(slave.custom.label)}.`, ["custom-label"]);
		}
