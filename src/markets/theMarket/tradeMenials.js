/** Buy and sell menial slaves
 * @param {boolean} embedded - if true, this element is embedded in a page other than the market and should omit fluff and results
 */
App.UI.tradeMenials = function(embedded) {
	...
	if (!embedded) {
		App.UI.DOM.appendNewElement("h2", menialSpan, "Menial Slaves");

		menialSpan.append(slaveDemand());
		menialSpan.append(slaveSupply());

		App.UI.DOM.appendNewElement("div", menialSpan, popCap.text);
		App.UI.DOM.appendNewElement("div", menialSpan, `The parts of your arcology you own can house a total of ${num(popCap.value)} menial slaves.`);
	}

	menialSpan.append(menialTrading());

	if (!embedded) {
		if (!FutureSocieties.isActive('FSPaternalist')) {
			menialSpan.append(fuckDollConversion());
			menialSpan.append(fuckDollTrading());
		}

		menialSpan.append(bioreactorTrading());
	}
		...
		if (bulkMax > 0 && V.cash > menialPrice) {
			...
			linkArray.push(
				App.UI.DOM.link(
					...
					() => {
						buySomeMenials(1);
						jQuery(menialSpan).empty().append(App.UI.tradeMenials(embedded));
					}
				)
			);
