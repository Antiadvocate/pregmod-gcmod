App.Markets["Household Liquidator"] = function() {
	...
	/** @type {ReturnType<typeof prepSlaveSale>} */
	let newSlaves = null;
	if (jsRandom(1, 100) > 50) {
		...
		const genes = GenerateChromosome();
		const minAge = (genes === "XX" ? V.fertilityAge : V.potencyAge) + V.minimumSlaveAge + 1;
		const slave = GenerateNewSlave(genes, {
			minAge: minAge, maxAge: 42, ageOverridesPedoMode: 1, disableDisability: 1
		});
		slave.butt += 1;
		if (slave.vagina > -1) {
			slave.boobs += 100;
