...
App.EndWeek.personalBusiness = function() {
	...
	if (V.mods.food.enabled && V.mods.food.market) {
		if (App.Facilities.Farmyard.foodAvailable() < App.Facilities.Farmyard.foodConsumption() && V.cash < App.Facilities.Farmyard.foodBuyCost(App.Facilities.Farmyard.foodConsumption())) {
			...
			if (V.mods.food.warned === true) {
				V.temp.gameover = "starving citizens";
				V.nextLink = "Gameover";
			} else {
	...
	function whoring() {
		...
		if (V.PC.visualAge < V.minimumSlaveAge) {
			if (onBedRest(V.PC)) {
				...
				if (!isPCCareerInCategory("escort")) {
					...
					if (cervixPump > 0) {
						r.push(`You end the week with`);
						if (V.PC.cervixImplantTarget === "all") {
							r.push(`<span class="change positive">noticeable bloating across your body</span>`);
						} else if (V.PC.cervixImplantTarget === "TnA") {
							r.push(`<span class="change positive">more prominent tits and ass</span>`);
						} else if (V.PC.cervixImplantTarget === "mombod") {
							r.push(`<span class="change positive">a more motherly form</span>`);
						} else if (V.PC.cervixImplantTarget === "bottomHeavy") {
							r.push(`<span class="change positive">noticeable swelling in your butt and belly</span>`);
						} else if (V.PC.cervixImplantTarget === "boobsImplant") {
							r.push(`<span class="change positive">some noticeable weight to your chest</span>`);
						} else if (V.PC.cervixImplantTarget === "buttImplant") {
							r.push(`<span class="change positive">some noticeable expansion to your rear</span>`);
						} else {
							r.push(`<span class="change positive">some noticeable swelling in your belly</span>`);
						}
						r.push(`from all the loads pumped into`);
						...
						cervixPumpFillsImplant(V.PC, cervixPump);
					}
			...
			if (onBedRest(V.PC)) {
				...
				if (V.PC.bellyPreg >= 1500 && FutureSocieties.isActive('FSRestart') && V.PC.pregSource !== SID.PC && V.PC.pregSource !== SID.ELITE) {
		...
		if (sceneContinues) {
			...
			if (cervixPump > 0) {
				if (V.PC.cervixImplantTarget === "all") {
					r.push(`You feel a little heavier after all the sex, a byproduct of your <span class="change positive">body expanding</span>`);
				} else if (V.PC.cervixImplantTarget === "TnA") {
					r.push(`You start to find it difficult getting dressed after all the sex, a byproduct of <span class="change positive">the hourglass expansion</span>`);
				} else if (V.PC.cervixImplantTarget === "mombod") {
					r.push(`You feel a little heavier up front after all the sex, a byproduct of your <span class="change positive">belly and breasts expanding</span>`);
				} else if (V.PC.cervixImplantTarget === "bottomHeavy") {
					r.push(`Your lower body feels a little heavier after all the sex, a byproduct of your <span class="change positive">butt and belly expanding</span>`);
				} else if (V.PC.cervixImplantTarget === "boobsImplant") {
					r.push(`Your bra feels tighter after all the sex, a byproduct of your <span class="change positive">breasts expanding</span>`);
				} else if (V.PC.cervixImplantTarget === "buttImplant") {
					r.push(`You start to find it difficult getting dressed after all the sex, a byproduct of your <span class="change positive">butt expanding</span>`);
				} else {
					r.push(`You feel a little heavier around the middle after all the sex, a byproduct of your <span class="change positive">belly expanding</span>`);
				}
				r.push(`from the loads pumped into`);
				if (V.PC.cervixImplant === 1) {
					r.push(`your pussy.`);
				} else if (V.PC.cervixImplant === 2) {
					r.push(`your asshole.`);
