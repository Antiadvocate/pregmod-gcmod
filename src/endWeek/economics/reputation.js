...
App.EndWeek.reputation = function() {
	...
	if (V.PC.bellyPreg >= 1500) { // 5.0.0
		if (FutureSocieties.isActive('FSRestart')) {
			if (V.arcologies[0].FSRestartDecoration === 100) {
				if (V.PC.pregSource !== SID.PC && V.PC.pregSource !== SID.ELITE) {
	...
	if (FutureSocieties.isActive('FSRomanRevivalist')) {
		...
		const racialVarieties = new Set(getSlaves().map((s) => s.race));
		...
		if (getSlaves().filter(s => [Job.CLUB, Job.DJ, Job.PUBLIC].includes(s.assignment) || (s.assignment === Job.RECRUITER && V.recruiterTarget === "other arcologies" && V.arcologies[0].influenceTarget !== -1)).size <= threshold) {
