...
App.EndWeek.arcManagement = function() {
	...
	function fsImpact() {
		...
		if (FutureSocieties.isActive('FSAntebellumRevivalist')) {
			...
			slaveDemandU *= 1 + Math.trunc(Math.min(V.arcologies[0].FSAntebellumRevivalist, 100) / 20) * 0.08;
			slaveDemandT *= 1 + Math.trunc(Math.min(V.arcologies[0].FSAntebellumRevivalist, 100) / 20) * 0.05;
			welfareFS *= 1 + Math.trunc(Math.min(V.arcologies[0].FSAntebellumRevivalist, 100) / 20) * 0.025;
			lowerClass += Math.trunc(Math.min(V.arcologies[0].FSAntebellumRevivalist, 100) / 20) * -16;
			...
			r.push(`Antebellum Revivalism prioritizes the wealthy while supporting a moderately healthy yeoman class, but offers little assistance, or sympathy, to the poor.`);
		}
	 ...
	 * @returns {number}
	 */
	function processDemand(isFrozen) {
		let transportHub = 1;
		let crime = 0.8;
		if (V.secExpEnabled > 0) {
			transportHub = 0.7;
	...
	/** @returns {Array<string|HTMLSpanElement>} */
