...
App.SlaveAssignment.recruitGirls = function recruitGirls(slave) {
	...
	} else if (slavesLength < idleTarget) {
		recruiting(slave);
	} else {
		idlePublicity(slave);
	}
		...
		if (targetArcology !== undefined) {
			...
			if (arcologyInfo.fsActive('FSRepopulationFocus')) {
				...
				if (targetArcologyInfo.fsActive('FSRestart')) {
					...
					if (slave.belly < 1500 && !canGetPregnant(slave, !fertKnown(slave))) {
	...
	function idlePublicity(slave) {
		...
		if (arcologyInfo.fsActive('FSRomanRevivalist')) {
			...
			if (slave.skill.entertainment > 95 && App.Entity.facilities.servantsQuarters.employeesIDs().size > 5 && (slave.behavioralQuirk === BehavioralQuirk.FUNNY || totalInt > 50)) {
				pushFS(`${He} puts together a charming little video touring your spotless estate, smartly directing your large staff of servants to always appear busy or ready to serve as she moves about the grounds.`);

				let decorationScore = 0;
				const activeFacilities = Object.values(App.Entity.facilities).filter(f => f.isDecorated && f.established && f.isInPenthouse);
				for (const facility of activeFacilities) {
					if (facility.decoration !== "Antebellum Revivalist") {
						decorationScore++;
					}
				}

				if (0.67 >= (decorationScore / activeFacilities.length)) {
					pushFS(`Your commitment to the style and architecture of the era is clear to viewers, leaving even skeptics doubtless of your authenticity.`);
					seed += 3;
				} else if ( 0.33 >= (decorationScore / activeFacilities.length)) {
					pushFS(`The Antebellum decoration helps to sell the image of a Southern aristocrat.`);
					seed += 1;
				} else {
					pushFS(`Most viewers are impressed, but some apparently feel that your interior decoration is inconsistent with the Antebellum style.`);
				}
			...
			} else if (!App.Entity.facilities.servantsQuarters.established) {
