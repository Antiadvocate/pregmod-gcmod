		...
		switch (slave.drugs) {
			...
			case Drug.GROWTHSTIM:
				...
				if (growth > 0) {
					...
					if (slave.geneMods.NCS === 1) {
						r += ` ${His} <span class="orange">NCS</span> inhibits ${his} body's response to the treatment.`;
						ageMod = 0.25;
			...
			case Drug.SAGBGONE:
				if (slave.assignment === Job.CONCUBINE) {
					r += ` Before bed and when you awake, you take the time to sensually <span class="coral">massage sag-B-gone into ${his} breasts,</span> which, while enjoyable to the both of you, doesn't seem to be doing much.`;
