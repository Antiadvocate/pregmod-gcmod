...
App.SlaveAssignment.serveThePublic = function saServeThePublic(slave) {
	...
	function comingOfAge(slave) {
		if (slave.physicalAge === V.minimumSlaveAge && slave.physicalAge === V.fertilityAge && canGetPregnant(slave, !fertKnown(slave)) && (arcologyInfo.fsActive('FSRepopulationFocus') || arcologyInfo.fsActive('FSGenderFundamentalist')) && !arcologyInfo.fsActive('FSRestart')) {
		...
		} else if (slave.physicalAge === V.fertilityAge && canGetPregnant(slave, !fertKnown(slave)) && (arcologyInfo.fsActive('FSRepopulationFocus') || arcologyInfo.fsActive('FSGenderFundamentalist'))) {
			if (slave.birthWeek === 0) {
	...
	function physicalEffects(slave) {
		...
		if (canDoVaginal(slave)) {
			if (slave.vagina === 0) {
				...
				if (jsRandom(1, 100) > ((110 - vaginalUse) + (slave.vagina * 10) + (slave.skill.vaginal / 3))) {
		...
		if (canDoAnal(slave)) {
			if (slave.anus === 0) {
				...
				if (slave.vagina < 0) {
					if (jsRandom(1, 100) > ((100 - analUse) + (slave.anus * 10) + (slave.skill.anal / 6))) {
					...
					if (jsRandom(1, 100) > ((110 - analUse) + (slave.anus * 10) + (slave.skill.anal / 6))) {
	...
	function slaveSkills(slave) {
		...
		if (slave.aphrodisiacs > 1 || slave.inflationType === InflationLiquid.APHRO) {
			...
			if ((slave.skill.oral >= 100) && ((slave.skill.anal >= 100) || !canDoAnal(slave)) && ((slave.skill.vaginal >= 100) || !canDoVaginal(slave)) && (slave.skill.penetrative >= 100 || penetrativeSocialUse(slave) <= 0)) {
				...
				const weakSkills = [];
				for (const skill of ["oral", "anal", "vaginal", "penetrative"]) {
					if (slave.skill[skill] < 100) {
						weakSkills.push(skill);
					}
				}
				r += ` ${He} shows diligence, and ${his} <span class="skill inc">${toSentence(weakSkills)} skills improve,</span> according to what the citizens demand`;
				if (!canDoVaginal(slave)) {
			...
			if (!isAmputee(slave)) {
				...
				if (slave.skill.entertainment > jsRandom(50, 99) && slave.skill.whoring < 50) {
					...
					slave.skill.whoring += (2.5 + Math.floor((slave.intelligence + slave.intelligenceImplant) / 64));
				}
			}
		}
	}
	...
	function publicReactions(slave) {
		...
		if (totalRelatives(slave) > 0) {
			let children = getSlaves().filter((s) => areRelated(slave, s) && (s.assignment === slave.assignment));
			if (children.size > 1) {
				r += ` Since ${his} relatives, ${toSentence(children.map(s => s.slaveName))}, are public sluts too, ${he} earns extra attention.`;
			} else if (children.size > 0) {
				r += ` Since ${his} ${relativeTerm(slave, children.atIndex(0).value)}, ${children.atIndex(0).value.slaveName}, is a public slut too, ${he} earns extra attention.`;
			}
			if (children.size > 0) {
		...
		if (slave.bellyPreg >= 1500 || App.Data.misc.fakeBellies.includes(slave.bellyAccessory) || slave.bellyImplant >= 1500) {
			if (arcologyInfo.fsActive('FSRepopulationFocus')) {
				...
				if (slave.breedingMark === 0 || V.propOutcome === 0) {
					r += ` In the new culture of ${arcology.name}, ${his}`;
					if (App.Data.misc.fakeBellies.includes(slave.bellyAccessory) || slave.bellyImplant >= 1500) {
						r += ` apparent`;
					}
		...
		if (arcologyInfo.fsActive('FSRestart') && slave.breedingMark === 1 && V.propOutcome === 1) {
			r += ` Citizens are ecstatic at the opportunity to use an ordinarily exclusive breeding slave. `;
		}
		...
		if (V.seeRace === 1) {
			if (V.racialAttractivenessBonus === 2 && !FutureSocieties.isActive("FSSupremacist") && slave.race === arcology.FSSupremacistRace) {
				r += ` The public tastes favor ${arcology.FSSupremacistRace} ${girl}s like ${slave.slaveName}.`;
			} else if (V.racialAttractivenessBonus === 1 && slave.race === "white") {
		...
		let needText = ``;
		if (slave.energy > 95) {
			needText = ` As a nymphomaniac ${he} gets sexual release from every sex act${arcologyInfo.fsActive('FSSlaveProfessionalism') ? `` : ` and is a crowd favorite`}.`;
		...
		if (slave.attrKnown === 1) {
			...
			if (arcologyInfo.fsActive('FSSlaveProfessionalism') && slave.energy > 80) {
				r += ` ${His} intense sex drive is a turn off for some citizens. `;
			}
		}
	...
	function sexualSatiation(slave) {
		...
		if (cervixPump > 0) {
			if (slave.cervixImplantTarget === "all") {
				r += ` ${slave.slaveName} notices ${his} <span class="change positive">body has swollen</span>`;
			} else if (slave.cervixImplantTarget === "TnA") {
				r += ` ${slave.slaveName} notices ${his} <span class="change positive">tits and ass have expanded</span>`;
			} else if (slave.cervixImplantTarget === "mombod") {
				r += ` ${slave.slaveName} notices ${his} <span class="change positive">belly and breasts have swollen</span>`;
			} else if (slave.cervixImplantTarget === "bottomHeavy") {
				r += ` ${slave.slaveName} notices ${his} <span class="change positive">butt and belly have grown</span>`;
			} else if (slave.cervixImplantTarget === "boobsImplant") {
				r += ` ${slave.slaveName} notices ${his} <span class="change positive">boobs have grown</span>`;
			} else if (slave.cervixImplantTarget === "buttImplant") {
				r += ` ${slave.slaveName} notices ${his} <span class="change positive">butt has grown</span>`;
			} else {
				r += ` ${slave.slaveName} notices ${his} <span class="change positive">belly has swollen</span>`;
			}
			r += ` from all the `;
	...
	function pregnancy(slave) {
		...
		if (jsRandom(0, 99) < pregChance) {
			...
			App.EndWeek.saVars.addPregEvent(slave, SID.CITIZEN, `Due to ${citizenDesc} cumming in ${his} fertile ${slave.mpreg === 1 ? anusDesc(slave, true) : vaginaDesc(slave, true)}, <span class="pregnant">${he} has become pregnant.</span>`);
		} else if (isVirile(slave) && isFertile(slave) && slave.preg !== -1 && slave.geneMods.aggressiveSperm === 1 && canBreed(slave, slave) && ((slave.chastityVagina === 0 && slave.dick === 0) || (slave.chastityPenis === 0 && slave.dick > 0))) { // Possible Self Impreg, aggressive sperm ignores chastity
			let baseCum = cumAmount(slave); // DL
			const loadAmount = baseCum / 7;
			if (slave.dick > 0) { // Cumming inside others significantly reduces risk
				baseCum *= (1 - (0.80 * (Math.min((loadAmount * penetrativeUse), baseCum) / baseCum))); // Up to 80% reduction of base cum amount
			}
			if (isHorny(slave)) {
				baseCum *= 1.1;
			} else {
				baseCum *= (slave.energy / 100);
			}
			let chance = Math.log(baseCum + 1) * 5;
			if (slave.dick === 0) { // Increased chance if no dick (cumming is messier)
				chance *= 1.1;
			}
			if (!canWalk(slave)) {
				chance *= 1.1;
			}
			if (jsRandom(0, 999) < chance) { // Ends up being about 1% at 7DL of cum -> 3.2% at 700DL of cum, but will be lower as it relies on the previous preg event not triggering.
				App.EndWeek.saVars.addPregEvent(slave, slave.ID, `Due to ${his} aggressive sperm reaching ${his} fertile ${slave.mpreg === 1 ? `anal womb` : `womb`}, <span class="pregnant">${he} has become pregnant with ${his} own child.</span>`);
			}
		}
	}
};
