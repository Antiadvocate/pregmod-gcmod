...
App.SlaveAssignment.inflation = function saInflation(slave) {
	...
	function cervixImplantFluidConversion(slave) {
		if (slave.inflationMethod === 2 && slave.cervixImplantTarget !== "none") {
			r.push(`${His} rectal micropump implant filters out some fluid from the ${slave.inflationType} within ${him} during the week,`);
			if (slave.cervixImplantTarget === "all") {
				r.push(`<span class="change positive">adding it to ${his} fillable implants.</span>`);
			} else if (slave.cervixImplantTarget === "TnA") {
				r.push(`<span class="change positive">adding it to ${his} breast and butt implants.</span>`);
			} else if (slave.cervixImplantTarget === "mombod") {
				r.push(`<span class="change positive">adding most of it to ${his} abdominal implant and the rest to ${his} breast implants.</span>`);
			} else if (slave.cervixImplantTarget === "bottomHeavy") {
				r.push(`<span class="change positive">adding it to ${his} butt and abdominal implants.</span>`);
			} else if (slave.cervixImplantTarget === "boobsImplant") {
				r.push(`<span class="change positive">adding it to ${his} breast implants.</span>`);
			} else if (slave.cervixImplantTarget === "buttImplant") {
				r.push(`<span class="change positive">adding it to ${his} butt implants.</span>`);
			} else {
				r.push(`<span class="change positive">adding it to ${his} abdominal implant.</span>`);
			}
			cervixPumpFillsImplant(slave, 200);
		}
	}
};
