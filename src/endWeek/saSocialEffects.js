	...
	function makeSocialEffects() {
		...
		if (FutureSocieties.isActive('FSGenderRadicalist')) {
			...
			if (!FutureSocieties.isActive('FSRestart')) {
				...
				} else if (slave.preg === 0 && isFertile(slave, true)) {
		...
		if (FutureSocieties.isActive('FSRepopulationFocus')) {
			...
			if (slave.breedingMark === 1 && V.propOutcome === 1) {
				if (slave.pregKnown === 1 && (slave.pregSource === SID.PC || slave.pregSource === SID.ELITE)) {
					let r = [];
		...
		if (FutureSocieties.isActive('FSBodyPurist')) {
			...
			if (slave.fuckdoll === 0) {
				if (slave.vagina === 0 && slave.anus === 0 && slave.counter.reHymen === 0 && slave.counter.anal === 0) {
				...
				} else if (slave.vagina === 0 && slave.counter.reHymen === 0) {
