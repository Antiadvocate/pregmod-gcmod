...
App.SlaveAssignment.guardYou = function saGuardYou(slave) {
	...
	function trainReplacements(slave) {
		...
		if (requirements.filter(s => s[0]).length > 0) {
			...
			const successorCandidates = getSlaves().filter(s => (assignmentVisible(s) || [Job.CONCUBINE, Job.WARDEN, Job.HEADGIRL, Job.QUARTER, Job.MASTERSUITE].includes(s.assignment)) && bodyguardSuccessorEligible(s));
			...
			if (combatSkilled.size < 2) {
				...
				if (!candidate) {
					candidate = successorCandidates.find(function(s) { return s.skill.combat <= 60; })?.value;
					if (candidate) {
