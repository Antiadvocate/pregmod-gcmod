...
App.EndWeek.servantsQuartersReport = function() {
	let facility = App.Entity.facilities.servantsQuarters;
	const beforeFrag = new DocumentFragment();
	App.EndWeek.saVars.flSex = App.EndWeek.getFLSex(App.Entity.facilities.servantsQuarters);
	App.EndWeek.saVars.flImpreg = {};

	const slaves = App.Utils.sortedEmployees(facility);
	const devBonus = (facility.decoration !== "standard") ? 1 : 0;
	...
	function stewardessText() {
		...
		if (S.Stewardess) {
			...
			if (App.Entity.facilities.servantsQuarters.rule('stewardessImpregnates') === 1 && canPenetrate(S.Stewardess) && isVirile(S.Stewardess) && !slaveResting(S.Stewardess)) {
				let potentiallyFertile = false;
				let cumLeft = resetHGCum(S.Stewardess); // get max slaves she can impregnate
				for (const slave of slaves) {
					if (!fertKnown(slave)) {
						potentiallyFertile = true;
					}
					if (cumLeft > 0 && canImpreg(slave, S.Stewardess, !fertKnown(slave)) && slave.pregKnown === 0) {
						const result = tryKnockMeUp(slave, 100, 2, S.Stewardess);
						let virginityLoss = false;
						if (slave.mpreg === 1) {
							seX(S.Stewardess, "penetrative", slave, "anal", jsRandom(8, 12));
							if (slave.anus === 0) {
								slave.anus = 1;
							...
							seX(S.Stewardess, "penetrative", slave, "vaginal", jsRandom(8, 12));
							if (slave.vagina === 0) {
								slave.vagina = 1;
								stewardessTookVirginity++;
								virginityLoss = true;
						...
						slave.need -= slave.fetish === Fetish.PREGNANCY ? 40 : 20;
						stewardessImpregnated++;
						cumLeft--;
						App.EndWeek.saVars.flImpreg[slave.ID] = {success: !_.isEmpty(result), virginityLoss};
					}
				}
				if (stewardessImpregnated > 0) {
					r.push(`It's ${his} responsibility to keep ${his} charges pregnant, so`);
					if (stewardessImpregnated === 1) {
						r.push(`${he} cums in the only ${potentiallyFertile ? `potentially fertile` : `fertile`} slave's pussy all week.`);
					} else {
						r.push(`${he} ensures the ${stewardessImpregnated} ${potentiallyFertile ? `potentially fertile` : `fertile`} servants are full of ${his} cum at all times.`);
					}
					...
					S.Stewardess.need -= stewardessImpregnated * (S.Stewardess.fetish === Fetish.PREGNANCY ? 40 : 20);
				}
			}
			if (FLsFetish === 1) {
	...
	for (const slave of App.SlaveAssignment.reportSlaves(slaves)) {
		...
		if (V.showEWD || (V.favSeparateReport === 1 && V.favorites.includes(slave.ID))) {
			...
			slaveEntry.append(App.SlaveAssignment.saSlaveIntro(slave, `is working out of ${facility.name}.`));
	...
	if (SQMilk > 0) {
		App.Events.addNode(afterFrag, [`Since your lactating servants spend most of their time working in the penthouse, they use the milkers there, giving ${SQMilk} liters of milk over the week, which is sold for <span class="yellowgreen">${cashFormat(SQMilkSale)}.</span>`], "p", ["indent"]);
	}

	if (facility.decoration !== "standard") {
		App.Events.addNode(afterFrag, [`${capFirstChar(facility.name)}'s ${facility.decoration} atmosphere <span class="hotpink">has a minor impact on your servants.</span>`], "p", ["indent"]);
	}
