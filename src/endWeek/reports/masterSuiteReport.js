...
App.EndWeek.masterSuiteReport = function() {
			...
			if (V.verboseDescriptions === 1 || (V.favSeparateReport === 1 && V.favorites.includes(S.Concubine.ID))) {
				...
				if (S.Concubine.fetish !== Fetish.MINDBROKEN) {
					...
					if (S.Concubine.prestigeDesc === "$He is remembered for winning best in show as a breeder.") {
						...
						} else if (isFertile(S.Concubine, true) || S.Concubine.preg > 0) {
	...
	function commonText(slave) {
		...
		if (V.masterSuiteUpgradePregnancy === 1) {
			...
			/* Keeps them on as long as they aren't menstruating will more less ensure they are on the drugs during their fertile week. */
			if (V.masterSuiteHyperPregnancy === 1 && slave.drugs !== Drug.SUPERFERTILITY && canGetPregnant(slave, true)) {
				slave.drugs = Drug.SUPERFERTILITY;
			} else if (V.masterSuitePregnancyFertilityDrugs === 1 && slave.drugs !== Drug.FERTILITY && canGetPregnant(slave, true)) {
				...
				(!canGetPregnant(slave, true) && [Drug.FERTILITY, Drug.SUPERFERTILITY].includes(slave.drugs))
			) {
		...
		if (V.verboseDescriptions === 1 || (V.favSeparateReport === 1 && V.favorites.includes(slave.ID))) {
			...
			if (partTime.length > 0) {
				App.Events.addNode(frag, partTime, "div", "indent");
			}
