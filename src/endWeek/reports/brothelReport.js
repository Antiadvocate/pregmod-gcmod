...
App.EndWeek.brothelReport = function() {
				...
				if (slave.prestigeDesc === "$He is a famed Free Cities whore, and commands top prices.") {
					...
					if (slave.bellyPreg >= 5000) {
						...
						if (slave.pregKnown === 1 && slave.bellyPreg < 1500) {
							...
							r.push(`is menstruating this week.`);
						} else {
	...
	if (SL > 0) {
		...
		App.Events.addNode(afterFrag, [`${brothelNameCaps} makes you <span class="cash inc">${cashFormat(profits)}</span> this week.`], "p", ["indent"]);

		if (V.brothelDecoration !== "standard") {
			App.Events.addNode(afterFrag, [`${brothelNameCaps}'s customers enjoyed <span class="reputation inc">fucking whores in ${V.brothelDecoration} surroundings.</span>`], "p", ["indent"]);
		}
