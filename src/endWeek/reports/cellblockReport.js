...
App.EndWeek.cellblockReport = function() {
	...
	if (S.Wardeness) {
		...
		if ((S.Wardeness.dick > 2 || S.Wardeness.clit > 3) && canPenetrateAny(S.Wardeness)) {
			...
			if (S.Wardeness.dick > 2) {
				...
				r.push(`${His} molestation of the prisoners is more varied and effective because ${he} has a pseudophalic clit to fuck them with.`);
			}
	...
	if (slaves.length > 0) {
		...
		if (slaves.length > 1) {
			$(intro).append(`<strong>There are ${slaves.length} slaves being confined in ${V.cellblockName} until they are willing to obey.</strong>`);
		} else {
			$(intro).append(`<strong>There is one slave being confined in ${V.cellblockName} until they are willing to obey.</strong>`);
		}
	}
	...
	if (softenedQuirks || brokenSlaves) {
		...
		// add the description block for both softening and breaking
		if (softenedQuirks + brokenSlaves > 0) {
			if (V.cellblockDecoration !== "standard") {
				App.Events.addNode(afterFrag, [`${cellblockNameCaps}'s ${V.cellblockDecoration} atmosphere <span class="hotpink">had an impact on them while they were</span> imprisoned.`], "p", ["indent"]);
			}
		}
	}
