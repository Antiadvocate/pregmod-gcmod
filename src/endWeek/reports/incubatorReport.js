...
App.EndWeek.incubatorReport = function() {
	...
	const speedUpgrade = incubator.upgrade('speed');
	...
	for (const tank of getTankSlaves().values()) {
		...
		if (tank.incubatorSettings.growTime > 0) {
			tank.incubatorSettings.growTime -= speedUpgrade;
			r.push(`<span class="pink">${tank.slaveName}'s</span> growth is currently being accelerated. ${He}`);
			if (Math.ceil(tank.incubatorSettings.growTime/speedUpgrade) <= 0) {
				r.push(`is <span class="lime">ready for release.</span> ${He} will be ejected from ${his} tank upon your request.`);
			} else {
				r.push(`will be ready for release in about ${Math.ceil(tank.incubatorSettings.growTime/speedUpgrade)} weeks.`);
			}
		} else {
		...
		if (incubator.upgrade('weight') === 1) {
			if (tank.incubatorSettings.weight === 1) {
				if (tank.weight < 200) {
					if (speedUpgrade === 52) {
						tank.weight += 70;
					} else if (speedUpgrade === 18) {
						tank.weight += 40;
					} else if (speedUpgrade === 9) {
						tank.weight += 20;
					} else if (speedUpgrade === 6) {
						tank.weight += 10;
					} else if (speedUpgrade === 5) {
						tank.weight += 5;
					}
				}
		...
		r = [];
		if (incubator.upgrade('growthStims') === 1 && tank.incubatorSettings.growthStims !== 0) {
		...
		if (tank.incubatorSettings.pregAdaptation > 0 && tank.incubatorSettings.growTime > 0) {
			if (tank.incubatorSettings.pregAdaptationInWeek <= 0) {
				...
				let weekAdapt = tank.incubatorSettings.pregAdaptationInWeek * speedUpgrade;
				if (isNaN(weekAdapt)) {
				...
				weekAdapt = tank.incubatorSettings.pregAdaptationInWeek * speedUpgrade;
				...
				if (random(1, 100) <= (tank.incubatorSettings.pregAdaptationPower - 1) * (speedUpgrade / 2 + 1)) {
