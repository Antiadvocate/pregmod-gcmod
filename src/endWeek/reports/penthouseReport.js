...
App.EndWeek.penthouseReport = function() {
	...
	const penthouseSlaves = getSlaves().filter(s => assignmentVisible(s) || !App.EndWeek.saVars.slaveCheckedIn.includes(s.ID));
	...
	for (const slave of App.SlaveAssignment.reportSlaves(penthouseSlaves.asArray())) {
	...
	let fcs = App.Entity.facilities;
	V.brothelSpots = fcs.brothel.freeSpace;
	V.clubSpots = fcs.club.freeSpace;
	V.dairySpots = fcs.dairy.freeSpace;
	App.EndWeek.saVars.facilitySpots.servantsQuarters = fcs.servantsQuarters.freeSpace;
	...
	function slavesToTrain() {
		if (S.HeadGirl) {
			...
			for (const slave of penthouseSlaves.values()) {
			...
			// @ts-ignore
			return HGPossibleSlaves.flat(Number.POSITIVE_INFINITY).slice(0, App.EndWeek.saVars.HGEnergy);
		} else {
			return [];
		}
	}
