...
App.EndWeek.dairyReport = function() {
	...
	if (V.MilkmaidID !== 0) {
		...
		for (const slave of slaves) {
			...
			if (
				App.Facilities.Dairy.milkmaidWillImpreg(slave)
			) {
				milkmaidImpregnated++;
				if (slave.mpreg === 1) {
					mpregImpregnated++;
			...
			if (App.Facilities.Dairy.milkmaidWillSodomize(slave)) {
				prostateStim++;
			}
		...
		if (milkmaidImpregnated > 0) {
			...
			if (milkmaidImpregnated === 1) {
				...
				// mpregImpregnated === milkmaidImpregnated checks if there are only mpreg this week.
				r.push(`fills the ${milkmaidImpregnated} cows'${mpregImpregnated === milkmaidImpregnated ? ` anal` : ``} wombs with ${his} cum.`);
			}
			...
			S.Milkmaid.need -= milkmaidImpregnated * (S.Milkmaid.fetish === Fetish.PREGNANCY ? 20 : 10);
		}
		switch (FLsFetish) {
			case 1:
		...
		if (S.Milkmaid.skill.oral < 90) {
			r.push(slaveSkillIncrease('oral', S.Milkmaid, 3));
		}
		...
		if (prostateStim > 0) {
			...
			S.Milkmaid.need -= prostateStim * 10;
		}
		if (milkmaidImpregnated || (prostateStim > 0)) {
			r.push(slaveSkillIncrease('penetrative', S.Milkmaid, 2));
		}
	...
	for (const slave of App.SlaveAssignment.reportSlaves(slaves)) {
		...
		if (V.dairyPregUpgrade === 1 && V.dairyPregSetting > 0) {
			if ((slave.ovaries === 1 && slave.vagina > -1) || (slave.mpreg === 1 && V.dairyStimulatorsSetting > 0)) {
				if (slave.vagina > -1) {
				...
				if (isFertile(slave)) {
					if (slave.eggType === "human") {
						...
						WombImpregnate(slave, slave.pregType, SID.CITIZEN, 1);
					} /* closes eggType */
					if (slave.vagina === 0) {
						slave.vagina++;
					}
	...
	if (V.dairyDecoration !== "standard") {
		App.Events.addNode(afterFrag, [`${dairyNameCaps}'s <span class="green">${V.dairyDecoration} style is well known.</span>`], "p", ["indent"]);
	}
