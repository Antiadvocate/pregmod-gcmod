...
App.Facilities.Nursery.nurseryReport = function nurseryReport() {
	...
	if (slaves) {
		let r = [];
		r.push(`<strong>${slaves.length > 1 ? `There are ${slaves.length} slaves` : `There is one slave`} working in ${V.nurseryName}.</strong>`);
	...
	if (S.Matron) {
		...
		if (V.showEWD || (V.favSeparateReport === 1 && V.favorites.includes(slave.ID))) {
			const matronEntry = App.UI.DOM.appendNewElement("div", beforeFrag, '', ["slave-report"]);
	...
	for (const slave of App.SlaveAssignment.reportSlaves(slaves)) {
		...
		if (V.showEWD || (V.favSeparateReport === 1 && V.favorites.includes(slave.ID))) {
			...
			const slaveEntry = App.UI.DOM.makeElement("div", '', ["slave-report"]);
	...
	if (V.nurseryDecoration !== "standard") {
		App.Events.addNode(afterFrag, [`${capFirstChar(V.nurseryName)}'s ${V.nurseryDecoration} atmosphere <span class="devotion inc">has a minor impact on your nannies.</span>`], "p", ["indent"]);
	}
