...
App.EndWeek.spaReport = function() {
	...
	if (restedSlaves > 0) {
		...
		if (V.spaDecoration !== "standard") {
			const subjectText = restedSlaves === 1 ? `${him} while ${he} was` : `them while they were`;
			App.Events.addNode(afterFrag, [`${capFirstChar(V.spaName)}'s ${V.spaDecoration} atmosphere <span class="hotpink">had an impact on ${subjectText}</span> resting.`], "p", ["indent"]);
		}
	}
