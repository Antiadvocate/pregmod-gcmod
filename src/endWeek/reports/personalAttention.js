...
App.PersonalAttention.slaveReport = function(slave) {
	...
	switch (pa.objective) {
						...
						if (overpowerCheck(slave, V.PC) < random(1, 100)) {
							...
							if (canPenetrate(slave)) {
								...
								if ((slave.prostate > 0 || cumTotal >= 1) && slave.dick === 0) {
									r.push(`and thoroughly soaked you with ${isVirile(slave) ? "semen and " : ""}sexual fluids.`);
		...
		case "ravished": {
			const inControl = (slave.trust > 20 && slave.devotion < 20 && overpowerCheck(slave, V.PC) < 120 - slave.trust) ? false : true; // may need fine tuning
			...
			r.push(`You've had so much pent up sexual energy lately that you frequently need to avail yourself of ${his} dick in order to get some sense fucked back into you.`);
			if (V.PC.vagina === 0) {
			...
			if (playerOnTop && !canLift(V.PC, slave)) {
