...
App.Facilities.Nursery.childrenReport = function childrenReport() {
	...
	function musclesRulesEffects(child) {
		...
		if (V.nurseryMuscles) {
			...
			const muscleSpan = App.UI.DOM.makeElement("span", 'rapid muscle development. ', ["improvement"]);
	...
	function childFriendshipRivalries(child) {
		...
		const cribsFiltered = getInfants().filter(c => c.ID !== child.ID);

		for (const target of cribsFiltered.values()) {
	...
	function newChildFetish(fetish = "none") {
		switch (fetish) {
			...
			case Fetish.BESTIALITY:
				return `has developed a fascination for getting fucked by animals`;
			default:
	...
	function childGrowTime(child) {
		...
		if (child.growTime > 0) {
			...
			mainSpan.append(nameSpan, ' is ', limeSpan, ` ${He} will be removed from ${V.nurseryName} upon your request. `);
		}
	...
	/**
	 * Returns the number of children over the target age in the Nursery.
	 * @param {number} [targetAge=3]
	 */
	function childrenOverTargetAge(targetAge = 3) {
		return getInfants().filter(c => c.actualAge >= targetAge).size;
	}
