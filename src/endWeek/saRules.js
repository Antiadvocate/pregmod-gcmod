			...
			switch (slave.assignment) {
					...
					if (slave.devotion < -50) {
						...
						switch (partner.type) {
							...
							case "nurse":
								...
								break;
							case "sub":
								r.push(`is regularly serviced by ${his} assigned ${numberWithPlural(App.EndWeek.saVars.subSlaveMap[slave.ID].length, `subordinate slave`)} at ${his} pleasure.`);
								// Sex taken care of on the sub side.
								slave.need = 0;
								break;
							default:
				...
				case Job.CELLBLOCK: {
					...
					if (slave.devotion < -50) {
						...
						if (canImpreg(slave, V.PC) && ((slave.vagina > 0 && slave.ovaries === 1) || (slave.anus !== 0 && slave.mpreg === 1))) {
							knockMeUp(slave, 10, 0, -1);
							if (slave.preg > 0 && slave.pregKnown === 1) {
				...
				case Job.STEWARD:
					...
					r.push(`${He} is <span class="hotpink">very happy</span> with ${his} private room off of ${App.Entity.facilities.servantsQuarters.name} and <span class="mediumaquamarine">trusts</span> you a bit more for placing ${him} in charge of it. It also helps to offset the tiring nature of ${his} position.`);
					slave.devotion += 1;
					slave.trust += 1;
				...
				case Job.QUARTER:
					...
					if (slave.devotion <= 20) {
						switch (App.Entity.facilities.servantsQuarters.decoration) {
							case "Degradationist":
						...
						switch (App.Entity.facilities.servantsQuarters.decoration) {
							case "Degradationist":
								if (slave.trust > 40) {
									r.push(`The abysmal living conditions of ${App.Entity.facilities.servantsQuarters.name} <span class="gold">remind ${him} that ${his} life is absolutely meaningless to you.</span>`);
									slave.trust -= 3;
								} else if (slave.trust > 10) {
									r.push(`The abysmal living conditions of ${App.Entity.facilities.servantsQuarters.name} <span class="gold">remind ${him} that ${he} is worthless as a person to you.</span>`);
									slave.trust -= 2;
								}
								break;
							case "Subjugationist":
							...
							case "Antebellum Revivalist":
								if (slave.trust > 40) {
									r.push(`The spare living conditions of ${App.Entity.facilities.servantsQuarters.name} <span class="gold">remind ${him} not to get too comfortable</span> with ${his} life.`);
									slave.trust -= 2;
								} else if (slave.trust > 10) {
									r.push(`The spare living conditions of ${App.Entity.facilities.servantsQuarters.name} <span class="gold">keep ${him} aware of ${his} place.</span>`);
									slave.trust -= 1;
								}
								break;
							case "Aztec Revivalist":
							...
							case "Edo Revivalist":
							case "Roman Revivalist":
								r.push(`The living conditions of ${App.Entity.facilities.servantsQuarters.name} might be spare, but they are no means uncomfortable.`);
								break;
							case "Arabian Revivalist":
							case "Egyptian Revivalist":
							case "Neo-Imperialist":
								r.push(`The living conditions of ${App.Entity.facilities.servantsQuarters.name} might be spare, but ${he} loves the little luxuries that come with them.`);
								break;
							default:
								r.push(`${He} likes ${his} personal space in ${App.Entity.facilities.servantsQuarters.name}'s dormitory.`);
						}
					}
					if (slave.rules.living === "luxurious") {
						r.push(`They provide <span class="health inc">satisfying rest</span> every time ${he} drifts off to sleep.`);
