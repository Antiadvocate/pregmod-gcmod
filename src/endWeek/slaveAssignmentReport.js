...
App.EndWeek.slaveAssignmentReport = function() {
	...
	for (const slave of getSlaves().values()) {
		...
		if (slave.devotion >= -50) {
			if (slave.energy > 20) {
				/* get slave need from last week before it is reset below */
				const frustratedLastWeek = slave.need > slave.energy / 2;
				...
				/* slightly increase need this week if slave was unsatisfied last week */
				if (frustratedLastWeek) {
					slave.need *= 1.10;
				}
	...
	if (facilities.spa.established) {
		...
		for (const slave of App.Utils.sortedEmployees(App.Entity.facilities.spa)) {
			if (slave.geneMods.aggressiveSperm === 1 && isVirile(slave) && (slave.rules.release.masturbation || App.Utils.hasNonassignmentSex(slave) || slave.rules.release.facilityLeader) && slave.energy > 20 && (!S.Attendant || V.spaAggroSpermBan !== 1)) {
				...
				App.EndWeek.saVars.poolJizzers.push({ID: slave.ID, weight: cum}); // FIXME: weight is being stored in App.EndWeek.saVars.poolJizzers, but isn't used anywhere
			}
		}
	}
	...
	if (V.StudID) {
		...
		for (const slave of getSlaves().values()) {
			...
			if (canGetPregnant(slave, !fertKnown(slave)) && canBreed(stud, slave) && slave.ID !== V.StudID) {
				stud.need -= 20;
	...
	if (V.seeImages && V.seeReportImages) {
		...
		const drawnSlaveIDs = getSlaves().filter(s => !assignmentVisible(s) && !undrawnJobs.includes(s.assignment)).map(s => s.ID);
