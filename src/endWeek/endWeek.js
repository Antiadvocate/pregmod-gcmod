globalThis.endWeek = (function() {
	function doEndWeek() {
		...
		for (const s of getSlaves().values()) {
			if (V.experimental.slaveHistory) {
				// save last week to history
				App.SlaveAssignment.progressHistory(s);
			}
			slavePrep(s);
		}
		setUseWeights();
	...
	function nursery() {
		for (const c of getInfants().values()) {
			c.growTime--;
			c.birthWeek++;
			if (c.birthWeek >= 52) {
				c.birthWeek %= 52;
	...
	function advance() {
		// check for Gameover and if not gameover then continue to the Slave Assignments Report
		if (slaveCount() < 1) {
			V.temp.gameover = "no slaves";
			Engine.play("Gameover");
		} else if (V.arcologies[0].ownership < V.arcologies[0].minority) {
			V.temp.gameover = "ownership";
			Engine.play("Gameover");
		} else {
	...
	function confirmEndWeek() {
		...
		if (
			...
			(V.releaseID >= App.Version.release || confirm(`Ending the week without running BC is likely to break things. Are you sure you want to continue?`))
		) {
