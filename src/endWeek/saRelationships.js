...
App.SlaveAssignment.relationships = function saRelationships(slave) {
	...
	function generateRelationship(slave) {
		...
		if (slave.devotion + slave.trust > 175 && random(1, 3) === 1) { // player + emotional slut
			...
			const potentialFriend = getSlaves().find((s) => s.career === "a Futanari Sister" && s.rules.relationship === "permissive" && canStartFriendship(slave, s))?.value;
			...
			const potentialFriend = getSlaves().find((s) => (s.prestigeDesc === "You bankrupted and enslaved $him in revenge for $his part in the attack on your arcology by the Daughters of Liberty." && s.fuckdoll === 0 && s.fetish !== Fetish.MINDBROKEN && s.newGamePlus === 0))?.value;
			if (potentialFriend !== undefined) {
			...
			if (slave.trust > -90) { // used to be devotion > -90. Why a slave hating you wouldn't try to make friends with others that hate you doesn't make sense. Almost tempted to raise it to -50.
				...
				if (randomSeed > 75) {
					let resentment = 0;
					for (const potentialFriend of getSlaves().values()) {
						if (canStartFriendship(slave, potentialFriend) && potentialFriend.assignment !== Job.CONFINEMENT) {
							...
							if (potentialFriend.ID === V.MadamID && slave.assignment === Job.BROTHEL) {
							...
							} else if (mutualChildren(slave, potentialFriend, getSlaves().asArray()) > 0) {
								if (App.Utils.sexAllowed(slave, potentialFriend)) {
									if (potentialFriend.rules.relationship === "permissive") {
										...
										if (mutualChildren(slave, potentialFriend, getSlaves().asArray()) === 1) {
											r.push(`a child`);
										} else {
											r.push(`children`);
										}
			...
			switch (slave.relationship) {
				case 1: // friends
					...
					if (slave.devotion + slave.trust > 170 && random(1, 100) > 95) { // 5% chance to abandon friend to ebond/eslut
						...
						slave.relationship++;
						friend.relationship = slave.relationship;
					} else if (mutualChildren(slave, friend, getSlaves().asArray()) > 0 && (random(1, 100) < (50 + (10 * mutualChildren(slave, friend, getSlaves().asArray()))))) {
				...
				case 2: // BFFs;
					...
					if (slave.devotion + slave.trust > 95 && random(1, 100) === 100) { // 1% chance to abandon best friend to ebond/eslut
						...
						if (mutualChildren(slave, friend, getSlaves().asArray()) > 0 && random(1, 100) < (50 + (10 * mutualChildren(slave, friend, getSlaves().asArray())) + seed)) {
				...
				case 3: // FWB
					...
					if (V.seeIncest === 1 && slave.energy > 95 && areRelated(slave, friend) && random(1, 100) > (90 - seed)) {
						...
						slave.relationship++;
						friend.relationship = slave.relationship;
					} else if (mutualChildren(slave, friend, getSlaves().asArray()) > 0 && random(1, 100) < (50 + (10 * mutualChildren(slave, friend, getSlaves().asArray())) + seed)) {
	...
	function incestReactions(slave, relative) {
		if (areRelated(slave, relative)) {
			...
			const repType = relative.ID === SID.PC ? "PCRelationships" : "SlaveRelationships";
			const whose = relative.ID === SID.PC ? "your" : "their";

			if (slave.fetish !== Fetish.MINDBROKEN) {
				if (relative.ID === SID.PC) {
					r.push(`${He}`);
				} else {
					r.push(`${slave.slaveName}`);
				}
				if ((slave.origin === "$He offered $himself to you for enslavement hoping you would preserve $his incestuous relationship with $his sibling." || slave.origin === "$He offered to become your slave to protect $his incestuous relationship.") && relative.ID !== SID.PC) {
	...
	function familyFeelings(slave) {
		...
		const overwhelmed = 5;
		if (slave.trust <= 95) {
			let relatives = getSlaves().filter((s) => areRelated(slave, s)).values();
			if (slave.trust < -20) {
