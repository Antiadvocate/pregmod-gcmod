...
App.SlaveAssignment.pregnancy = function saPregnancy(slave) {
	...
	function pregnancyDiscovery(slave) {
		if (slave.preg === slave.pregData.normalBirth / 8) { /* BAD design - with speed control drugs and animal pregnancy 5th week can be not integer value, and block will be skipped as it's will be not strict == */
			...
			if (slave.womb[slave.womb.length - 1].motherID === SID.PC) { // if the PC is the real mother (transferred the ova)
				...
				if (slave.womb[slave.womb.length - 1].fatherID === SID.PC) { // if PC is the father
			...
			} else if (slave.pregSource === SID.PC) {
				V.PC.counter.slavesKnockedUp++;
	...
	function pregnancyMentalEffects(slave) {
		...
		if (slave.career === "a dairy cow" && slave.devotion <= 50) {
			...
			slave.devotion += 1;
		} else if (slave.devotion <= 20 && slave.pregSource === SID.PC) {
		...
		} else if (slave.devotion > 50 && slave.pregSource === SID.PC) {
		...
		if (slave.pregSource === slave.relationshipTarget && slave.relationship > 2) {
			...
			slave.trust += 1;
		} else if (slave.pregSource === SID.FUTA) {
		...
		switch (slave.pregControl) {
			...
			case GestationDrug.SLOW:
				if (slave.devotion <= 20 && slave.pregSource === SID.PC) {
	...
	function preconception(slave) {
		 ...
		 *
		 * Impregnation will also be attempted if they are potentially fertile if their cycle is not known.
		 */
		if (canGetPregnant(slave, !fertKnown(slave)) && ((slave.assignment === Job.DAIRY && V.dairyPregSetting === 0) || slave.assignment !== Job.DAIRY) && !V.temp.slaveIdsImpregnanted?.has(slave.ID)) {
			impregnation(slave);
		}
	...
	function pcDoKnockUpSlave(slave) {
		if (slave.pregKnown === 0) {
			r.push(`You frequently avail yourself of ${his} fertile`);
			if (slave.mpreg === 1) {
				r.push(`ass.`);
			} else {
				r.push(`pussy.`);
	...
	function impregnation(slave) {
		...
		let StudTakesV = 0;
		const cycleKnown = fertKnown(slave);
		const fertileText = cycleKnown ? `fertile` : `potentially fertile`;
		...
		function tryImpreg(slave, PC) {
			if (!isVirile(PC)) {
				...
				return false;
			} else if (!canGetPregnant(slave, !cycleKnown)) {
				return false;
		...
		if (V.universalRulesImpregnation === "PC" && tryImpreg(slave, V.PC) && (slave.pregKnown === 0 || (V.universalRulesSuperfetationImpregnation === 1 && slave.geneticQuirks.superfetation === 2 && (slave.pregKnown === 1 || V.geneticMappingUpgrade > 0 || slave.counter.birthsTotal > 0))) && slave.PCExclude !== 1) {
			if (isPlayerFrigid()) {
				r.push(`${slave.slaveName} is ready to be bred, so whenever you feel up to it, you ejaculate onto ${his} ${fertileText}`);
				if (slave.mpreg === 1) {
					r.push(`ass.`);
				} else {
					r.push(`cunt.`);
				...
				if (slave.fuckdoll === 0 && slave.fetish !== Fetish.MINDBROKEN) {
					if (slave.devotion <= 20 && slave.trust >= -20) {
						...
						r.push(`This regimen fills ${him} with <span class="devotion dec">hatred,</span> <span class="trust dec">fear,</span>`);
						if (cycleKnown) {
							r.push(`and eventually, <span class="pregnant">a pregnancy.</span>`);
						} else {
							r.push(`and possibly a pregnancy.`);
						}
						slave.devotion -= 5;
						slave.trust -= 5;
						...
						r.push(`${He} is <span class="devotion inc">absurdly pleased</span> by this treatment, <span class="trust inc">trustingly</span> serving as your cum receptacle until ${he}`);
						if (cycleKnown) {
							r.push(`<span class="pregnant">conceives.</span>`);
						} else {
							r.push(`hopefully conceives.`);
						}
						r.push(`${He}'s so aroused by having you pour in semen that the sensation alone of it creeping into ${his} depths is often enough to bring ${him} to orgasm.`);
						slave.devotion += 5;
						slave.trust += 5;
						...
						r.push(`${He} serves you dutifully in this, <span class="trust inc">trustingly</span> serving as your cum receptacle until ${he}`);
						if (cycleKnown) {
							r.push(`<span class="pregnant">conceives.</span>`);
						} else {
							r.push(`hopefully conceives.`);
				...
				r.push(`${slave.slaveName} is ripe for breeding, so you ejaculate inside ${him} often. When you bore of ${his} ${fertileText}`);
				if (slave.mpreg === 1) {
					r.push(`ass,`);
				} else {
					r.push(`cunt,`);
				...
				if (slave.fuckdoll === 0 && slave.fetish !== Fetish.MINDBROKEN) {
					if (slave.devotion <= 20 && slave.trust >= -20) {
						...
						slave.trust -= 5;
						if (slave.sexualFlaw === SexualFlaw.NONE) {
							r.push(`This unpleasant interlude leaves ${him} <span class="flaw gain">hating penetration</span> of ${his}${cycleKnown ? ` now-pregnant` : ``}`);
							if (slave.mpreg === 1) {
								r.push(`ass.`);
								slave.sexualFlaw = SexualFlaw.HATESANAL;
							} else {
						...
						r.push(`${He} is <span class="devotion inc">absurdly pleased</span> by this treatment, <span class="trust inc">trustingly</span> serving as your breeding bitch until ${he}`);
						if (cycleKnown) {
							r.push(`<span class="pregnant">conceives.</span>`);
						} else {
							r.push(`hopefully conceives.`);
						}
						r.push(`${He}'s so aroused by the constant insemination that having your dick, wet from another slave, pushed inside ${him} to climax is often enough to bring ${him} to orgasm in turn.`);
						slave.devotion += 5;
						slave.trust += 5;
						if (slave.fetishKnown === 1 && slave.fetishStrength <= 95) {
							r.push(`Such total satisfaction of ${his} pregnancy fantasies <span class="fetish inc">strengthens ${his} fetish.</span>`);
						...
						r.push(`${He} serves you dutifully in this, <span class="trust inc">trustingly</span> serving as your breeding bitch until ${he}`);
						if (cycleKnown) {
							r.push(`<span class="pregnant">conceives.</span>`);
						} else {
							r.push(`hopefully conceives.`);
				...
				r.push(`${slave.slaveName} is ripe for breeding, so you spray ejaculate onto ${his} ${fertileText}`);
				if (slave.mpreg === 1) {
					r.push(`ass`);
				} else {
					r.push(`cunt`);
				...
				if (slave.fuckdoll === 0 && slave.fetish !== Fetish.MINDBROKEN) {
					if (slave.devotion <= 20 && slave.trust >= -20) {
						...
						r.push(`up in the air. This regimen fills ${him} with <span class="devotion dec">hatred,</span> <span class="trust dec">fear,</span>`);
						if (cycleKnown) {
							r.push(`and <span class="pregnant">a pregnancy.</span>`);
						} else {
							r.push(`and possibly a pregnancy.`);
						}
						slave.devotion -= 5;
						...
						r.push(`${He}'s so aroused by the constant insemination that having you push up against ${him} to climax is often enough to bring ${him} to orgasm in turn.`);
						slave.devotion += 5;
						slave.trust += 5;
						if (slave.fetishKnown === 1 && slave.fetishStrength <= 95) {
							r.push(`Such total satisfaction of ${his} pregnancy fantasies <span class="fetish inc">strengthens ${his} fetish.</span>`);
			...
			if (slave.HGExclude === 1) {
				r.push(`It's ${S.HeadGirl.slaveName}'s responsibility to impregnate ${fertileText} slaves, but your Head Girl is forbidden from impregnating ${slave.slaveName}.`);
			} else if (V.universalHGImpregnateMasterSuiteToggle === 1 && [Job.MASTERSUITE, Job.CONCUBINE].includes(slave.assignment)) {
				r.push(`It's ${S.HeadGirl.slaveName}'s responsibility to impregnate ${fertileText} slaves, but ${slave.slaveName} is off-limits.`);
			} else if (App.EndWeek.saVars.HGCum === 0) {
				r.push(`It's ${S.HeadGirl.slaveName}'s responsibility to impregnate ${fertileText} slaves, but your Head Girl can only fuck a limited number of slaves enough to ensure impregnation each week.`);
			} else if (!canBreed(slave, S.HeadGirl)) {
				r.push(`It's ${S.HeadGirl.slaveName}'s responsibility to impregnate ${fertileText} slaves, but ${S.HeadGirl.slaveName}'s sperm is unable to fertilize ${slave.slaveName}'s ova, so ${he2} doesn't waste ${his2} seed trying.`);
			} else {
				...
				if (slave.fuckdoll === 0 && slave.fetish !== Fetish.MINDBROKEN) {
					if (slave.career === "a dairy cow" && slave.devotion <= 20) {
						r.push(`${slave.slaveName} feels a need to be bred by the Head Girl, and submits ${himself} to ${his} superior's virile cock until`);
						if (cycleKnown) {
							r.push(`<span class="pregnant">conception</span> is verified.`);
						} else {
							r.push(`conception is likely.`);
						}
					} else if (slave.devotion <= 20 && slave.trust >= -20) {
						if ((S.HeadGirl.fetish === Fetish.SADIST || S.HeadGirl.fetish === Fetish.DOM) && S.HeadGirl.fetishStrength > 60) {
							r.push(`${His2} interest is piqued, however, when ${slave.slaveName} shows signs of resistance. ${S.HeadGirl.slaveName} <span class="devotion inc">enthusiastically</span> <span class="devotion dec">rapes the poor ${girl}</span> pregnant,`);
							r.push(`ejaculating inside ${his2} victim more often than is really necessary for ${cycleKnown ? `<span class="pregnant">conception.</span>` : `possible conception.`}`);
							S.HeadGirl.devotion += 2;
							slave.devotion -= 5;
						} else {
							r.push(`${slave.slaveName} tries to resist ${him2}, so ${S.HeadGirl.slaveName} is forced to <span class="devotion dec">rape the poor ${girl}</span> pregnant, regularly ejaculating inside ${him} until`);
							if (cycleKnown) {
								r.push(`<span class="pregnant">conception</span> is confirmed.`);
							} else {
								r.push(`conception is likely.`);
							}
							slave.devotion -= 4;
						...
						if ((S.HeadGirl.fetish === Fetish.SADIST || S.HeadGirl.fetish === Fetish.DOM) && S.HeadGirl.fetishStrength > 60) {
							r.push(`${His2} interest is piqued, however, when it becomes clear that ${slave.slaveName}, though fearfully obedient, is not happy with being bred. ${S.HeadGirl.slaveName} <span class="devotion inc">enthusiastically</span> ensures that ${his2} victim <span class="devotion dec">does not enjoy</span> a week of being`);
							if (cycleKnown) {
								r.push(`<span class="pregnant">raped pregnant.</span>`);
							} else {
								r.push(`raped.`);
							}
							...
							r.push(`${slave.slaveName}, though fearfully obedient, is not happy with being bred, but ${S.HeadGirl.slaveName} <span class="devotion dec">rapes the poor ${girl}</span>${cycleKnown ? ` pregnant` : ``} anyway, regularly ejaculating inside ${him} until.`);
							if (cycleKnown) {
								r.push(`<span class="pregnant">conception</span> is confirmed.`);
							} else {
								r.push(`conception is likely.`);
						...
						if (slave.fetish === Fetish.PREGNANCY && slave.fetishStrength > 60) {
							...
							r.push(`is <span class="devotion inc">very willing to be bred</span> by your Head Girl, and eagerly takes ${his} superior's cock bareback until.`);
							if (cycleKnown) {
								r.push(`<span class="pregnant">conception</span> is verified.`);
							} else {
								r.push(`conception is likely.`);
							...
							r.push(`${slave.slaveName} is willing to be bred by the Head Girl, and takes ${his} superior's cock bareback until.`);
							if (cycleKnown) {
								r.push(`<span class="pregnant">conception</span> is verified.`);
							} else {
								r.push(`conception is likely.`);
						...
						if (slave.fetish === Fetish.PREGNANCY && slave.fetishStrength > 60) {
							...
							r.push(`considers getting bred by your Head Girl <span class="devotion inc">a dream come true,</span> and gets fucked bareback daily until.`);
							if (cycleKnown) {
								r.push(`<span class="pregnant">conception</span> is verified.`);
							} else {
								r.push(`conception is likely.`);
							...
							r.push(`${slave.slaveName} is <span class="devotion inc">quite willing to be bred</span> by the Head Girl, whom ${he} respects, and submissively takes ${his} superior's cock bareback until.`);
							if (cycleKnown) {
								r.push(`<span class="pregnant">conception</span> is verified.`);
							} else {
								r.push(`conception is likely.`);
			...
			if ((slave.StudExclude === 1 || slave.breedingMark === 1) && (Stud.career !== "a breeding bull" || Stud.fetish !== Fetish.MINDBROKEN || !canMove(Stud))) {
				r.push(`It's ${Stud.slaveName}'s role to provide sperm for ${fertileText} slaves, but ${slave.slaveName} is not included on the list.`);
			} else if (V.universalHGImpregnateMasterSuiteToggle === 1 && [Job.MASTERSUITE, Job.CONCUBINE].includes(slave.assignment)) {
				r.push(`It's ${Stud.slaveName}'s role to provide sperm for ${fertileText} slaves, but ${slave.slaveName} is off-limits.`);
			} else if (App.EndWeek.saVars.StudCum === 0) {
				r.push(`It's ${Stud.slaveName}'s role to provide sperm for ${fertileText} slaves, but your Stud can only cum enough to ensure impregnation in a limited number of slaves each week.`);
			} else if (!canBreed(slave, Stud)) {
				r.push(`It's ${Stud.slaveName}'s role to provide sperm for ${fertileText} slaves, but ${his2} sperm is unable to fertilize ${slave.slaveName}'s ova, so there is no point in forcing the union.`);
			} else {
				...
				if (Stud.fuckdoll > 0) {
					...
					if (Stud.career === "a breeding bull" && canMove(Stud)) {
						r.push(`impregnate ${fertileText} slaves, a task the amorous Stud performs with gusto and little regard for rules.`);
						StudRandiness = 1;
					} else {
				...
				if (slave.fuckdoll === 0 && slave.fetish !== Fetish.MINDBROKEN) {
					if (Stud.fuckdoll > 0) {
						if (slave.career === "a dairy cow" && slave.devotion <= 20) {
							r.push(`${slave.slaveName} <span class="devotion dec">is not amused</span> that ${he} is expected to use a syringe to`);
							if (cycleKnown) {
								r.push(`<span class="pregnant">inseminate ${himself}.</span>`);
							} else {
								r.push(`inseminate ${himself}.`);
							}
							slave.devotion -= 2;
						} else if (slave.devotion <= 20 && slave.trust >= -20) {
							r.push(`${slave.slaveName} <span class="trust dec">is horrified</span> by the state of ${Stud.slaveName} and <span class="devotion dec">quite upset</span> that ${he} is being forced not only to carry ${his2} child, but to milk ${him2} and inject ${his2} cum into ${his} ${pussy} until ${he}`);
							if (cycleKnown) {
								r.push(`<span class="pregnant">conceives.</span>`);
							} else {
								r.push(`potentially conceives.`);
							}
							slave.trust -= 4;
							slave.devotion -= 4;
						} else if (slave.devotion <= 20) {
							r.push(`${slave.slaveName} <span class="trust dec">is utterly horrified</span> by the state of ${Stud.slaveName} and <span class="devotion dec">has to be forced</span> to`);
							if (cycleKnown) {
								r.push(`<span class="pregnant">conceive ${his2} child.</span>`);
							} else {
								r.push(`potentially conceive ${his2} child.`);
							}
							slave.trust -= 10;
							slave.devotion -= 10;
						} else if (slave.devotion <= 75) {
							if (slave.fetish === Fetish.PREGNANCY && slave.fetishKnown === 1 && slave.fetishStrength > 60) {
								r.push(`${slave.slaveName}, a pregnancy fetishist, is <span class="devotion inc">very willing</span> to inject ${his2} cumshots deep into ${his} ${pussy} until ${he}`);
								if (cycleKnown) {
									r.push(`<span class="pregnant">conceives.</span>`);
								} else {
									r.push(`hopefully conceives.`);
								...
								r.push(`${slave.slaveName} is willing to carry your Stud's child and injects ${his2} loads into ${his} ${pussy} until ${he}`);
								if (cycleKnown) {
									r.push(`<span class="pregnant">conceives.</span>`);
								} else {
									r.push(`hopefully conceives.`);
							...
							if (slave.fetish === Fetish.PREGNANCY && slave.fetishKnown === 1 && slave.fetishStrength > 60) {
								r.push(`${slave.slaveName}, a pregnancy fetishist, <span class="devotion inc">eagerly</span> injects cumshot after cumshot into ${his} hungry ${pussy} throughout the week,`);
								if (cycleKnown) {
									r.push(`<span class="pregnant">guaranteeing conception.</span>`);
								} else {
									r.push(`hopefully guaranteeing conception.`);
								}
								slave.devotion += 1;
								satisfiedPregFetish = 1;
								slave.need -= 10;
							} else {
								r.push(`${slave.slaveName} is <span class="devotion inc">quite willing</span> to carry your Stud's child and dutifully injects ${himself} with ${his2} loads until`);
								if (cycleKnown) {
									r.push(`<span class="pregnant">conception</span> is verified.`);
								} else {
									r.push(`conception is likely.`);
						...
						if (slave.career === "a dairy cow" && slave.devotion <= 20) {
							...
							if (StudRandiness === 1) {
								r.push(`but has <span class="devotion dec">trouble coming to terms with</span> being ${cycleKnown ? `<span class="pregnant">inseminated</span>` : `inseminated`} by a mate with no dick.`);
								slave.need -= 10;
								StudVaginal = 1;
							} else {
								r.push(`but between ${his} mate's lack of a dick and having to ${cycleKnown ? `<span class="pregnant">inseminate ${himself}</span>` : `inseminate ${himself}`} with a syringe, the experience <span class="devotion dec">is not very pleasant.</span>`);
							}
							slave.devotion -= 1;
						} else if (slave.devotion <= 20 && slave.trust >= -20) {
							if (StudRandiness === 1) {
								r.push(`${slave.slaveName} tries to resist ${him2}, so ${Stud.slaveName} teaches ${him2} that ${he2} doesn't need a dick <span class="devotion dec">to rape the poor ${girl}</span> pregnant.`);
								r.push(`${He2} regularly ejaculating inside ${him} until`);
								if (cycleKnown) {
									r.push(`<span class="pregnant">conception</span> is confirmed,`);
								} else {
									r.push(`conception is likely,`);
								}
								r.push(`<span class="trust dec">much to ${his} terror.</span>`);
								slave.devotion -= 4;
								slave.trust -= 4;
								StudVaginal = 1;
							} else {
								r.push(`${slave.slaveName} finds ${Stud.slaveName} is in a similar situation as ${he} is, so ${he} tries to make it as <span class="trust inc">mutually enjoyable as possible,</span> despite the awkwardness involved,`);
								r.push(`while trying ${his} best to overlook that ${Stud.slaveName} is being forced to ${cycleKnown ? `<span class="pregnant">impregnate ${him}.</span>` : `impregnate ${him}.`}`);
								slave.trust += 4;
								slave.need = 0;
							}
						} else if (slave.devotion <= 20) {
							if (StudRandiness === 1) {
								r.push(`${slave.slaveName}, though fearfully obedient, is not happy with being bred, so ${Stud.slaveName} is forced to <span class="devotion dec">rape the poor ${girl}</span>${cycleKnown ? ` pregnant` : ``}, regularly squirting cum into ${his} ${pussy} until`);
								if (cycleKnown) {
									r.push(`<span class="pregnant">conception</span> is confirmed.`);
								} else {
									r.push(`conception is likely.`);
								...
								r.push(`${slave.slaveName} finds that ${Stud.slaveName} is less than enthusiastic about fucking ${him}, <span class="devotion dec">making the resulting rape extra unpleasant</span> when ${he2}`);
								if (cycleKnown) {
									r.push(`<span class="pregnant">forces a baby into ${him}.</span>`);
								} else {
									r.push(`attempts to force a baby into ${him}.`);
								}
								slave.devotion -= 6;
							}
						} else if (slave.devotion <= 75) {
							if (slave.fetish === Fetish.PREGNANCY && slave.fetishKnown === 1 && slave.fetishStrength > 60) {
								...
								if (StudRandiness === 1) {
									r.push(`<span class="devotion inc">very willing to be bred</span> by your Stud, but never expected to be ${cycleKnown ? `<span class="pregnant">impregnated via tribbing.</span>` : `impregnated via tribbing.`}`);
									slave.need -= 20;
									StudVaginal = 1;
								} else {
									r.push(`<span class="devotion inc">happy to bear</span> your Stud's child by injecting ${his2} cumshots deep into ${his} ${pussy} until ${he}`);
									if (cycleKnown) {
										r.push(`<span class="pregnant">conceives.</span>.`);
									} else {
										r.push(`hopefully conceives.`);
								...
								if (StudRandiness === 1) {
									r.push(`allows ${him2} to`);
									if (cycleKnown) {
										r.push(`effectively <span class="pregnant">trib ${him} pregnant.</span>`);
									} else {
										r.push(`attempt to trib ${him} pregnant.`);
									}
									slave.need -= 20;
									StudVaginal = 1;
								} else {
									...
									if (cycleKnown) {
										...
										r.push(`until conception is likely.`);
									}
								}
							}
						} else {
							if (slave.fetish === Fetish.PREGNANCY && slave.fetishKnown === 1 && slave.fetishStrength > 60) {
								...
								if (StudRandiness === 1) {
									r.push(`but never expected to be ${cycleKnown ? `<span class="pregnant">impregnated via tribbing.</span>` : `impregnated via tribbing.`}`);
									slave.need -= 20;
									StudVaginal = 1;
								} else {
									r.push(`and injects milked cum into ${his} hungry ${pussy} daily until`);
									if (cycleKnown) {
										r.push(`until <span class="pregnant">conception</span> is verified.`);
									} else {
										r.push(`until conception is likely.`);
								...
								if (StudRandiness === 1) {
									...
									if (cycleKnown) {
										r.push(`<span class="pregnant">conceive a child.</span>`);
									} else {
										r.push(`hopefully conceive a child.`);
									}
									slave.need -= 20;
									StudVaginal = 1;
								} else {
									r.push(`and dutifully injects ${himself} with ${his2} loads until.`);
									if (cycleKnown) {
										r.push(`<span class="pregnant">conception</span> is verified.`);
									} else {
										r.push(`conception is likely.`);
						...
						if (slave.career === "a dairy cow" && slave.devotion <= 20) {
							...
							if (StudRandiness === 1) {
								r.push(`${slave.slaveName} tries to resist ${him2}, so ${Stud.slaveName} just <span class="devotion dec">rapes the poor ${girl}</span>${cycleKnown ? ` pregnant` : ``}, regularly ejaculating inside ${him} until`);
								if (cycleKnown) {
									r.push(`<span class="pregnant">conception</span> is confirmed.`);
								} else {
									r.push(`conception is probable.`);
								}
								slave.devotion -= 4;
								rapeAddsFlaw = 1;
							} else {
								r.push(`${slave.slaveName} finds ${Stud.slaveName} is in a similar situation as ${he} is, so ${he} tries to make it as <span class="trust inc">mutually enjoyable as possible,</span> despite the awkwardness involved, while trying ${his} best to overlook that ${Stud.slaveName} is being forced to`);
								if (cycleKnown) {
									r.push(`<span class="pregnant">impregnate ${him}.</span>`);
								} else {
									r.push(`try to impregnate ${him}.`);
								}
								slave.trust += 4;
								slave.need = 0;
							}
							...
							if (StudRandiness === 1) {
								r.push(`${slave.slaveName}, though fearfully obedient, is not happy with being bred, but ${Stud.slaveName} <span class="devotion dec">rapes the poor ${girl}</span>${cycleKnown ? ` pregnant` : ``} anyway, regularly ejaculating inside ${him} until`);
								if (cycleKnown) {
									r.push(`<span class="pregnant">conception</span> is confirmed.`);
								} else {
									r.push(`conception is probable.`);
							...
							if (slave.fetish === Fetish.PREGNANCY && slave.fetishKnown === 1 && slave.fetishStrength > 60) {
								...
								if (cycleKnown) {
									...
									r.push(`daily until conception is likely.`);
								}
								slave.devotion += 2;
								satisfiedPregFetish = 1;
								slave.need = 0;
						...
						if (slave.career === "a dairy cow" && slave.devotion <= 20) {
							r.push(`${slave.slaveName} feels a need to be bred by your Stud, but try as ${he} might, just can't fit ${his2} massive dick inside ${his} ${pussy}.`);
							r.push(`Having to take just the tip until ${he}`);
							if (cycleKnown) {
								r.push(`<span class="pregnant">conceives</span>`);
							} else {
								r.push(`potentially conceives`);
							}
							r.push(`<span class="devotion dec">isn't satisfying</span> to the ex-dairy cow.`);
							slave.devotion -= 1;
							StudPenetrative = 1;
							StudTakesV = 1;
						} else if (slave.devotion <= 20 && slave.trust >= -20) {
							if (StudRandiness === 1) {
								...
								if (cycleKnown) {
									...
									r.push(`attempt to impregnate ${him}.`);
								}
								slave.trust += 4;
								slave.need = 0;
								StudPenetrative = 1;
						...
						if (slave.career === "a dairy cow" && slave.devotion <= 20) {
							r.push(`${slave.slaveName} feels a need to be bred by your Stud, but try as ${he} might, just can't get ${him2} hard.`);
							r.push(`Being forced to milk ${him2} and inject ${his2} seed into ${himself} until ${he}`);
							if (cycleKnown) {
								r.push(`<span class="pregnant">conceives</span>`);
							} else {
								r.push(`potentially conceives`);
							}
							r.push(`<span class="devotion dec">disappoints</span> the ex-dairy cow.`);
							slave.devotion -= 1;
						} else if (slave.devotion <= 20 && slave.trust >= -20) {
							r.push(`${slave.slaveName} finds ${Stud.slaveName} is in a similar situation as ${he} is, so the two of them work together to make the process <span class="trust inc">as painless as possible,</span> given that ${he} is`);
							if (cycleKnown) {
								r.push(`<span class="pregnant">coming out of this pregnant,</span> one way or the other.`);
							}  else {
								r.push(`probably coming out of this pregnant, one way or the other.`);
							}
							slave.trust += 4;
							Stud.trust += 4;
						} else if (slave.devotion <= 20) {
							r.push(`${Stud.slaveName} shows mercy to the pitiable ${slave.slaveName}, <span class="devotion dec">building upon their mutual dislike of you.</span> ${He2} still has to`);
							if (cycleKnown) {
								r.push(`<span class="pregnant">force ${him} to bear ${his2} child,</span> however.`);
							} else {
								r.push(`try to force ${him} to bear ${his2} child, however.`);
							}
							slave.devotion -= 4;
							Stud.devotion -= 4;
						} else if (slave.devotion <= 75) {
							if (slave.fetish === Fetish.PREGNANCY && slave.fetishKnown === 1 && slave.fetishStrength > 60) {
								r.push(`${slave.slaveName}, a pregnancy fetishist, is <span class="devotion inc">very willing</span> to milk ${him2} and inject ${his} harvest deep into ${his} ${pussy} until ${he}`);
								if (cycleKnown) {
									r.push(`<span class="pregnant">conceives.</span>`);
								} else {
									r.push(`potentially conceives.`);
								...
								r.push(`${slave.slaveName} is willing to carry your Stud's child and milks loads into ${his} ${pussy} until ${he}`);
								if (cycleKnown) {
									r.push(`<span class="pregnant">conceives.</span>`);
								} else {
									r.push(`potentially conceives.`);
							...
							if (slave.fetish === Fetish.PREGNANCY && slave.fetishKnown === 1 && slave.fetishStrength > 60) {
								r.push(`${slave.slaveName}, a pregnancy fetishist, <span class="devotion inc">eagerly</span> milks cumshot after cumshot into ${his} hungry ${pussy} throughout the week,`);
								if (cycleKnown) {
									r.push(`<span class="pregnant">guaranteeing conception.</span>`);
								} else {
									r.push(`possibly conceiving a child.`);
								}
								slave.devotion += 1;
								satisfiedPregFetish = 1;
							} else {
								r.push(`${slave.slaveName} is <span class="devotion inc">quite willing</span> to carry your Stud's child and dutifully milks loads from ${him2} until`);
								if (cycleKnown) {
									r.push(`<span class="pregnant">conception</span> is verified.`);
								} else {
									r.push(`conception is likely.`);
			...
			const potentialSlaveFathers = getSlaves().filter(s => s.ID !== slave.ID && canBreed(slave, s) && ![Job.AGENT, Job.AGENTPARTNER].includes(s.assignment));
			if (potentialSlaveFathers.size === 0) {
				...
				if (potentialSlaveFathers.size > 1) {
					r.push(`You have no way of knowing which slave ${he}`);
					if (cycleKnown) {
						r.push(`<span class="pregnant">conceives</span> from, so it's a mystery until the father can be verified.`);
					} else {
						r.push(`will conceive from, so it's a mystery until the pregnancy and father can be verified.`);
					}
				} else {
					r.push(`You have a fairly safe idea of which slave ${he} `);
					if (cycleKnown) {
						r.push(`<span class="pregnant">conceived</span> from, given there was only one donor.`);
					} else {
						r.push(`might conceive from, given there was only one donor.`);
					}
				}
				...
				const dadHash = potentialSlaveFathers.asArray().reduce((acc, cur, i) => Object.assign(acc, {[i]: score(cur)}), {});
				const rouletteWinner = hashChoice(dadHash);
				if (rouletteWinner && isFertile(slave)) {
			...
			if (slave.fuckdoll > 0) {
				r.push(`positioned on a cot in the square outside the penthouse for citizens to fuck ${his} ${fertileText}`);
				if (slave.mpreg === 1) {
					r.push(`rear`);
				} else {
					r.push(`pussy`);
				}
				r.push(`until ${his} suit alerts you that that ${he} has likely ${cycleKnown ? `<span class="pregnant">conceived.</span>` : `conceived.`}`);
				...
				r.push(`until ${he} is ${cycleKnown ? `<span class="pregnant">thoroughly bred.</span>` : `thoroughly bred.`}`);
			} else if (slave.devotion <= 20 && slave.trust >= -20) {
				r.push(`chained out in the square for citizens to avail themselves to ${his} ${fertileText}`);
				if (slave.mpreg === 1) {
					r.push(`anus.`);
				} else {
					r.push(`pussy.`);
				}
				r.push(`Being reduced to a public cumdump fills ${him} with <span class="devotion dec">hatred,</span> and <span class="trust dec">fear,</span>`);
				r.push(`and ${cycleKnown ? `<span class="pregnant">pregnant</span>` : `possibly pregnant`} with some unknown man's spawn.`);
				slave.devotion -= 7;
				slave.trust -= 7;
			} else if (slave.devotion <= 20) {
				r.push(`shackled out in the square for citizens to avail themselves to ${his} ${fertileText}`);
				if (slave.mpreg === 1) {
					r.push(`anus.`);
				} else {
					r.push(`pussy.`);
				}
				r.push(`${He} complies out of fear, allowing man after man to cum inside ${him} until`);
				if (cycleKnown) {
					r.push(`<span class="pregnant">pregnancy is assured.</span>`);
				} else {
					r.push(`pregnancy is likely.`);
				}
			} else if (slave.fetish === Fetish.PREGNANCY && slave.fetishStrength > 60) {
				r.push(`<span class="trust inc">permitted</span> outside the penthouse to <span class="devotion inc">gleefully</span> offer ${his} ${fertileText}`);
				if (slave.mpreg === 1) {
					r.push(`rear`);
				} else {
					r.push(`pussy`);
				}
				r.push(`to passersby until ${he} feels ${he} is thoroughly`);
				if (cycleKnown) {
					r.push(`<span class="pregnant">impregnated.</span>`);
				} else {
					r.push(`inseminated.`);
				}
				r.push(`${He}'s so aroused by the erection selection that ${he} often climaxes just by being penetrated.`);
				slave.devotion += 5;
				slave.trust += 5;
				if (slave.fetishKnown === 1 && slave.fetishStrength <= 95) {
					r.push(`Such total satisfaction of ${his} pregnancy fantasies <span class="fetish inc">strengthens ${his} fetish.</span>`);
				...
				r.push(`ordered to leave the penthouse and offer ${his} ${fertileText}`);
				if (slave.mpreg === 1) {
					r.push(`rear`);
				} else {
					r.push(`pussy`);
				}
				r.push(`to citizens until ${he} has taken enough dicks to`);
				if (cycleKnown) {
					r.push(`<span class="pregnant">conceive.</span>`);
				} else {
					r.push(`likely conceive.`);
				}
				if (!isVirile(V.PC)) {
				...
				r.push(`allowed to leave the penthouse to offer ${his} ${fertileText}`);
				if (slave.mpreg === 1) {
					r.push(`rear`);
				} else {
					r.push(`pussy`);
		...
		} else if (impregEvent && isFertile(slave)) {
			...
			switch (slave.assignment) {
				...
				case Job.CELLBLOCK:
					...
					break;
				case Job.CONCUBINE:
					if (slave.fuckdoll === 0 && slave.fetish !== Fetish.MINDBROKEN && canImpreg(slave, V.PC, !fertKnown(slave))) {
						const pregKnown = slave.pregKnown;
						r.push(`As your concubine, ${he} takes care to only share ${his} ${fertileText}`);
						if (slave.mpreg === 1) {
							r.push(`ass`);
						} else {
							r.push(`pussy`);
						...
						r.push(`with you.`);
						tryKnockMeUp(slave, 100, 2, V.PC);
						if (!pregKnown && slave.pregKnown) { // Discovered pregnancy
							r.push(`${His} efforts paid off; <span class="pregnant">${he} has become pregnant with your child.</span>`);
						}
					}
					break;
				case Job.MASTERSUITE:
					if (pcCanKnockUpSlave(slave)) {
						...
						const msSlaves = getSlaves().filter((s) => s.assignment === Job.MASTERSUITE).asArray().shuffle();
						for (const msSlave of msSlaves) {
							if (canImpreg(slave, msSlave, !fertKnown(slave)) && App.Utils.sexAllowed(slave, msSlave)) {
								...
								const pregKnown = slave.pregKnown;
								tryKnockMeUp(slave, 100, 2, msSlave);
								if (!pregKnown && slave.pregKnown) { // discovered pregnancy
				...
				case Job.GLORYHOLE:
				case Job.ARCADE:
					if (slave.eggType === "human" && isFertile(slave)) {
						const pregKnown = slave.pregKnown;
						knockMeUp(slave, 100, 2, -2);
						if (!pregKnown && slave.pregKnown) { // Discovered pregnancy
				...
				default: /* random impregnation chance on other assignments - consider relationships first */
					if (App.Utils.hasNonassignmentSex(slave) || slave.devotion <= 50) {
						...
						if (conceptionSeed > 80 && ((slave.counter.vaginal > 0) || (slave.counter.anal > 0 && slave.mpreg > 0))) { /* TODO: compare to previous week totals? */
							...
							if (((slave.relationship === -3) || (slave.relationship === -2)) && canImpreg(slave, V.PC, fertKnown(slave)) && App.Utils.sexAllowed(slave, V.PC)) {
								pregSource = SID.PC;
							} else if (tempLover && canImpreg(slave, tempLover, !fertKnown(slave))) { /* erection not needed for impregnation via consensual sex play (FWB or better) */
							...
							} else if (tempSub && canAchieveErection(tempSub) && canImpreg(slave, tempSub, !fertKnown(slave))) { /* subordinate must have erection to impregnate target */
								pregSource = tempSub.ID;
							} else if (random(1, 100) > 95 && tempRival && canAchieveErection(tempRival) && canImpreg(slave, tempRival, !fertKnown(slave))) { /* 5% chance to be raped and knocked up by bitter rival - erection needed */
								if (App.Utils.sexAllowed(slave, tempRival)) {
									r.push(`Driven by the bitter rivalry between them, ${tempRival.slaveName} successfully manages to overpower ${slave.slaveName} and rape ${him}, cumming deep in ${his} ${fertileText}`);
									if (slave.mpreg === 1) {
										r.push(`ass.`);
									} else {
										r.push(`pussy.`);
									...
									r.push(`Driven by the bitter rivalry between them, ${tempRival.slaveName} <span class="trust dec">breaks the rules,</span> successfully overpowering ${slave.slaveName} and raping ${him}, cumming deep in ${his} ${fertileText}`);
									if (slave.mpreg === 1) {
										r.push(`ass.`);
									} else {
										r.push(`pussy.`);
							...
							} else if (random(1, 100) > 60 && (canImpreg(slave, V.PC, !fertKnown(slave)) || canFemImpreg(slave, V.PC, !fertKnown(slave))) && App.Utils.sexAllowed(slave, V.PC)) { /* still 40% chance of impregnation by PC */
								pregSource = SID.PC;
								...
								pregSource = SID.CITIZEN;
							} else {
								const potentialFathers = getSlaves().filter(s => canImpreg(slave, s, !fertKnown(slave))).asArray().shuffle();
						...
						if (pregSource !== SID.GENERIC && slave.eggType === "human" && isFertile(slave)) {
							const pregKnown = slave.pregKnown;
					...
					if (canGetPregnant(slave) && canWalk(slave) && isSlaveAvailable(slave) && (V.personalAttention.task !== PersonalAttention.MAID || onBedRest(V.PC))) {
						...
						let slobs = getSlaves().filter(s => canFemImpreg(slave, s) && isSlaveAvailable(s) && s.geneMods.aggressiveSperm === 1 && (s.fetish === Fetish.MINDBROKEN || s.energy > 95 || (s.devotion < -20 && s.trust > 20) || (s.intelligence + s.intelligenceImplant < -10)));
						if (slobs.size > (totalServantCapacity() / 5)) {
							tryKnockMeUp(slave, -50, 2, slobs.randomValue());
						}
					}
