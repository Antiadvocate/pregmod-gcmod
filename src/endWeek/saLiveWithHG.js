...
App.SlaveAssignment.liveWithHG = function saliveWithHG(slave) {
	...
	function HGSetsDiet(slave, HG) {
		const t = [];
		if ((arcology.FSRepopulationFocusPregPolicy === 1 || (!arcologyInfo.fsActive('FSRestart') && HG.fetish === Fetish.PREGNANCY)) && canImpreg(slave, HG, true) && V.dietFertility === 1) {
	...
	function HGSetsDrugs(slave, HG) {
		...
		if (V.HGSuiteDrugs !== 0) {
			...
			if (slave.indentureRestrictions < 2) {
				...
				} else if (canImpreg(slave, HG, true) && !arcologyInfo.fsActive('FSRestart') && (getHGTastes() === 4 || HG.fetish === Fetish.PREGNANCY)) {
				...
				} else if (arcology.FSRepopulationFocusPregPolicy === 1 && canImpreg(slave, HG, true)) {
	...
	function slaveHGRelations(slave, HG) {
		if (HG.rivalry !== 0 && HG.rivalryTarget === slave.ID) {
			...
			if (HG.rules.relationship !== 'restrictive' && slave.rules.relationship !== 'restrictive') {
			...
			if (HG.relationshipTarget === slave.ID) {
				if (areRelated(HG, slave) && V.seeIncest === 0) {
					...
					if (HG.rules.relationship === 'permissive' && slave.rules.relationship === 'permissive') {
						if (slave.relationship <= 2) {
	...
	function HGManagesPregnancy(slave, HG) {
		if (HG.fetish === Fetish.PREGNANCY && canImpreg(slave, HG, !fertKnown(slave)) && slave.pregKnown === 0) {
			if (!FutureSocieties.isActive('FSRestart', arcology)) {
				r.push(`${HG.slaveName} promptly ${fertKnown(slave) ? `<span class="pregnant">impregnates</span>` : `attempts to impregnate`} `);
				if (HG.fetishKnown === 1) {
				...
				tryKnockMeUp(slave, 100, 2, HG);
				if (HG.fetishStrength > 70 && canImpreg(HG, slave, !fertKnown(HG))) {
					r.push(`Unsurprisingly, ${he2} gives in to ${his2} own cravings and also takes ${slave.slaveName}'s loads until ${he2} ${fertKnown(HG) ? `<span class="pregnant">gets pregnant</span> too` : `is possibly pregnant as well`}.`);
					tryKnockMeUp(HG, 100, 2, slave);
				}
			} else if (HG.fetishKnown === 1) {
				r.push(`${HG.slaveName} knows better than to even consider knocking up ${slave.slaveName}.`);
			}
		} else if (HG.fetish === Fetish.PREGNANCY && canImpreg(HG, slave, !fertKnown(HG)) && HG.pregKnown === 0) {
			if (!FutureSocieties.isActive('FSRestart', arcology) && HG.fetishStrength > 70) {
				r.push(`${HG.slaveName} promptly ${fertKnown(HG) ? `<span class="pregnant">knocks ${himself2} up</span>` : `attempts to knock ${himself2} up`} with ${slave.slaveName}'s`);
				if (HG.fetishKnown === 1) {
					r.push(`seed, to your Head Girl's considerable <span class="devotion inc">satisfaction.</span>`);
					HG.devotion += 4;
				} else {
					r.push(`seed. ${His2} <span class="devotion inc">pride</span> over ${his2} eagerness to get pregnant completely exposes ${his2} hidden, and powerful, <span class="fetish gain">pregnancy fetish.</span>`);
					HG.fetishKnown = 1;
					HG.devotion += 4;
				}
				tryKnockMeUp(HG, 100, 2, slave);
		...
		} else if (arcology.FSRepopulationFocusPregPolicy === 1 && canImpreg(slave, HG, !fertKnown(slave)) && slave.pregKnown === 0) {
			r.push(`${HG.slaveName} wastes no time in chasing the current trend and hurries to ${fertKnown(slave) ? ` <span class="pregnant">impregnate</span>` : `try to impregnate`} ${slave.slaveName}.`);
			tryKnockMeUp(slave, 100, 2, HG);
	...
	function HGSexualSatiation(slave, HG) {
		...
		if (cervixPump > 0) {
			r.push(`${slave.slaveName} notices ${his}`);
			if (slave.cervixImplantTarget === "all") {
				r.push(`<span class="change positive">body swelling</span>`);
			} else if (slave.cervixImplantTarget === "TnA") {
				r.push(`<span class="change positive">tits and ass grow</span>`);
			} else if (slave.cervixImplantTarget === "mombod") {
				r.push(`<span class="change positive">belly and breasts swell</span>`);
			} else if (slave.cervixImplantTarget === "bottomHeavy") {
				r.push(`<span class="change positive">butt and belly growing</span>`);
			} else if (slave.cervixImplantTarget === "boobsImplant") {
				r.push(`<span class="change positive">boobs grow</span>`);
			} else if (slave.cervixImplantTarget === "buttImplant") {
				r.push(`<span class="change positive">butt grow</span>`);
			} else {
				r.push(`<span class="change positive">belly swell</span>`);
			}
			r.push(`after`);
	...
	function slaveReport(slave, el) {
		if (V.showEWD !== 0 || (V.favSeparateReport === 1 && V.favorites.includes(slave.ID))) {
			...
			App.SlaveAssignment.PartTime.saPartTime(slave);
			/* App.SlaveAssignment.choosesOwnClothes(slave) */
