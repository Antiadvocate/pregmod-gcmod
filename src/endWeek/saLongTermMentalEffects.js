		...
		switch (slave.sexualFlaw) {
			...
			case SexualFlaw.BREEDER:
				...
				if (slave.broodmother > 0 && slave.pregKnown === 1) {
					...
					slave.paraphiliaSatisfied = 1;
				} else if (canGetPregnant(slave, !fertKnown(slave)) && V.PC.dick !== 0 && slave.toyHole === ToyHole.PUSSY && slave.mpreg !== 1 && [Job.CONCUBINE, Job.FUCKTOY, Job.MASTERSUITE].includes(slave.assignment)) {
					r.push(`${His} paraphilia is satisfied by constant unprotected vaginal sex with you.`);
					slave.paraphiliaSatisfied = 1;
				} else if (canGetPregnant(slave, !fertKnown(slave)) && V.PC.dick !== 0 && slave.toyHole === ToyHole.ASS && slave.mpreg === 1 && [Job.CONCUBINE, Job.FUCKTOY, Job.MASTERSUITE].includes(slave.assignment)) {
		...
		switch (slave.career) {
			...
			case "a bioreactor":
				if (V.dairyRestraintsSetting < 2 || slave.assignment !== Job.DAIRY && slave.assignment !== Job.MILKED) {
					...
					if (isFertile(slave, true) && (V.dairyPregUpgrade > 0)) {
						r.push(`baby-filled,`);
					}
					if (slave.balls !== 0) {
				...
				case Fetish.PREGNANCY:
					if (slave.sexualFlaw !== SexualFlaw.BREEDER) {
						if (slave.preg >= slave.pregData.normalBirth * .75 && slave.geneticQuirks.uterineHypersensitivity === 2) {
							...
							slave.fetishStrength = 100;
							FSApproves = 1;
						} else if (slave.bellyPreg > 100 && slave.pregKnown === 1 && slave.pregSource === SID.PC && slave.breedingMark === 1 && V.propOutcome === 1 && slave.devotion > 75) {
