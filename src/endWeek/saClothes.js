				...
				switch (slave.clothes) {
					...
					case "slave rags":
						if (slave.sexualFlaw === SexualFlaw.SELFHATING) {
							r.push(`Since ${he} has no value as a person, ${he} is <span class="devotion inc">happy</span> to wear clothing equally as worthless.`);
							slave.devotion += 2;
						} else if (slave.fetish === Fetish.HUMILIATION) {
							if (slave.fetishKnown === 0) {
								r.push(`${He} pretends to be embarrassed over wearing old rags but seems to get off on it. <span class="fetish gain">${He}'s into humiliation.</span>`);
								slave.fetishKnown = 1;
							} else if (slave.fetishStrength > 60) {
								r.push(`${He} pretends to be embarrassed over wearing old rags but <span class="devotion inc">openly gets off on it.</span>`);
								slave.devotion += 2;
							} else {
								r.push(`${He} pretends to be embarrassed over wearing old rags but <span class="devotion inc">secretly gets off on it.</span>`);
								slave.devotion += 1;
							}
						} else if (slave.devotion < -20) {
							r.push(`Being forced to wear nothing but old discarded rags serves as a constant reminder of <span class="trust dec">how little value</span> ${his} life holds.`);
							slave.trust -= 1;
						}
						break;
					default:
			...
			if (slave.weight > 130) {
				...
				if (slave.fuckdoll === 0 && slave.fetish !== Fetish.MINDBROKEN) {
					if (slave.sexualFlaw === SexualFlaw.BREEDER) {
						...
						slave.devotion -= 1;
					} else if (slave.vagina === 0 && isFertile(slave, true) && !FutureSocieties.isActive('FSRepopulationFocus', arcology) && slave.devotion <= 20) {
