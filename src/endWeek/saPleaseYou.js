...
App.SlaveAssignment.pleaseYou = function saPleaseYou(slave) {
	...
	function useVagina(slave) {
		...
		if (slave.fuckdoll === 0) {
			...
			if (nullPC || PCNoTool) {
				...
				if (V.PC.vagina >= 0 && V.PC.clit < 3 && V.PC.dick === 0) {
	...
	function useAllHoles(slave) {
		...
		if (slave.fuckdoll > 0) {
			...
			if (showExcessReport && slave.sexualQuirk !== SexualQuirk.GAGFUCK) {
	...
	function physicalEffects(slave) {
		...
		if (cervixPump > 0) {
			if (slave.fuckdoll === 0) {
				if (slave.fetish === Fetish.MINDBROKEN) {
					r.push(`${He} is completely oblivious to ${his}`);
					if (slave.cervixImplantTarget === "all") {
						r.push(`<span class="change positive">increasingly bulbous body</span>`);
					} else if (slave.cervixImplantTarget === "TnA") {
						r.push(`<span class="change positive">more shapely assets</span>`);
					} else if (slave.cervixImplantTarget === "mombod") {
						r.push(`<span class="change positive">rapidly progressing faux pregnancy</span>`);
					} else if (slave.cervixImplantTarget === "bottomHeavy") {
						r.push(`<span class="change positive">increasingly bloated lower body</span>`);
					} else if (slave.cervixImplantTarget === "boobsImplant") {
						r.push(`<span class="change positive">expanding breasts</span>`);
					} else if (slave.cervixImplantTarget === "buttImplant") {
						r.push(`<span class="change positive">increasingly expansive rear</span>`);
					} else {
						r.push(`<span class="change positive">increasingly swollen belly</span>`);
					}
					r.push(`and stands no chance of linking it to`);
				} else {
					r.push(`${He} notices ${his}`);
					if (slave.cervixImplantTarget === "all") {
						r.push(`<span class="change positive">body has swollen</span>`);
					} else if (slave.cervixImplantTarget === "TnA") {
						r.push(`<span class="change positive">tits and ass have expanded</span>`);
					} else if (slave.cervixImplantTarget === "mombod") {
						r.push(`<span class="change positive">belly and breasts have swollen</span>`);
					} else if (slave.cervixImplantTarget === "bottomHeavy") {
						r.push(`<span class="change positive">butt and belly have grown</span>`);
					} else if (slave.cervixImplantTarget === "boobsImplant") {
						r.push(`<span class="change positive">boobs have grown</span>`);
					} else if (slave.cervixImplantTarget === "buttImplant") {
						r.push(`<span class="change positive">butt has grown</span>`);
					} else {
						r.push(`<span class="change positive">belly has swollen</span>`);
					}
					r.push(`after`);
				}
				r.push(`all the`);
				...
				r.push(`With each deposit of your seed into ${him}, ${his}`);
				if (slave.cervixImplantTarget === "all") {
					r.push(`<span class="change positive">body grows ever more bulbous.</span>`);
				} else if (slave.cervixImplantTarget === "TnA") {
					r.push(`<span class="change positive">assets grow a little larger.</span>`);
				} else if (slave.cervixImplantTarget === "mombod") {
					r.push(`<span class="change positive">faux pregnancy progresses a little more.</span>`);
				} else if (slave.cervixImplantTarget === "bottomHeavy") {
					r.push(`<span class="change positive">belly and butt swell a little larger.</span>`);
				} else if (slave.cervixImplantTarget === "boobsImplant") {
					r.push(`<span class="change positive">chest swells a little larger.</span>`);
				} else if (slave.cervixImplantTarget === "buttImplant") {
					r.push(`<span class="change positive">rear swells a little larger.</span>`);
				} else {
					r.push(`<span class="change positive">stomach swells a little larger.</span>`);
				}
			}
	...
	function familyBonus(slave) {
		...
		const fucktoys = getSlaves().filter(s => isFucktoy(s));
		...
		if (children.size > 1) {
			const childPronouns = getPronouns(children.atIndex(0).value);
			...
			const childPronouns = getPronouns(children.atIndex(0).value);
			r.push(`Since you are also keeping ${his} ${childPronouns.daughter}, ${children.atIndex(0).value.slaveName}, as a sexual servant, you often use them together, which leaves quite a public impression.`);
		}

		const siblings = fucktoys.filter(s => areSisters(slave, s) > 0);
		if (siblings.size > 1) {
			const sibPronouns = getPronouns(siblings.atIndex(0).value);
			...
			const sibPronouns = getPronouns(siblings.atIndex(0).value);
			r.push(`Since you are also keeping ${his} ${sibPronouns.sister}, ${siblings.atIndex(0).value.slaveName}, as a sexual servant, you often use them together, which leaves quite a public impression.`);
		}
