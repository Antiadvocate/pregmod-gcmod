{
	...
	App.SlaveAssignment.getMilked = function saGetMilked(slave, multiplier = 1.0, preview = false) {
		...
		let implantEffect;
		// if milkmaid is impregnating during prostate stim. Prevents doubling up on sex.
		let prostateStimImpreg = false;
		...
		if ([Job.ARCADE, Job.MILKED, Job.DAIRY].includes(slave.assignment) || preview) {
			if (slave.balls > 0 && !App.Facilities.Dairy.cumMilkingBlocked(slave)) {
				harvestCum(slave, multiplier);
				if (!preview) {
					cumEffects(slave);
				}
			}
			if (App.Facilities.Dairy.canMilkGirlcum(slave)) {
		...
		function harvestCum(slave, multiplier) {
			...
			// Text and effects are in cumEffects
			if (slave.assignment === Job.DAIRY) {
				const milkmaidSodomizes = App.Facilities.Dairy.milkmaidWillSodomize(slave);
		...
		function cumEffects(slave) {
			if (slave.energy > 95 && slave.dick > 0) {
			...
			if (slave.need > 0) {
				r.text += ` ${His} ${slave.dick > 0 ? `cock and balls` : `balls`} are milked so thoroughly that ${he}'s involuntarily sexually sated, regardless of ${his} feelings and tastes.`;
				slave.need = 0;
			}
			...
			if (slave.assignment === Job.DAIRY) {
				const milkmaidSodomizes = App.Facilities.Dairy.milkmaidWillSodomize(slave);
				if (V.dairyStimulatorsUpgrade === 1 && V.dairyStimulatorsSetting > 0 && milkmaidSodomizes) {
					r.text += ` Although unnecessary, ${S.Milkmaid.slaveName} sometimes stands in for the sodomizer, fucking ${slave.slaveName}'s ass to help ${him} cum.`;
					// We assume sodomizer will take the slave's virginity, so no check here.
					const sexCount = isHorny(S.Milkmaid) ? jsRandom(8, 12) : jsRandom(1, Math.trunc(S.Milkmaid.energy / 10));
					seX(slave, 'anal', S.Milkmaid, 'penetrative', sexCount);
					if (slave.mpreg === 1 && App.Facilities.Dairy.milkmaidWillImpreg(slave)) {
						prostateStimImpreg = true;
						r.text += ` ${S.Milkmaid.slaveName} spends some extra time after each prostate milking to fill ${his} ${fertKnown(slave) ? `fertile` : `potentially fertile`} anal womb.`;
					}
				}
				if (V.dairyStimulatorsSetting === 0 && milkmaidSodomizes) {
					r.text += ` ${S.Milkmaid.slaveName} sometimes fucks ${slave.slaveName}'s ass to help ${him} cum.`;
					if (slave.anus === 0) {
						r.text += ` This takes ${his} <span class="virginity loss">anal virginity</span>.`;
						slave.anus++;
					}
					seX(slave, 'anal', S.Milkmaid, 'penetrative', jsRandom(8, 12));
					if (slave.mpreg === 1 && App.Facilities.Dairy.milkmaidWillImpreg(slave)) {
						r.text += ` ${S.Milkmaid.slaveName} spends some extra time after each prostate milking to fill ${his} ${fertKnown(slave) ? `fertile` : `potentially fertile`} anal womb.`;
					}
				}
			...
			if (!canAchieveErection(slave) && canDoAnal(slave)) {
				if (slave.dick === 0) {
					r.text += ` Since ${he} is lacking a penis,`;
				} else {
					r.text += ` Since ${he} cannot maintain an erection,`;
				}
				r.text += ` ${he} requires <span class="trust dec">painful</span> and <span class="devotion dec">degrading</span> anal electrostimulation to produce.`;
				slave.devotion -= 2;
				slave.trust -= 2;
				if (slave.anus === 0) {
			...
			} else if (slave.devotion <= 20 && !isHorny(slave) && canDoAnal(slave)) {
		...
		function pregnancy(slave) {
			if (slave.assignment === Job.DAIRY) {
				if (prostateStimImpreg) { // This conditional prevents doubling up on anal impregnation
					App.EndWeek.saVars.addPregEvent(slave, V.MilkmaidID, `Due to ${S.Milkmaid.slaveName} cumming in ${his} ass, <span class="pregnant">${he} has become pregnant.</span>`);
				} else if (App.Facilities.Dairy.milkmaidWillImpreg(slave)) {
					r.text += ` ${S.Milkmaid.slaveName} keeps ${his} ${fertKnown(slave) ? `fertile` : `potentially fertile`} ${slave.mpreg === 1 ? `anal womb` : `womb`} full of cum.`;
					if (slave.mpreg === 1 && slave.anus === 0) {
						r.text += ` This breaks in ${his} <span class="virginity loss">virgin anus.</span>`;
						slave.anus++;
					}
					if (slave.mpreg === 0 && slave.vagina === 0) {
						r.text += ` This breaks in ${his} <span class="virginity loss">virgin pussy.</span>`;
						slave.vagina++;
					}
					seX(slave, slave.mpreg ? 'anal' : 'vaginal', S.Milkmaid, 'penetrative', jsRandom(8, 12));
					App.EndWeek.saVars.addPregEvent(slave, V.MilkmaidID, `Due to ${S.Milkmaid.slaveName} inseminating ${his} ${slave.mpreg === 1 ? `ass` : `vagina`}, <span class="pregnant">${he} has become pregnant.</span>`);
				}
			}
		}
