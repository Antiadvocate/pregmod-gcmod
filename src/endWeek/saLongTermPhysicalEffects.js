...
App.SlaveAssignment.longTermPhysicalEffects = function saLongTermPhysicalEffects(slave) {
	...
	function sexualSatisfaction(slave) {
		...
		if (App.Utils.hasPartnerSex(slave)) {
			...
			if (sexPartner && isSlaveAvailable(slave) && isSlaveAvailable(sexPartner)) {
				// sexy time
				if (slave.relationship === 5 && canPenetrate(sexPartner)) {
					let virginityText;
					if (slave.vagina === 0 && canDoVaginal(slave)) {
						seX(slave, "vaginal", sexPartner, "penetrative");
						slave.vagina++;
						if (canImpreg(slave, sexPartner)) {
							knockMeUp(slave, 50, 0, sexPartner.ID);
						}
						virginityText = `${He} <span class="virginity loss">gives ${his} virginity</span> to ${his} ${relationshipTerm(slave)} ${sexPartner.slaveName}.`;
					}
					if (slave.anus === 0 && canDoAnal(slave)) {
						seX(slave, "anal", sexPartner, "penetrative");
						slave.anus++;
						if (canImpreg(slave, sexPartner)) {
							knockMeUp(slave, 50, 1, sexPartner.ID);
						}
						if (virginityText) { // already lost vaginal
							virginityText = `${He} allows ${his} ${relationshipTerm(slave)} ${sexPartner.slaveName} to take <span class="virginity loss">both of ${his} virgin holes.</span>`;
						} else {
							virginityText = `${He} <span class="virginity loss">gives ${his} virgin anus</span> to ${his} ${relationshipTerm(slave)} ${sexPartner.slaveName}.`;
						}
					}
					if (virginityText) {
						r.push(virginityText);
					}
				}
				fuckCount = random(Math.min(4, Math.ceil(slave.energy / 20)), 5);
				slave.need = 0;
				SimpleSexAct.Slaves(slave, sexPartner, fuckCount);
			}
		}
	...
	function boobsEffects(slave) {
		...
		if (slave.lactation === 1) {
			...
			if (slave.lactationDuration === 0) {
				if (S.Wardeness && slave.assignment === Job.CELLBLOCK && ([SexualFlaw.MALICIOUS, SexualFlaw.ABUSIVE, SexualFlaw.BREASTEXP].includes(S.Wardeness.sexualFlaw))) {
	...
	function bellyEffects(slave) {
		...
		if (slave.cervixImplant >= 1) {
			...
			if (slave.cervixImplantTubing) {
				if (slave.boobsImplantType === "hyper fillable" && slave.boobsImplant >= 50500) {
					r.push(`${His} cervix implant detected that ${his} breast implants are over capacity, and <span class="change negative">pumps filler out</span> several times during week in an effort to maintain their integrity.`);
					slave.boobsImplant -= 1000;
					slave.boobs -= 1000;
				} else if (slave.boobsImplantType === "advanced fillable" && slave.boobsImplant >= 10500) {
					r.push(`${His} cervix implant detected that ${his} breast implants are at risk of rupturing, and <span class="change negative">pumps filler out</span> several times during week in an effort to reduce the pressure.`);
					slave.boobsImplant -= 1000;
					slave.boobs -= 1000;
				} else if (slave.boobsImplantType === "fillable" && slave.boobsImplant >= 2050) {
					r.push(`${His} cervix implant detected that ${his} breast implants are at risk of rupturing, and <span class="change negative">pumps filler out</span> several times during week in an effort to reduce the pressure.`);
			...
			if (slave.chem > 10) {
				if (random(1, 200) < slave.chem + slave.physicalAge - slave.health.condition - (5 * slave.curatives)) {
					...
					if (V.arcologies[0].FSBodyPuristLaw === 1) {
						r.push(`Your society's focus on preventing things like this is starting to have an effect, but long term damage like what ${he} has suffered takes time to resolve itself.`);
					} else if (V.healthyDrugsUpgrade === 1) {
						r.push(`Your effort to provide drug treatments free of negative side effects is starting to show results, but long term damage like what ${he} has suffered takes time to resolve itself.`);
					}
				}
			}
		}
