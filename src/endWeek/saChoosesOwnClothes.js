		...
		if (slave.fetish === Fetish.MINDBROKEN) {
			...
			if (slave.assignment === Job.NURSE) { /* Chooses clothes according to assignment (no exceptions)*/
				...
				if (slave.assignment === Job.RECRUITER) {
					...
					if (isItemAccessible.entry("attractive lingerie for a pregnant woman") === true && slave.belly >= 10000 && slave.pregSource === SID.PC) {
				...
				if (V.arcologies[0].FSSlaveProfessionalism > 0) {
					...
					wardrobeFS.push({text: `and ties on the skimpiest bikini ${he} can find.`, clothes: "a nipple bikini"});
				...
				if (slave.sexualFlaw === SexualFlaw.SHAMEFAST) {
					...
					if (isItemAccessible.entry("a slutty magical girl costume") === true) {
						wardrobeTastes.push({text: `and throws on a slutty magical girl costume in the hopes of being "defeated" and getting taken advantage of.`, clothes: "a slutty magical girl costume"});
					}
				} else if (slave.sexualFlaw === SexualFlaw.SELFHATING) {
					wardrobeTastes.push({text: `but chooses to wear some discarded rags instead; clothing shouldn't be wasted on trash like ${him}.`, clothes: "slave rags"});
				...
				if (slave.fetishKnown === 1) {
					if (slave.fetish === Fetish.SUBMISSIVE) {
						...
						if (isItemAccessible.entry("a maternity dress") === true && canGetPregnant(slave, !fertKnown(slave))) {
	...
	function todaysCollar(slave) {
		...
		if (slave.fetish === Fetish.MINDBROKEN) {
			...
			if (V.arcologies[0].FSRepopulationFocus > 0 && slave.pregKnown === 1) {
				...
				} else if (slave.fetish === Fetish.PREGNANCY && (canGetPregnant(slave, true, true) || slave.pregKnown === 1)) {
