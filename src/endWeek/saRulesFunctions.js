	...
	if (slave.fetishKnown === 1 && slave.fetishStrength > 60) {
		switch (slave.fetish) {
			...
			case Fetish.PREGNANCY:
				if (isFertile(slave, !fertKnown(slave)) && !isMenstruating(slave) && V.PC.dick !== 0) {
...
App.EndWeek.Rules.playerDrugEffects = function(slave) {
	...
	if (slave.drugs === Drug.SUPERFERTILITY && canImpreg(slave, V.PC)) { // Check is fine here for fert known as this represents slave in heat
		if (slave.devotion > 20 || slave.trust < -20) {
...
App.EndWeek.Rules.masturbationDrugEffects = function(slave) {
	...
	if (slave.drugs === Drug.SUPERFERTILITY && canGetPregnant(slave)) { // Preg check here is fine, as it represents slave in heat.
...
App.EndWeek.Rules.partnerDrugEffects = function(slave) {
	...
	if (slave.drugs === Drug.SUPERFERTILITY && canImpreg(slave, sr)) { // Check is fine here for fert known as this represents slave in heat
		if ((slave.devotion > 20) || (slave.trust < -20)) {
...
App.EndWeek.Rules.consentRules = function(slave) {
	...
	function getJobText(assignment) {
		...
		switch (assignment) {
			...
			case Job.STEWARD:
				return Object.assign(obj, {
					comingDay: `you assign more maids to ${App.Entity.facilities.servantsQuarters.name}`,
			...
			case Job.HOUSE:
				return Object.assign(obj, {
					peers: getSlaves().asArray(), // ALL THE SLAVES! (Not all the slaves, so hopefully thats okay -FranklyGeorge)
