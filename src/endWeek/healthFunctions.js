...
/** Determine whether a given human can catch an illness at all, so we don't have to check chances.
 * @param {FC.HumanState} human
 * @returns {boolean}
 */
globalThis.canCatchIllness = function(human) {
	if (V.seeIllness === 0) { // illness is disabled globally
		return false;
	}
	if (human.health.illness > 0) { // already sick
		return false;
	}

	if (human.curatives === 1 || human.inflationType === InflationLiquid.CURATIVE) { // on preventatives, or inflated with curatives
		return false;
	}

	if (asSlave(human)?.assignment === Job.CLINIC) { // slaves in the clinic are monitored
		return false;
	}
	return true;
};
...
globalThis.nurseEffectiveness = function(slave) {
	...
	if (S.Nurse) {
		...
		let nurseEffectiveness = Math.trunc((nurseSkill * clinicUpgrade / Math.max((App.Entity.facilities.clinic.employeesIDs().size * 10 + (slaveCount() * 2) * clinicScreening), 1)) * 20);
...
globalThis.endWeekHealthDamage = function(slave) {
	...
	if (slave.ID !== SID.PC) {
		...
		if (H.shortDamage > 0) {
			...
			const assignment = slave.ID !== SID.PC ? asSlave(slave).assignment : null;
			...
			if (slave.curatives > 0 || slave.ID === SID.PC) { // transferred damage is half if on preventatives/curatives or target is the player
