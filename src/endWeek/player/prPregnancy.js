...
App.EndWeek.Player.pregnancy = function(PC = V.PC) {
	...
	// PC will always know their own ovulation cycle
	...
	function impregnation() {
		if (PC.vagina === 0 || (PC.anus === 0 && PC.mpreg > 0)) {
			...
			if (V.policies.sexualOpenness === 1) {
				eligibleSlaves = getSlaves().filter(s => canImpreg(V.PC, s) && App.Utils.sexAllowed(V.PC, s) && isSlaveAvailable(s) && canAchieveErection(s) && s.devotion > 20 && raceIsAcceptable).asArray();
			} else {
				eligibleSlaves = getSlaves().filter(s => canImpreg(V.PC, s) && isSlaveAvailable(s) && s.toyHole === ToyHole.DICK).asArray();
			}
		...
		if (canGetPregnant(PC) && !onBedRest(PC, true) && !isPCCareerInCategory("servant")) {
			...
			let slobs = getSlaves().filter(s => isSlaveAvailable(s) && s.geneMods.aggressiveSperm === 1 && (s.fetish === Fetish.MINDBROKEN || s.energy > 95 || (s.devotion < -20 && s.trust > 20) || (s.intelligence + s.intelligenceImplant < -10)));
			if (slobs.size > (totalServantCapacity() / 5)) {
				tryKnockMeUp(PC, -50, 2, slobs.randomValue());
			}
		}
	}
