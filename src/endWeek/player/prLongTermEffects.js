...
App.EndWeek.Player.longTermEffects = function(PC = V.PC) {
				...
				*/
				...
				if (PC.drugs === Drug.FERTILITY) {
					...
					if (PC.geneMods.NCS === 0) {
						...
						if (App.Medicine.fleshSize(PC, 'boobs') < 400 * gigantomastiaMod && random(1, 100) < 30) {
	...
	function period() {
		...
		if ((V.PC.ovaries === 1 && V.PC.vagina >= 0) || V.PC.mpreg === 1) {
			if (isMenstruating(V.PC)) {
				if (!fertKnown(V.PC) && V.PC.pubertyXX === 1) {
			...
			} else if (fertKnown(V.PC) && PC.fertLate < 0) {
				if (PC.fertLate === -1) {
