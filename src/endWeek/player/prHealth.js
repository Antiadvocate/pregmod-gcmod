	...
	function illness() {
		...
		if (roll < 2 && canCatchIllness(PC)) { // Always a 2% per week base chance of you, even in otherwise-safe conditions, spontaneously catching an illness
			...
			if (PCH.illness > 0 && roll > (30 + (PCH.illness * 10)) / (healthFactor)) {
				...
				*/
				const sexPartners = getSlaves().filter(s => App.Utils.sexAllowed(PC, s) && isSlaveAvailable(s)).asArray();
				const fucktoys = getSlaves().filter(s => [Job.FUCKTOY, Job.MASTERSUITE, Job.CONCUBINE].includes(s.assignment)).asArray();
				const wives = getSlaves().filter(s => s.relationship === -3).asArray();
