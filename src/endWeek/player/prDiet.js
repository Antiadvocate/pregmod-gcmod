	...
	function slaveFoodEffects() {
		...
		switch (PC.diet) {
			...
			case PCDiet.FERTILITY: // + ovum and small boosts to energy and attrXY
				...
				if (!isFertile(PC, true) || (PC.preg !== 0 && !superFetKnown)) {
	...
	function gainMuscle() {
		if (onBedRest(PC)) {
			...
			if (PC.geneticQuirks.mGain === 2) {
				...
				r.push(`you <span class="change positive">rapidly bulk up.</span>`);
				PC.muscles += 10;
			} else if (PC.drugs === Drug.STEROID) {
				r.push(`and since you're on so much gear, you <span class="change positive">rapidly bulk up.</span>`);
				...
				r.push(`you barely <span class="change positive">gain any muscle.</span>`);
				PC.muscles += 2;
			} else if (PC.balls > 0 && PC.ballType !== "sterile" && PC.hormoneBalance >= 100) {
				r.push(`but with the natural testosterone and artificial female hormones clashing in your system, you <span class="change positive">gain muscle slowly.</span>`);
				PC.muscles += 3 + PC.geneticQuirks.mGain;
			} else if (PC.balls > 0 && PC.ballType !== "sterile" && PC.hormoneBalance <= -100) {
				r.push(`and with the natural testosterone and artificial male hormones in your system, you <span class="change positive">rapidly bulk up.</span>`);
				PC.muscles += 8;
			} else if (PC.balls > 0 && PC.ballType !== "sterile") {
				r.push(`and with the natural testosterone in your system, you <span class="change positive">efficiently bulk up.</span>`);
				PC.muscles += 5 + PC.geneticQuirks.mGain;
			} else if (PC.balls > 0) {
				r.push(`but with your useless balls making little testosterone for you to use, you barely <span class="change positive">gain muscle.</span>`);
				PC.muscles += 2 + PC.geneticQuirks.mGain;
			} else if (PC.hormoneBalance <= -100) {
				r.push(`and with the artificial testosterone in your system, you <span class="change positive">gain muscle.</span>`);
				PC.muscles += 5 + PC.geneticQuirks.mGain;
			} else if (PC.hormoneBalance >= 100) {
				r.push(`but with lots of female hormones in your system, you barely <span class="change positive">gain muscle.</span>`);
				PC.muscles += 2 + PC.geneticQuirks.mGain;
			} else {
