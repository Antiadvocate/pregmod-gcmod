...
App.EndWeek.Player.inflation = function(PC = V.PC) {
	...
	function cervixImplantFluidConversion() {
		if (PC.inflationMethod === 2 && PC.cervixImplantTarget !== "none") {
			r.push(`Your rectal micropump implant filters out some fluid from the ${PC.inflationType} you're holding inside you during the week,`);
			if (PC.cervixImplantTarget === "all") {
				r.push(`<span class="change positive">adding it to your fillable implants.</span>`);
			} else if (PC.cervixImplantTarget === "TnA") {
				r.push(`<span class="change positive">adding it to your breast and butt implants.</span>`);
			} else if (PC.cervixImplantTarget === "mombod") {
				r.push(`<span class="change positive">adding most of it to your abdominal implant and the rest to your breast implants.</span>`);
			} else if (PC.cervixImplantTarget === "bottomHeavy") {
				r.push(`<span class="change positive">adding it to your butt and abdominal implants.</span>`);
			} else if (PC.cervixImplantTarget === "boobsImplant") {
				r.push(`<span class="change positive">adding it to your breast implants.</span>`);
			} else if (PC.cervixImplantTarget === "buttImplant") {
				r.push(`<span class="change positive">adding it to your butt implants.</span>`);
			} else {
				r.push(`<span class="change positive">adding it to your abdominal implant.</span>`);
			}
			cervixPumpFillsImplant(PC, 200);
		}
	}
