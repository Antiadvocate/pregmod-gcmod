...
App.EndWeek.Player.longTermPhysicalEffects = function(PC = V.PC) {
	...
	const averageDicking = getSlaves().filter(s => canAchieveErection(s) && isSlaveAvailable(s) && ((V.policies.sexualOpenness === 1 && s.devotion > 20 && App.Utils.sexAllowed(V.PC, s)) || s.toyHole === ToyHole.DICK));
	...
	function holeRelaxation() {
		...
		if (averageDicking.size > 0) {
			averageDicking.forEach(s => averageDickSize += s.dick);
			averageDickSize = averageDickSize / averageDicking.size;
		}
	...
	function nullSexualFrustration() {
		...
		if (PC.energy > 20) {
			r.push(`You`);
			if (averageDicking.size > 0 && PC.anus > 0) {
				if (PC.prostate !== 0) {
					r.push(`lack`);
					if (V.seeDicks > 0) {
						r.push(`a dick or`);
					}
					r.push(`a pussy, but you do have a prostate and ${averageDicking.size > 1 ? "slaves" : "a slave"} to work it, giving you at least one sexual outlet.`);
					frustration = 1;
				} else {
					r.push(`have no`);
					if (V.seeDicks > 0) {
	...
	function sexualSatisfaction() {
		const sexPartners = getSlaves().filter(s => App.Utils.sexAllowed(V.PC, s) && isSlaveAvailable(s) && s.assignment !== Job.FUCKTOY).asArray().shuffle();
		...
		if (isPlayerFrigid()) {
			...
			let partner = sexPartners.pop();
			while (PC.need > 0 && partner) {
				if (averageDicking.has(partner.ID)) {
					if (getPCPreferredHole() === "vagina" && PC.vagina > 0) {
						seX(PC, "vaginal", partner, "penetrative");
						PC.need -= 2;
					} else if (PC.anus > 0) {
						seX(PC, "anal", partner, "penetrative");
						if (PC.prostate > 0) {
							PC.need--;
						}
					} else {
						seX(PC, "oral", partner, "penetrative");
					}
				} else if (!isNull) {
					SimpleSexAct.Player(partner);
					PC.need--;
				}
				partner = sexPartners.pop();
			}
		}
		PC.lusty = 0;
		// Needs more holes to fuck, or more dedicated fucktoys.
		const fuckpitSlaves = getSlaves().filter(s => s.assignment === Job.MASTERSUITE || s.assignment === Job.CONCUBINE || (s.assignment === Job.FUCKTOY && (canWalk(s) || (canMove(s) && s.rules.mobility === "permissive"))) && s.rules.release.master !== 0);
		if (V.masterSuiteUpgradeLuxury === 2 && fuckpitSlaves.size > 1 && !isPlayerFrigid()) {
			...
			while (fuckpitSex > 0) {
				SimpleSexAct.Player(fuckpitSlaves.randomValue());
				fuckpitSex--;
			}
			PC.need = 0;
		} else if (PC.need > 0) {
		...
		if (PC.pubertyXX === 1 && PC.vagina !== -1) {
			if (PC.energy > 60 && PC.vaginaLube === 0 && (V.policies.sexualOpenness === 1 || averageDicking.size > 0) && random(1, 100) > 95) {
				r.push(`With all the excitement in your life, your formerly dry pussy <span class="change positive">starts moistening up.</span>`);
				PC.vaginaLube++;
			} else if (PC.energy <= 20 && PC.vaginaLube > 0 && V.policies.sexualOpenness === 0 && averageDicking.size === 0 && random(1, 100) <= 5) {
	...
	function bellyEffects() {
		...
		if (PC.cervixImplant >= 1) {
			if (PC.bellyImplant >= 800000 || (PC.bellyImplant > 130000 && V.arcologies[0].FSTransformationFetishistResearch !== 1)) {
				r.push(`Your cervix implant detected that the volume of filler in your belly is at the implant's threshold, and <span class="change negative">opens backflow</span> several times throughout week.`);
			...
			if (PC.cervixImplantTubing) {
				if (PC.boobsImplantType === "hyper fillable" && PC.boobsImplant >= 50500) {
					r.push(`Your cervix implant detected that your breast implants are over capacity, and <span class="change negative">pumps filler out</span> several times during week in an effort to maintain their integrity.`);
					PC.boobsImplant -= 1000;
					PC.boobs -= 1000;
				} else if (PC.boobsImplantType === "advanced fillable" && PC.boobsImplant >= 10500) {
					r.push(`Your cervix implant detected that your breast implants are at risk of rupturing, and <span class="change negative">pumps filler out</span> several times during week in an effort to reduce the pressure.`);
					PC.boobsImplant -= 1000;
					PC.boobs -= 1000;
				} else if (PC.boobsImplantType === "fillable" && PC.boobsImplant >= 2050) {
					r.push(`Your cervix implant detected that your breast implants are at risk of rupturing, and <span class="change negative">pumps filler out</span> several times during week in an effort to reduce the pressure.`);
					PC.boobsImplant -= 100;
					PC.boobs -= 100;
				}
				// butt implants require conversion prior to application
			}
		}
	}
};
