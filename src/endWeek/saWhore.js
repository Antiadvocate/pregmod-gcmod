...
App.SlaveAssignment.whore = function(slave) {
	...
	function slaveSkills(slave) {
		...
		if (slave.aphrodisiacs > 1 || slave.inflationType === InflationLiquid.APHRO) {
			...
			if ((slave.skill.oral >= 100) && ((slave.skill.anal >= 100) || !canDoAnal(slave)) && ((slave.skill.vaginal >= 100) || !canDoVaginal(slave)) && (slave.skill.penetrative >= 100 || penetrativeSocialUse(slave) <= 0)) {
				...
				r += ` ${He} shows diligence, and ${his} <span class="skill inc">${toSentence(weakSkills)} skills improve,</span> according to what the customers demand`;
				if (!canDoVaginal(slave)) {
	...
	function publicReactions(slave) {
		...
		if (totalRelatives(slave) > 0) {
			...
			if (children.size > 1) {
				...
				const child0 = children.atIndex(0).value;
				const relativePronouns = getPronouns(child0);
				r += ` Since ${his} ${relativeTerm(slave, child0)}, ${child0.slaveName}, is selling ${relativePronouns.objectReflexive} too, ${he} earns extra ¤ by working with ${relativePronouns.object}.`;
			}
			if (children.size > 0) {
		...
		if (arcologyInfo.fsActive('FSRestart') && slave.breedingMark === 1 && V.propOutcome === 1) {
			r += ` Customers are overjoyed at the prospect of fucking an ordinarily exclusive breeding slave. `;
		}
		...
		if (V.seeRace === 1) {
			if (V.racialAttractivenessBonus === 2 && !FutureSocieties.isActive("FSSupremacist") && slave.race === arcology.FSSupremacistRace) {
				r += ` Many customers prefer ${arcology.FSSupremacistRace} ${girl}s like ${slave.slaveName}.`;
		...
		let needText = ``;
		if (slave.energy > 95) {
			needText = ` As a nymphomaniac ${he} gets sexual release from every transaction${arcologyInfo.fsActive('FSSlaveProfessionalism') ? `` : ` and is wildly successful`}.`;
			slave.need -= beauty;
		...
		if (slave.attrKnown === 1) {
			...
			if (arcologyInfo.fsActive('FSSlaveProfessionalism') && slave.energy > 80) {
				r += ` ${His} intense sex drive is unappealing to some customers. `;
			}
		}
	...
	function pregnancy(slave) {
		...
		if (jsRandom(0, 99) < pregChance) {
			...
			App.EndWeek.saVars.addPregEvent(slave, SID.CITIZEN, `Due to ${customerDesc} cumming in ${his} fertile ${slave.mpreg === 1 ? anusDesc(slave, true) : vaginaDesc(slave, true)}, <span class="pregnant">${he} has become pregnant.</span>`);
