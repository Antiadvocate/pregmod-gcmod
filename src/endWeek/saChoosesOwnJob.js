...
App.SlaveAssignment.choosesOwnJob = function saChoosesOwnJob(slave) {
	...
	if (slave.fuckdoll > 0 || slave.fetish === Fetish.MINDBROKEN) {
		...
		r.push(`was assigned to choose ${his} own job, but is <span style="warning">mentally incapable</span> of doing so,`);
	...
	function assignServant(slave) {
		...
		if (V.universalRulesAssignsSelfFacility === 1 && App.Entity.facilities.servantsQuarters.capacity > servQL) {
			r.push(`from ${App.Entity.facilities.servantsQuarters.name}</span> to make your penthouse as clean and homelike as possible.`);
	...
	function jobSelection(slave) {
		...
		choice.push(`was assigned to choose ${his} own job. ${He} ${canSee(slave) ? "watches" : "observes"} your other slaves to decide what to do, <span class="devotion inc">happy</span> to be permitted a choice and a little more <span class="trust inc">confident</span> than if you had just given ${him} orders. The ${SlaveTitle(slave)}`);
		...
		if (slave.devotion <= 20 && slave.trust >= -20) {
			...
			if (V.universalRulesAssignsSelfFacility === 1 && App.Entity.facilities.servantsQuarters.capacity > servQL) {
				choice.push(`is obedient but not devoted, so ${he} <span class="job change">decides to work from ${App.Entity.facilities.servantsQuarters.name}</span> since it's the least sexually demanding job available.`);
		...
		} else if (V.universalRulesAssignsSelfFacility === 1 && slave.devotion > 50 && canWalk(slave) && canSee(slave) && V.nursery > nurseryL && getInfants().find((c) => (c.mother === slave.ID || c.father === slave.ID))) {
			if (getInfants().find((c) => (c.mother === slave.ID || c.father === slave.ID))) {
			...
			if (slave.fetish === Fetish.SUBMISSIVE && canWalk(slave) && canSee(slave)) {
				if (V.universalRulesAssignsSelfFacility === 1 && App.Entity.facilities.servantsQuarters.capacity > servQL) {
					choice.push(`thinks ${he} belongs at the bottom of the penthouse hierarchy, so ${he} <span class="job change">goes to live in ${App.Entity.facilities.servantsQuarters.name}.</span>`);
					choice.push(assignJob(slave, Job.QUARTER));
				} else {
				...
				if (V.PC.dick > 0 && isFertile(slave, !fertKnown(slave)) && !isMenstruating(slave)) {
					if (V.universalRulesAssignsSelfFacility === 1 && V.masterSuite > masterSL) {
				...
				} else if (V.universalRulesAssignsSelfFacility === 1 && V.dairyPregSetting > 0 && V.dairy > dairyL && isFertile(slave, !fertKnown(slave)) && !isMenstruating(slave)) {
					if (V.dairyPregSetting > 1) {
						choice.push(`eagerly <span class="job change">rushes to ${V.dairyName}</span> in the hopes that ${his} ${fertKnown(slave) ? `fertile` : `potentially fertile`} womb will be packed full of children.`);
						choice.push(assignJob(slave, Job.DAIRY));
					} else {
						choice.push(`<span class="job change">rushes to ${V.dairyName}</span> in the hopes that ${his} ${fertKnown(slave) ? `fertile` : `potentially fertile`} womb will be rented out.`);
						choice.push(assignJob(slave, Job.DAIRY));
					}
				...
				} else if (canGetPregnant(slave, !fertKnown(slave))) {
					if (V.universalRulesAssignsSelfFacility === 1 && V.brothel > brothelL) {
