 ...
 * @returns {{type: "player" | "lover" | "friend" | "family" | "nurse" | "sub" | null, slave?: FC.SlaveState}}
 */
App.EndWeek.getClinicPartner = function(slave) {
	if (!_.isEmpty(App.EndWeek.saVars.subSlaveMap[slave.ID])) {
		return {type: "sub"};
	}
...
App.EndWeek.getFLSex = function(facility) {
	...
	function flWillFuck(emp) {
		...
		// only the warden or a non-frigid stewardess will routinely rape frigid/unhappy slaves
		if (horny(emp) || fl.assignment === Job.WARDEN || (fl.assignment === Job.STEWARD && S.Stewardess.energy > 20)) {
