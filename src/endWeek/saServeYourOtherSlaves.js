...
App.SlaveAssignment.serveYourOtherSlaves = function saServeYourOtherSlaves(slave) {
	...
	// Calls App.Events.spaceSentences and removes extra spaces "  "
	...
	function validateJob(slave) {
		if (slave.subTarget === -1) {
			...
			// Send to general population if dom is not available.
			if (domSlave && !isSlaveAvailable(domSlave)) {
				domName = domSlave.slaveName;
				r.push(`was supposed to be serving ${domName} who is currently unavailable, so instead ${he}`);
				domSlave = undefined;
				domSlaveUnavailable = true;
			}
			if (domSlave) {
				jobType = "sub";
				domName = domSlave.slaveName;
	...
	function cumdumpLife(slave) {
		...
		if (domSlaveUnavailable) {
			r.push(`It takes some time for your slaves to realize ${he} is no longer exclusive to ${domName}, so ${he} gets little attention this week.`);
			return;
		}
		...
		// TODO: This needs some enhancement as currently cumDumps will have sex even if slaves aren't using them, and there's no tracking of who is actually fucking them.
	...
	function subLife(slave) {
		...
		const domFacility = App.Utils.jobForAssignment(domSlave.assignment)?.facility;
		otherSubs = App.EndWeek.saVars.subSlaveMap.get(domSlave.ID)?.filter(id => id !== slave.ID) ?? [];

		r.push(`is serving <strong>${domName}</strong> this week`);
		if (otherSubs.length > 1) {
			r.push(`alongside ${otherSubs.length} other slaves`);
		} else if (otherSubs.length > 0) {
			const otherSub = getSlave(otherSubs[0]);
			if (otherSub) {
				r.push(`alongside`);
				if (isRivalP(slave, otherSub)) {
					r.push(`${his} ${rivalryTerm(slave)}`);
				} else if (haveRelationshipP(slave, otherSub)) {
					r.push(`${his} ${relationshipTerm(slave)}`);
				} else if (areRelated(slave, otherSub)) {
					r.push(`${his} ${relativeTerm(slave, otherSub)}`);
				}
				r.push(otherSub.slaveName);
			}
		}
		if (!assignmentVisible(domSlave) && domFacility) {
			r.push(`in ${domFacility.name}.`);
		} else {
			r.push(`in the penthouse.`);
		}
		...
		if (domSlave.energy <= 20 || (!onBedRest(V.PC, true) && ["ravish", "ravished"].includes(getPersonalAttentionType(slave.ID)))) { // Dom doesn't fuck sub if frigid or being ravished by Player
			if (domSlave.energy <= 20) {
				r.push(`${domName} is frigid,`);
			} else { // Ravaged
				r.push(`${domName} is thoroughly ravaged by you,`);
			}
			r.push(`so ${he2} simply uses ${subName} for comfort and convenience. ${subName} washes ${his} superior's ${domRace} body thoroughly and uses ${his} own ${subRace} body to warm ${domName}'s bed at night. <span class="hotpink">${domName} enjoys the ease and companionship.</span>`);
		} else if (domSlave.fetishStrength > 60 && domSlave.fetish === Fetish.BUTTSLUT) {
			...
			if (canPenetrate(slave)) {
				...
				if (isAmputee(slave)) {
					if (canDoVaginal(domSlave)) {
						...
						oralUse = random(9, 12);
						seX(domSlave, "vaginal", slave, "oral", oralUse);
			...
			domSlaveUsedFetish = 1;
			if (canImpreg(slave, domSlave, !fertKnown(slave))) {
				if (domFetishKnown) {
				...
				const pregText = tryKnockMeUp(slave, 100, 2, domSlave);
				if (slave.fertKnown) {
					r.push(pregText);
				}
			...
			} else if (canImpreg(domSlave, slave, !fertKnown(domSlave))) {
				if (domFetishKnown) {
					r.push(`${domName} loves pregnancy, and`);
				} else {
					r.push(`${domName}, it turns out, <span class="lightcoral">really likes pregnancy,</span> so`);
				...
				const pregText = tryKnockMeUp(domSlave, 100, 2, slave);
				if (domSlave.pregKnown) {
					r.push(pregText);
				}
				fuckCount = random(10, 16);
		...
		if (jobType === "stud") {
			...
			switch (slave.fetish) {
				...
				case Fetish.PREGNANCY:
					if (canGetPregnant(slave, !fertKnown(slave))) {
						if (slave.mpreg === 0 && vaginalUse > 7) {
							if (slave.fetishKnown) {
								r.push(`Having ${his} ${fertKnown(slave) ? `potentially ` : ``}fertile pussy see so much use really satisfies ${subName}.`);
							} else {
								r.push(`Frequent unprotected sex in ${his} ${fertKnown(slave) ? `potentially ` : ``}fertile hole exposes ${him} for the <span class="lightcoral">pregnancy fetishist ${he} is.</span>`);
								slave.fetishKnown = 1;
							}
							subSlaveLikedFetish = 1;
						} else if (slave.mpreg === 1 && analUse > 7) {
							if (slave.fetishKnown) {
								r.push(`Having ${his} ${fertKnown(slave) ? `potentially ` : ``}fertile ass see so much use really satisfies ${subName}.`);
							} else {
								r.push(`Frequent unprotected sex in ${his} ${fertKnown(slave) ? `potentially ` : ``}fertile hole exposes ${him} for the <span class="lightcoral">pregnancy fetishist ${he} is.</span>`);
								slave.fetishKnown = 1;
							}
					...
					case Fetish.PREGNANCY:
						if (canGetPregnant(slave, !fertKnown(slave))) {
							if (slave.mpreg === 0 && vaginalUse > 0) {
								if (slave.fetishKnown) {
									r.push(`Having ${his} ${fertKnown(slave) ? `potentially ` : ``}fertile pussy serve as someone's toy really satisfies ${subName}.`);
								} else {
									r.push(`Having ${his} ${fertKnown(slave) ? `potentially ` : ``}fertile pussy used exposes ${him} for the <span class="lightcoral">pregnancy fetishist ${he} is.</span>`);
									slave.fetishKnown = 1;
								}
								slave.need = 0;
							} else if (slave.mpreg === 1 && analUse > 0) {
								if (slave.fetishKnown) {
									r.push(`Having ${his} ${fertKnown(slave) ? `potentially ` : ``}fertile ass serve as someone's toy really satisfies ${subName}.`);
								} else {
									r.push(`Having ${his} ${fertKnown(slave) ? `potentially ` : ``}fertile asspussy used exposes ${him} for the <span class="lightcoral">pregnancy fetishist ${he} is.</span>`);
									slave.fetishKnown = 1;
								}
								slave.need = 0;
							}
	...
	function physicalEffects(slave) {
		...
		// Check if dom slave actually produces enough cum to fill implant.
		// TODO: Adjust for Cumdump - filling can happen even if no slave produces cum.
		const fillsImplant = (jobType === "sub" && domSlave && cumAmount(domSlave) > 5) || jobType === "cumdump";
		if (fillsImplant) {
		...
		if (cervixPump > 0) {
			r.push(`${He} notices ${his}`);
			if (slave.cervixImplantTarget === "all") {
				r.push(`<span class="change positive">body is a little more bloated</span>`);
			} else if (slave.cervixImplantTarget === "TnA") {
				r.push(`<span class="change positive">tits and ass have a little more heft to them</span>`);
			} else if (slave.cervixImplantTarget === "mombod") {
				r.push(`<span class="change positive">belly and breasts are a little more motherly</span>`);
			} else if (slave.cervixImplantTarget === "bottomHeavy") {
				r.push(`<span class="change positive">butt and belly are a little bloated</span>`);
			} else if (slave.cervixImplantTarget === "boobsImplant") {
				r.push(`<span class="change positive">boobs have expanded a little</span>`);
			} else if (slave.cervixImplantTarget === "buttImplant") {
				r.push(`<span class="change positive">butt has grown a little</span>`);
			} else {
				r.push(`<span class="change positive">belly is a little heavier</span>`);
			}
			r.push(`after all the`);
			...
			cervixPumpFillsImplant(slave, cervixPump);
		}
	...
	function increaseSkillFromCumdump(slave, skillName, skillUse) {
		if (slave.skill[skillName] < 30) {
			r.push(slaveSkillIncrease(skillName, slave, Math.max((Math.floor((skillUse / 2) + Math.floor((slave.intelligence + slave.intelligenceImplant) / 32))), 0)));
		} else if (slave.skill[skillName] < 100) {
			r.push(slaveSkillIncrease(skillName, slave, Math.max((Math.floor((skillUse / 4) + Math.floor((slave.intelligence + slave.intelligenceImplant) / 32))), 0)));
		}
	}
	...
	function slaveSkills(slave) {
		if (jobType === "stud") {
			...
			const {his2, he2, him2} = getPronouns(domSlave).appendSuffix('2');
			if ((slave.rivalry === 0 || slave.rivalryTarget !== domSlave.ID) && slave.devotion > 20 && domSlave.devotion > 20) {
				const intAdjustment = Math.trunc(slave.intelligence + slave.intelligenceImplant) / 32; // -4 to 4
				/**
				 * Get skill increase amount with a min of 1
				 * @param {number} use
				 * @param {number} [min=1]
				 */
				const skillIncreaseAmount = (use, min=1) => {
					return Math.trunc(Math.max(Math.min(use, 5) + intAdjustment, min)); // min - +9
				};
				// Cap skill increases from domSlave per week to 2 categories.
				let skillIncreased = 0;
				const skillIncreaseCap = 2;
				if (oralUse > 0) {
					if (slave.skill.oral < domSlave.skill.oral && skillIncreased < skillIncreaseCap) {
						r.push(`${domName} is better at oral than ${subName} and lets ${him} in on some of ${his2} techniques.`);
						r.push(slaveSkillIncrease('oral', slave, skillIncreaseAmount(oralUse)));
						skillIncreased++;
					} else if (slave.skill.oral < 30) {
						r.push(slaveSkillIncrease('oral', slave, skillIncreaseAmount(oralUse / 4, 0)));
					}
				}
				if (vaginalUse > 0) {
					if (slave.skill.vaginal < domSlave.skill.vaginal && domSlave.vagina >= 0  && skillIncreased < skillIncreaseCap) {
						r.push(`${domName} knows how to use ${his2} pussy better than ${subName} does and passes on some of ${his2} knowledge.`);
						r.push(slaveSkillIncrease('vaginal', slave, skillIncreaseAmount(vaginalUse)));
						skillIncreased++;
					} else if (slave.skill.vaginal < 30) {
						r.push(slaveSkillIncrease('vaginal', slave, skillIncreaseAmount(vaginalUse / 2, 0)));
					}
				}
				if (analUse > 0) {
					if (slave.skill.anal < domSlave.skill.anal && skillIncreased < skillIncreaseCap) {
						r.push(`${domName} is more familiar with anal sex than ${subName} is and lets some of ${his2} skill rub off on ${him}.`);
						r.push(slaveSkillIncrease('anal', slave, skillIncreaseAmount(analUse)));
						skillIncreased++;
					} else if (slave.skill.anal < 30) {
						r.push(slaveSkillIncrease('anal', slave, skillIncreaseAmount(analUse / 2, 0)));
					}
				}
				if (penetrativeUse > 0) {
					if (slave.skill.penetrative < domSlave.skill.penetrative && skillIncreased < skillIncreaseCap) {
						r.push(`${domName} is better at fucking others than ${subName} is, so ${he2} gives ${him} some pointers to improve ${his} skills.`);
						r.push(slaveSkillIncrease('penetrative', slave, skillIncreaseAmount(penetrativeUse)));
						skillIncreased++;
					} else if (slave.skill.penetrative < 30) {
						r.push(slaveSkillIncrease('penetrative', slave, skillIncreaseAmount(penetrativeUse / 2, 0)));
					}
				}
				if (skillIncreased < skillIncreaseCap && [Job.WHORE, Job.BROTHEL, Job.MADAM].includes(domSlave.assignment)) {
					if (slave.skill.whoring < 30 && domSlave.skill.whoring > 30) {
						r.push(`${He} manages to pick up some basic street smarts and knowledge about prostitution from ${domName} while assisting ${him2}.`);
						r.push(slaveSkillIncrease('whoring', slave, skillIncreaseAmount(domSlave.skill.whoring / 20)));
						skillIncreased++;
					}
				}
				if (skillIncreased < skillIncreaseCap && [Job.PUBLIC, Job.CLUB, Job.DJ].includes(domSlave.assignment)) {
					if (slave.skill.entertainment < 30 && domSlave.skill.entertainment > 30) {
						r.push(`${He} gains some experience and basic understanding regarding entertaining citizens from ${domName} while helping ${him2}.`);
						r.push(slaveSkillIncrease('entertainment', slave, skillIncreaseAmount(domSlave.skill.entertainment / 20)));
						skillIncreased++;
					}
				}
			}
