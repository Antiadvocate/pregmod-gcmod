...
App.Mods.DinnerParty.Execution = function() {
	...
	if (dinnerRating >= 12) { // successful evening
		...
		for (const aliveSlave of getSlaves().filter(s => s.fetish !== Fetish.MINDBROKEN && s.devotion < 50).values()) {
			let relation;
			let feeling;
			const {his3, sister3, daughter3} = getPronouns(aliveSlave).appendSuffix("3");
		...
		for (const aliveSlave of getSlaves().filter(s => s.fetish !== Fetish.MINDBROKEN && s.devotion >= 85).values()) {
			let relation;
			let feeling;
			const {his3, sister3, daughter3} = getPronouns(aliveSlave).appendSuffix("3");
			if (slave.mother === aliveSlave.ID || slave.father === aliveSlave.ID) {
		...
		let aliveSlave = getSlaves().find(s => s.ID === slave.relationshipTarget && s.fetish !== Fetish.MINDBROKEN)?.value;
		...
		aliveSlave = getSlaves().find(s => s.ID === slave.rivalryTarget && s.fetish !== Fetish.MINDBROKEN)?.value;
		...
		if (generateEnemies === 1) {
			...
			for (let i = 0; i < 6; i++) {
					...
					V.MOD_enemy.numSlave = random(Math.trunc(getSlaves().length * 0.7), Math.trunc(getSlaves().length * 1.3));
		...
		*/
		...
		for (const aliveSlave of getSlaves().values()) {
			if (random(0, 1) > 0) {
