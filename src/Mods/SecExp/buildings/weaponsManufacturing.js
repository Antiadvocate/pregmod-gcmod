App.Mods.SecExp.weapManu = (function() {
	...
	function GUI() {
		...
		function menials() {
			...
			App.UI.DOM.appendNewElement("div", div, App.UI.tradeMenials(true), ['indent']);
	 ...
	 function purchase(upgradeID) {
		...
		if (!V.SecExp.buildings.weapManu.upgrades.completed.includes(upgradeID)) {
			...
			if (!found) {
				...
				if (item.type !== "all") {
					el.append(`the base ${item.type} value${item.type.includes("and") ? 's' : ''}`);
				} else {
