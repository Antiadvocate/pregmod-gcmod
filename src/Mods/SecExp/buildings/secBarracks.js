App.Mods.SecExp.barracks = (function() {
	...
	function GUI() {
		...
		if (V.SecExp.buildings.barracks.size < 5) {
			const cost = Math.trunc(5000 * (V.SecExp.buildings.barracks.size + 1) * V.upgradeMultiplierArcology);
			...
			if (reasons.length > 0 || sectionSF) {
				...
				const cost = Math.trunc( (750000 * (1.15 + (App.Mods.SF.upgrades.total() / 1000)) * (1.15 + (V.SF.Squad.Firebase / 10))) * App.Mods.SF.env() );
		...
		function page(u) {
			...
			switch (u) {
				...
				case "militia":
					...
					break;
				case "mercs":
					App.UI.DOM.appendNewElement("div", r, `With the installation of a mercenary company in the arcology, many others are attracted to your free city, hoping to land a contract with you.`);
					App.UI.DOM.appendNewElement("div", r, `You are able to organize them in units to use in the defense of the arcology. Excluding the defense force you set up, there are ${num(App.Mods.SecExp.Manpower.totalMerc)} mercenaries in your arcology, of which ${num(App.Mods.SecExp.Manpower.employedMerc)} are actively employed and ${num(V.SecExp.units.mercs.free)} not yet under contract. In total ${num(V.SecExp.units.mercs.dead)} mercenaries have died defending your arcology.`);
					break;
			}
