App.Events.conflictReport = function() {
	...
	if ([3, -3, 2, -2].includes(result)) {
		if (inBattle) {
			V.SecExp.battles.lossStreak = (result === 3 ? 0 : V.SecExp.battles.lossStreak++);
			V.SecExp.battles.victoryStreak = (result === -3 ? V.SecExp.battles.victoryStreak++ : 0);
		}
	...
	if (![-1, 0, 1].includes(result)) {
		...
		if (allKilled) {
			...
			r.push(`inflicting ${num(V.SecExp.war.attacker.losses)} casualties, while sustaining`);
		}
		if (V.SecExp.war.losses > 1) {
			r.push(`${num(Math.trunc(V.SecExp.war.losses))} casualties`);
		} else {
			r.push(`${V.SecExp.war.losses > 0 ? 'a casualty' : 'zero casualties'}`);
		}
		r.addToLast(`${allKilled ? '' : ' themselves'}.`);
		if (inRebellion) {
		...
		if (result === 3) {
			...
			r.push(`The fight was long and hard, but in the end our men managed to ${inBattle ? 'repel the' : 'stop the revolt'}`);
			if (V.SecExp.war.attacker.type === "raiders") {
				r.push(`raiders`);
			} else if (V.SecExp.war.attacker.type === "free city") {
				r.push(`mercenaries`);
			} else if (V.SecExp.war.attacker.type === "freedom fighters") {
				r.push(`freedom fighters`);
			} else if (V.SecExp.war.attacker.type === "old world") {
				r.push(`old world soldiers`);
			}
			r.addToLast(`, though not without difficulty.`);
		} else if (result === -2) {
			r.push(`The fight was long and hard. Our men in the end had to yield to the`);
			if (inBattle) {
				if (V.SecExp.war.attacker.type === "raiders") {
					r.push(`horde of raiders`);
				} else if (V.SecExp.war.attacker.type === "free city") {
					r.push(`slavers`);
				} else if (V.SecExp.war.attacker.type === "freedom fighters") {
					r.push(`freedom fighters`);
			...
			r.addToLast(`, which were fortunately unable to capitalize on their victory.`);
		}
		...
		if (result === 3 || result === 2) {
			...
			if (inRebellion) {
				App.UI.DOM.appendNewElement("div", node, `Many of the rebelling ${slaveRebellion ? 'slaves' : 'citizens'} were ${slaveRebellion ? 're' : ''}captured and punished${slaveRebellion ? '' : ', many others enslaved'}.`);
				...
				V[slaveRebellion ? "NPCSlaves" : "lowerClass"] -= random(10, 30);
				...
				if (result === 3) {
					...
					if (V.SecExp.war.attacker.troops - V.SecExp.war.attacker.losses <= 50) {
						...
						r.push(`a great amount`);
					}
					r.push(`of attackers, and some of their equipment,`);
				} else {
					r.push("Our men were not able to capture any combatants, however some equipment was seized during the enemy's hasty retreat,");
				}
				if (V.SecExp.war.attacker.type === "raiders") {
			...
			r.push(`Due to your defeat, your`, App.UI.DOM.makeElement("span", `reputation`, ["red"]), `and`, App.UI.DOM.makeElement("span", `authority`, ["red"]), `decreased.`);
			if (inRebellion) {
				if (slaveRebellion) {
					r.push(`After the battle most of the rebelling slaves managed to escape, while others remained in the arcology for days looting and hunting their former masters. The arcology will bear the scars of this day for a long time.`);
					...
					r.push(`After the battle most of the rebelling citizens remained in the arcology for days looting and hunting their former arcology. We will bear the scars of this day for a long time.`);
				...
				if (result === -3) {
					r.push("Obviously your troops were not able to capture anyone or anything.");
					r.toParagraph();
		...
		if (inRebellion) {
			...
			if (!V.SecExp.war.reactorDefense) {
				if (random(1, 100) <= (75 - ((V.SecExp.buildings.riotCenter ? V.SecExp.buildings.riotCenter.fort.reactor : 0) * 25))) {
					...
					r.push(`It will cost <span class="red">time and money to repair the damage.</span>`);
		...
		if (hasLosses || V.SecExp.war.losses === 0) {
			...
			throw Error(`Losses are ${num(V.SecExp.war.losses)}.`);
		}
		...
		if (inRebellion && V.SecExp.war.irregulars > 0) {
			...
			r.push("The volunteering citizens were quickly organized into an irregular militia unit and deployed in the arcology:", casualtiesReport('irregulars', loss));
			r.toNode("div");
			if (hasLosses) {
		...
		if (V.SF.Toggle && V.SF.Active >= 1 && (inRebellion || inBattle && V.SecExp.war.deploySF)) {
			...
			r.push(`${capFirstChar(V.SF.Lower)}, ${num(V.SF.ArmySize)} strong, is called to arms:`, casualtiesReport('SF', loss));
			r.toNode("div");
		}
		...
		if (inRebellion) {
			...
			if (captives > 0) {
				let candidates = [];
				r.push(`During the battle ${num(captives)} attackers were captured.`);
				let totalPrice = 0;
				if (random(1, 100) <= 25) {
...
const rebellingUnitsFate = function(unit, averageLosses) {
	...
	for (const u of V.SecExp.units[unit].squads.filter(s => s.active === 1)) {
		if (V.SecExp.war.rebellingID.includes(u.ID)) {
