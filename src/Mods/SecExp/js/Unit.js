App.Mods.SecExp.unit = (function() {
	...
	function barracksList(type) {
		...
		if (type === "slaves") {
			App.UI.DOM.appendNewElement("div", list, App.UI.tradeMenials(true));
		}
		...
		for (const unit of V.SecExp.units[type].squads) {
			...
			linkArray.push(App.UI.DOM.link(`Disband the unit`, () => {
				...
				V.SecExp.battles.lastSelection.deleteAll(unit);
				App.UI.reload();
			}
			));
	...
	function squads(type = '') {
		const array = Object.values(V.SecExp.units).map(s => s.squads).flat(Number.POSITIVE_INFINITY);
		switch (type) {
