...
App.Mods.SecExp.Obj = (function() {
	...
	function BC() {
		// Do not add new BC to this. It will not be executed for new releases.
		// Create a patch following the instructions in `/src/data/patches/patch.js` instead.
		// @ts-ignore BC
		...
		for (const [unit, data] of App.Mods.SecExp.unit.list()) {
			...
			V.SecExp.units[unit].squads.forEach(u => {
				...
				if (unit !== 'bots' && u.platoonName.includes('undefined')) {
