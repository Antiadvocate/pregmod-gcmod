App.Intro.init = function() {
	...
	*/
	...
	App.Utils.setNonexistentProperties(V, App.Data.defaultGameStateVariables);
	// @ts-expect-error V.genePoolDefaults is read only, but we need to set it if it doesn't exist
	V.genePoolDefaults = V.genePoolDefaults || new App.Entity.GenePoolRecord();

	/* These variables must be created AND set to default values, NG+ or not */
	App.Utils.setExistentProperties(V, App.Data.resetOnNGPlus);
