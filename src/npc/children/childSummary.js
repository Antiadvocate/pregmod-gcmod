...
App.Facilities.Nursery.ChildSummary = function(child) {
	...
	function shortSmartFetish(child) {
		if (child.fetishKnown === 1) {
			if (child.clitSetting === "off") {
				...
				r += `SP:preg`;
			} else if (((child.fetish !== "bestiality") || (child.fetishStrength <= 95)) && (child.clitSetting === "bestiality")) {
				r += `SP:beast`;
	...
	function shortExtendedFamily(child) {
		...
		let handled = 0;
		if (child.mother > 0) {
			const mother = getSlaves().get(child.mother);
			if (mother) {
				r += `${SlaveFullName(mother)}'s ${daughter}`;
				if (child.relationshipTarget === mother.ID) {
		...
		if (child.father > 0 && child.father !== child.mother) {
			const father = getSlaves().get(child.father);
			if (father) {
				r += `${SlaveFullName(father)}'s ${daughter}`;
				if (child.relationshipTarget === father.ID && handled !== 1) {
					const friendShipShort = relationshipTermShort(child);
					r += ` & ${friendShipShort}`;
					handled = 1;
				}
			...
			r += " ";
		} else if (child.father === SID.PC && child.mother !== SID.PC) {
			r += `Your ${daughter}`;
		...
		if (child.daughters === 1) {
			const mother = getSlaves().find(s => s.mother === child.ID)?.value;
			if (mother) {
				r += `${SlaveFullName(mother)}'s mother`;
				if (child.relationshipTarget === mother.ID) {
					const friendShipShort = relationshipTermShort(child);
					r += ` & ${friendShipShort}`;
					handled = 1;
			...
			r += " ";
			const father = getSlaves().find(s => s.father === child.ID)?.value;
			if (father) {
				r += `${SlaveFullName(father)}'s father`;
				if (child.relationshipTarget === father.ID && handled !== 1) {
					const friendShipShort = relationshipTermShort(child);
					r += ` & ${friendShipShort}`;
					handled = 1;
		...
		if (child.sisters === 1) {
			const sisters = getSlaves().find(s => areSisters(s, child) > 0)?.value;
			if (sisters) {
				r += `${SlaveFullName(sisters)}'s ${sister}`;
				if (child.relationshipTarget === sisters.ID) {
					const friendShipShort = relationshipTermShort(child);
					r += `& ${friendShipShort}`;
					handled = 1;
				}
		...
		if (child.relationship > 0 && handled !== 1) {
			const rTarget = getSlaves().find(s => s.ID === child.relationshipTarget)?.value;
			if (rTarget) {
				r += `${SlaveFullName(rTarget)}'s`;
				const friendShipShort = relationshipTermShort(child);
				r += ` ${friendShipShort}`;
			}
		} else if (child.relationship === -3 && child.mother !== SID.PC && child.father !== SID.PC) {
			r += `Your ${wife}`;
	...
	function shortRival(child) {
		if (child.rivalry !== 0) {
			r += `&nbsp;&nbsp;&nbsp;&nbsp;`;
			const rival = getSlaves().find(s => s.ID === child.rivalryTarget)?.value;
			if (rival) {
				r += `<span class="lightsalmon">`;
				if (child.rivalry <= 1) {
					r += `Disl ${SlaveFullName(rival)}`;
				} else if (child.rivalry <= 2) {
					r += `${SlaveFullName(rival)}'s rival`;
				} else {
					r += `Hates ${SlaveFullName(rival)}`;
				}
				r += `</span> `;
			}
		}
	...
	function longExtendedFamily(child) {
		let handled = 0;
		if (child.mother > 0) {
			const mother = getSlaves().find(s => s.ID === child.mother)?.value;
			if (mother) {
				r += `${SlaveFullName(mother)}'s <span class="lightgreen">daughter`;
				if (child.relationshipTarget === mother.ID) {
		...
		if (child.father > 0 && child.father !== child.mother) {
			const father = getSlaves().find(s => s.ID === child.father)?.value;
			if (father) {
				r += `${SlaveFullName(father)}'s <span class="lightgreen">daughter`;
				if (child.relationshipTarget === father.ID) {
					const friendShipShort = relationshipTerm(child);
					r += ` and ${friendShipShort}`;
					handled = 1;
				}
		...
		} else if (child.father === SID.PC && child.father !== child.mother) {
			r += `Your `;
			if (child.relationship < -1) {
				r += `<span class="lightgreen">daughter and ${PCrelationshipTerm(child)}.</span> `;
				handled = 1;
		...
		if (child.daughters === 1) {
			let daughter = getSlaves().find(s => s.mother === child.ID)?.value;
			if (daughter) {
				r += `${SlaveFullName(daughter)}'s <span class="lightgreen">mother`;
				if (child.relationshipTarget === daughter.ID) {
					const friendShipShort = relationshipTerm(child);
					r += ` and ${friendShipShort}`;
					handled = 1;
				}
			...
			daughter = getSlaves().find(s => s.father === child.ID)?.value;
			if (daughter) {
				r += `${SlaveFullName(daughter)}'s <span class="lightgreen">father`;
				if (child.relationshipTarget === daughter.ID) {
					const friendShipShort = relationshipTerm(child);
					r += ` and ${friendShipShort}`;
					handled = 1;
		...
		if (child.sisters === 1) {
			const sisters = getSlaves().find(s => areSisters(s, child) > 0)?.value;
			if (sisters) {
				r += `${SlaveFullName(sisters)}'s <span class="lightgreen">sister`;
				if (child.relationshipTarget === sisters.ID) {
					const friendShipShort = relationshipTerm(child);
					r += ` and ${friendShipShort}`;
					handled = 1;
		...
		if (child.rivalry !== 0) {
			...
			if (rival) {
				if (child.rivalry <= 1) {
					...
					r += `${SlaveFullName(rival)}'s <span class="lightsalmon">rival.</span> `;
				} else {
					r += `<span class="lightsalmon">Hates</span> ${SlaveFullName(rival)}. `;
				}
			}
			r += " ";
		}
