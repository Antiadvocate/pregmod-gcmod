 ...
 * @param {object} root0
 * @param {number} [root0.market]
 * @param {number} [root0.eventDescription]
 * @returns {string}
 */
App.Facilities.Nursery.LongChildDescription = function(child, {market = 0, eventDescription = 0} = {}) {
	...
	const father = child.father === SID.PC ? PC : getSlave(child.father);
	const mother = child.mother === SID.PC ? PC : getSlave(child.mother);
				...
				if (child.preg > 0 && !child.pregKnown) {
					...
					if (pregCollar === 1) {
						...
						if (child.vagina < 0 && child.mpreg < 1) {
							...
							if (child.pregSource === SID.PC) {
								r += `"Womb claimed by my ${getWrittenTitle(child)}!" `;
							} else if (child.pregSource === SID.CITIZEN || child.pregSource === SID.GENERIC) {
								r += `"Baby made by slutting around!" `;
							} else if (child.pregSource === SID.LAB) {
								r += `"My baby was made with science!" `;
							} else if (child.pregSource === SID.FUTA) {
