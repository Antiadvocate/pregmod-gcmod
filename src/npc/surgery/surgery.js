 ...
 */
App.Medicine.Surgery.makeLink = function(procedure, refresh, cheat, facilityControlledAspects, onApply) {
	let disabledReasons = procedure.disabledReasons(facilityControlledAspects);
	if (disabledReasons.length > 0) {
		return App.UI.DOM.disabledLink(procedure.name, disabledReasons);
	}
	...
	/** @returns {"substantial" | "significant" | "moderate" | "light" | "insignificant"} */
 ...
 * @param {Set<FC.Facilities.FacilityControlledAspect>} facilityControlledAspects
 * @returns {FC.Medicine.Surgery.SizingOptions<any>}
 */
App.Medicine.Surgery.allSizingOptions = function(facilityControlledAspects) {
	return {
		replace: true,
...
App.Medicine.Surgery.sizingProcedures = function() {
	class ForbiddenDummy extends App.Medicine.Surgery.Procedure {
		...
		disabledReasons(facilityControlledAspects) {
			return super.disabledReasons(facilityControlledAspects).concat(this._forbidden);
		}
	}
	...
	function substituteImplicitOptions(options, target) {
		return {
			...
			facilityControlledAspects: options.facilityControlledAspects || new Set()
		};
	}
	...
	function boobSizingProcedures(slave, options = {}) {
		...
		if (options.facilityControlledAspects.has(FacilityControlledAspect.SURGERY_BOOBS_IMPLANTS)) {
			const {his, His} = getPronouns(slave);
			return [new ForbiddenDummy(slave, "Implants", `${His} assignment controls the size of ${his} boob implants.`)];
		}
	...
	function buttSizingProcedures(slave, options = {}) {
		...
		if (options.facilityControlledAspects.has(FacilityControlledAspect.SURGERY_BOOBS_IMPLANTS)) {
			const {his, His} = getPronouns(slave);
			return [new ForbiddenDummy(slave, "Implants", `${His} assignment controls the size of ${his} ass implants.`)];
		}
	...
	function lipsSizingProcedures(slave, options) {
		...
		if (options.facilityControlledAspects.has(FacilityControlledAspect.SURGERY_BOOBS_IMPLANTS)) {
			const {his, His} = getPronouns(slave);
			return [new ForbiddenDummy(slave, "Implants", `${His} assignment controls the size of ${his} lip implants.`)];
		}
...
globalThis.eyeSurgery = function(slave, side, action) {
	...
	function addEye(actor) {
		actor.eye[side] = new App.Entity.SingleEyeState();
		resetEyeColor(actor, side);
	}
};
 ...
 */
globalThis.setEyeColor = function(actor, color, side = "both") {
	if (!color || color === "" || typeof color !== "string") { throw new Error(`Expected a non empty string for 'color'. Got '${color}' of type '${typeof color}' instead.`); }
	if (side === "both") {
		setEyeColor(actor, color, "left");
		setEyeColor(actor, color, "right");
		return;
	}
...
globalThis.setEyeColorFull = function(slave, iris = "brown", pupil = "circular", sclera = "white", side = "both") {
	if (!iris || iris === "" || typeof iris !== "string") { throw new Error(`Expected a non empty string for 'iris'. Got '${iris}' of type '${typeof iris}' instead.`); }
	if (side === "both") {
 ...
 */
globalThis.setGeneticEyeColor = function(actor, color, heterochromia = false) {
	if (!color || color === "" || typeof color !== "string") { throw new Error(`Expected a non empty string for 'color'. Got '${color}' of type '${typeof color}' instead.`); }
	if (heterochromia) {
		actor.geneticQuirks.heterochromia = color;
	} else {
		actor.eye.origColor = color;
	}
};
...
/**
 * @param {FC.SlaveState} slave
 * @param {2|3|4|5|6|7|8|9|10} newId
 * @returns {boolean}
 */
globalThis.upgradeLimbs = function(slave, newId) {
	...
	/**
	 * @param {FC.LimbArgumentAll} limb
	 * @param {function(FC.SlaveState): FC.LimbType} idFunction
	 */
