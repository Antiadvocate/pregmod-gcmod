			...
			if (body.pregKnown === 1 && soul.preg > 0) {
				...
				if (soul.pregKnown === 1) {
					if (body.fetish === "pregnancy") {
						if (body.devotion > 50) {
							r.push(`${He} <span class="mediumorchid">scowls with momentary wrath</span> before regaining ${his} composure. ${He} resents being separated from ${his} pregnancy${(canGetPregnant(body, true, true)) ? `, though that is easily remedied` : ``}.`);
							body.devotion -= 5;
						} else if (body.devotion > 20) {
							r.push(`${He} <span class="mediumorchid">scowls angrily</span> at this turn of events${(canGetPregnant(body, true, true)) ? `, though that will be easily remedied by putting another child in ${him}` : ``}.`);
							body.devotion -= 10;
						} else {
