...
globalThis.bodySwap = function(soul, body, fromGenepool) {
	...
	soul.electrolarynx = body.electrolarynx;
	...
	if (!fromGenepool) { // swapping NOT gene pool records
		...
		soul.cervixImplantTubing = body.cervixImplantTubing;
		soul.cervixImplantTarget = body.cervixImplantTarget;
		soul.scar = body.scar;
...
globalThis.bodySwapSelection = function(soul) {
	...
	for (const body of getSlaves().values()) {
...
globalThis.huskSwapSelection = function(body) {
	...
	for (const soul of getSlaves().values()) {
				...
				if (!hasAnyProstheticLimbs(soul)) {
					if (soul.indenture === -1) {
						if (soul.breedingMark === 0 || V.propOutcome === 0 || V.eugenicsFullControl === 1 || !FutureSocieties.isActive('FSRestart')) {
							if (WombReserveCount(soul, "both") === 0) {
								if (soul.ID !== body.ID) {
									App.UI.DOM.appendNewElement("div", el,
										App.UI.DOM.link(
											soul.slaveName,
											() => {
												V.temp.swappingSlave = soul.ID;
