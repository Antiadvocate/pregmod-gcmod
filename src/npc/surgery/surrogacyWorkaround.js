App.UI.surrogacyWorkaround = function() {
	...
	App.UI.DOM.appendNewElement("p", node, `${(donatrix === -1) ? `You've prepared yourself to have an egg taken from your ovaries;` : `${getSlave(V.temp.AS).slaveName} is prepped to have an egg harvested from ${getPronouns(getSlave(V.temp.AS)).possessive} ovaries;`} now you must select a target to fertilize it and who will carry it to term.`, "scene-intro");
