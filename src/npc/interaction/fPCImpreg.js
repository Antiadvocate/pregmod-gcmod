...
App.Interact.fPCImpreg = function(slave) {
	...
	if (!hasPenis) {
		text.push(`You wear a special strap-on with a micro-pump that ensures any ejaculate will be expelled through the artificial member.`);
		if (V.PC.clit > 3) {
			text.push(`The attached dildo is hollow and comfortablely fits around your ${clitDesc(V.PC, true)}, providing additional stimulation.`);
		}
	}
	...
	const attempts = jsRandom(3, 9);
	seX(slave, slave.mpreg ? `anal` : `vaginal`, V.PC, "penetrative", attempts);
	...
	if (slave.pregKnown === 1) {
		...
		text.push(`Despite repeating this ritual throughout the week, ${slave.slaveName} <span class="red">failed to conceive your child.</span>`);
	} else {
		text.push(`You repeat this ritual throughout the week, expecting to`);

		if (superfetation) {
			text.push(`add your child to ${slave.slaveName}'s pregnancy.`);
		} else {
			text.push(`impregnate ${slave.slaveName} with your child.`);
		}
	}

	if (FutureSocieties.isActive('FSRestart') && (!slave.breedingMark || V.propOutcome === 0) && V.eugenicsFullControl !== 1) {
		text.push(`Rumors spread about ${slave.pregKnown === 1 && success ? `you` : `your attempts at`} fucking your slaves pregnant; the Societal Elite are <span class="reputation dec">very displeased</span> by these rumors.`);
		V.failedElite += 5;
	}

	if (FutureSocieties.isActive('FSGenderRadicalist') && slave.mpreg) {
		text.push(`Society <span class="reputation inc">approves</span> of ${slave.pregKnown === 1 && success ? `you` : `your attempts at`} fucking your slaves' asses pregnant; this advances the ideal all a slave needs is ${his} rear.`);
		...
		if (slave.mpreg) {
			...
			text.push(`Society <span class="reputation inc">approves</span> of ${slave.pregKnown === 1 && success ? `you` : `your attempts at`} putting a new slave in ${him}; this advances the idea that all slaves should bear their masters' babies.`);
			FutureSocieties.Change("FSGenderFundamentalist", 1);
		}
	}
