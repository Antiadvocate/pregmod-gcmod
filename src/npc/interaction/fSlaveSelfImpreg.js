/**
 * Works with slaves that only have balls and no penis.
 *
...
App.Interact.fSlaveSelfImpreg = function(slave) {
	...
	let masturbationAction = `jacking off`;
	if (slave.dick === 0) {
		if (canDoVaginal(slave)) {
			masturbationAction = `rubbing ${his} ${clitDesc(slave, true)}`;
		} else if (canDoAnal(slave)) {
			if (slave.prostate > 0) {
				masturbationAction = `massaging ${his} prostate`;
			} else {
				masturbationAction = `fingering ${his} anus`;
			}
		} else {
			masturbationAction= `stimulating ${his} nipples`;
		}
	}
	...
	if (slave.fetish === Fetish.MINDBROKEN) {
		r.push(`${slave.slaveName} is dully cooperative as you ensure ${he} is positioned conveniently on a bench to give you access`);
		if (slave.dick > 0) {
			r.push(`to both aspects of ${his} genitalia. A quick dose of vasodilators ensures that ${he} is ready to perform, even if ${he} is unable to fully comprehend what is about to happen. Once ${he} is fully erect,`);
		} else {
			r.push(`to ${his} fertile ${slave.mpreg === 1 ? 'anus' : 'pussy'}. Once positioned correctly,`);
		}
		r.push(`your personal assistant uses a mechanical toy to efficiently stimulate ${him} to climax, while you hold a small container in place to collect ${his} emission. Moments later, you've loaded ${slave.slaveName}'s seed into a syringe and dispensed the contents deep within ${his}`);
		if (slave.mpreg === 1) {
			r.push(`ass.`);
		} else {
			r.push(`vagina.`);
		...
		if (coop && enjoy) {
			...
			r.push(`in helpless horror as`);
			if (slave.dick > 0) {
				r.push(`the vasodilators you inject bring ${his} manhood to complete erectness and`);
			}
		...
		if (enjoy) {
			if (slave.dick > 6 && slave.balls <= 4) { // TODO: canPenetrate() needs to be here
				...
				if (slave.trust < -20 && slave.dick > 0) {
				...
				} else if (slave.dick > maxErectionSize(slave) && slave.dick > 0) {
					r.push(`Although ${he} is aroused by the idea of impregnating ${himself}, ${slave.slaveName}'s cock is simply too large to easily become erect. A quick injection of vasodilators later, and ${he}'s ready to go. Because ${he}'s so eager, you simply stand back and let the magic happen.`);
				} else {
					r.push(`Merely thinking about what you're about to do has ${slave.slaveName} ${slave.dick > 0 ? `fully erect, with a small bead of precum running down ${his} shaft and blending with the` : 'looking at you with anticipation, with copious amounts of'}`);
					if (slave.mpreg === 1) {
						r.push(`lubricant dripping from between ${his} cheeks.`);
					} else {
						r.push(`feminine wetness dripping from`);
				...
				if (slave.dick < 1) {
					r.push(`Since ${he} has no member to fuck ${himself} with, you produce a special toy for ${slave.slaveName} — a catheter attached by a flexible tube to a dildo, with an embedded micro-pump to ensure any fluid makes its way out of the dildo.`);
				...
				if (slave.dick > 0) {
					...
					if (slave.prostate > 0) {
						r.push(`You massage ${his} prostate while ${he} fucks ${himself} with the dildo in ${his} ${slave.mpreg ? anusDesc(slave, true) : vaginaDesc(slave, true)}. The stimulation of being filled and having ${his} prostate stimulated`);
					} else {
						r.push(`You massage ${his} ${boobsDesc(slave, true)} while ${he} fucks ${himself} with the dildo in ${his} ${slave.mpreg ? anusDesc(slave, true) : vaginaDesc(slave, true)}. The stimulation of being filled and having ${his} nipples stimulated`);
					}
					r.push(`doesn't give ${him} much time to enjoy what ${he}'s doing before ${his} hips buck wildly and you see semen traveling from ${his} urethra, along the clear plastic tube, and into the base of the dildo embedded deeply in ${slave.slaveName}'s throbbing`);
				}
				if (slave.mpreg === 1) {
					r.push(`asshole.`);
				} else {
			...
			if (slave.devotion > 50 && slave.dick > 0) {
				if (slave.dick > maxErectionSize(slave)) {
			...
			// TODO: if slave has lover/wife, let them "do the honors"
			r.push(`You produce a sample container and syringe; ${slave.slaveName} gets the idea and immediately begins ${masturbationAction}. When ${he} ejaculates, ${he} carefully holds the cup`);
			if (hasBothArms(slave)) {
		...
		if (enjoy) {
			...
			if (slave.dick > maxErectionSize(slave) && slave.balls <= 4 && slave.dick > 0) {
			...
			r.push(`The repulsion ${slave.slaveName} feels toward both you and what you're about to force ${him} to do make ${his} cooperation entirely out of the question. You are forced to restrain ${him} with straps, kicking and sobbing.`);
			if (slave.dick > 0) {
				r.push(`The sobbing only gets louder when ${he} feels the prick of a needle shooting vasodilators into ${his} crotch. Moments later, despite ${his} best efforts, ${he} is painfully erect and ready for you to proceed.`);
			}
			r.push(`Since ${he}'s guaranteed to make this as difficult as possible, you opt for the simplest solution:`);
			if (slave.dick > 0) {
				r.push(`you take a condom and apply it to ${his} penis; once ${he}'s sheathed, you instruct ${V.assistant.name}, using toys, to forcibly stimulate ${him} until ${he} ejaculates and remove the condom.`);
			} else {
				r.push(`you grab a catheter and attach one end to a cup; once it's inserted in ${his} urethra, you instruct ${V.assistant.name}, using toys, to forcibly stimulate ${him} until ${he} ejaculates.`);
			}
			r.push(`Loading the contents into a syringe and injecting them into ${his}`);
			if (slave.mpreg === 1) {
				r.push(`ass`);
			} else {
				r.push(`vagina`);
	...
	const riskyWeek = isFertile(slave);
	let success = riskyWeek;
	if (V.menstruation === 1 && V.menstruationKnown > 0) {
		const pregChance = riskyWeek ? 95 : 0;
		success = jsRandom(0, 100) < pregChance;
	}
	...
	} else if (V.pregnancyKnown === 0 && !success) {
		r.push(`Despite repeating this ritual throughout the week, ${slave.slaveName} <span class="red">failed to conceive ${his} own child.</span>`);
	} else {
		r.push(`You repeat this ritual throughout the week, expecting to`);

		if (superfetation) {
			r.push(`add another child to ${slave.slaveName}'s pregnancy.`);
		} else {
			r.push(`impregnate ${slave.slaveName} with ${his} own child.`);
		}
	}

	if (FutureSocieties.isActive('FSRestart')) {
		if ((slave.breedingMark !== 1 || V.propOutcome === 0) && V.eugenicsFullControl !== 1) {
			r.push(`Rumors spread about ${slave.pregKnown === 1 && success ? `you` : `your attempts at`} having your slaves breed; the Societal Elite are <span class="red"> very displeased</span> by these rumors.`);
			V.failedElite += 5;
		}
	}
