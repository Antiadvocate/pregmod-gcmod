...
App.Interact.fBeg = function(slave) {
	...
	function consummation() {
		...
		if (V.PC.dick > 0) {
			if (slave.energy > 50) {
				text.push(`${He} can't help but stare in lust at your`);
				if (V.PC.balls >= 30) {
				...
				if (slave.fetishStrength > 60) {
					switch (slave.fetish) {
						...
						case "pregnancy":
							...
							text.push(`${his} stomach. ${Spoken(slave, `"Use me as your breeder, ${Master}, please! I just want to ${slave.pregSource === SID.PC ? 'be filled with your seed' : 'bear your children'} forever!"`)}`);
							break;
						default:
							text.push(`${He} kneels`);
