App.Interact.fSlaveImpregChoosePartner = class extends App.Interact.BaseChoosePartnerRenderer {
	constructor(slave) {
		super(slave);
		this.intro = `${slave.slaveName} is fertile; now you must select a slave with potent testicles.`;
		this.noneEligible = `You have no slaves capable of inseminating others.`;
		this.execute = (slave1, slave2) => {
			// Prevents repeated attempts
			if (!V.temp.slaveIdsImpregnanted) {
				V.temp.slaveIdsImpregnanted = new Set();
			}
			V.temp.slaveIdsImpregnanted.add(slave.ID);
			return App.Interact.fSlaveImpreg(slave1, slave2);
		};
	}

	eligible(candidate) {
		return (canImpreg(this.slave, candidate, !fertKnown(this.slave)) && canPenetrate(candidate)) || canFemImpreg(this.slave, candidate, !fertKnown(this.slave));
	}
...
App.Interact.fSlaveImpreg = function(slave, impregnatrix) {
	...
	const hasPenis = impregnatrix.dick > 0;
	...
	if (impregnatrix.fetish === "pregnancy" && impregnatrix.fetishKnown === 1 && impregnatrix.fetishStrength > 60 && impregnatrix.devotion >= -20) {
		...
		if (!canAchieveErection(impregnatrix) && hasPenis) {
			...
			r.push(`A special strap-on is fitted onto ${his2} urethra. The dildo includes a micro-pump that ensures any seed released will be sent out from the tip of the artificial member.`);
		}
	} else if (impregnatrix.attrXX > 65 && impregnatrix.attrKnown === 1 && impregnatrix.devotion >= -20 && hasPenis) {
		r.push(`Since ${impregnatrix.slaveName} likes sticking ${his2} cock in girls, ${he2} doesn't take much convincing.`);
		if (!canAchieveErection(impregnatrix)) {
		...
		if (!canAchieveErection(impregnatrix) && hasPenis) {
			...
			r.push(`A special strap-on is fitted onto ${his2} urethra, `);
		}
		r.push(`only a certain confusion in ${his2} look betraying that ${he2} realizes how special an occasion this is.`);
		if (!hasPenis) {
			r.push(`The dildo includes a micro-pump that ensures any seed released will be sent out from the tip of the artificial member.`);
		}
	} else if (impregnatrix.devotion > 20 && hasPenis) {
		...
		r.push(`Since ${impregnatrix.slaveName} is unlikely to comply willingly, you simply restrain ${him2}`);
		if (hasPenis) {
	...
	const penetrationTool = hasPenis ? dickDesc(impregnatrix, true) : 'artificial cock';

	if (slave.devotion < -20 && impregnatrix.devotion < -20) {
		r.push(`Since you have two restrained slaves, it's up to you to do all the work. You put ${slave.slaveName} on the couch with ${his} ${assPussy} available, and then maneuver ${impregnatrix.slaveName}'s ${penetrationTool} into place. The two slaves make no further moves until you deal ${impregnatrix.slaveName} a terrific swat across the ass and promise to give ${him2} more of the same until ${he2} gets going. After watching them mechanically go at it for a while, you stop ${impregnatrix.slaveName}, insert an electrostimulator up ${his2} rectum, and administer a shock to ${his2} ${prostate} that forces ${him2} to empty ${his2} nuts into ${slave.slaveName}. Both slaves <span class="mediumorchid">resent</span> what you made them do and <span class="gold">fear you</span> as a result.`);
		slave.devotion -= 5;
		slave.trust -= 5;
		...
		r.push(`cowgirl position. They take their time, humping slowly and gazing into each other's eyes. After a little while, though, ${slave.slaveName} looks over to where you're sitting, the invitation clear in ${his} eyes. As soon as you stand to come over, ${slave.slaveName} turns around on ${impregnatrix.slaveName}'s ${penetrationTool} and opens wide for you. You and ${impregnatrix.slaveName} enjoy the`);
		if (superfetation === 1) {
			r.push(`gravid ${girl}`);
		} else {
			r.push(`mother-to-be`);
		...
		r.push(`as ${he} continues riding${hasPenis ? ` cock` : ``}. Once`);
		if (V.PC.dick !== 0) {
			r.push(`stiff,`);
		} else {
	...
	if (slave.pregKnown === 1 && success) {
		r.push(`Throughout the week, you keep ${slave.slaveName}'s ${assPussy} intimate with ${impregnatrix.slaveName}'s ${penetrationTool}. In the end, you are certain ${slave.slaveName}`);
		if (superfetation === 1) {
		...
		r.push(`Despite repeating this ritual throughout the week, ${slave.slaveName} <span class="red">failed to conceive ${impregnatrix.slaveName}'s child.</span>`);
	} else {
		r.push(`Throughout the week, you keep ${slave.slaveName}'s ${assPussy} intimate with ${impregnatrix.slaveName}'s ${penetrationTool}, intending to`);

		if (superfetation) {
			r.push(`add ${impregnatrix.slaveName}'s child to ${slave.slaveName}'s pregnancy.`);
		} else {
			r.push(`impregnate ${slave.slaveName} with ${impregnatrix.slaveName}'s child.`);
		}
	}

	if (FutureSocieties.isActive('FSRestart') && V.eugenicsFullControl !== 1) {
		r.push(`Rumors spread about ${slave.pregKnown === 1 && success ? `you` : `your attempts at`} breeding your slaves; the Societal Elite are <span class="red">displeased</span> by these rumors.`);
		V.failedElite += 1;
	}

	if (FutureSocieties.isActive('FSGenderRadicalist') && slave.mpreg) {
		r.push(`Society <span class="reputation inc">approves</span> of ${slave.pregKnown === 1 && success ? `you` : `your attempts at`} breeding your slave's ass; this advances the ideal all a slave needs is ${his} rear.`);
		...
		if (slave.mpreg) {
			...
			r.push(`Society <span class="reputation inc">approves</span> of ${slave.pregKnown === 1 && success ? `you` : `your attempts at`} breeding your slaves; this advances the ideal of a durable, self propagating race of slaves`);
			FutureSocieties.Change("FSGenderFundamentalist", 1);
		}
	}
	...
	if (success && slave.pregKnown === 1 || V.pregnancyKnown !== 0) {
		...
		// face
		childStats = (getGenePoolRecord(slave).face + getGenePoolRecord(impregnatrix).face) / 2;
		if (childStats > 95) {
		...
		// intelligence
		childStats = (getGenePoolRecord(slave).intelligence + getGenePoolRecord(impregnatrix).intelligence) / 2;
		if (childStats > 95) {
			r.push(`genius-level`);
		} else if (childStats > 50) {
			r.push(`great`);
