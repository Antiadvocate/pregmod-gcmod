 ...
 * @param {Partial<InstanceType<typeof GenerateNewSlavePram>>} [params]
...
App.StartingGirls.listOfSlavesWithParent = function(parent, id) {
	...
	const slavesWithParent = getSlaves().filter((s) => s[parent] === id);
...
App.StartingGirls.uncommittedFamilyTree = function(slave) {
	/** @type {FC.HumanState[]} */
	let tSlaves = getSlaves().filter(s => s.ID !== slave.ID).asArray(); // exclude the unedited copy of this slave, if it exists
...
App.StartingGirls.playerOrigin = function(slave) {
	...
	switch (V.PC.career) {
		...
		case "escort":
			...
			data.nonPCpregSource = SID.CLIENT;
			break;
		case "prostitute":
			if (slave.mother === SID.PC && slave.father === SID.PC) {
		...
		case "child servant":
			...
			data.nonPCpregSource = SID.FORMERMASTER;
			break;
		case "BlackHat":
			if (slave.mother === SID.PC && slave.father === SID.PC) {
	...
	function updateSlave() {
		...
		if (data.nonPCpregSource && slave.preg > 0 && slave.pregSource !== SID.PC) {
...
App.StartingGirls.mental = function(slave, cheat = false) {
	...
	App.StartingGirls.addSet(option, App.Data.StartingGirls.devotion, cheat);
	...
	App.StartingGirls.addSet(option, App.Data.StartingGirls.trust, cheat);
...
App.StartingGirls.finalize = function(slave) {
	...
	const options = new App.UI.OptionsGroup();
	options.addOption("Apply Career bonus:", "applyCareerBonus", V.temp)
	...
	if (V.cash >= cost) {
		const addGirl = () => {
			...
			if (V.temp.applyCareerBonus) {
				if (_.isArray(V.temp.careerBonusNeeded)) {
					V.temp.careerBonusNeeded.push(slave.ID);
				} else {
					V.temp.careerBonusNeeded = [slave.ID];
				}
			}
			App.StartingGirls.cleanup(slave);
			App.Verify.slaveState(`<starting girl with id ${slave.ID}>`, slave, "none");
		...
		if (V.cash - cost >= minimumSlaveCost() / (isPCCareerInCategory("slaver") ? 2 : 1)) {
			...
			// (Of course, the player can always click "Take control of your arcology" if they cannot afford V.temp.activeSlave,
			...
			App.UI.DOM.appendNewElement("div", el,
				App.UI.DOM.link(
					...
					() => {
						addGirl();
						V.temp.activeSlave = this.generate();
					},
					[],
					"Starting Girls"
				)
