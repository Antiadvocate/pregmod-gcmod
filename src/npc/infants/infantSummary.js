...
App.Facilities.Nursery.InfantSummary = function(child) {
	...
	function shortExtendedFamily() {
		...
		if (child.mother > 0) {
			const target = getSlaves().find(s => s.ID === child.mother)?.value;
			if (target) {
				r += `${SlaveFullName(target)}'s ${daughter}`;
				if (child.relationshipTarget === target.ID) {
		...
		if (child.father > 0 && child.father !== child.mother) {
			const target = getSlaves().find(s => s.ID === child.father)?.value;
			if (target) {
				r += `${SlaveFullName(target)}'s ${daughter}`;
				if (child.relationshipTarget === target.ID && handled !== 1) {
					const friendShipShort = relationshipTermShort(child);
					r += ` & ${friendShipShort}`;
					handled = 1;
				}
		...
		if (child.daughters === 1) {
			let target = getSlaves().find(s => s.mother === child.ID)?.value;
			if (target) {
				r += `${SlaveFullName(target)}'s mother`;
				if (child.relationshipTarget === target.ID) {
					const friendShipShort = relationshipTermShort(child);
					r += ` & ${friendShipShort}`;
					handled = 1;
			...
			r += " ";
			target = getSlaves().find(s => s.father === child.ID)?.value;
			if (target) {
				r += `${SlaveFullName(target)}'s father`;
				if (child.relationshipTarget === target.ID && handled !== 1) {
					const friendShipShort = relationshipTermShort(child);
					r += ` & ${friendShipShort}`;
					handled = 1;
		...
		if (child.sisters === 1) {
			const target = getSlaves().find(s => areSisters(s, child) > 0)?.value;
			if (target) {
				r += `${SlaveFullName(target)}'s ${sister}`;
				if (child.relationshipTarget === target.ID) {
					const friendShipShort = relationshipTermShort(child);
					r += `& ${friendShipShort}`;
					handled = 1;
		...
		if (child.relationship > 0 && handled !== 1) {
			const target = getSlaves().find(s => s.ID === child.relationshipTarget)?.value;
			if (target) {
				r += `${SlaveFullName(target)}'s`;
				const friendShipShort = relationshipTermShort(child);
				r += ` ${friendShipShort}`;
			}
	...
	function longExtendedFamily() {
		...
		if (child.mother > 0) {
			const target = getSlaves().find(s => s.ID === child.mother)?.value;
			if (target) {
				r += `${SlaveFullName(target)}'s <span class="lightgreen">${daughter}`;
				if (child.relationshipTarget === target.ID) {
		...
		if (child.daughters === 1) {
			let target = getSlaves().find(s => s.mother === child.ID)?.value;
			if (target) {
				r += `${SlaveFullName(target)}'s <span class="lightgreen">mother`;
				if (child.relationshipTarget === target.ID) {
					const friendShipShort = relationshipTerm(child);
					r += ` and ${friendShipShort}`;
					handled = 1;
			...
			target = getSlaves().find(s => s.father === child.ID)?.value;
			if (target) {
				r += `${SlaveFullName(target)}'s <span class="lightgreen">father`;
				if (child.relationshipTarget === target.ID) {
					const friendShipShort = relationshipTerm(child);
					r += ` and ${friendShipShort}`;
					handled = 1;
		...
		if (child.sisters === 1) {
			const target = getSlaves().find(s => areSisters(s, child) > 0)?.value;
			if (target) {
				r += `${SlaveFullName(target)}'s <span class="lightgreen">${sister}`;
				if (child.relationshipTarget === target.ID) {
					const friendShipShort = relationshipTerm(child);
					r += ` and ${friendShipShort}`;
					handled = 1;
	...
	function shortRival() {
		...
		if (child.rivalry !== 0) {
			r += `&nbsp;&nbsp;&nbsp;&nbsp;`;
			const target = getSlaves().find(s => s.ID === child.rivalryTarget)?.value;
			if (target) {
				r += `<span class="lightsalmon">`;
				if (child.rivalry <= 1) {
					r += `Disl ${SlaveFullName(target)}`;
				} else if (child.rivalry <= 2) {
					r += `${SlaveFullName(target)}'s rival`;
				} else {
					r += `Hates ${SlaveFullName(target)}`;
				}
				r += `</span> `;
			}
		}
	...
	function longRival() {
		...
		if (child.rivalry !== 0) {
			...
			if (target) {
				if (child.rivalry <= 1) {
					r += `<span class="lightsalmon">Dislikes</span> ${SlaveFullName(target)}. `;
				} else if (child.rivalry <= 2) {
					r += `${SlaveFullName(target)}'s <span class="lightsalmon">rival.</span> `;
				} else {
					r += `<span class="lightsalmon">Hates</span> ${SlaveFullName(target)}. `;
				}
			}
			r += " ";
		}
