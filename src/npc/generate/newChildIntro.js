...
App.UI.newChildIntro = function(slave) {
	...
	const isTankBaby = slave.tankBaby > 0;
	...
	if (isTankBaby) {
		...
		r.push(`You completed the legalities before heading to ${V.nurseryName}, knowing the facility will present the child on your`);
	}

	if (tempDad) {
		if (tempDad.ID !== SID.PC) {
			tempParents = "father";
		}
	}
	if (tempMom) {
		if (tempMom.ID !== SID.PC) {
			tempParents = "mother";
		}
		if (tempDad) {
			if (tempMom.ID !== SID.PC && tempDad.ID !== SID.PC && tempMom.ID !== tempDad.ID) {
				tempParents = "parents";
			} else if (tempMom.ID === tempDad.ID && tempMom.ID !== SID.PC) {
	...
	if (isTankBaby) {
		...
		if (slave.preg > 0) {
			...
			const isReproductivelyAdvanced = slave.energy > 40; /* not really the correct check for this flavor text, but energy is only set by the reproductive upgrade, so it works. TODO: check actual body development vs. expected for age? */
			r.push(`you help ${him} to ${his} feet${isReproductivelyAdvanced ? `, making sure to feel up ${his} overdeveloped body,` : ``} and walk ${him} to your penthouse.`);
		}
	}
	...
	if (getMinRelative(slave.mother) || getMinRelative(slave.father)) {
		App.UI.DOM.appendNewElement(
			"div",
			surnaming,
			App.UI.DOM.link(
	...
	if (V.PC.slaveSurname && (slave.mother === SID.PC || slave.father === SID.PC)) {
		App.UI.DOM.appendNewElement(
			"div",
			surnaming,
			App.UI.DOM.link(
	...
	if (tempMom && tempDad && slave.father !== slave.mother) {
		...
		r.push(`alone, having no family${(slave.mother === SID.PC || slave.father === SID.PC) ? `, other than you,` : ``} in your stock.`);
	}
	...
	if (slave.vagina > -1) {
		if ((V.PC.dick !== 0) && (V.PC.boobs >= 300)) {
			...
			r.push(`${He} notices your privates differ from ${hers}, and thanks to the ${isTankBaby ? 'tank' : 'nursery'}'s teachings, <span class="devotion inc">can't look away.</span>`);
			slave.devotion += 4;
			if (isTankBaby && slave.energy > 40) {
