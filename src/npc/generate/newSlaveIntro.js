			...
			if (PC.career === "escort") { // switch to .belly
				if (PC.belly >= 1000) {
					if (slave.fetish === "pregnancy") {
						...
						r.push(`Your pregnancy is obvious to ${him}, which means two things: you won't be as capable of disciplining ${him}, leading ${him} to be <span class="mediumorchid">more willing to disobey,</span> and you are going to be demanding and needy, possibly even <span class="gold">using force to get ${him} to do what you want.</span>`);
						slave.trust -= 10;
						slave.devotion -= 10;
					}
				}
				...
				choice({
					...
					result(slave) {
						...
						if (tankBorn) {
							...
							const canLactate = slave.energy > 40; // TODO: this works because energy is only set by the reproductive upgrade. Maybe better to test something else?
							if (canLactate && slave.boobs > 400) {
				...
				if (PC.vagina !== -1 && slave.dick > 0 && canAchieveErection(slave)) {
					choice({
						...
						result(slave) {
							...
							if (tankBorn && (overpowerCheck(slave, V.PC) < random(1, 100)) && slave.energy > 40) {
						...
						choice({
							...
							result(slave) {
								...
								if (tankBorn) {
									...
									if (tankBorn && (overpowerCheck(slave, V.PC) < random(1, 100)) && slave.muscles > 30 && slave.energy > 40 && canAchieveErection(slave)) {
