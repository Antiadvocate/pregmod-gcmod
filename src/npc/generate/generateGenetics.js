...
globalThis.generateGenetics = (function() {
	...
	function generateGenetics(actor1, actor2, name) {
		...
		if (actor1.ID > 0) {
			mother = getGenePoolRecord(actor1);
			if (mother === undefined) {
				mother = actor1;
			}
			activeMother = getSlave(actor1.ID, true);
		...
		if (actor2 > 0) {
			father = getGenePoolRecord(actor2);
	...
	function setIntelligence(father, mother, activeMother, actor2, inbreedingCoeff) {
		...
		if (mother.ID === SID.PC) {
			if (actor2 === -6) {
				...
				while (
					smarts < 50 &&
					count < 500 // this fixes a infinite loop where a father and a mother may come through where father + mother intelligence < 50
				) {
					smarts = Math.max(smarts, fuzzy(father.intelligence, mother.intelligence));
					count++;
				}
			} else {
	...
	function setGeneticQuirks(father, mother, sex) {
		...
		if (father !== 0) {
			...
			} else if (father.geneticQuirks.potent === 1) {
				chance = random(0, 1000);
				if (mother.geneticQuirks.potent >= 1) {
					if (sex === "XY") {
						if (chance > 500) {
							quirks.potent = 2;
						} else if (chance > 50) {
 ...
 * @returns {FC.SlaveState|FC.InfantState}
 */
globalThis.generateChild = function(mother, ovum, incubator = false) {
	...
	if (!incubator) { // does extra work for the incubator if defined, otherwise builds a simple object
		child = new App.Entity.InfantState(String(genes.artSeed));
		...
		child.cervixImplantTubing = 0;
		child.cervixImplantTarget = "bellyImplant";
		child.makeup = 0;
		child.nails = 0;
		child.stampTat = 0;
		child.bellyTat = 0;
...
function regenerateSurname(child) {
	...
	if (child.clone) {
		const cloneSeed = getMinRelative(child.cloneID);
	...
	const father = getMinRelative(child.father);
	const mother = getMinRelative(child.mother);
	...
	function getPatronym() {
		if (child.mother === SID.PC && V.surnamePCOverride === 1) {
			return V.PC.slaveName;
		} else if (motherName && (!fatherName || (child.father === SID.PC && V.surnamePCOverride === 2))) {
			return motherName;
		} else if (fatherName) {
			return fatherName;
		}
	...
	function getMatronym() {
		if (child.father === SID.PC && V.surnamePCOverride === 1) {
			return V.PC.slaveName;
		} else if (fatherName && (!motherName || (child.mother === SID.PC && V.surnamePCOverride === 2))) {
			return fatherName;
		} else if (motherName) {
			return motherName;
		}
	...
	switch (V.surnameScheme) {
		case 0: // family, patrilineal
			if (V.PC.slaveSurname && child.mother === SID.PC && V.surnamePCOverride === 1) {
				child.slaveSurname = V.PC.slaveSurname;
			} else if (motherSurname && (!fatherSurname || (child.father === SID.PC && V.surnamePCOverride === 2))) {
		...
		case 1: // family, matrilineal
			if (V.PC.slaveSurname && child.father === SID.PC && V.surnamePCOverride === 1) {
				child.slaveSurname = V.PC.slaveSurname;
			} else if (fatherSurname && (!motherSurname || (child.mother === SID.PC && V.surnamePCOverride === 2))) {
				child.slaveSurname = fatherSurname;
