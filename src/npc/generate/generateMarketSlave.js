	...
	switch (market) {
		case "corporate": {
			...
			if (V.corp.SpecAccent === 1) {
				...
				slave.accent = /** @type {FC.GenePoolRecord["accent"]} */ (Math.clamp(slave.accent, 0, 1));
		...
		case "neighbor": {
			...
			if (neighbor.FSRepopulationFocus > 50) {
				...
				if (slave.preg < 0) {
					...
					if (V.menstruation) {
						slave.fertPeak = 0; /* ensure they will be fertile */
					}
					if (isFertile(slave)) {
			...
			if (neighbor.FSGenderRadicalist > 50) {
				...
				if (slave.dick > 0) {
					...
					if (neighbor.FSGenderRadicalistResearch === 1 && jsRandom(1, 100) <= 20) {
						...
						slave.mpreg = 1;
						if (V.menstruation) {
							slave.fertPeak = 0; /* ensure they will be on a risky week */
						}
						if (isFertile(slave)) {
			...
			if (neighbor.FSPaternalist > 20) {
				...
				if (!FutureSocieties.isActive('FSRepopulationFocus', neighbor) && !FutureSocieties.isActive('FSBodyPurist', neighbor)) {
					if (isFertile(slave, true) && slave.vagina > 0 && jsRandom(1, 10) === 9) {
		...
		case "TSS": {
			if (V.TSS.schoolUpgrade === 1) {
				if (V.pedoMode === 1) {
					SGProp.minAge = 24;
					SGProp.maxAge = 42; // Otherwise pedo mode sets max age to 18, and all graduates up being 24.
				} else {
					SGProp.minAge = 36;
				}
			} else if (V.pedoMode === 1 || (V.TSS.schoolPresent !== 0 &&random(0, 2) === 1)) { // mix of local and off branch slaves; local slaves have a hard age floor due to need for education/training
				const hardMin = (V.TSS.schoolUpgrade === 2 ? 12: 10);
				SGProp.minAge = Math.max(V.minimumSlaveAge, hardMin);
				SGProp.maxAge = SGProp.minAge + random(0, 1);
			} else {
				SGProp.minAge = 18;
		...
		case "TUO": {
			if (V.pedoMode || (V.TUO.schoolPresent !== 0 && random(0, 2) === 1)) { // mix of local and off branch slaves; local slaves have a hard age floor due to need for education/training
				const hardMin = (V.TUO.schoolUpgrade > 0 ? 12: 10);
				SGProp.minAge = Math.max(V.minimumSlaveAge, hardMin);
				SGProp.maxAge = Math.max(V.fertilityAge + jsRandom(0, 1), SGProp.minAge + jsRandom(0, 1));
				SGProp.ageOverridesPedoMode = 1;
			} else {
				SGProp.minAge = 18;
		...
		case "GRI": {
			...
			if (slave.physicalAge >= 16) { // The child aging code won't deal well with underage height growth.
				slave.height = Math.max(jsRandom(160, 210), slave.natural.height);
			}
			let ageButtModifier = 0;
			if (slave.physicalAge <= 12) {
				ageButtModifier = -2;
			} else if (slave.physicalAge <= 15) {
				ageButtModifier = -1;
			}
			slave.butt = jsRandom(4, 10) + ageButtModifier;
			const ageBoobsMultiplier = Math.min((slave.physicalAge - 3) / 15, 1);
			if (V.GRI.schoolUpgrade === 2) { // This doesn't assume even boob growth for pre-pubescents; it's more about what the body can support.
				slave.boobs = Math.round((200 * jsRandom(15, 30) * ageBoobsMultiplier) / 50) * 50;
				slave.lactation = 2;
				slave.lactationDuration = 2;
			} else {
				slave.boobs = Math.round((200 * jsRandom(4, 20) * ageBoobsMultiplier) / 50) * 50;
			}
			slave.nipples = jsEither(["huge", "inverted"]);
		...
		case "SCP": {
			if (V.pedoMode === 1) {
				if (V.SCP.schoolUpgrade === 2) { // Plastic surgery doesn't really work on growing bodies, but we can assume periodic implant replacements.
					SGProp.minAge = 10;
				} else {
					SGProp.minAge = V.minimumSlaveAge;
				}
			...
			slave.buttImplantType = "normal";
			slave.butt += slave.buttImplant;
			console.log(slave.boobs);
			const ageBoobsMultiplier = Math.min((slave.physicalAge - 2) / 16, 1);
			const boobsTargetSize = Math.round((2000 * ageBoobsMultiplier) / 50) * 50; // This assumes an older and taller slave can support bigger implants.
			slave.boobsImplant = (boobsTargetSize - slave.boobs);
			slave.boobs += slave.boobsImplant;
			console.log(slave.boobs - slave.boobsImplant);
		...
		case "LDE": {
			if (V.pedoMode || (V.LDE.schoolPresent !== 0 && random(0, 2) === 1)) { // mix of local and off branch slaves
				SGProp.minAge = V.minimumSlaveAge + 1;
			...
			SGProp.disableDisability = 1;
			SGProp.ageOverridesPedoMode = 1; // Minimal difference from pure pedo mode--we're guaranteeing a year for the drugs to work.
			slave = GenerateNewSlave("XY", SGProp);
		...
		case "TGA": {
			if (V.pedoMode === 1 || (V.TGA.schoolPresent !== 0 && random(0, 2) === 1)) { // mix of local and off branch slaves; local slaves have a hard age floor due to need for education/training
				const hardMin = (V.TGA.schoolUpgrade > 0 ? 12 : 10);
				SGProp.minAge = Math.max(V.minimumSlaveAge, hardMin);
				SGProp.maxAge = SGProp.minAge + random(0, 1);
				SGProp.ageOverridesPedoMode = 1;
			} else {
