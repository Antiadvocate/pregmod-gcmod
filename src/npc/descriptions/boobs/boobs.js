		...
		if (clothing && clothing.desc && "boobs" in clothing.desc) {
			...
			switch (slave.clothes) {
				...
				case "a nipple bikini":
					r += `${slave.slaveName}'s ${adjNoun} `;
					if (slave.boobs > 25000) {
						r += `are completely bare, save for the proportionally tiny pieces of cloth covering ${his} ${slave.nipples} nipples held up by a thin string tied behind ${his} neck.`;
					} else {
						r += `are completely bare, save for two tiny pieces of cloth covering ${his} ${slave.nipples} nipples held up by a thin string tied behind ${his} neck.`;
					}
					if (slave.nipples === NippleShape.FUCKABLE) {
						r += ` A pair of plugs hold the outfit together, acting as anchor points in place of ${his} nipples.`;
					}
					break;
				...
				case "slave rags":
					if (slave.boobs < 300) {
						r += `${slave.slaveName}'s rags rest over ${his} flat chest.`;
					} else if (slave.boobs > 4000) {
						r += `There aren't enough rags available to cover ${his} massive breasts; instead they only conceal a small amount of cleavage.`;
					} else if (slave.boobs > 800) {
						r += `${slave.slaveName} has to tie multiple rags to stand any chance of covering ${his} ${adjNoun}.`;
					} else {
						r += `${slave.slaveName}'s rags barely cover ${his} ${adjNoun}.`;
					}
					break;
				case "a slutty magical girl costume":
					if (slave.boobs < 300) {
						r += `${slave.slaveName}'s narrow bodysuit is so tightly form-fitting that it clings to ${his} flat chest and prominently displays ${his} ${slave.nipples} nipples.`;
					} else {
						r += `${slave.slaveName}'s narrow bodysuit is widest at the bust, with ${his} ${adjNoun} each individually hugged and supported by the material.`;
					}
					break;
				// WIP
		...
		if (V.showClothing === 1 && descType !== DescType.MARKET) {
			switch (slave.clothes) {
				case "a monokini":
					...
					break;
				case "a slutty magical girl costume":
					r += ` Every detail of them can be seen through the material of ${his} costume.`;
					break;
				default:
		...
		if (V.showBodyMods === 1) { // this seems wrong
			if (slave.piercing.areola.weight === 0) {
				switch (slave.areolae) {
					case 1:
						r.push(`${His} areolae form large, lovely ${slave.areolaeShape}s of ${nipColor} skin around each nipple.`);
						break;
					case 2:
						r.push(`${His} areolae are unusually wide, eye-catching ${slave.areolaeShape}s of ${nipColor} skin around each nipple.`);
						break;
					case 3:
			...
			if (slave.areolae > 1) {
				switch (slave.clothes) {
					case "a string bikini":
						...
						break;
					case "a nipple bikini":
						r.push(`${His} nipple bikini simply conceals its namesake, affording no cover to the areolae lewdly visible around them.`);
						break;
					case "slutty business attire":
					...
					case "a monokini":
						...
						break;
					case "a slutty magical girl costume":
						r.push(`${His} bodysuit is so lewdly fitted that not only are ${his} nipples obvious, the outline of ${his} areolae can be made out, too.`);
						break;
				}
			}
		}
