...
App.Desc.crotch = function(slave, descType = DescType.NORMAL) {
	...
	if (V.showClothing === 1 && descType !== DescType.MARKET) {
		// Industrial Dairy - does not need to check chastity settings as slaves are stripped bare.
		if (slave.assignment === Job.DAIRY && V.dairyRestraintsSetting > 1) {
			if (V.dairyPregSetting > 1 && slave.vagina > -1) {
				r.push(`${slave.slaveName}'s pussy is occupied by a massive dildo,`);
				if (isFertile(slave) || (!fertKnown(slave) && !isMenstruating(slave) && isFertile(slave, true))) {
					r.push(`servicing ${his} womb with its ejaculate.`);
				} else {
					r.push(`ejaculating drugs and curatives regularly.`);
				}
			}
			if (slave.balls > 0) {
				if (slave.dick > 0) {
					...
					r.push(`The milking machine keeps a cup against ${slave.slaveName}'s tiny cumhole, since ${he} has balls, but lacks a penis.`);
				}
			}
		} else {
				...
				switch (slave.clothes) {
					...
					case "body oil":
						if (slave.dick > 0 && slave.vagina > -1) {
							r.push(`Every surface of ${slave.slaveName}'s hermaphroditic genitalia has been carefully`);
						} else if (slave.dick > 0) {
					...
					case "a nipple bikini":
						...
						if (slave.dick > 7 && canAchieveErection(slave) && slave.devotion > 20 && slave.belly >= 5000) {
							...
							r.push(`erection is so massive that ${his} bikini bottom is more an accessory to it than any sort of support. ${He}'d restrain it with ${his} top, but even the slightest throb would tear it right off.`);
						} else if (slave.dick > 5 && canAchieveErection(slave) && slave.devotion > 20) {
							r.push(`erection is so big that it proudly extends out of ${his} bikini bottom. ${He}'d restrain it with ${his} top, but even the slightest throb would tear it right off.`);
						} else if ((slave.dick > 0) && (slave.vagina > -1)) {
							r.push(`just given up and pushed ${his} bikini bottom to one side, since the bikini bottom that fits a hermaphrodite has not yet been designed.`);
						} else if (slave.dick > 0) {
							r.push(`bikini bottom is more of a banana hammock, in front.`);
						} else if (slave.vagina === -1) {
							r.push(`bikini conceals the featurelessness of ${his} groin.`);
						} else {
							r.push(`bikini tightly hugs ${his} pussylips.`);
						}
						break;
					...
					case "a bimbo outfit":
						...
						break;
					case "slave rags":
						if ((slave.dick > 3) && canAchieveErection(slave) && (slave.devotion > 20)) {
							r.push(`${slave.slaveName}'s rags can't handle erections, often leaving the tattered fabric to hang to the side of ${his} turgid cock.`);
						} else if (slave.dick > 5) {
							r.push(`${slave.slaveName}'s rags can't hide ${his} huge cock; it hangs out for all to see beneath the tattered fabric.`);
						} else if (slave.dick > 3) {
							r.push(`${slave.slaveName}'s rags can barely conceal ${his} cock; every motion threatens to swing it out from behind the tattered fabric.`);
						} else if ((slave.dick > 0) && (slave.vagina > -1)) {
							r.push(`${slave.slaveName}'s rags conceal ${his} hermaphroditic genitalia, as long as ${he} doesn't move too much.`);
						} else if (slave.dick > 0) {
							r.push(`${slave.slaveName}'s rags conceal ${his} cock, as long as ${he} doesn't move too much.`);
						} else if (slave.chastityVagina) {
							r.push(`${slave.slaveName}'s rags can't hide ${his} chastity belt.`);
						} else if (slave.vagina === -1) {
							r.push(`${slave.slaveName}'s rags conceal ${his} featureless groin, as long as ${he} doesn't move too much.`);
						} else {
							r.push(`${slave.slaveName}'s rags conceal ${his} pussy, as long as ${he} doesn't move too much.`);
						}
						break;
					case "a slutty magical girl costume":
						if (canAchieveErection(slave) && slave.devotion > 20) {
							r.push(`${slave.slaveName}'s bodysuit has a nigh undetectable slit in its crotch, just for those magical girls with a little something extra jutting out from between their legs.`);
						} else if ((slave.dick > 0) && (slave.vagina > -1)) {
							r.push(`${slave.slaveName}'s bodysuit tightly hugs ${his} hermaphroditic genitalia, revealing each and every detail.`);
						} else if (slave.dick > 0) {
							r.push(`${slave.slaveName}'s bodysuit tightly embraces ${his} cock, revealing every little detail.`);
						} else if (slave.chastityVagina) {
							r.push(`${slave.slaveName}'s bodysuit tightly hugs ${his} chastity belt, begging to be let it.`);
						} else if (slave.vagina === -1) {
							r.push(`${slave.slaveName}'s bodysuit tightly hugs ${his} featureless groin.`);
						} else {
							r.push(`${slave.slaveName}'s bodysuit tightly hugs ${his} pussy, revealing its every fold.`);
						}
						break;
					// WIP
