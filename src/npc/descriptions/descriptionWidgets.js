...
App.Desc.ageAndHealth = function(slave) {
	...
	if (!slave.fuckdoll) {
		...
		if (slave.actualAge === V.idealAge || (slave.actualAge === V.idealAge - 1 && slave.birthWeek >= 52 && V.seeAge)) {
			...
			} else if (slave.birthWeek === 0 && V.seeAge) {
...
App.Desc.sexualHistory = function(slave) {
	...
	if (slave.counter.reHymen > 0) {
		r += `${His} hymen has been regenerated ${numberWithPluralOne(slave.counter.reHymen, "time")}. `;
	}
	...
	if (slave.counter.slavesKnockedUp > 0 || slave.counter.slavesFathered > 0 || slave.counter.PCKnockedUp > 0) {
		let PCChildrenFatheredCount = getSlaves().filter(s => s.father === slave.ID && s.mother === SID.PC).size; // The slave is also the father of the children transferred from the PC's womb to other slave's one.
