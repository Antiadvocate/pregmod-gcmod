		...
		if (clothing && clothing.desc && "summary" in clothing.desc) {
			...
			switch (slave.clothes) {
				...
				case "a nipple bikini":
					r.push(`half typical bikini, half wardrobe malfunction waiting to `);
					if (hasAnyLegs(slave)) {
						r.push(`happen, and`);
						r.push(App.Desc.footwear(slave));
					} else {
				...
				case "a courtesan dress":
					...
					break;
				case "slave rags":
					r.push(`consisting of several pieces of ragged cloth held in place with frayed cordage.`);
					if (hasAnyLegs(slave)) {
						r.push(`${He} is`);
						r.push(App.Desc.footwear(slave));
					}
				...
				case "a slutty magical girl costume":
					r.push(`consisting of narrow bodysuit that hugs every nook and cranny and an ultra short skirt that covers nothing, all in garishly bright colors.`);
					if (hasAnyArms(slave)) {
						r.push(`${He} is wearing`);
				...
				case "a superhero outfit":
					r.push(`an outfit that consists of a blue leotard with a superhero symbol and a long red cape.`);
					if (hasAnyArms(slave)) {
						r.push(`${His} arms are bare.`);
					}
					if (hasAnyLegs(slave)) {
						r.push(`${He} is wearing`);
						r.push(App.Desc.footwear(slave));
					...
					break;
				case "a cardigan":
					r.push(`worn loose and open to show ${his} crop-top underneath. To round out the casual look, ${he} wears tight leather pants.`);
					if (hasAnyArms(slave)) {
						r.push(`${His} arms are covered by the long cardigan.`);
					}
					if (hasAnyLegs(slave)) {
						r.push(`${He} is wearing`);
						r.push(App.Desc.footwear(slave));
				...
				case "a streetwear outfit":
					r.push(`consisting of a short denim skirt, a crop-top and a stylish leather jacket. To compliment the rebellious look, ${he} wears ripped fishnet stockings.`);
					if (hasAnyArms(slave)) {
						r.push(`${His} arms are covered by ${his} jacket.`);
					}
					if (hasAnyLegs(slave)) {
						r.push(`${He} is wearing`);
						r.push(App.Desc.footwear(slave));
				...
				case "a country outfit":
					r.push(`consisting of a white, puffed blouse with an embroidered corset and a flared short skirt.`);
					if (hasAnyArms(slave)) {
						r.push(`${His} arms are covered by the long sleeves of ${his} blouse.`);
					}
					if (hasAnyLegs(slave)) {
						r.push(`${He} is wearing`);
						r.push(App.Desc.footwear(slave));
				...
				case "a leather outfit":
					r.push(`consisting of a black, strapless leather corset, paired with tight leather pants.`);
					if (hasAnyArms(slave)) {
						r.push(`To compliment the rocker look, ${he} wears a pair of leather bracers and armbands.`);
					}
					if (hasAnyLegs(slave)) {
						r.push(`${He} is wearing`);
						r.push(App.Desc.footwear(slave));
				...
				case "a turtleneck and miniskirt":
					r.push(`including a conservative turtleneck sweater paired with a sexy miniskirt.`);
					if (hasAnyLegs(slave)) {
						r.push(`${He} is wearing`);
						r.push(App.Desc.footwear(slave));
					}
				...
				case "a red priestess outfit":
					r.push(`a crimson robe with detached sleeves and a long flowing skirt. ${His} head is crowned by an ornamental bone headdress.`);
					if (hasAnyArms(slave)) {
						r.push(`${His} arms are ornamented with religious jewelry.`);
					}
					if (hasAnyLegs(slave)) {
						r.push(`${He} is wearing`);
						r.push(App.Desc.footwear(slave));
				...
				case "an eastern european outfit":
					r.push(`consisting of a white blouse with detached sleeves, a corset and a long flowing skirt. ${His} head is ornamented with a flower crown.`);
					if (hasAnyLegs(slave)) {
						r.push(`${He} is wearing`);
						r.push(App.Desc.footwear(slave));
					}
					break;
				case "a samba outfit":
					r.push(`a colorful carnival outfit. It includes a colorful, sparkling bikini, is decorated with beads and sequins and has an elaborate feathered headdress and backpiece.`);
					if (hasAnyArms(slave)) {
						r.push(`${His} arms are decorated with colorful arm bands.`);
					}
					if (hasAnyLegs(slave)) {
						r.push(`${He} is wearing`);
						r.push(App.Desc.footwear(slave));
				...
				case "a tribal outfit":
					r.push(`consisting of a beaded halter top decorated with feathers, a fringed miniskirt with leather straps and ornamental patterns, and a feathered headdress.`);
					if (hasAnyArms(slave)) {
						r.push(`On ${his} arms ${he} wears leather armbands with ornamental patterns.`);
					}
					if (hasAnyLegs(slave)) {
						r.push(`${He} is wearing`);
						r.push(App.Desc.footwear(slave));
				...
				case "a tropical outfit":
					r.push(`consisting of a simple woven grass skirt and bra as well as a flower lei.`);
					if (hasAnyArms(slave)) {
						r.push(`On ${his} arms ${he} wears decorative arm bands.`);
					}
					if (hasAnyLegs(slave)) {
						r.push(`${He} is wearing`);
						r.push(App.Desc.footwear(slave));
				...
				case "a valkyrie outfit":
					r.push(`an ensemble that presents a fanciful medieval viking look with a fur-trimmed leather corset with metal accents, leather miniskirt, and fur coat.`);
					if (hasAnyArms(slave)) {
						r.push(`On ${his} arms ${he} wears metal bracers.`);
					}
					if (hasAnyLegs(slave)) {
						r.push(`${He} is wearing`);
						r.push(App.Desc.footwear(slave));
				...
				case "a baroque dress":
					r.push(`consisting of a long and wide petticoat as well as a corset, reminding you of a long gone era.`);
					if (hasAnyLegs(slave)) {
						r.push(`${He} is wearing`);
						r.push(App.Desc.footwear(slave));
					}
				...
				case "a cocktail dress":
					r.push(`a short frilly dress in an off-the-shoulder design with a fitted waist.`);
					if (hasAnyLegs(slave)) {
						r.push(`${He} is wearing`);
						r.push(App.Desc.footwear(slave));
					}
				...
				case "an ice queen outfit":
					r.push(`consisting of a long white lace dress with a fur coat and a white crown.`);
					if (hasAnyArms(slave)) {
						r.push(`${His} arms are covered by the coat.`);
					}
					if (hasAnyLegs(slave)) {
						r.push(`${He} is wearing`);
						r.push(App.Desc.footwear(slave));
				...
				case "a magical girl costume":
					r.push(`consisting of a cute frilly dress in pastel colors.`);
					if (hasAnyLegs(slave)) {
						r.push(`${He} is wearing`);
						r.push(App.Desc.footwear(slave));
					}
					break;
				case "a sundress":
					r.push(`a short cute dress with floral motif and a wide-brimmed sunhat.`);
					if (hasAnyArms(slave)) {
						r.push(`${His} arms are bare.`);
					}
					if (hasAnyLegs(slave)) {
				...
				case "a wedding dress":
					r.push(`truly worthy of the highest class of slaves. It comes with a veil and a rose bouquet.`);
					if (hasAnyArms(slave)) {
						r.push(`${His} arms are covered by silk.`);
					}
					if (hasAnyLegs(slave)) {
						r.push(`${He} is wearing`);
						r.push(App.Desc.footwear(slave));
				...
				case "an armored dress":
					r.push(`which looks like it came from an art designer's wet dream; A dress, featuring a built-in corset and a long skirt, gold embroidery and a long royal cloak.`);
					if (hasAnyArms(slave)) {
						r.push(`${His} arms are covered by the long sleeves of the dress.`);
					}
					if (hasAnyLegs(slave)) {
						r.push(`${He} is wearing`);
						r.push(App.Desc.footwear(slave));
					...
					break;
				case "a babydoll":
					r.push(`a short little transparent piece of lingerie with cute little bows.`);
					if (hasAnyLegs(slave)) {
						r.push(`${He} is wearing`);
						r.push(App.Desc.footwear(slave));
					}
				...
				case "a coat and corset":
					r.push(`consisting of a big trench coat that hides a raunchy lingerie combo of a corset and lace miniskirt beneath.`);
					if (hasAnyLegs(slave)) {
						r.push(`${He} is wearing`);
						r.push(App.Desc.footwear(slave));
					}
				...
				case "a frilly bikini":
					r.push(`a tight swimsuit that shows off ${his} figure while accentuating ${his} playful side with cute little frills.`);
					if (hasAnyLegs(slave)) {
						r.push(`${He} is wearing`);
						r.push(App.Desc.footwear(slave));
					}
