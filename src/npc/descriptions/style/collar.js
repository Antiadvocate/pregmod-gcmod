...
App.Desc.collar = function(slave) {
	...
	const fertilityUnknown = !fertKnown(slave);
	const {
		he, him, his, He
	} = getPronouns(slave);
	switch (slave.collar) {
		...
		case "preg biometrics":
			if (slave.pregSource > 0 || slave.pregSource < -9) {
				daddy = findFather(slave.pregSource);
				if (!daddy){
					const missingFatherID = slave.womb[0].fatherID;
					daddy = V.missingTable[missingFatherID];
				}
				daddy = daddy.slaveName;
			}
			r.push(`${He} is wearing a heavy metal collar with a digital display; it currently reads:`);
			if (!isMenstruating(slave) && ((slave.preg > 0 && slave.pregKnown === 0) || (V.menstruation && V.menstruationKnown === 1 && slave.fertKnown && slave.fertLate < 0))) {
				...
				if (pregCollar === 1) {
					...
					} else if (V.menstruation && !fertKnown(slave) && (slave.ovaries > 0 || slave.mpreg > 0) && slave.pubertyXX === 1 && slave.physicalAge <= V.fertilityAge) {
						r.push(`"I'm not old enough to get pregnant yet!"`);
					} else if (isFertile(slave, fertilityUnknown)) {
						r.push(`"Knock me up!"`);
					} else if ((slave.ovaries > 0 || slave.mpreg > 0) && slave.pubertyXX === 0 && slave.physicalAge < V.fertilityAge) {
						r.push(`"I'm not old enough to get pregnant yet!"`);
					} else if (V.menstruation === 1 && slave.fertPeak > 0 && isFertile(slave, true)) {
					...
					} else if (V.menstruation && V.menstruationKnown > 0 && (slave.ovaries > 0 || slave.mpreg > 0) && slave.pubertyXX === 1 && slave.physicalAge <= V.fertilityAge && !slave.fertKnown) {
						r.push(`"Can you help me never experience a period?"`);
					} else if (isFertile(slave, fertilityUnknown)) {
						r.push(`"My womb needs filling!"`);
					} else if (V.menstruation === 1 && slave.fertPeak > 0 && (V.menstruationKnown === 0 || slave.fertKnown)) {
					...
					if (slave.vagina < 0 && slave.mpreg < 1) {
						...
						} else if (slave.pregSource === SID.GENERIC || slave.pregSource === SID.CITIZEN || slave.pregSource === SID.CLIENT) {
