		...
		if (slave.geneticQuirks.superfetation === 2 && slaveWD.litters.length > 1) {
			if (V.pregnancyMonitoringUpgrade !== 1) {
				...
				if (!sameFather) {
					...
					if (slaveWD.litterData[slaveWD.litters.length - 1][0].age <= slave.pregData.normalBirth / 8) {
					...
					} else if (slave.pregSource === SID.GENERIC) {
						...
						r.push(`all of them from an unknown father.`);
					}
				}
			}
		} else {
			if (slave.pregSource === SID.LAB) {
				...
				if (slave.pregSource === SID.PC) {
					...
					if (slave.pregType > 1) {
						...
						if (daddy) {
							...
							r.push(`an unknown father's growing children.`);
						}
					} else {
						if (daddy) {
							r.push(`${daddy}'s growing child.`);
						} else {
							r.push(`an unknown father's growing child.`);
						}
					}
				}
				...
				if (daddy > 0) {
					...
					let desc = new Map([
						[SID.GENERIC, `an unknown person`],
						[SID.PC, `you`],
						[SID.CITIZEN, `one of the citizens of ${V.arcologies[0].name}`],
						[SID.FORMERMASTER, `your former Master`],
						[SID.ARCOWNER, `the owner of another arcology`],
						[SID.ELITE, `a member of the Societal Elite`],
						[SID.ANIMAL, `one of your pets`],
						[SID.FUTA, `a Futanari Sister`],
						[SID.RAPIST, `a rapist`],
						[SID.CLIENT, `a client`],
					]);
					daddy = desc.get(daddy);
				}
