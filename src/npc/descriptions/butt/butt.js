...
App.Desc.butt = function(slave, descType = DescType.NORMAL) {
		...
		if (clothing && clothing.desc && "butt" in clothing.desc) {
			...
			switch (slave.clothes) {
				...
				case "a nipple bikini":
					r.push(`${His}`);
					if (slave.butt > 10) {
						r.push(`bikini bottom barely covers ${his} inhuman`);
					} else if (slave.butt > 6) {
						r.push(`bikini bottom only somewhat covers ${his} huge`);
					} else if (slave.butt > 3) {
						r.push(`bikini bottom enticingly covers ${his} big`);
					} else {
						r.push(`bikini bottom completely covers ${his}`);
					}
					r.push(`buttocks.`);
					break;
				...
				case "an evening dress":
					r.push(`${His} sensual evening dress`);
					if (slave.butt > 10) {
						r.push(`is tailored as much as possible for ${his} inhuman derriere, yet the garment's split is still dramatically shifted upwards, exposing most of ${his} lower body.`);
					} else if (slave.butt > 6) {
						r.push(`is tailored, though the thigh cut nevertheless rides up ${his} leg, giving a tantalizing view of ${his} massive ass.`);
					} else if (slave.butt > 3) {
						r.push(`is tailored to accentuate ${his} big butt.`);
					} else {
						r.push(`clings to ${his} ass.`);
					}
					break;
				case "a dirndl":
				...
				case "slave rags":
					r.push(`${His} rags`);
					if (slave.butt > 10) {
						r.push(`rest on top of ${his} enormous butt, leaving ${his} behind entirely exposed.`);
					} else if (slave.butt > 6) {
						r.push(`rest on top of ${his} huge butt, only covering the top of ${his} asscrack.`);
					} else if (slave.butt > 3) {
						r.push(`can only cover about half of ${his} big butt.`);
					} else {
						r.push(`just barely manage to cover ${his} rear.`);
					}
					break;
				case "a slutty magical girl costume":
					r.push(`${His} ultra short skirt catches the eye with exactly how little it covers as the rear of ${his} bodysuit`);
					if (slave.butt > 6) {
						r.push(`lewdly vanishes between ${his} huge asscheeks.`);
					} else if (slave.butt > 3) {
						r.push(`lewdly rests between ${his} asscheeks.`);
					} else {
						r.push(`lewdly embraces each of ${his} asscheeks.`);
					}
					break;
				// WIP
	...
	} else if (slave.tailShape === "hyenaspotted") {
		r.push(`${He} has a short and brush-like, black-tipped and ${slave.tailEffect === "none" ? `${slave.tailColor} colored hyena tail` : `${slave.tailColor} colored hyena tail with ${slave.tailEffect}`}. It wags when ${he} gets excited, and raises cutely when ${he} focuses intently on a task.`);
	} else if (slave.tailShape === "hyenastriped") {
		r.push(`${He} has a short, bushy ${slave.tailEffect === "none" ? `${slave.tailColor} colored hyena tail` : `${slave.tailColor} colored hyena tail with ${slave.tailEffect}`}.`);
	...
	} else if (slave.tailShape === "rat") {
		r.push(`${He} has a long, thick and almost scaly ${slave.tailEffect === "none" ? `${slave.skin} tail` : `${slave.skin} tail with ${slave.tailEffect}`}. ${He} sometimes curls it around various things, but it's not strong or dextrous enough to use meaningfully.`);
	} else if (slave.tailShape === "mouse") {
		r.push(`${He} has a long, thin ${slave.tailEffect === "none" ? `${slave.skin} tail,` : `${slave.skin} tail with ${slave.tailEffect},`} covered in a very thin layer of ${slave.hColor} fur.`);
