...
App.Desc.buttplug = function(slave, descType = DescType.NORMAL) {
	...
	// Industrial Dairy View: Does not need to check chastity as slaves are stripped bare.
				...
				switch (slave.clothes) {
					...
					case "a nipple bikini":
						if (slave.chastityAnus) {
							...
							r.push(`${His}`);
							if (slave.anus > 1) {
								r.push(`well-fucked asshole is concealed by ${his} bikini.`);
							} else {
								r.push(`bikini conceals ${his} tight asshole.`);
							}
						}
						break;
					...
					case "slave rags":
						if (slave.chastityAnus) {
							r.push(`${His} anal chastity belt can easily be seen beneath ${his} butt rag.`);
						} else {
							r.push(`${His} rags can easily be torn off to expose ${his} anus.`);
						}
						break;
					case "a slutty magical girl costume":
						if (slave.chastityAnus) {
							r.push(`${His} anal chastity belt prevents ${his} bodysuit from hugging ${his} anus.`);
						} else {
							r.push(`${His} bodysuit might tightly hug ${his} anus, but it can easily be pulled aside to expose it.`);
						}
						break;
