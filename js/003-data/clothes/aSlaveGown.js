...
App.Data.clothes.set("a hoodie",
	{
		...
		desc: {
			summary: function(slave) {
				...
				r.push(`a comfy hoodie with long sleeves and a big hood.`);
				r.push(`It covers ${his} chest and arms, but leaves ${his} lower body bare.`);
				r.push(`${He} looks intentionally bottomless.`);
			...
			boobs: function(slave) {
				...
				if (slave.boobs < 300) {
					return `${slave.slaveName}'s hoodie lies flat across ${his} chest.`;
				} else if (slave.boobs > 3000) {
					return `${slave.slaveName}'s hoodie is pulled taut over ${his} breasts, making their shape obvious through the fabric.`;
				}
				return `${slave.slaveName}'s hoodie drapes over ${his} breasts, hiding nipples but not curves.`;
			},
			...
			butt: function(slave) {
				...
				if (!hasAnyLegs(slave)) {
					return `The hoodie bunches around ${slave.slaveName}'s waist, leaving ${his} rear bare.`;
				}
				return `From behind, the hoodie hides the top of ${slave.slaveName}'s ass, but ${his} bare cheeks still show.`;
			},
			...
			crotch: function(slave) {
				...
				r.push(`The hem of ${slave.slaveName}'s hoodie hangs low, but ${his} groin is still completely exposed.`);
				if (slave.dick > 0) r.push(`${His} cock${slave.scrotum > 0 ? " and balls" : ""} are out in the open.`);
				if (slave.vagina > -1) r.push(`${His} pussy is on display beneath it.`);
...
App.Data.clothes.set("a tankini",
	{
		name: "Tankini",
		...
		desc: {
			summary: function(slave) {
				...
				r.push(`a cute tankini: a tank-top style swim top with matching bottoms.`);
				r.push(`It covers ${his} torso more than a bikini while still fitting snugly to ${his} shape.`);
				r.push(`${He} looks ready for the pool, beach, or a very suggestive photoshoot.`);
			...
			boobs: function(slave) {
				...
				if (slave.boobs < 300) {
					return `${slave.slaveName}'s tankini top sits smoothly across ${his} chest.`;
				} else if (slave.boobs > 3000) {
					return `${slave.slaveName}'s tankini top strains over ${his} breasts, stretching the fabric and making every curve obvious.`;
				}
				return `${slave.slaveName}'s tankini top cups ${his} breasts neatly, showing shape without exposing too much.`;
			},
			...
			butt: function(slave) {
				...
				if (!hasAnyLegs(slave)) {
					return `The tankini bottom clings around ${slave.slaveName}'s hips, emphasizing ${his} rear.`;
				}

				r.push(`From behind, the tankini bottom hugs ${slave.slaveName}'s ass closely.`);
				if (slave.butt > 6) r.push(`It was not made for someone with this much cake, and it shows.`);
				return r.join(" ");
			},
			...
			crotch: function(slave) {
				...
				r.push(`${slave.slaveName}'s tankini bottom covers ${his} groin, but the thin swim fabric leaves little to the imagination.`);
				if (slave.dick > 0) r.push(`${His} cock${slave.scrotum > 0 ? " and balls" : ""} create a noticeable outline.`);
				if (slave.vagina > -1) r.push(`${His} pussy is covered, but the fit draws the eye anyway.`);
