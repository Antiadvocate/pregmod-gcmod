...
App.Medicine.Surgery.Procedures.ShortenTendons = class extends App.Medicine.Surgery.Procedure {
	...
	get _blockedByFacilityControlledAspect() {
		return FacilityControlledAspect.SURGERY_TENDONS;
	}

	disabledReasons(facilityControlledAspects) {
		const reasons = super.disabledReasons(facilityControlledAspects);
		if (!hasAnyNaturalLegs(this._slave)) {
			const {He} = getPronouns(this.originalSlave);
			reasons.push(`${He} has no natural legs to modify.`)
		}
		return reasons;
	}
