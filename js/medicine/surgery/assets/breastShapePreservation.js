	...
	reaction(slave, diff) {
		...
		if (slave.boobs >= 7000) {
			...
			if (slave.fetish === Fetish.MINDBROKEN) {
				...
				r.push(`${He}'s come to terms with the fact that ${he}'s a slave, but both you and ${him} expected something other than this when ${he} was sent to the surgery. ${He} isn't much affected mentally. As with all invasive surgery <span class="health dec">${his} health has been affected.</span> ${He} is <span class="trust inc">thankful</span> that you removed the literal weight off ${his} chest.`);
				reaction.trust += 5;
			} else {
				if (canSee(slave)) {
