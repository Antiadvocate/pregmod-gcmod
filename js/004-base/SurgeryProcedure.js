App.Medicine.Surgery.Procedure = class {
	...
	/**
	 * Facility controlled aspect of a slave that blocks commitment of the procedure, if any.
	 * @protected
	 * @returns {FC.Facilities.FacilityControlledAspect}
	 */
	...
	disabledReasons(facilityControlledAspects) {
		...
		const blockedBy = this._blockedByFacilityControlledAspect;
		if (blockedBy !== null && facilityControlledAspects.has(blockedBy)) {
			const {His, him} = getPronouns(this.originalSlave);
			reasons.push(`${His} assignment automatically performs equivalent or conflicting procedure on ${him}.`);
		}
