...
App.RA.Activation.populateGetters = function() {
	...
	gm.addBoolean("hasimplants", {
		name: "Has implants?", description: "Does the slave have any cosmetic implants?",
		val: c => SlaveStatsChecker.hasImplants(c.slave)
	});
	gm.addBoolean("hasallimplants", {
		name: "Has all Implants?", description: "Does the slave have the full set of cosmetic implants (boobs, butt, lips)?",
		val: c => SlaveStatsChecker.hasAllImplants(c.slave)
	});
	gm.addBoolean("issurgicallyimproved", {
		name: "Is Surgically Improved?", description: "Does the slave undergone breast, butt, lips, and waist enhancement surgeries?",
		val: c => SlaveStatsChecker.isSurgicallyImproved(c.slave)
	});
